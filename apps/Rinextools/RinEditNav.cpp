...
class RinEditNav : public BasicFramework
{
  public:
    ...
    struct UniqueNav
    {
        UniqueNav() : PRNID(-1), fitint2(0.)
        {
        }
        UniqueNav(const Rinex3NavData &rnd)
            : time(rnd.time), satSys(rnd.satSys), PRNID(rnd.PRNID), fitint2(rnd.fitint * 1800.)
        {
        ...
        bool operator<(const UniqueNav &right) const
        {
            ...
            if (right.time < time)
                return false;
            if (satSys < right.satSys)
                return true;
            if (right.satSys < satSys)
                return false;
            if (PRNID < right.PRNID)
                return true;
            return false;
        }
...
RinEditNav ::RinEditNav(const string &applName)
    : BasicFramework(applName, "reads one or more RINEX (v.2+) navigation"
      ...
      timeOpt('t', "time", "%Y %m %d %H %M %S",
              "Start time (of data) for"
              " processing (time in RINEX 3 format, i.e. \"YYYY mm dd HH MM"
              " SS\")"),
      eTimeOpt('e', "end-time", "%Y %m %d %H %M %S",
               "End time (of data) for"
               " processing"),
      excludeOpt('x', "exclude",
                 "Exclude satellite [system] from output"
                 " [e.g. G17,R]"),
      output3Opt('o', "out",
                 "Output (ARG=[sys@]filename) to RINEX ver. 3"
                 " file fn [repeatable]"),
      output2Opt('2', "out2",
                 "Output (ARG=[sys@]filename) to RINEX ver. 2"
                 " file fn [repeatable]"),
      filesOpt("RINEX-NAV-FILE [...]", true), minTime(CommonTime::BEGINNING_OF_TIME), maxTime(CommonTime::END_OF_TIME)
{
...
bool RinEditNav ::initialize(int argc, char *argv[], bool pretty) noexcept
{
    ...
    for (unsigned i = 0; i < files.size(); i++)
    {
        ...
        {
            cerr << "\"" << files[i] << "\" specified multiple times, only using once" << endl;
            continue;
        }
        ...
        if (debugLevel)
        {
            cerr << printTime(dataMap[sp].time, "time:%Y/%02d/%02m-%02H:%02M:%02S") << endl;
    ...
    for (unsigned i = 0; i < exsats.size(); i++)
    {
        ...
        {
            cerr << "\"" << exsats[i] << "\" is not a valid satellite designation" << endl;
...
void RinEditNav ::process()
{
    ...
    while (!done)
    {
        ...
        if ((omi->first >= minTime) && (omi->first < maxTime) && (exclSats.count(dataMap[sp].sat) == 0))
        {
            ...
            if (uniques.count(key) == 0)
            {
                ...
                if ((tbegin == CommonTime::BEGINNING_OF_TIME) || (tbegin > key.beginFit()))
                {
                    tbegin = key.beginFit();
                }
                if ((tend == CommonTime::BEGINNING_OF_TIME) || (tend < key.endFit()))
                {
                    tend = key.endFit();
                }
                ...
                for (auto o3i : output3Map)
                {
                    ...
                    if ((o3i.first == SatelliteSystem::Unknown) || (o3i.first == dataMap[sp].sat.system))
                    {
                ...
                {
                    // Treat "Unknown" as a wildcard here
                    if ((o2i.first == SatelliteSystem::Unknown) || (o2i.first == dataMap[sp].sat.system))
                    {
    ...
    cout << "Nav table for all satellites has " << uniques.size() << " entries; Valid time span is "
         << printTime(tbegin, "%Y/%02m/%02d %02H:%02M:%02S %P") << " to "
         << printTime(tend, "%Y/%02m/%02d %02H:%02M:%02S %P") << endl;
}

bool RinEditNav ::parseOut(const CommandOptionWithAnyArg &opt, OutputMap &outMap, set<string> &outFNs)
{
    ...
    for (unsigned i = 0; i < outs.size(); i++)
    {
        ...
        {
            cerr << "Can't open \"" << sysFn[1] << "\" for output more than once" << endl;
            exitCode = BasicFramework::OPTION_ERROR;
            return false;
        }
...
void RinEditNav ::writeHeaders()
{
    ...
    merged.valid = Rinex3NavHeader::validVersion | Rinex3NavHeader::validRunBy | Rinex3NavHeader::validEoH;
    ...
    {
        if (firstRec && (dmi.first->header.valid & Rinex3NavHeader::validLeapSeconds))
        {
        ...
        for (const auto &i : dmi.first->header.mapIonoCorr)
        {
            ...
            merged.valid |=
                (dmi.first->header.valid & (Rinex3NavHeader::validIonoCorrGPS | Rinex3NavHeader::validIonoCorrGal));
        }
    ...
    for (auto &i : output3Map)
    {
        ...
        merged.fileSys = string(1, sat.systemChar()) + ": " + sat.systemString();
        (*i.second) << merged;
    }
