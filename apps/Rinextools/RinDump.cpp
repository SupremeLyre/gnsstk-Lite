 ...
 * \dictentry{\argarg{DATA},is the quantity to be output\, either raw data\, satellite-dependent data or linear
 * combinations\, as given by one of the following tags:}
 * \enddictable
 *
 ...
 * \dicdef{System(s) (GNSSs) \argarg{S}=S[,S], where S=RINEX system [repeatable] (GPS,GLO). RINEX systems are
 * GPS,GLO,GAL,GEO|SBAS,BDS,QZS}
 * \dicterm{\--code \argarg{S:C}}
 * \dicdef{System \argarg{S} allowed tracking codes \argarg{C}, in order [see --typehelp] [repeatable] (). Defaults:
 * GPS:PYMNIQSLXWCN, GLO:PC, GAL:ABCIQXZ, GEO:CIQX, BDS:IQX, QZS:CSLXZ}
 ...
 * \dicdef{Trop model \argarg{M} [one of Zero,Black,Saas,NewB,Neill,GG,GGHt with optional weather T(C),P(mb),RH(%)]
 * (NewB,20.0,1013.0,50.0)}
...
class LinCom
{
  public:
    ...
    double Compute(const RinexSatID sat, Rinex3ObsHeader &Rhead, const vector<RinexDatum> &vrdata);
...
double getObsData(string tag, RinexSatID sat, Rinex3ObsHeader &Rhead, const vector<RinexDatum> &vrdata);
...
#include "NewNavInit.h"
    ...
    try
    {
        ...
        if (iret < 0)
        {
            if (iret == -2)
            {
                LOG(INFO) << "Error - Memory error.";
            }
            else if (iret == -3)
            {
                LOG(INFO) << "Error - invalid command line";
            }
            else if (iret == -4)
            {
                LOG(INFO) << "Error - log file could not be opened";
            }
            else if (iret == -5)
                ; //{ LOG(INFO) << "Error - invalid input"; }
            else
            {
                LOG(INFO) << "Error - some other error code";
            }
            return iret;
        }
...
int Initialize(string &errors)
{
    try
    {
        ...
        // run out of memory.
        MultiFormatNavDataFactory *ndfp = dynamic_cast<MultiFormatNavDataFactory *>(C.ndfp.get());
      ...
      //      * C.SP3EphStore.getPositionTimeStep() + 1.);
            ...
            if (C.debug > -1)
            {
                C.navLib.dump(LOGstrm, (C.debug > 6 ? DumpDetail::Full : DumpDetail::Brief));
            }
            ...
            set<SatID> sats(C.navLib.getIndexSet(CommonTime::BEGINNING_OF_TIME, CommonTime::END_OF_TIME));
            // loop over sats
            for (const auto &sat : sats)
            {
                ...
                if (sat.system == SatelliteSystem::Glonass)
                {
                    ...
                    if (it == C.GLOfreqChan.end() && sat.system == SatelliteSystem::Glonass)
                    {
        ...
        if (ndfp->size() == 0 && C.InputNavFiles.size() > 0)
        {
            try
            {
                for (nread = 0, nfile = 0; nfile < C.InputNavFiles.size(); nfile++)
                {
                    ...
                    {
                        LOG(WARNING) << "Unable to load \"" << C.InputNavFiles[nfile] << "\"" << endl;
                        continue;
                    }
        ...
        if (C.decimate > 0.0)
        {
            ...
            C.decTime = static_cast<CommonTime>(GPSWeekSecond(static_cast<GPSWeekSecond>(C.decTime).week, 0.0));
        }
        ...
        if (!C.haveRef)
        {
            ...
            {
                string tag(C.InputTags[i]);
                if (tag == "RNG" || tag == "ELE" || tag == "AZI" || tag == "TRP" || tag == "LAT" || tag == "LON")
                {
                    ossE << "Error : Rx-dependent data " << tag << " requires --ref input\n";
                    isValid = false;
                }
                if (tag == "REL" || tag == "SVX" || tag == "SVY" || tag == "SVZ" || tag == "SCL" || tag == "SVA" ||
                    tag == "SVO" || tag == "SVH")
                    needPos = true;
            }
            ...
            if (needPos)
                C.knownPos = Position(1., 2., 3., Position::Cartesian);
        ...
        LOG(DEBUG) << (C.haveObs ? "" : "not ") << "haveObs " << (C.haveNonObs ? "" : "not ") << "haveNonObs "
                   << (C.haveRCL ? "" : "not ") << "haveRCL " << (C.havePOS ? "" : "not ") << "havePOS "
        ...
        if (!C.noHeader)
        {
            ...
            oss << "# Input file(s)";
            for (i = 0; i < C.InputObsFiles.size(); i++)
                oss << " " << C.InputObsFiles[i];
            LOG(INFO) << oss.str();
            oss.str("");
            ...
            if (C.knownPos.getCoordinateSystem() != Position::Unknown)
            {
                LOG(INFO) << "# Refpos " << C.knownPos.printf("XYZ(m): %.3x %.3y %.3z = LLH(ddm): %.9AN %.9LE %.3h");
            }
...
void Configuration::SetDefaults(void)
{
    ...
    debLimit["NL"] = 0.0;
    debLimit0["NL"] = false;
    LinComTags.push_back("WLC");
    debLimit["WLC"] = 10.;
    debLimit0["WLC"] = true;
    LinComTags.push_back("NLC");
    debLimit["NLC"] = 10.;
    debLimit0["NLC"] = true;
    ...
    for (size_t i = 0; i < validSys.size(); i++)
    {
        ...
        {
            LOG(WARNING) << "Warning - system \"" << validSys[i] << "\" does not have 3-char entry in map1to3Sys";
        }
        else
        {
...
int Configuration::ProcessUserInput(int argc, char **argv)
{
    ...
    if (typehelp)
    {
            ...
            for (CarrierBand carrierBand : CarrierBandIterator())
            {
                ...
                for (TrackingCode trackCode : TrackingCodeIterator())
                {
                    ...
                    for (ObservationType obsType : ObservationTypeIterator())
                    {
                        ...
                        try
                        {
                            string tag(string(1, syss[s]) + string(1, RinexObsID::ot2char[obsType]) +
                            ...
                            if (name.find("Unknown") != string::npos || name.find("undefined") != string::npos ||
                                name.find("Any") != string::npos || !isValidRinexObsID(tag))
                            {
                                continue;
                            }
                            ...
                            if (find(goodtags.begin(), goodtags.end(), tag) == goodtags.end())
                            {
                                ...
                                if ((codedesc.find("GPS") == 0) || (codedesc.find("GLO") == 0) ||
                                    (codedesc.find("GAL") == 0) || (codedesc.find("BDS") == 0))
                                {
                                    codedesc.erase(0, 3);
                                }
                                else if ((codedesc.find("SBAS") == 0) || (codedesc.find("QZSS") == 0))
                                {
                                    codedesc.erase(0, 4);
                                }
        ...
        LOG(INFO) << " Sys " << leftJustify("Freq", len2) << " " << center("Track", len3) << " "
                  << center("Pseudo-", len4) << " " << center("Carrier", len4) << " " << center("Doppler", len4) << " "
                  << center("Signal", len4) << " " << center("Ionosph.", len4) << " " << center("Channel", len4);
        LOG(INFO) << "     " << leftJustify("    ", len2) << " " << center("     ", len3) << " "
                  << center("range", len4) << " " << center("phase", len4) << " " << center("   ", len4) << " "
                  << center("Strength", len4) << " " << center("Delay", len4) << " " << center("Number", len4);
        ...
        for (size_t i = 0; i < syss.size(); ++i)
        {
            ...
            for (jt = it->second.begin(); jt != it->second.end(); ++jt)
            {
                for (kt = jt->second.begin(); kt != jt->second.end(); ++kt)
                {
                    ...
                    {
                        char ot(obsTypes[j]);
                        LOGstrm << " " << center((kt->second[ot] == "" ? "----" : kt->second[ot]), len4);
                    }
                    LOGstrm << endl;
                }
            }
...
string Configuration::BuildCommandLine(void)
{
    ...
    string PrgmDesc = " Program " + PrgmName +
                      ...
                      " Usage: " +
                      PrgmName +
                      " [options] <file> [<sat>] <data>\n"
                      "      E.g. " +
                      PrgmName +
                      " test2820.11o G17 C1C L1C R09 ELE AZI\n"
    ...
    opts.Add(0, "sat", "sat", true, false, &InputSats, "", "sat is a RINEX satellite id (see above)");
    opts.Add(0, "dat", "data", true, false, &InputTags, "", "data (raw,combination, or other) to dump (see above)");
    opts.Add(0, "combo", "spec", true, false, &InputCombos, "",
             "custom linear combination; spec is co[co[co]]; see --combohelp");
    opts.Add(0, "header", "", false, false, &dumpHeader, "", "Dump RINEX header");

    opts.Add(0, "sys", "s", true, false, &InputSyss, "# Define or restrict values used in --dat and --combo",
    ...
    opts.Add(0, "obspath", "p", false, false, &Obspath,
             "# Paths of input files (optional):", "Path of input RINEX observation file(s)");
    ...
    opts.Add(0, "debias0", "type", true, false, &typeLimit0, "", "Toggle initial debias of data <type> ()");
    opts.Add(0, "elevlim", "lim", false, false, &elevlimit, "",
             "Limit output to data with elevation angle > lim degrees [ELE req'd]");

    string rvstr("# Rinex version (current, latest version is " + asString(currentVersion, 2) + "):");
    opts.Add(0, "currentRinex", "", false, false, &doCurrRversion, rvstr,
             "Process in current, not header, RINEX version");
    opts.Add(0, "RinexVer", "V", false, false, &Rversion, "", "Process in RINEX version V (default is header.version)");
    ...
    opts.Add(0, "ionoht", "ht", false, false, &IonoHt, "", "Ionospheric height in kilometers [for VI, LAT, LON]");

    opts.Add(0, "timefmt", "fmt", false, false, &userfmt,
             "# Output:", "Format for time tags (see GNSSTK::Epoch::printf) in output");
    opts.Add(0, "headless", "", false, false, &noHeader, "",
             "Turn off printing of headers and no-eph-warnings in output");
    opts.Add(0, "TECU", "", false, false, &doTECU, "", "Compute iono delay (SI,VI) in TEC units (else meters)");
    opts.Add(0, "verbose", "", false, false, &verbose, "", "Print extra output information");
    opts.Add(0, "debug", "", false, false, &debug, "", "Print debug output at level 0 [debug<n> for level n=1-7]");
    opts.Add(0, "help", "", false, false, &help, "", "Print this syntax page, and quit");
    opts.Add(0, "typehelp", "", false, false, &typehelp, "", "Print all valid RINEX obs IDs, and quit");
...
int Configuration::ExtraProcessing(string &errors, string &extras)
{
    ...
    for (i = 0; i < GLOfreqStrs.size(); i++)
    {
        ...
        if (fld.size() != 2)
            oss << "Error - invalid GLO sat:channel pair in --GLOfreq: " << GLOfreqStrs[i] << endl;
        else
        {
    ...
    {
        if (InputSyss[i] == string("SBAS"))
            InputSyss[i] = string("GEO");
    ...
    { // otherwise the defaults are in place
        for (i = 0; i < InputCodes.size(); i++)
        {
            ...
            for (size_t j = 0; j < fld[1].size(); j++)
                if (msg.find(fld[1][j], 0) == string::npos)
                {
                    ok = false;
                    break;
                }
            if (ok)
                mapSysCodes[fld[0]] = fld[1];
        }
    }
...
int ProcessFiles(void)
{
    try
    {
        ...
        for (nfiles = 0, nfile = 0; nfile < C.InputObsFiles.size(); nfile++)
        {
            ...
            if (C.useVersion == 0.0)
                C.useVersion = Rhead.version;
            LOG(INFO) << "# RINEX version: file " << fixed << setprecision(2) << Rhead.version << " / output "
                      << C.useVersion;

            if (C.debug > -1)
            {
                LOG(DEBUG) << "Input header (ver " << fixed << setprecision(2) << C.useVersion << ") for RINEX file "
                           << filename;
            ...
            if (!C.noHeader)
            {
                ...
                if (C.dumpHeader)
                {
                    LOG(INFO) << "# Header dump follows, RINEX ver. (file: " << fixed << setprecision(2)
                ...
                for (kt = Rhead.mapObsTypes.begin(); kt != Rhead.mapObsTypes.end(); kt++)
                {
                    ...
                    bool ok = false;
                    for (i = 0; i < C.InputSats.size(); i++)
                        if (C.InputSats[i].system == sat.system)
                        {
                            ok = true;
                            break;
                        }
            ...
            while (1)
            {
                ...
                {
                    LOG(ERROR) << " Error : Failed to read obs data (Exception " << e.getText(0) << "); dump follows.";
                ...
                // LOG(INFO) << "";
                LOG(DEBUG) << " Read RINEX data: flag " << Rdata.epochFlag << ", timetag "
                           << printTime(Rdata.time, C.longfmt);

                // stay within time limits
                if (Rdata.time < C.beginTime)
                {
                    LOG(DEBUG) << " RINEX data timetag " << printTime(Rdata.time, C.longfmt) << " is before begin time "
                               << printTime(C.beginTime, C.longfmt);
                    continue;
                }
                if (Rdata.time > C.endTime)
                {
                    LOG(DEBUG) << " RINEX data timetag " << printTime(Rdata.time, C.longfmt) << " is after end time "
                               << printTime(C.endTime, C.longfmt);
                    break;
                }
                ...
                if (C.decimate > 0.0)
                {
                    if (C.decTime == CommonTime::BEGINNING_OF_TIME)
                        C.decTime = Rdata.time;
                ...
                if (Rdata.epochFlag > 1)
                {
                    ...
                    for (j = 0; j < Rdata.auxHeader.commentList.size(); j++)
                    {
                        ...
                        if (fld[0] == string("XYZT"))
                        { // R2
                            if (C.knownPos.getCoordinateSystem() != Position::Unknown)
                            {
                                oss << "    NA    " << fixed << setprecision(3) << " " << setw(8)
                                    << (asDouble(fld[1]) - C.knownPos.X()) << " " << setw(8)
                                    << (asDouble(fld[2]) - C.knownPos.Y()) << " " << setw(8)
                                    << (asDouble(fld[3]) - C.knownPos.Z()) << " GPS " << fld[4];
                            }
                            else
                            {
                                oss << "    NA    "
                                    << " " << fld[1] << " " << fld[2] << " " << fld[3] << " GPS " << fld[4];
                            }
                        }
                        else if (fld[0] == string("DIAG"))
                        { // R2
                        ...
                        else if (fld[0] == string("XYZ"))
                        { // R3
                            if (C.knownPos.getCoordinateSystem() != Position::Unknown)
                            {
                                oss << " " << fld[4] << fixed << setprecision(3) << " " << setw(8)
                                    << (asDouble(fld[1]) - C.knownPos.X()) << " " << setw(8)
                                    << (asDouble(fld[2]) - C.knownPos.Y()) << " " << setw(8)
                                    << (asDouble(fld[3]) - C.knownPos.Z()) << " GPS " << fld[4];
                            }
                            ...
                            {
                                oss << " " << fld[4] << " " << fld[1] << " " << fld[2] << " " << fld[3];
                            }
                        }
                        else if (fld[0] == string("CLK"))
                        { // R3
                            for (k = 1; k < fld.size() - 1; k++)
                                oss << " " << fld[k];
                        }
                        else if (fld[0].substr(0, 3) == string("DIA"))
                        { // R3
                            oss << " " << fld[0].substr(3);
                            for (k = 1; k < fld.size() - 1; k++)
                                oss << " " << fld[k];
                ...
                else
                {
                    if (C.haveObs || C.haveNonObs || C.haveCombo)
                    {
                        ...
                        if (C.haveRCL)
                            LOG(INFO) << line << " RCL " << fixed << setprecision(3) << setw(width)
                                      << Rdata.clockOffset * C_MPS;
                        ...
                        for (it = Rdata.obs.begin(); it != Rdata.obs.end(); ++it)
                        {
                            ...
                            // output this sat?
                            if (C.InputSats.size() > 0 &&
                                find(C.InputSats.begin(), C.InputSats.end(), sat) == C.InputSats.end())
                            {
                                // check for all sats of this system
                                RinexSatID tsat(-1, sat.system);
                                if (find(C.InputSats.begin(), C.InputSats.end(), tsat) == C.InputSats.end())
                                {
                            ...
                            if (badele)
                                continue; // don't compute lincombos due to removeBias
...
double getObsData(string tag, RinexSatID sat, Rinex3ObsHeader &Rhead, const vector<RinexDatum> &vrdata)
{
    try
    {
        ...
        if (jt != Rhead.mapObsTypes[sys].end())
        { // its in the header
...
double getNonObsData(string tag, RinexSatID sat, const CommonTime &time)
{
    try
    {
        ...
        if (C.mapSatCER.find(sat) == C.mapSatCER.end())
        {
            ...
            {
                if (!C.noHeader)
                    LOG(VERBOSE) << "# Warning - no ephemeris for (" << tag << ") sat " << sat << " at time "
                                 << printTime(time, C.longfmt);
                return data;
            }
        }
...
bool LinCom::ParseAndSave(const string &lab, bool save)
{
    ...
    else if (tag == string("SI") || tag == string("VI") || tag == string("IF") || tag == string("GF") ||
             tag == string("WL") || tag == string("NL"))
    { // tag:t:ij
        string type;
        if (fld[1] == "C" || fld[1] == "R")
            type = string("C");
        else if (fld[1] == "L" || fld[1] == "P")
            type = string("L");
        else
            return false;
        ...
        { // loop over systems
            ...
            if (tag == string("SI") || tag == string("VI"))
            { // iono delay
                double alpha(getAlpha(sat.system, n1, n2));
                // convert to TECU
                double TECUperM(1.0);
                if (C.doTECU)
                {
                    ...
                    {
                        static const double GLOL1((L1_FREQ_GLO + C.GLOfreqChan.count(sat) * L1_FREQ_STEP_GLO) * 1.e-8);
            ...
            else if (tag == string("GF"))
            { // geo-free
                sysConsts[sys].push_back(1.0);
                sysConsts[sys].push_back(-1.0);
            }
            else if (tag == string("WL"))
            { // widelane
        ...
        if (sysConsts.size() == 0 || sysObsids.size() == 0)
            return false;
    }

    else if (tag == string("WLC"))
    { // MW:ij
        if (fld.size() == 1)
        { // MW alone : ij from --freq
            for (i = 0; i < C.InputFreqs.size(); i++)
            {
                sysConsts.clear();
                sysObsids.clear();
    ...
    else if (tag == string("NLC"))
    { // NLC:ij
        if (fld.size() == 1)
        { // NLC alone : ij from --freq
            for (i = 0; i < C.InputFreqs.size(); i++)
            {
                sysConsts.clear();
                sysObsids.clear();
    ...
    bool ok(true);
    for (map<string, vector<double>>::const_iterator it = sysConsts.begin(); it != sysConsts.end(); ++it)
    {
        sys = it->first;
        LOG(DEBUG2) << "Final sys " << sys << " consts " << sysConsts[sys].size() << " obsids "
                    << sysObsids[sys].size();
        if (sysConsts[sys].size() != sysObsids[sys].size())
        {
            ok = false;
            break;
        }
    ...
    if (save)
    {
        ...
        LOG(DEBUG2) << "Parse dont save >" << label << "<";

    return true;
}

//------------------------------------------------------------------------------------
double LinCom::Compute(const RinexSatID sat, Rinex3ObsHeader &Rhead, const vector<RinexDatum> &vrdata)
{
    Configuration &C(Configuration::Instance());
    ...
    {
        LOG(DEBUG2) << msg << " System not found";
        return 0.0;
    }
    ...
    for (size_t i = 0; i < sysConsts[sys1].size(); i++)
    {
        ...
        if (obsid.size() == 4 && obsid[0] != sys1[0])
        ...
        if (obsid.size() == 3)
            obsid = sys1 + obsid; // add system char to obsid
        if (!isValidRinexObsID(obsid))
        { // TD do this earlier?
        ...
        for (size_t k = 0; k < allObsIDs.size(); k++)
        {
            ...
            jt = find(Rhead.mapObsTypes[sys1].begin(), Rhead.mapObsTypes[sys1].end(), allObsIDs[k]);
        ...
        LOG(DEBUG2) << msg << msg2 << " ok, sum: " << fixed << setprecision(4) << sysConsts[sys1][i] << " * " << data;
...
bool LinCom::removeBias(const RinexSatID &sat)
{
    ...
    if (biases.find(sat) == biases.end())
    { // sat not found - initial point
...
{
    ostringstream oss;
    oss << "Dump LC " << lc.label << " freq " << lc.f1 << "," << lc.f2 << " limit " << fixed << setprecision(3)
        << lc.limit << " limit0 " << (lc.limit0 ? "T" : "F");
    ...
    while (it != lc.sysConsts.end())
    {
        ...
        for (size_t i = 0; i < it->second.size(); ++i)
            oss << (i == 0 ? " " : " + ") << lc.sysConsts[sys][i] << " * " << lc.sysObsids[sys][i];
        ++it;
    }
    os << oss.str();
    return os;
