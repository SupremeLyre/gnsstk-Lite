...
class Configuration : public Singleton<Configuration>
{
  ...
  private:
    ...
    void SetDefaults(void) noexcept
    {
        ...
        userfmt = gpsfmt;
        help = verbose = brief = nohead = notab = gpstime = sorttime = vistab = dogaps = doms = ycode = quiet = false;
        debug = -1;
        dt = -1.0;
  ...
  public:
    ...
    bool help, verbose, brief, nohead, notab, gpstime, sorttime, dogaps, doms, vistab, ycode, quiet;
    int debug, vres;
    double dt;
...
string Configuration::BuildCommandLine(void) noexcept
{
    ...
    opts.Add(0, "obs", "file", true, false, &InputObsFiles,
             "# Required input [file names may appear w/o --obs if unambiguous]", "Input RINEX observation file name");
    ...
    opts.Add(0, "exSat", "sat", true, false, &exSats, "", "Exclude satellite (or system) <sat> e.g. G24,R");
    opts.Add(0, "onlySat", "sat", true, false, &onlySats, "", "Include ONLY satellites (or systems) <sat> e.g. G,R");

    string rvstr("# Rinex version (current, latest version is " + asString(Rinex3ObsBase::currentVersion, 2) + "):");
    opts.Add(0, "currentRinex", "", false, false, &doCurrRversion, rvstr,
             "Output in current, not header, RINEX version");
    opts.Add(0, "RinexVer", "V", false, false, &Rversion, "", "Output in RINEX version V (default is header.version)");

    opts.Add(0, "timefmt", "fmt", false, false, &userfmt,
             "# Output:", "Format for time tags (see GNSSTK::Epoch::printf) in output");
    opts.Add('b', "brief", "", false, false, &brief, "", "Produce a brief output");
    opts.Add(0, "nohead", "", false, false, &nohead, "", "Omit header from output");
    opts.Add(0, "notable", "", false, false, &notab, "", "Omit sat/obs table from output");
    opts.Add(0, "dt", "sec", false, false, &dt, "",
             "Nominal time step of data (sec); required only for gaps and millisec");
    opts.Add(0, "milli", "", false, false, &doms, "", "Find millisecond clock adjusts; req's --dt");
    opts.Add(0, "gaps", "", false, false, &dogaps, "", "Print a table of gaps in the data; req's --dt");
    opts.Add(0, "vis", "n", false, false, &vres, "",
             "Print graphical visibility, resolution <n> [n~20 @ 30s; req's --gaps]");
    opts.Add(0, "vtab", "", false, false, &vistab, "", "Print tabular visibility [req's --gaps and --vis]");

    opts.Add(0, "ycode", "", false, false, &ycode, "# Other:", "Assume v2.11 P mean Y");
    opts.Add('q', "quiet", "", false, false, &quiet, "", "Make output a little quieter");
    ...
    if (exSats.size() > 0 && onlySats.size() > 0)
    {
        for (i = 0; i < onlySats.size(); i++)
        {
            ...
            if ((find(exSats.begin(), exSats.end(), sat) != exSats.end()) ||
                ...
                oss << "Error : satellite " << asString(sat) << " found in both --exSat and --onlySat" << endl;
        }
    }
...
int ProcessFiles()
{
    try
    {
        ...
        if (C.doCurrRversion)
            outputVersion = Rinex3ObsBase::currentVersion;
        ...
        for (nfiles = 0, nfile = 0; nfile < C.InputObsFiles.size(); nfile++)
        {
            ...
            if (C.quiet)
            {
                ...
                LOG(INFO) << "+++++++++++++ " << C.PrgmName << " summary of Rinex obs file " << choppedFN
                          << " +++++++++++++";
            }
            else if (!C.brief)
            {
                LOG(INFO) << "+++++++++++++ " << C.PrgmName << " summary of Rinex obs file " << filename
                          << " +++++++++++++";
            }
            ...
            if (C.brief)
            {
                if (nfile > 0)
                    LOG(INFO) << "";
                LOG(INFO) << "File name: " << filename << " (RINEX ver. " << Rhead.version << ")";
                LOG(INFO) << "Marker name: " << Rhead.markerName;
                LOG(INFO) << "Antenna type: " << Rhead.antType;
                LOG(INFO) << "Position (XYZ,m) : " << fixed << setprecision(4) << Rhead.antennaPosition << ".";
                LOG(INFO) << "Antenna offset (UEN,m) : " << fixed << setprecision(4) << Rhead.antennaDeltaHEN << ".";
            }
            else if (!C.nohead)
            {
                ...
                if (outputVersion == 0.0)
                    ...
                    LOG(INFO) << " (Header has version " << Rhead.version << "; output as version " << outputVersion
                              << ")";
            ...
            if (!Rhead.isValid())
            {
                ...
                if (C.quiet)
                {
                    std::string choppedFN(filename);
                    choppedFN.erase(0, 1 + filename.find_last_of("/\\"));
                    LOG(INFO) << "\n+++++++++++++ End of RinSum summary of " << choppedFN << " +++++++++++++";
                }
                else if (!C.brief)
                {
            ...
            for (sit = Rhead.mapObsTypes.begin(); sit != Rhead.mapObsTypes.end(); ++sit)
            {
                ...
                LOG(DEBUG) << "GNSS " << (sit->first) << " is present with " << (sit->second).size()
                           << " observations...";
            ...
            if (C.doms)
            {
                ...
                for (i = 0; i < msots.size(); i++)
                    LOG(DEBUG) << " " << msots[i] << fixed << setprecision(6) << " " << waves[i];
            }
            ...
            while (1)
            {
                ...
                {
                    if (nepochs == 0 && Rdata.time.getTimeSystem() != Rhead.lastObs.getTimeSystem())
                    {
                ...
                if (prevObsTime != CommonTime::BEGINNING_OF_TIME && Rdata.time - prevObsTime < 1.e-3)
                {
                    // save it
                    if (!cacheon)
                    {
                ...
                for (it = Rdata.obs.begin(); it != Rdata.obs.end(); ++it)
                {
                    ...
                    // is sat included?
                    if (C.onlySats.size() > 0 && find(C.onlySats.begin(), C.onlySats.end(), sat) == C.onlySats.end() &&
                        find(C.onlySats.begin(), C.onlySats.end(), RinexSatID(-1, sat.system)) == C.onlySats.end())
                        continue;
                    ...
                    else if (find(C.exSats.begin(), C.exSats.end(), RinexSatID(-1, sat.system)) != C.exSats.end())
                        continue;
                    ...
                    for (size_t index = 0; index != vecData.size(); index++)
                    {
                        ...
                        if (C.debug > -1)
                            oss << fixed << setprecision(3) << " " << asString(Rhead.mapObsTypes[sysStr][index]) << " "
                                << setw(13) << vecData[index].data << " " << vecData[index].lli << " "
                                << vecData[index].ssi;
                ...
                if (prevObsTime != CommonTime::BEGINNING_OF_TIME)
                {
                    ...
                    {
                        LOG(WARNING) << "Warning - repeated time tag at " << printTime(lastObsTime, C.longfmt);
                    }
                    else
                    {
                        LOG(WARNING) << "Warning - time tags out of order: " << printTime(prevObsTime, C.longfmt)
            ...
            if (nepochs <= 0)
            {
                LOG(INFO) << "File " << filename << " : no data found. Are time limits wrong?";
                continue;
            }
            ...
            // Summary info -----------------------------------------------------
            LOG(INFO) << "Computed interval " << fixed << setw(5) << setprecision(2) << dt << " seconds.";
            ...
            delta.day = remainder / 86400;
            remainder %= 86400;
            delta.hour = remainder / 3600;
            remainder %= 3600;
            delta.minute = remainder / 60;
            remainder %= 60;
            delta.second = remainder;
            if (delta.day > 0)
                oss << delta.day << "d ";

            LOG(INFO) << oss.str() << delta.hour << "h " << delta.minute << "m " << delta.second << "s = " << secs
                      << " seconds.";
            ...
            LOG(INFO) << "There were " << nepochs << " epochs (" << fixed << setprecision(2)
                      << double(nepochs * 100) / i << "% of " << i << " possible epochs in this timespan) and "
            ...
            else
            {
                ...
                for (; sit != Rhead.mapObsTypes.end(); ++sit)
                {
                    ...
                    for (i = 0, k = 0; k < vectot.size(); k++)
                        i += vectot[k];
                    if (i == 0)
                        continue; // no, skip it

                    oss.str("");
                    oss << "System " << RinexSatID(sysCode).systemString3() << " Obs types(" << vec.size() << "): ";
            ...
            if (C.dogaps)
            {
                ...
                oss << "GAP ALL " << setw(5) << C.gapcount[0] << " - " << setw(5) << C.gapcount[k];
                ...
                for (tabIt = table.begin(); tabIt != table.end(); ++tabIt)
                {
                    ...
                    oss << "GAP " << tabIt->sat << " " << setw(5) << tabIt->gapcount[0] << " - " << setw(5)
                        << tabIt->gapcount[k];
                    // NB DO NOT make ii size_t
                    for (ii = 1; ii <= k - 2; ii += 2)
                ...
                // print for timetags = all sats
                k = C.gapcount.size() - 1; // size() is at least 2
                oss << "GAP ALL " << fixed << setprecision(1) << setw(8) << t + d * C.gapcount[0] << " - " << setw(8)
                    << t + d * C.gapcount[k];
                // NB DO NOT make ii size_t
                for (ii = 1; ii <= k - 2; ii += 2)
                ...
                {
                    k = tabIt->gapcount.size() - 1;
                    oss << "GAP " << tabIt->sat << " " << fixed << setprecision(1) << setw(8)
                        << t + d * tabIt->gapcount[0] << " - " << setw(8) << t + d * tabIt->gapcount[k];
                    // NB DO NOT make ii size_t
                    for (ii = 1; ii <= k - 2; ii += 2)
                ...
                if (C.vres > 0)
                {
                    ...
                    if (C.vistab)
                    {
                        LOG(INFO) << "\n Visibility Timetable - resolution is " << dn << " epochs = " << dn * C.dt
                                  << " seconds.\n"
                        ...
                        while (vt != vtab.end())
                        {
                            ...
                            oss.str("");
                            oss << "VTAB " << setw(4) << printTime(ttag, C.longfmt) << " " << setw(4) << k << " "
                                << setw(5) << k * C.vres << " " << setw(3) << vt->first - k << fixed << setprecision(1)
                                << " " << setw(8) << (vt->first - k) * C.dt * dn << " " << setw(5) << sats.size();
                        ...
                        LOG(INFO) << "VTAB " << setw(4) << printTime(ttag, C.longfmt) << " " << setw(4) << k << " "
                                  << setw(5) << int(0.5 + (ttag - firstObsTime) / C.dt) << " END";
                    }
            ...
            if (cache.size() > 0)
            {
                for (i = 0; i < cache.size(); i++)
                    LOG(INFO) << " Warning: " << setw(4) << cache[i].size() << " data records following epoch "
                              << printTime(cachetime[i], C.calfmt) << " are out of time order,"
                              << "\n         with epochs " << printTime(cache[i][0].time, C.calfmt) << " to "
                              << printTime(cache[i][cache[i].size() - 1].time, C.calfmt) << endl;
            }

            if ((Rhead.valid & Rinex3ObsHeader::validInterval) && fabs(dt - Rhead.interval) > 1.e-3)
                LOG(INFO) << " Warning - Computed interval is " << setprecision(2) << dt
                          << " sec, while input header has " << setprecision(2) << Rhead.interval << " sec.";

            if (C.beginTime == CommonTime::BEGINNING_OF_TIME && fabs(firstObsTime - Rhead.firstObs) > 1.e-8)
                LOG(INFO) << " Warning - Computed first time does not agree with header";

            if (C.endTime == CommonTime::END_OF_TIME && (Rhead.valid & Rinex3ObsHeader::validLastTime) &&
            ...
            for (sit = Rhead.mapObsTypes.begin(); sit != Rhead.mapObsTypes.end(); ++sit)
            {
                ...
                {
                    RinexSatID sat(sit->first);
                    if ((find(C.exSats.begin(), C.exSats.end(), RinexSatID(-1, sat.system)) ==
                         C.exSats.end())                                                        // sys not excluded
                        && (C.onlySats.size() > 0 && find(C.onlySats.begin(), C.onlySats.end(), // only system
                                                          RinexSatID(-1, sat.system)) != C.onlySats.end()))
                        LOG(INFO) << " Warning - System " << sit->first << " = " << sat.systemString()
                                  << " should be removed from the header.";
                }
            }
            ...
            for (sit = Rhead.mapObsTypes.begin(); sit != Rhead.mapObsTypes.end(); ++sit)
            {
                ...
                for (k = 0; k < totvec.size(); k++)
                {
                    if (totvec[k] == 0)
                    {
                        ...
                        LOG(INFO) << " Warning - Obs type " << sit->first << asString((sit->second)[k]) << " ("
                                  << sat.systemString() << " " << asString((sit->second)[k]) << tag
