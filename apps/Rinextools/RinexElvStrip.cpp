 ...
 * <b>rinexelvstrip</b> <b>-i</b>&nbsp;\argarg{ARG} <b>-o</b>&nbsp;\argarg{ARG} <b>-n</b>&nbsp;\argarg{ARG}
 * <b>[-d</b><b>]</b> <b>[-v</b><b>]</b> <b>[-p</b>&nbsp;\argarg{POSITION}<b>]</b> <b>[-e</b>&nbsp;\argarg{ARG}<b>]</b>
 *
...
RinexElvStrip::RinexElvStrip(const string &app) noexcept
    : BasicFramework(app, "Strip low elevation data from observations."),
      inputArg('i', "input", "Input RINEX obs file", true), outputArg('o', "output", "Output RINEX obs file", true),
      ...
      elvArg('e', "elevation", "elevation strip angle (default = 10 degrees)")
{
    ...
    navArg.setDescription("Input nav file(s). Can be " + ndfp->getFactoryFormats() + ".");
...
void RinexElvStrip::process()
{
    ...
    if (!oros)
    {
        cerr << "Can't open \"" << outputArg.getValue()[0] << "\" for output" << endl;
        exitCode = 1;
        return;
    }
    if (!iros)
    {
        cerr << "Can't open \"" << inputArg.getValue()[0] << "\" for input" << endl;
    ...
    while (iros >> irod)
    {
        ...
        for (RinexObsData::RinexSatMap::iterator i = irod.obs.begin(); i != irod.obs.end(); i++)
        {
            try
            {
                Xvt xvt;
                if (!navLib.getXvt(NavSatelliteID(i->first), irod.time, xvt, SVHealth::Any, NavValidityType::ValidOnly,
                                   NavSearchOrder::Nearest))
                {
                    cerr << "Couldn't get XVT for " << i->first << " at "
                         << printTime(irod.time, "%02m/%02d/%02Y %02H:%02M:%03.1f") << endl;
                    continue;
                }
                ...
                if (elv >= elvmask)
                    ...
                    cout << "Stripped PRN " << i->first.id << " (elv = " << setprecision(2) << elv << ") at "
                         << printTime(irod.time, "%02m/%02d/%02Y %02H:%02M:%03.1f") << endl;
            }
