...
class Configuration : public Singleton<Configuration>
{
  ...
  public:
    ...
    string messHDp, messHDr, messHDo, messHDa, messHDx, messHDm, messHDn, messHDt, messHDh, messHDj, messHDk, messHDl,
        messHDs;
...
int initialize(string &errors)
{
    try
    {
        ...
        if (C.decimate > 0.0)
        {
            ...
            LOG(DEBUG) << "Decimate, with final decimate ref time " << printTime(C.decTime, C.longfmt) << " and step "
                       << C.decimate;
        }
    ...
    try
    {
        ...
        for (nfiles = 0, nfile = 0; nfile < C.messIF.size(); nfile++)
        {
            ...
            for (kt = Rhead.mapObsTypes.begin(); kt != Rhead.mapObsTypes.end(); kt++)
            {
                ...
                oss.str("");
                oss << "# Header ObsIDs " << sat.systemString3() << " (" << kt->second.size() << "):";
            ...
            for (it = C.vecCmds.begin(); it != C.vecCmds.end(); it++)
            {
                ...
                if (it->type == EditCmd::doCT)
                {
                    ...
                    for (jt = RHout.mapObsTypes.begin(); jt != RHout.mapObsTypes.end(); ++jt)
                    {
                        if (sys != string("?") && sys != jt->first)
                            continue;
                        RinexObsID obsid(jt->first + it->obs.asString(), Rhead.version);
                ...
                else if (it->type == EditCmd::dsCT && it->sign >= 0 && it->ttag == CommonTime::BEGINNING_OF_TIME)
                {
                    if (it->sat.id == -1)
                    {
            ...
            if (mungeData)
            {
                ...
                for (jt = Rhead.mapObsTypes.begin(); jt != Rhead.mapObsTypes.end(); ++jt)
                {
                    ...
                    {
                        kt = find(RHout.mapObsTypes[sys].begin(), RHout.mapObsTypes[sys].end(), jt->second[i]);
                        mapSysObsIDTranslate[sys][i] = (kt == RHout.mapObsTypes[sys].end()
                                                            ? -1                                      // not found
        ...
        { // regular data
            ...
            while (it != C.vecCmds.end())
            {
                if (it->ttag <= now || ::fabs(it->ttag - now) < C.timetol)
                {
                    ...
                    if (iret < 0)
                        return iret; // fatal error

                    // keep this command on the current list
                    if (iret > 0)
                        toCurr.push_back(*it); // C.currCmds.push_back(*it);
...
int executeEditCmd(const vector<EditCmd>::iterator &it, Rinex3ObsHeader &Rhead, Rinex3ObsData &Rdata)
{
    ...
    try
    {
        ...
        else if (it->type == EditCmd::ofCT)
        {
            ...
            LOG(INFO) << " Opened output file " << it->field << " at time " << printTime(Rdata.time, C.longfmt);
            ...
            if (it->ttag == CommonTime::BEGINNING_OF_TIME)
            {
                ...
                if (!C.messHDp.empty())
                    Rhead.fileProgram = C.messHDp;
                if (!C.messHDr.empty())
                    Rhead.fileAgency = C.messHDr;
                if (!C.messHDo.empty())
                    Rhead.observer = C.messHDo;
                if (!C.messHDa.empty())
                    Rhead.agency = C.messHDa;
                if (!C.messHDj.empty())
                    Rhead.recNo = C.messHDj;
                if (!C.messHDk.empty())
                    Rhead.recType = C.messHDk;
                if (!C.messHDl.empty())
                    Rhead.recVers = C.messHDl;
                if (!C.messHDs.empty())
                    Rhead.antNo = C.messHDs;
                if (!C.messHDx.empty())
                {
                    flds = split(C.messHDx, ',');
                    // TD check n==3,doubles in initialize
                    for (i = 0; i < 3; i++)
                        Rhead.antennaPosition[i] = asDouble(flds[i]);
                }
                if (!C.messHDm.empty())
                    Rhead.markerName = C.messHDm;
                if (!C.messHDn.empty())
                {
                ...
                if (!C.messHDh.empty())
                {
                    ...
                    for (i = 0; i < 3; i++)
                        Rhead.antennaDeltaHEN[i] = asDouble(flds[i]);
                }
                if (C.messHDdc)
                {
        ...
        else
        {
            ...
            jt = find(Rhead.mapObsTypes[sys].begin(), Rhead.mapObsTypes[sys].end(), it->obs);
...
string Configuration::buildCommandLine(void) noexcept
{
    ...
    opts.Add(0, "IF", "fn", true, false, &messIF, "# RINEX input and output files",
             "Input RINEX observation file name");
    opts.Add(0, "ID", "p", false, false, &InObspath, "", "Path of input RINEX observation file(s)");
    opts.Add(0, "OF", "fn", true, false, &messOF, "", "Output RINEX obs files [also see --OF <f,t> below]");
    opts.Add(0, "OD", "p", false, false, &OutObspath, "", "Path of output RINEX observation file(s)");

    opts.Add('f', "file", "fn", true, false, &cfgfile, "# Other file I/O",
             "Name of file containing more options [#->EOL = comment]");
    opts.Add('l', "log", "fn", false, false, &logfile, "", "Output log file name");
    opts.Add(0, "ver2", "", false, false, &outver2, "", "Write out RINEX version 2");

    opts.Add(0, "verbose", "", false, false, &verbose, "# Help", "Print extra output information");
    ...
    opts.Add(0, "HDp", "p", false, false, &messHDp,
             "# ------ Editing commands ------\n"
             "# RINEX header modifications (arguments with whitespace must be quoted)",
             "Set header 'PROGRAM' field to <p>");
    opts.Add(0, "HDr", "rb", false, false, &messHDr, "", "Set header 'RUN BY' field to <rb>");
    opts.Add(0, "HDo", "obs", false, false, &messHDo, "", "Set header 'OBSERVER' field to <obs>");
    opts.Add(0, "HDa", "a", false, false, &messHDa, "", "Set header 'AGENCY' field to <a>");
    opts.Add(0, "HDx", "x,y,z", false, false, &messHDx, "", "Set header 'POSITION' field to <x,y,z> (ECEF, m)");
    opts.Add(0, "HDm", "m", false, false, &messHDm, "", "Set header 'MARKER NAME' field to <m>");
    opts.Add(0, "HDn", "n", false, false, &messHDn, "", "Set header 'MARKER NUMBER' field to <n>");
    opts.Add(0, "HDj", "n", false, false, &messHDj, "", "Set header 'REC #' field to <n>");
    opts.Add(0, "HDk", "t", false, false, &messHDk, "", "Set header 'REC TYPE' field to <t>");
    opts.Add(0, "HDl", "v", false, false, &messHDl, "", "Set header 'REC VERS' field to <v>");
    opts.Add(0, "HDs", "n", false, false, &messHDs, "", "Set header 'ANT #' field to <n>");
    opts.Add(0, "HDt", "t", false, false, &messHDt, "", "Set header 'ANT TYPE' field to <t>");
    opts.Add(0, "HDh", "h,e,n", false, false, &messHDh, "",
             "Set header 'ANTENNA OFFSET' field to <h,e,n> (Ht,East,North)");
    opts.Add(0, "HDc", "c", true, false, &messHDc, "", "Add 'COMMENT' <c> to the output header");
    opts.Add(0, "HDdc", "", false, false, &messHDdc, "", "Delete all comments [not --HDc] from input header");
    opts.Add(0, "HDda", "", false, false, &messHDda, "", "Delete all auxiliary header data");
    ...
    opts.Add(0, "TE", "t[:f]", false, false, &stopStr, "", "Stop  time: Reject data after this time");
    opts.Add(0, "TT", "dt", false, false, &timetol, "", "Tolerance in comparing times, in seconds");
    opts.Add(0, "TN", "dt", false, false, &decimate, "", "If dt>0, decimate data to times = TB + N*dt [sec, w/in tol]");
    ...
    opts.Add(0, "DA+", "t", true, false, &messDAp, "", "Delete all data beginning at time <t>");
    opts.Add(0, "DA-", "t", true, false, &messDAm, "", "Stop deleting at time <t>");

    opts.Add(0, "DO", "OT", true, false, &messDO, "", "Delete RINEX obs type <OT> entirely (incl. header)");
    ...
    opts.Add(0, "DS+", "SV,t", true, false, &messDSp, "", "Delete data for satellite <SV> beginning at time <t>");
    opts.Add(0, "DS-", "SV,t", true, false, &messDSm, "", "Stop deleting data for sat <SV> beginning at time <t>");

    opts.Add(0, "DD", "SV,OT,t", true, false, &messDD, "", "Delete a single RINEX datum(SV,OT) at time <t>");
    opts.Add(0, "DD+", "SV,OT,t", true, false, &messDDp, "", "Delete all RINEX data(SV,OT) starting at time <t>");
    opts.Add(0, "DD-", "SV,OT,t", true, false, &messDDm, "", "Stop deleting RINEX data(SV,OT) at time <t>");

    opts.Add(0, "SD", "SV,OT,t,d", true, false, &messSD, "", "Set data(SV,OT) to value <d> at single time <t>");
    opts.Add(0, "SS", "SV,OT,t,s", true, false, &messSS, "", "Set SSI(SV,OT) to value <s> at single time <t>");
    opts.Add(0, "SL", "SV,OT,t,l", true, false, &messSL, "", "Set LLI(SV,OT) to value <l> at single time <t>");
    opts.Add(0, "SL+", "SV,OT,t,l", true, false, &messSLp, "", "Set all LLI(SV,OT) to value <l> starting at time <t>");
    opts.Add(0, "SL-", "SV,OT,t,l", true, false, &messSLm, "", "Stop setting LLI(SV,OT) to value <l> at time <t>");

    opts.Add(0, "BZ", "", false, false, &messBZ, "# Bias cmds: (BD cmds apply only when data is non-zero, unless --BZ)",
             "Apply BD command even when data is zero (i.e. 'missing')");
    opts.Add(0, "BS", "SV,OT,t,s", true, false, &messBS, "", "Add the value <s> to SSI(SV,OT) at single time <t>");
    opts.Add(0, "BL", "SV,OT,t,l", true, false, &messBL, "", "Add the value <l> to LLI(SV,OT) at single time <t>");
    opts.Add(0, "BD", "SV,OT,t,d", true, false, &messBD, "", "Add the value <d> to data(SV,OT) at single time <t>");
    opts.Add(0, "BD+", "SV,OT,t,d", true, false, &messBDp, "",
             "Add the value <d> to data(SV,OT) beginning at time <t>");
    opts.Add(0, "BD-", "SV,OT,t,d", true, false, &messBDm, "", "Stop adding the value <d> to data(SV,OT) at time <t>");
...
void Configuration::parseEditCmds(vector<string> &vec, const string lab, ostringstream &os) noexcept
{
    for (size_t i = 0; i < vec.size(); i++)
    {
        EditCmd ec(lab, vec[i]);
        if (ec.isValid())
            vecCmds.push_back(ec);
        else
            os << "Error: invalid argument in " << lab << " cmd: >" << vec[i] << "<\n";
    }
}
...
EditCmd::EditCmd(const string intypestr, const string inarg)
{
    try
    {
        ...
        {
            if (n != 1 && n != 3 && n != 7)
                return; // DS DS,w,sow and DS,y,m,d,h,m,s
            try
            {
                sat.fromString(flds[0]);
            }
            catch (Exception)
            {
                return;
        ...
        else
        {
            ...
            if (n < 4)
                return; // at minimum SV,OT,week,sow

            stripLeading(arg, flds[0] + "," + flds[1] + ","); // remove 'SV,OT,' from arg

            string dat;
            if (tag != "DD")
            { // strip and save last arg (dsl)
            ...
            if (n != 5 && n != 9)
                return; // rest have SV,OT,t,d = 5 or 9 args

            if (tag == "SD" || tag == "BD")
            { // double data
                if (isScientificString(dat))
                    data = asDouble(dat);
                else
                    return;
            }
...
bool EditCmd::parseTime(const string arg, CommonTime &ttag) noexcept
{
    ...
    if (n == 2 || n == 6)
    {
        ...
        try
        {
            ep.scanf(arg, (n == 2 ? fmtGPS : fmtCAL));
        }
...
string EditCmd::asString(string msg)
{
    try
    {
        ...
        if (msg.size())
            os << msg;
        os << " " << typeLabel[type] << " " << (sign == 0 ? "0" : (sign < 0 ? "-" : "+")) << " SV:" << sat.toString()
           << " OT:" << obs.asString() << " d:" << fixed << setprecision(4) << data << " i:" << idata
           << " t:" << (ttag == CommonTime::BEGINNING_OF_TIME ? "BeginTime" : printTime(ttag, C.longfmt)) << " >"
           << field << "<";
