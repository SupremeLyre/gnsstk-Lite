...
#ifdef DEBUG_NAVFIT
#include "GLOCNavEph.hpp" // debug code
#include "NavFit.hpp"     // debug code
#include "OrbitData.hpp"  // debug code
#endif
...
class FindParams
{
  public:
    FindParams()
        : xmitHealth(SVHealth::Any), valid(NavValidityType::Any), order(NavSearchOrder::User), tgtSys(TimeSystem::Any)
    {
    }
     ...
     * @return true if successful. */
    bool getXvt(std::shared_ptr<NavLibrary> &navLib, bool useAlm, Xvt &xvt) const;
...
class CommandOptionHelpNavMessageType : public CommandOptionHelp
{
  public:
     ...
     */
    CommandOptionHelpNavMessageType(const char shOpt, const std::string &loOpt, const std::string &desc);
...
class CommandOptionNavLibraryFind : public CommandOptionWithAnyArg
{
  public:
     ...
     *   nav message type.
     */
    CommandOptionNavLibraryFind(const char shOpt, const std::string &loOpt, const std::string &desc,
 ...
 * <b>navdump</b> <b>[-d</b><b>]</b> <b>[-v</b><b>]</b> <b>[-t</b>&nbsp;\argarg{ARG}<b>]</b>
<b>[-l</b>&nbsp;\argarg{ARG}<b>]</b> <b>[-F</b>&nbsp;\argarg{ARG}<b>]</b> <b>[-X</b>&nbsp;\argarg{ARG}<b>]</b>
<b>[\--xvt-file</b>&nbsp;\argarg{ARG}<b>]</b> <b>[-g</b>&nbsp;\argarg{ARG}<b>]</b>
<b>[\--to-file</b>&nbsp;\argarg{ARG}<b>]</b> \argarg{ARG} <b>[</b>...<b>]</b>
 *
 ...
 * \dicdef{Get help for enums, specify one of system, obstype, carrier, range, antenna, navtype, navmsgtype, health,
validity, order, detail}
...
"navmsgtype YYYY/DOY/HH:MM:SS.SSS[/SYS/TGT] subject-sat-id system [carrier [range [navtype [xmit-health [validity-type
[xmit-sat-id system]]]]]]" (optional time systems SYS and TGT are used to convert the specified time system from SYS to
TGT before look-up; navmsgtype must be Ephemeris, Almanac or Unknown for Almanac-if-no-Ephemeris)}
 ...
 * \dicdef{Compute a satellite XVT using nav data matching the specified file's contents.  The file should contain the
same format specification as --xvt, one per line}
 * \dicterm{-g, \--geodetic=\argarg{ARG}}
 * \dicdef{Print position in geodetic coordinates in addition to an XVT in ECEF, using the specified ellipsoid model
(WGS84, GPS, PZ90, Galileo, CGCS2000)}
 * \dicterm{\--to-file=\argarg{ARG}}
 * \dicdef{File containing nav data to be loaded for the sole purpose of providing time offsets when doing time system
conversions during look-ups for everything else.  Only makes sense to use with -F -X or --xvt-file when a time system is
specified.}
 * \enddictionary
 *
...
bool FindParams ::find(std::shared_ptr<NavLibrary> &navLib, NavDataPtr &ndp) const
{
    CommonTime inqTime(when);
    if ((tgtSys != TimeSystem::Any) && (tgtSys != inqTime.getTimeSystem()))
    {
...
bool FindParams ::getXvt(std::shared_ptr<NavLibrary> &navLib, bool useAlm, Xvt &xvt) const
{
    CommonTime inqTime(when);
    if ((tgtSys != TimeSystem::Any) && (tgtSys != inqTime.getTimeSystem()))
    {
...
void FindParams ::decode(const std::string &s, bool warnNMT, std::string &warnings)
{
    ...
    if (nmid.messageType == NavMessageType::Unknown)
    {
        ...
        {
            cerr << "Warning: Unknown nav message type: \"" << words[0] << "\"" << endl;
        }
    }
    try
    {
    ...
    if ((nmid.sat.system == SatelliteSystem::Glonass) && (words[2].find('/') != std::string::npos))
    {
    ...
    if (numWords > 4)
    {
        ...
        {
            cerr << "Warning: Unknown carrier band \"" << words[4] << "\"" << endl;
        }
    }
    if (numWords > 5)
    {
        ...
        {
            cerr << "Warning: Unknown tracking code \"" << words[5] << "\"" << endl;
        }
    }
    if (numWords > 6)
    {
        ...
        {
            cerr << "Warning: Unknown nav type \"" << words[6] << "\"" << endl;
        }
    }
    if (numWords > 7)
    {
        ...
        {
            cerr << "Warning: Unknown health \"" << words[7] << "\"" << endl;
        }
    }
    if (numWords > 8)
    {
        ...
        {
            cerr << "Warning: Unknown validity type \"" << words[8] << "\"" << endl;
        }
    }
    if (numWords == 10)
    {
    ...
    if (numWords > 10)
    {
        ...
        nmid.xmitSat = SatID(StringUtils::asInt(words[9]), StringUtils::asSatelliteSystem(words[10]));
...
CommandOptionNavLibraryFind ::CommandOptionNavLibraryFind(const char shOpt, const std::string &loOpt,
                                                          const std::string &desc, bool warnUnknownNMT)
    : CommandOptionWithAnyArg(shOpt, loOpt, desc), warnNMT(warnUnknownNMT)
{
}

std::string CommandOptionNavLibraryFind ::checkArguments()
{
    std::string rv;
...
NavDump ::NavDump(const string &applName)
    : BasicFramework(applName, "Dump any and all navigation data (that we"
      ...
      typesOpt('t', "type",
               "Navigation data type(s) to dump (use \"-E"
               " navmsgtype\" for list, default=all types)"),
      detailOpt('l', "level",
                "Specify detail level of output (use \"-E"
                " detail\" for list, default=Full)"),
      inqOpt('F', "find", "Find a specific object in the input matching " + specHelp),
      xvtOpt('X', "xvt", "Compute a satellite XVT using nav data matching " + specHelp, false),
      xvtFileOpt(0, "xvt-file",
                 "Compute a satellite XVT using nav data"
                 " matching the specified file's contents.  The file should"
                 " contain the same format specification as --xvt, one per"
                 " line"),
      geodOpt('g', "geodetic",
              "Print position in geodetic coordinates"
              " in addition to an XVT in ECEF, using the specified ellipsoid"
              " model (WGS84, GPS, PZ90, Galileo, CGCS2000)"),
      toFileOpt(0, "to-file",
                "File containing nav data to be loaded for the"
      ...
      enumHelpOpt('E', "enum"), detail(DumpDetail::Full), ell(nullptr), filesOpt("", true)
{
    ...
    filesOpt.setDescription("NAV-FILE [...] (supported formats: " + ndfp->getFactoryFormats() + ")");
    detailOpt.setMaxCount(1);
    geodOpt.setMaxCount(1);
}

bool NavDump ::initialize(int argc, char *argv[], bool pretty) noexcept
{
    ...
    for (unsigned i = 0; i < types.size(); i++)
    {
        ...
        {
            cerr << "\"" << types[i] << "\" is not a valid nav message type" << endl;
            exitCode = BasicFramework::OPTION_ERROR;
            return false;
        }
        nmts.insert(nmt);
    ...
    // is needed.
    if ((nmts.size() > 0) && (xvtOpt.getCount() == 0) && (xvtFileOpt.getCount() == 0))
    {
    ...
    if (detailOpt.getCount())
    {
        ...
        {
            cerr << "\"" << detailStr << "\" is not a valid detail level" << endl;
            exitCode = BasicFramework::OPTION_ERROR;
            return false;
        }
    }
...
void NavDump ::process()
{
    ...
    if (xvtFileOpt.getCount() > 0)
    {
        ...
        for (unsigned i = 0; i < xvtfiles.size(); i++)
        {
            ...
            if (!is)
            {
                cerr << "Unable to open \"" << xvtfiles[i] << "\" for input" << endl;
                if (exitCode == 0)
                {
                    exitCode = BasicFramework::EXIST_ERROR;
                }
    ...
    if ((inqOpt.params.empty() && xvtOpt.params.empty() && (xvtFileOpt.getCount() == 0)) ||
...
bool NavDump ::getXvt(const FindParams &fp)
{
    bool found = true;
    if (verboseLevel)
    {
    ...
    case NavMessageType::Ephemeris:
        ...
        if (fp.find(navLib, ndp))
        {
            ...
            {
                cout << fp.nmid << gnsstk::printTime(fp.when, " %Y/%j/%02H:%02M:%015.12f %P ") << setprecision(15)
                     << xvt;
                if (ell != nullptr)
                {
        ...
        {
            cout << fp.nmid << gnsstk::printTime(fp.when, " %Y/%j/%02H:%02M:%015.12f %P ") << setprecision(15) << xvt;
            if (ell != nullptr)
            {
                Position coords(xvt.x, Position::Cartesian, ell, xvt.frame);
                coords.transformTo(Position::Geodetic);
    ...
    case NavMessageType::Unknown:
        ...
        break;
    default:
        cerr << "Can't compute an XVT using message type \"" << StringUtils::asString(fp.nmid.messageType) << "\""
             << endl;
        break;
    } // switch
    return found;
