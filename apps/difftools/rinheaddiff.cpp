...
RinexHeaderDiff::RinexHeaderDiff(const string &applName)
    : BasicFramework(applName, "Print the differences between the headers"
      ...
      inclOption('i', "include", "RINEX header lines to compare"), inputFileOption("FILES", true),
...
void RinexHeaderDiff::process()
{
    FileType ft1, ft2;
    FFData *header1, *header2;
    string fn1(inputFileOption.getValue()[0]), fn2(inputFileOption.getValue()[1]);
    ...
    if (ft1 == ft2)
    {
        ...
        {
        case RinexMet:
            if (dynamic_cast<RinexMetHeader *>(header1)->compare(*(dynamic_cast<RinexMetHeader *>(header2)), diffs,
                                                                 inclExclList, including))
            {
                // compare success
                exitCode = 0;
            }
            break;
        case RinexNav:
            if (dynamic_cast<Rinex3NavHeader *>(header1)->compare(*(dynamic_cast<Rinex3NavHeader *>(header2)), diffs,
                                                                  inclExclList, including))
            {
                // compare success
                exitCode = 0;
            ...
            break;
        case RinexObs:
            if (dynamic_cast<Rinex3ObsHeader *>(header1)->compare(*(dynamic_cast<Rinex3ObsHeader *>(header2)), diffs,
                                                                  inclExclList, including))
            {
                // compare success
                exitCode = 0;
...
RinexHeaderDiff::FileType RinexHeaderDiff::identFile(const string &fname, FFData *&hdr)
{
    hdr = NULL;
    try
    {
