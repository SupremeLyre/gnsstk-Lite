...
class RNWDiff : public DiffFrame
{
  public:
    ...
    RNWDiff(char *arg0)
        : DiffFrame(arg0, std::string("RINEX Nav")), precisionOption('p', "precision",
                                                                     "Ignore diffs smaller than "
...
void RNWDiff::process()
{
    try
    {
        FileFilterFrameWithHeader<Rinex3NavStream, Rinex3NavData, Rinex3NavHeader> ff1(inputFileOption.getValue()[0]),
        ...
        if (ff1.emptyHeader())
            cerr << "No header information for " << inputFileOption.getValue()[0] << endl;
        if (ff2.emptyHeader())
            cerr << "No header information for " << inputFileOption.getValue()[1] << endl;
        ...
        pair<list<Rinex3NavData>, list<Rinex3NavData>> difflist = ff1.diff(ff2, op);
        ...
        while (firstitr != difflist.first.end())
        {
            ...
            while ((!matched) && (seconditr != difflist.second.end()))
            {
                ...
                // different times.
                if ((firstitr->time == seconditr->time) && (firstitr->PRNID == seconditr->PRNID) &&
                    (firstitr->xmitTime == seconditr->xmitTime))
                {
                    YDSTime recTime(firstitr->time);
                    cout << fixed << setw(3) << recTime.doy << ' ' << setw(10) << setprecision(0) << recTime.sod << ' '
                         << setw(19) << setprecision(12) << scientific << (firstitr->af0 - seconditr->af0) << ' '
                         << (firstitr->af1 - seconditr->af1) << ' ' << (firstitr->af2 - seconditr->af2) << ' '
                         << (firstitr->IODE - seconditr->IODE) << ' ' << (firstitr->Crs - seconditr->Crs) << ' '
                         << (firstitr->dn - seconditr->dn) << ' ' << (firstitr->M0 - seconditr->M0) << ' '
                         << (firstitr->Cuc - seconditr->Cuc) << ' ' << (firstitr->ecc - seconditr->ecc) << ' '
                         << (firstitr->Cus - seconditr->Cus) << ' ' << (firstitr->Ahalf - seconditr->Ahalf) << ' '
                         << (firstitr->Toe - seconditr->Toe) << ' ' << (firstitr->Cic - seconditr->Cic) << ' '
                         << (firstitr->OMEGA0 - seconditr->OMEGA0) << ' ' << (firstitr->Cis - seconditr->Cis) << ' '
                         << (firstitr->i0 - seconditr->i0) << ' ' << (firstitr->Crc - seconditr->Crc) << ' '
                         << (firstitr->w - seconditr->w) << ' ' << (firstitr->OMEGAdot - seconditr->OMEGAdot) << ' '
                         << (firstitr->idot - seconditr->idot) << ' ' << (firstitr->codeflgs - seconditr->codeflgs)
                         << ' ' << (firstitr->weeknum - seconditr->weeknum) << ' '
                         << (firstitr->L2Pdata - seconditr->L2Pdata) << ' '
                         << (firstitr->accuracy - seconditr->accuracy) << ' ' << (firstitr->health - seconditr->health)
                         << ' ' << (firstitr->Tgd - seconditr->Tgd) << ' ' << (firstitr->IODC - seconditr->IODC) << ' '
                         << (firstitr->xmitTime - seconditr->xmitTime) << ' ' << (firstitr->fitint - seconditr->fitint)
                         << endl;
    ...
    catch (Exception &e)
    {
        ...
        cout << e << endl << endl << "Terminating.." << endl;
    }
    catch (std::exception &e)
    {
        exitCode = BasicFramework::EXCEPTION_ERROR;
        cout << e.what() << endl << endl << "Terminating.." << endl;
    }
    catch (...)
    {
        exitCode = BasicFramework::EXCEPTION_ERROR;
