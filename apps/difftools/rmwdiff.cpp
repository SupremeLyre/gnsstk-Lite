...
class RMWDiff : public DiffFrame
{
  public:
    ...
    RMWDiff(char *arg0) : DiffFrame(arg0, std::string("RINEX Met"))
    {
    }
...
void RMWDiff::process()
{
    try
    {
        FileFilterFrameWithHeader<RinexMetStream, RinexMetData, RinexMetHeader> ff1(inputFileOption.getValue()[0]),
        ...
        while (firstitr != difflist.first.end())
        {
            ...
            while ((!matched) && (seconditr != difflist.second.end()))
            {
                if (firstitr->time == seconditr->time)
                {
                    ...
                    cout << setw(3) << recTime.doy << ' ' << setw(10) << setprecision(0) << fixed << recTime.sod << ' '
                         << ff1.frontHeader().markerName << ' ' << ff2.frontHeader().markerName << ' ';
                    ...
                    for (m = intersection.begin(); m != intersection.end(); m++)
                    {
                        ...
                        cout << setw(7) << setprecision(1) << fixed << diff << ' ' << RinexMetHeader::convertObsType(*m)
                             << ' ';
                    }
                    cout << endl;
