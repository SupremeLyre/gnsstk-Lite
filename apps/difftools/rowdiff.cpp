...
class ROWDiff : public DiffFrame
{
  public:
    ...
    ROWDiff(char *arg0)
        : DiffFrame(arg0, std::string("RINEX Obs")), precisionOption('p', "precision",
...
{
    gnsstk::FileFilterFrameWithHeader<Rinex3ObsStream, Rinex3ObsData, Rinex3ObsHeader> ff1(
    ...
    std::list<Rinex3ObsData> a = ff1.halfDiff(ff2, Rinex3ObsDataOperatorLessThanFull(intersectRom), precision);
    std::list<Rinex3ObsData> b = ff2.halfDiff(ff1, Rinex3ObsDataOperatorLessThanFull(intersectRom), precision);

    pair<list<Rinex3ObsData>, list<Rinex3ObsData>> difflist = pair<list<Rinex3ObsData>, list<Rinex3ObsData>>(a, b);
    ...
    while ((firstDiffItr != difflist.first.end()) || (secondDiffItr != difflist.second.end()))
    {
        ...
        if ((firstDiffItr != difflist.first.end()) &&
            ((secondDiffItr == difflist.second.end()) || (firstDiffItr->time < secondDiffItr->time)))
        {
            for (const auto &firstObsItr : firstDiffItr->obs)
            {
                cout << "<" << setw(3) << (static_cast<YDSTime>(firstDiffItr->time)) << ' ' << setw(2)
                     << firstObsItr.first << ' ';
                ...
                for (const auto &romIt : intersectRom[sysString])
                {
                    ...
                    cout << setw(15) << setprecision(3) << fixed << firstObsItr.second[idx].data << ' '
                         << romIt.asString() << ' ';
                }
                cout << endl;
            }
        ...
        // Epoch only in second file
                 ...
                 ((firstDiffItr == difflist.first.end()) || (secondDiffItr->time < firstDiffItr->time)))
        {
            for (const auto &secondObsItr : secondDiffItr->obs)
            {
                cout << ">" << setw(3) << (static_cast<YDSTime>(secondDiffItr->time)) << ' ' << setw(2)
                     << secondObsItr.first << ' ';
                string sysString = string(1, secondObsItr.first.systemChar());
                for (const auto &romIt : intersectRom[sysString])
                {
                    size_t idx = header2.getObsIndex(sysString, romIt);
                    cout << setw(15) << setprecision(3) << fixed << secondObsItr.second[idx].data << ' '
                         << romIt.asString() << ' ';
                }
                cout << endl;
            }
        ...
        else if (firstDiffItr->time == secondDiffItr->time)
        {
            ...
            while ((firstObsItr != firstDiffItr->obs.end()) || (secondObsItr != secondDiffItr->obs.end()))
            {
                ...
                else if ((firstObsItr != firstDiffItr->obs.end()) && ((secondObsItr == secondDiffItr->obs.end()) ||
