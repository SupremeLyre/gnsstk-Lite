 ...
 * \dicdef{Solution(s) to compute: Sys:Freqs:Codes (cf. \--SOLhelp) [repeatable] () (require \--eph OR \--nav, but NOT
 * both)}
 ...
 * \dicdef{Trop model \argarg{M}, one of Zero,Black,Saas,NewB,Neill,GG,GGHt,Global with optional weather
 * T(C),P(mb),RH(%)] (NewB,20.0,1013.0,50.0)}
...
#include "BasicFramework.hpp" // for EXCEPTION_ERROR
...
class SolutionData
{
  public:
    ...
    bool ComputeData(const RinexSatID &sat, const vector<RinexDatum> &vrd)
    {
        ...
        // missing data?
        if (RawPR[0] == 0.0 || (frs.size() > 1 && RawPR[1] == 0.0))
            return false;

        // iono delay
        if (consts.size() > 1)
            RI = consts[1] * (RawPR[0] - RawPR[1]);
...
class SolutionObject
{
  public:
    // member functions
    // Default and only constructor
    SolutionObject(const string &desc) noexcept
    {
        Initialize(desc);
    }
    ...
    void CollectData(const RinexSatID &s, const double &elev, const double &ER, const vector<RinexDatum> &v) noexcept;
...
#include "NewNavInit.h"
    ...
    try
    {
        ...
        C.Title =
            C.PrgmName + ", part of the GNSS Toolkit, Ver " + Version + ", Run " + printTime(wallclkbeg, C.calfmt);
        // cout << C.Title << endl;
        ...
        for (;;)
        {
            ...
            if (nfiles < 0)
                break;
            LOG(VERBOSE) << "Successfully read " << nfiles << " RINEX observation file" << (nfiles > 1 ? "s." : ".");
...
int Initialize(string &errors)
{
    try
    {
        ...
        gnsstk::MultiFormatNavDataFactory *mfndfp = dynamic_cast<gnsstk::MultiFormatNavDataFactory *>(C.ndfp.get());
        ...
        if (C.InputObsFiles.size() > 0)
        {
            try
            {
                for (nread = 0, nfile = 0; nfile < C.InputObsFiles.size(); nfile++)
                {
                    ...
                    if (!rostrm.is_open())
                    {
                        ossE << "Error : failed to open RINEX obs file: " << C.InputObsFiles[nfile] << endl;
                        isValid = false;
                        continue;
                    }
                    ...
                    {
                        ossE << "Error : File: " << C.InputObsFiles[nfile] << " is not a valid RINEX file." << endl;
                        isValid = false;
                    }
                    nread++;
        ...
        if (C.InputSP3Files.size() > 0)
        {
            ...
            for (multimap<CommonTime, string>::const_iterator it = startNameMap.begin(); it != startNameMap.end(); ++it)
            ...
            if (isValid)
            {
                ...
                {
                    if (isValid)
                        for (nread = 0, nfile = 0; nfile < C.InputSP3Files.size(); nfile++)
                        {
        ...
        if (C.InputClkFiles.size() > 0)
        {
            ...
            LOG(VERBOSE) << "Read " << nread << " RINEX clock files into store.\n"
                         << "RINEX clock file store contains " << sp3fact->count(NavMessageType::Clock) << " data.";
        }
        else
            LOG(VERBOSE) << "No RINEX clock files";

        // ------------- configure and dump SP3 and clock stores -----------------
        if (isValid && sp3fact->size() > 0)
        {
            LOG(VERBOSE) << "Read " << nread << " SP3 ephemeris files into store.";
            LOG(VERBOSE) << "SP3 Ephemeris store contains " << sp3fact->size() << " data";
            ...
            set<SatID> sats(sp3fact->getIndexSet(CommonTime::BEGINNING_OF_TIME, CommonTime::END_OF_TIME));
            RinexSatID sat(*sats.rbegin());
            double dtp = sp3fact->getPositionTimeStep(sat);
            double dtc = sp3fact->getClockTimeStep(sat);
            LOG(VERBOSE) << "\nSP3 Ephemeris Store time intervals for " << sat << " are " << dtp << " (pos), and "
                         << dtc << " (clk)";
            ...
            sp3fact->setPosMaxInterval((sp3fact->getPositionInterpOrder() - 1) * dtp + 1.);
            ...
            if (C.verbose)
            {
                ...
                sp3fact->dump(LOGstrm, (C.debug > 6 ? DumpDetail::Full : DumpDetail::OneLine));
            }
            ...
            for (const auto &sat : sats)
            {
                ...
                if (sat.system == SatelliteSystem::Glonass)
                {
                    ...
                    if (it == C.GLOfreqChannel.end() && sat.system == SatelliteSystem::Glonass)
                    {
                ...
                LOG(VERBOSE) << " Sat: " << RinexSatID(sat) << " Neph: " << setw(3)
                             << sp3fact->count(sat, NavMessageType::Ephemeris)
                             << " Beg: " << printTime(sp3fact->getFirstTime(sat), C.longfmt)
                             << " End: " << printTime(sp3fact->getLastTime(sat), C.longfmt) << C.msg;
            } // end loop over sats
        }
        ...
        if (C.InputNavFiles.size() > 0)
        {
            try
            {
                for (nread = 0, nfile = 0; nfile < C.InputNavFiles.size(); nfile++)
                {
                    ...
                    if (C.verbose)
                    {
                        ...
                        LOG(VERBOSE) << "Read ephemeris data from file " << filename << ".";
                    }
            ...
            if (isValid)
            {
                LOG(VERBOSE) << "Read " << nread << " RINEX navigation files, containing " << mfndfp->size()
                             << " messages, into store.";
                LOG(VERBOSE) << "GPS ephemeris store contains "
                             << mfndfp->count(SatelliteSystem::GPS, NavMessageType::Ephemeris) << " ephemerides.";
                LOG(VERBOSE) << "GAL ephemeris store contains "
                             << mfndfp->count(SatelliteSystem::Galileo, NavMessageType::Ephemeris) << " ephemerides.";
                LOG(VERBOSE) << "BDS ephemeris store contains "
                             << mfndfp->count(SatelliteSystem::BeiDou, NavMessageType::Ephemeris) << " ephemerides.";
                LOG(VERBOSE) << "QZS ephemeris store contains "
                             << mfndfp->count(SatelliteSystem::QZSS, NavMessageType::Ephemeris) << " ephemerides.";
                LOG(VERBOSE) << "GLO ephemeris store contains "
                             << mfndfp->count(SatelliteSystem::Glonass, NavMessageType::Ephemeris) << " satellites.";
                // dump the entire store
                C.navLib.dump(LOGstrm, (C.debug > -1 ? DumpDetail::Full : DumpDetail::OneLine));
            }
        }
        ...
        if (C.InputMetFiles.size() > 0)
        {
            try
            {
                for (nfile = 0; nfile < C.InputMetFiles.size(); nfile++)
                {
                    ...
                    if (!mstrm.is_open())
                    {
                        ossE << "Error : failed to open RINEX meteorological file " << C.InputMetFiles[nfile] << endl;
                        isValid = false;
                        continue;
                    }
                ...
                if (isValid && C.verbose)
                {
                    ...
                    LOG(VERBOSE) << "Meteorological store contains " << C.MetStore.size() << " records:";
                    if (C.MetStore.size() > 0)
                    {
                        it = C.MetStore.begin();
                        if (C.MetStore.size() == 1)
                        {
                            LOG(VERBOSE) << "  Met store is at single time " << printTime(it->time, C.longfmt);
                        }
                        else
                        {
                            LOG(VERBOSE) << "  Met store starts at time " << printTime(it->time, C.longfmt);
                            it = C.MetStore.end();
                            it--;
                            LOG(VERBOSE) << "  Met store   ends at time " << printTime(it->time, C.longfmt);
                        }
                    }

                    if (C.debug > -1)
                    {
                        LOG(DEBUG) << "Dump of meteorological data store (" << C.MetStore.size() << "):";
                        for (it = C.MetStore.begin(); it != C.MetStore.end(); it++)
                        {
                            ...
                            {
                                os << "  " << RinexMetHeader::convertObsType(jt->first) << " = " << setw(6)
                                   << jt->second;
                            }
                            LOG(DEBUG) << os.str();
                        }
        ...
        if (C.InputDCBFiles.size() > 0)
        {
            for (nfile = 0; nfile < C.InputDCBFiles.size(); nfile++)
            {
                ...
                while (1)
                {
                    ...
                    if (sat.system == SatelliteSystem::Unknown || sat.id == -1)
                        continue;

                    word = stripFirstWord(line); // get bias
                    if (word.empty())
                        continue;
                    if (!isScientificString(word))
                        continue;
                    double bias(asDouble(word) * C_MPS * 1.e-9); // ns to m

                    if (C.P1C1bias.find(sat) != C.P1C1bias.end())
                    {
                        LOG(WARNING) << "Warning : satellite " << sat << " is duplicated in P1-C1 bias file(s)";
                    }
                    else
                    {
                        C.P1C1bias.insert(map<RinexSatID, double>::value_type(sat, bias));
                        LOG(DEBUG) << " Found P1-C1 bias for sat " << sat << " = " << setw(6) << word
                                   << " ns = " << fixed << setprecision(3) << setw(6) << bias << " m (from " << filename
                                   << ")";
                    }
                }
            }
        ...
        if (C.decimate > 0.0)
        {
            ...
            if (::fabs(s - sow) > 1.0)
                LOG(WARNING) << "Warning : decimation reference time "
        ...
        for (j = 0, i = 0; i < C.inSolDesc.size(); i++)
        {
            ...
            if (!SObj.isValid)
            {
                LOG(WARNING) << "Warning : solution descriptor " << C.inSolDesc[i] << " could not be created - ignore";
                continue;
            }

            if (i == 0)
                LOG(INFO) << SObj.prs.configString("PRS configuration:");
            ...
            for (size_t k = 0; k < SObj.sysChars.size(); k++)
            {
                ...
                {
                    LOG(INFO) << " Found system " << RinexObsID::map1to3sys[SObj.sysChars[k]] << " with " << n
                              << " ephemeris data.";
                }
            }
            if (!ok)
        ...
        for (i = 0; i < C.SolObjs.size(); i++)
        {
            ...
            {
                if (find(C.allSystemChars.begin(), C.allSystemChars.end(), C.SolObjs[i].sysChars[j]) ==
        ...
        if (C.debug > -1)
        {
            ...
            for (i = 0; i < C.allSystemChars.size(); i++)
                oss << " " << C.allSystemChars[i];
            LOG(DEBUG) << oss.str();
        }
    ...
    try
    {
        ...
        for (nfiles = 0, nfile = 0; nfile < C.InputObsFiles.size(); nfile++)
        {
            ...
            if (C.verbose)
            {
                ...
                Rhead.dump(LOGstrm);
                LOG(VERBOSE) << "Time system for RINEX file " << filename << " is "
                             << gnsstk::StringUtils::asString(istrm.timesystem);
            }
                ...
                for (; sit != Rhead.mapObsTypes.end(); ++sit)
                {
                    for (i = 0; i < sit->second.size(); i++)
                    {
                        if (asString(sit->second[i]) == string("C1C"))
                        {
                            ...
                            LOG(DEBUG) << "Correct for DCB: found " << asString(sit->second[i]) << " for system "
                                       << sit->first << " at index " << i;
                            break;
                        }
                    }
                }
            ...
            if (firstepoch)
            {
                ...
                if (!C.OutputObsFile.empty())
                {
                    ...
                    if (!ostrm.is_open())
                    {
                        LOG(WARNING) << "Warning : could not open output file " << C.OutputObsFile;
                ...
                if (!C.OutputORDFile.empty())
                {
                    ...
                    {
                        LOG(WARNING) << "Warning : failed to open output ORDs file " << C.OutputORDFile
                                     << " - abort ORD output.";
                        C.ORDout = false;
                    }
                    else
            ...
            for (i = 0; i < C.SolObjs.size(); ++i)
            {
                ...
                if (C.verbose)
                    for (j = 0; j < C.SolObjs[i].sysChars.size(); j++)
                    {
                        ...
                        if (C.SolObjs[i].sysChars[j] == "G")
                            ts = TimeSystem::GPS;
                        if (C.SolObjs[i].sysChars[j] == "R")
                            ts = TimeSystem::GLO;
                        if (C.SolObjs[i].sysChars[j] == "E")
                            ts = TimeSystem::GAL;
                        if (C.SolObjs[i].sysChars[j] == "C")
                            ts = TimeSystem::BDT;
                        if (C.SolObjs[i].sysChars[j] == "S")
                            ts = TimeSystem::GPS;
                        if (C.SolObjs[i].sysChars[j] == "J")
                            ts = TimeSystem::QZS;
                        NavDataPtr ndp;
                        if (C.navLib.getOffset(istrm.timesystem, ts, C.beginTime, ndp, SVHealth::Healthy))
                        {
                            ostringstream s;
            ...
            while (1)
            {
                ...
                {
                    LOG(WARNING) << " Warning : Failed to read obs data (Exception " << e.getText(0)
                                 << "); dump follows.";
                ...
                LOG(DEBUG) << "\n Read RINEX data: flag " << Rdata.epochFlag << ", timetag "
                ...
                {
                    LOG(DEBUG) << " RINEX data timetag " << printTime(C.endTime, C.longfmt) << " is after end time.";
                    break;
                }
                ...
                if (C.decimate > 0.0)
                {
                    ...
                    if (::fabs(dt) > 0.25)
                    {
                        LOG(DEBUG) << " Decimation rejects RINEX data timetag " << printTime(Rdata.time, C.longfmt);
                        continue;
                    }
                }
                ...
                for (it = Rdata.obs.begin(); it != Rdata.obs.end(); ++it)
                {
                    ...
                    if (find(C.allSystemChars.begin(), C.allSystemChars.end(), sys) == C.allSystemChars.end())
                    {
                        LOG(DEBUG) << " Sat " << sat << " : system " << sys << " is not needed.";
                        continue;
                    }
                    ...
                    if (DCBcorr && mapDCBindex.find(sys) != mapDCBindex.end())
                    {
                        ...
                        if (C.P1C1bias.find(sat) != C.P1C1bias.end())
                        {
                            LOG(DEBUG) << "Correct data " << asString(Rhead.mapObsTypes[sys][i]) << " = " << fixed
                                       << setprecision(2) << vrdata[i].data << " for DCB with " << C.P1C1bias[sat];
                    ...
                    if ((C.elevLimit > 0 || C.weight || C.ORDout) && PrevPos.getCoordinateSystem() != Position::Unknown)
                    {
                        CorrectedEphemerisRange CER;
                        try
                        {
                            CER.ComputeAtReceiveTime(Rdata.time, PrevPos, sat, C.navLib, C.searchOrder);
                            elev = CER.elevation;
                            ...
                            if (elev < C.elevLimit)
                            { // TD add elev mask [azim]
                                LOG(VERBOSE) << " Reject sat " << sat << " for elevation " << fixed << setprecision(2)
                                             << elev << " at time " << printTime(Rdata.time, C.longfmt);
                                continue;
                            }
                        }
                        catch (Exception &e)
                        {
                            LOG(WARNING) << "WARNING : Failed to get elevation for sat " << sat << " at time "
                                         << printTime(Rdata.time, C.longfmt);
                            continue;
                        }
                    }
                ...
                for (i = 0; i < C.SolObjs.size(); ++i)
                {
                    ...
                    if (firstepoch)
                        LOG(VERBOSE) << C.SolObjs[i].prs.outputString(string("RPF ") + C.SolObjs[i].Descriptor, -999);
                    if (firstepoch)
                        LOG(VERBOSE) << C.SolObjs[i].prs.outputPOSString(string("RPR ") + C.SolObjs[i].Descriptor,
                                                                         -999);
                    if (firstepoch)
                        LOG(VERBOSE) << C.SolObjs[i].prs.outputPOSString(string("RNE ") + C.SolObjs[i].Descriptor,
                                                                         -999);
                    j = C.SolObjs[i].ComputeSolution(Rdata.time);

                    // write ORDs, even if solution is not good
                    if (C.ORDout)
                        C.SolObjs[i].WriteORDs(Rdata.time, j);
                }
                ...
                if (!C.OutputObsFile.empty())
                {
                    ...
                    for (k = 0, i = 0; i < C.SolObjs.size(); ++i)
                        if (C.SolObjs[i].isValid)
                        {
                            ...
                            oss.str("");
                            oss << "XYZ" << fixed << setprecision(3) << " " << setw(12) << C.SolObjs[i].prs.Solution(0)
                                << " " << setw(12) << C.SolObjs[i].prs.Solution(1) << " " << setw(12)
                                << C.SolObjs[i].prs.Solution(2);
                            ...
                            for (j = 0; j < C.SolObjs[i].prs.dataGNSS.size(); j++)
                            {
                                ...
                                oss << " " << sat.systemString3() << " " << setw(11)
                                    << C.SolObjs[i].prs.Solution(3 + j);
                            }
                            oss << " " << C.SolObjs[i].Descriptor; // may get truncated
                            auxData.auxHeader.commentList.push_back(oss.str());
                            k++;
                            oss.str("");
                            oss << "DIA" << setw(2) << C.SolObjs[i].prs.Nsvs << fixed << setprecision(2) << " "
                                << setw(4) << C.SolObjs[i].prs.PDOP << " " << setw(4) << C.SolObjs[i].prs.GDOP << " "
...
void Configuration::SetDefaults(void) noexcept
{
    ...
    defaultPress = 1013.0;
    defaultHumid = 50.0;
    TropStr = TropType + string(",") + asString(defaultTemp, 1) + string(",") + asString(defaultPress, 1) +
              string(",") + asString(defaultHumid, 1);
...
void Configuration::SolDescHelp(void)
{
    ...
    for (i = 0; i < systs.size(); i++)
    {
        for (k = 0, j = 0; j < freqs.size(); j++)
        {
            ...
            if (systs[i] == 'G' && (freqs[j] == '1' || freqs[j] == '2'))
                strip(codes, 'N');
            if (codes.length() > k)
                k = codes.length();
        }
...
string Configuration::BuildCommandLine(void) noexcept
{
    ...
    opts.Add('f', "file", "fn", true, false, &filedummy,
             "# Input via configuration file:", "Name of file with more options [#->EOL = comment]");

    // required
    opts.Add(0, "obs", "fn", true, true, &InputObsFiles, "# Required input:", "RINEX observation file name(s)");
    opts.Add(0, "sol", "S:F:C", true, true, &inSolDesc, "", "Solution(s) to compute: Sys:Freqs:Codes (cf. --SOLhelp)");
    opts.Add(0, "eph", "fn", true, false, &InputSP3Files, "#   (require --eph OR --nav, but NOT both)",
             "Ephemeris+clock (SP3 format) file name(s)");
    opts.Add(0, "nav", "fn", true, false, &InputNavFiles, "", "RINEX nav file name(s) (also cf. --BCEpast)");

    // optional
    opts.Add(0, "clk", "fn", true, false, &InputClkFiles, "# Optional input\n# Other input files",
             "Clock (RINEX format) file name(s)");
    opts.Add(0, "met", "fn", true, false, &InputMetFiles, "", "RINEX meteorological file name(s)");
    opts.Add(0, "dcb", "fn", true, false, &InputDCBFiles, "", "Differential code bias (P1-C1) file name(s)");

    opts.Add(0, "obspath", "p", false, false, &Obspath,
             "# Paths of input files:", "Path of input RINEX observation file(s)");
    opts.Add(0, "ephpath", "p", false, false, &SP3path, "", "Path of input ephemeris+clock file(s)");
    opts.Add(0, "navpath", "p", false, false, &Navpath, "", "Path of input RINEX navigation file(s)");
    opts.Add(0, "clkpath", "p", false, false, &Clkpath, "", "Path of input RINEX clock file(s)");
    opts.Add(0, "metpath", "p", false, false, &Metpath, "", "Path of input RINEX meteorological file(s)");
    opts.Add(0, "dcbpath", "p", false, false, &DCBpath, "", "Path of input DCB (P1-C1) bias file(s)");
    ...
    opts.Add(0, "stop", "t[:f]", false, false, &stopStr, "", "Stop processing data at this epoch");
    opts.Add(0, "decimate", "dt", false, false, &decimate, "", "Decimate data to time interval dt (0: no decimation)");
    ...
    opts.Add(0, "exSat", "sat", true, false, &exclSat, "", "Exclude this satellite [eg. G24 | R | R23,G31]");
    opts.Add(0, "BCEpast", "", false, false, &searchUser, "",
             "Use 'User' find-ephemeris-algorithm (else nearest) (--nav only)");
    opts.Add(0, "PisY", "", false, false, &PisY, "", "P code data is actually Y code data");

    opts.Add(0, "wt", "", false, false, &weight,
             "# Solution Algorithm:", "Weight the measurements using elevation [--ref req'd]");
    opts.Add(0, "rms", "lim", false, false, &RMSLimit, "", "Upper limit on RMS post-fit residual (m)");
    opts.Add(0, "slope", "lim", false, false, &SlopeLimit, "", "Upper limit on maximum RAIM 'slope'");
    opts.Add(0, "nrej", "n", false, false, &maxReject, "", "Maximum number of satellites to reject [-1 for no limit]");
    opts.Add(0, "niter", "lim", false, false, &nIter, "", "Maximum iteration count in linearized LS");
    opts.Add(0, "conv", "lim", false, false, &convLimit, "", "Maximum convergence criterion in estimation in meters");
    ...
    opts.Add(0, "log", "fn", false, false, &LogFile,
             "# Output [for formats see "
    ...
    opts.Add(0, "SPSout", "", false, false, &SPSout, "", "Output autonomous pseudorange solution [tag SPS, no RAIM]");
    ...
    opts.Add(0, "timefmt", "f", false, false, &userfmt, "", "Format for time tags in output");
...
int Configuration::ExtraProcessing(string &errors, string &extras) noexcept
{
    ...
    if (!refPosStr.empty())
    {
        ...
        else
        {
            fld = split(refPosStr, ',');
            if (fld.size() != 3)
                oss << "Error : invalid format or number of fields in --ref arg: " << refPosStr << endl;
            else
            {
                try
                {
                ...
                {
                    oss << "Error : invalid position in --ref arg: " << refPosStr << endl;
                }
            }
        }
    }
    ...
    if (!TropStr.empty())
    {
        ...
        {
            oss << "Error : invalid format or number of fields in --Trop arg: " << TropStr << endl;
        }
        else
        {
            msg = fld[0];
            ...
            if (msg == "ZERO")
            {
                pTrop = new ZeroTropModel();
                TropType = "Zero";
            }
            else if (msg == "BLACK")
            {
                pTrop = new SimpleTropModel();
                TropType = "Black";
            }
            else if (msg == "SAAS")
            {
                pTrop = new SaasTropModel();
                TropType = "Saas";
            }
            else if (msg == "NEWB")
            {
                pTrop = new NBTropModel();
                TropType = "NewB";
            }
            else if (msg == "GG")
            {
                pTrop = new GGTropModel();
                TropType = "GG";
            }
            else if (msg == "GGHT")
            {
                pTrop = new GGHeightTropModel();
                TropType = "GGht";
            }
            else if (msg == "NEILL")
            {
                pTrop = new NeillTropModel();
                TropType = "Neill";
            }
            else if (msg == "GLOBAL")
            {
            ...
            else
            {
                ...
                oss << "Error : invalid trop model (" << fld[0] << "); choose one of "
                    << "Zero,Black,Saas,NewB,GG,GGht,Neill,Global (cf. gnsstk::TropModel)" << endl;
            }
...
void Configuration::setWeather(const CommonTime &ttag)
{
    try
    {
        ...
        while (it != MetStore.end())
        {
            ...
            if ((nextit != MetStore.end() && ttag < nextit->time &&
                 ttag >= it->time) || // OR there is no next, but ttag is w/in 15 min of current
                (nextit == MetStore.end() && (dt = ttag - it->time) >= 0.0 && dt < 900.0))
            {
                ...
                LOG(DEBUG) << "Reset weather at " << printTime(ttag, longfmt) << " to "
                           << printTime(currentTime, longfmt) << " " << defaultTemp << " " << defaultPress << " "
                           << defaultHumid;
...
void SolutionObject::ParseDescriptor(void) noexcept
{
    ...
    { // loop over component descriptors
...
bool SolutionObject::ChooseObsIDs(map<string, vector<RinexObsID>> &mapObsTypes) noexcept
{
    size_t i;
...
string SolutionObject::dump(int level, string msg1, string msg2) noexcept
{
    ...
    else if (level >= 1)
    {
        ...
        oss << " " << setw(2) << Satellites.size() << " " << setw(2) << UsedObsIDs.size();
...
//------------------------------------------------------------------------------------
void SolutionObject::CollectData(const RinexSatID &sat, const double &elev, const double &ER,
                                 const vector<RinexDatum> &vrd) noexcept
{
    if (!isValid)
        return;
    ...
    for (size_t i = 0; i < vecSolData.size(); i++)
    {
        if (vecSolData[i].ComputeData(sat, vrd))
        {
            ...
            if (vecSolData[i].RawPR.size() > 1)
                R2.push_back(vecSolData[i].RawPR[1]);
            else
                R2.push_back(0.0);
            UsedObsIDs.insert(multimap<RinexSatID, string>::value_type(sat, vecSolData[i].usedString()));
        }
    }
}
    ...
    try
    {
        ...
        if (Satellites.size() < 4)
        {
            LOG(VERBOSE) << "Solution algorithm failed, not enough data"
                         << " for " << Descriptor << " at time " << printTime(ttag, C.longfmt);
            return -3;
        }
        ...
        if (C.weight)
        {
            ...
            for (i = 0; i < n; i++)
                if (Elevations[i] < elev0)
                { // mod only el<30
                    double invsig(::sin(Elevations[i] * DEG_TO_RAD) / sin0);
                    invMCov(i, i) = invsig * invsig;
                }
            LOG(DEBUG) << "invMeasCov for " << Descriptor << " at time " << printTime(ttag, C.longfmt) << "\n"
        ...
        {
            Matrix<double> SVP;
            iret = prs.PreparePRSolution(ttag, Satellites, PRanges, C.navLib, SVP, C.searchOrder);

            if (iret > -3)
            {
                Vector<double> Resid, Slopes;
                iret = prs.SimplePRSolution(ttag, Satellites, SVP, invMCov, C.pTrop, prs.MaxNIterations,
                                            prs.ConvergenceLimit, Resid, Slopes);
            }

            if (iret < 0)
            {
                LOG(VERBOSE) << "SimplePRS failed "
                             << (iret == -4
                                     ? "to find ANY ephemeris"
                                     : (iret == -3
                                            ? "to find enough satellites with data"
                                            : (iret == -2 ? "because the problem is singular" :
                                                          /*iret==-1*/ "because the algorithm failed to converge")))
                             << " for " << Descriptor << " at time " << printTime(ttag, C.longfmt) << " iret " << iret;
            }
            else
            {
                ...
                if (prs.RMSFlag || prs.SlopeFlag || prs.TropFlag)
                    LOG(WARNING) << "Warning for " << Descriptor << " - possible degraded SPS solution at "
        ...
        iret = prs.RAIMCompute(ttag, Satellites, PRanges, invMCov, C.navLib, C.pTrop, C.searchOrder);
        ...
        if (prs.RMSFlag || prs.SlopeFlag || prs.TropFlag)
            LOG(WARNING) << "Warning for " << Descriptor << " - possible degraded RPF solution at "
                         << printTime(ttag, C.longfmt) << " due to"
        ...
        if (prs.hasMemory && ++nepochs > 1)
            LOG(VERBOSE) << "RPF " << Descriptor << " PFR"
                         ...
                         << fixed << setprecision(3) << " " << ::sqrt(prs.getAPV()) // sig(APV)
...
int SolutionObject::WriteORDs(const CommonTime &time, const int iret)
{
    try
    {
        ...
        for (i = 0; i < Satellites.size(); i++)
        {
            ...
            if (jt == prs.dataGNSS.end())
                continue;                  // should never happen
            j = jt - prs.dataGNSS.begin(); // index
            clk = prs.Solution(3 + j);

            C.ordstrm << "ORD " << RinexSatID(Satellites[i]).toString() << " " << printTime(time, C.userfmt) << fixed
                      << setprecision(3) << " " << setw(6) << Elevations[i] << " " << setw(6) << RIono[i] << " "
                      << setw(8) << R1[i] - ERanges[i] - clk << " " << setw(8) << R2[i] - ERanges[i] - clk << " "
                      << setw(8) << PRanges[i] - ERanges[i] - clk << " " << setw(13)
                      << clk
                      //<< " " << setw(12) << PRanges[i]
                      //<< " " << setw(12) << ERanges[i]
                      << " " << Descriptor << " " << iret << endl;
        }
