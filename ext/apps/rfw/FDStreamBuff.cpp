...
int FDStreamBuff::write(const char *buffer, const int n)
{
    ...
    while (buffer < buffer_end)
    {
        ...
        if (char_written > 0)
            buffer += char_written;
        else if (char_written < 0 && !(errno == EAGAIN || errno == EINTR))
            return EOF;
        else
...
void FDStreamBuff::dump(std::ostream &out) const
{
    ...
    ost << "FDStreamBuff: "
        << " H:" << handle << std::endl
        << " put: " << std::hex << pbase() << " - " << epptr() << " curr:" << pptr() - pbase() << std::endl
        << " get:" << eback() << " - " << egptr() << " curr:" << gptr() - eback() << std::endl
        << " buff:" << base() << " - " << ebuf() << std::endl;
    out << ost.str();
}
