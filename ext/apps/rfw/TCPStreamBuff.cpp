...
IPaddress::IPaddress(const std::string &host_name)
{
    ...
    {
        cout << "Host name '" << host_name << "' isn't an AF_INET address" << endl;
        return;
    }
...
{
    struct hostent *const host_ptr = ::gethostbyaddr((char *)&addr.address, sizeof(addr.address), AF_INET);
    if (host_ptr != 0)
    ...
    sprintf(buffer, "%0d.%0d.%0d.%0d", (native_addr >> 24) & 0xff, (native_addr >> 16) & 0xff,
            (native_addr >> 8) & 0xff, native_addr & 0xff);
    return os << buffer;
}
...
{
    return os << IPaddress(addr.sin_addr.s_addr) << ':' << (unsigned short)ntohs((short)addr.sin_port);
}
...
int TCPStreamBuff::connect(const SocketAddr target_address)
{
    ...
    {
        connect_status = ::connect(handle, (sockaddr *)target_address, sizeof(target_address));
    }
    if (connect_status != 0)
...
int TCPStreamBuff::accept(int listening_socket, SocketAddr &peeraddr)
{
    ...
    for (;;)
    {
        ...
        handle = ::accept(listening_socket, (sockaddr *)peeraddr, &target_addr_size);
        if (handle >= 0)
