...
void AshtechMBEN::decode(const std::string &data)
{
    ...
    else
    {
        ...
        char c;
        iss >> seq >> c >> left >> c >> svprn >> c >> el >> c >> az >> c >> chid >> c;
    ...
    if (csum != checksum)
    {
        setstate(crcbit);
        if (debugLevel)
            cout << "checksum error, computed:" << hex << (uint16_t)csum << " received:" << checksum << dec << endl;
    }
...
{
    char c;
    str >> warning >> c >> goodbad >> c >> polarity_known >> c >> ireg >> c >> qa_phase >> c >> full_phase >> c >>
        raw_range >> c >> doppler >> c >> smoothing >> c >> smooth_cnt >> c;
...
void AshtechMBEN::code_block::dump(ostream &out) const
{
    ...
    out << hex << "warn:" << (int)warning << " gb:" << (int)goodbad << " pol:" << (int)polarity_known << dec
        << " ireg:" << (int)ireg << " qa:" << (int)qa_phase << " phase:" << asString(full_phase, 1)
        << " range:" << asString(raw_range * 1e3, 3) << " doppler:" << doppler << " smo:" << smoothing
        << " smo_cnt:" << smooth_cnt;
}
...
void AshtechMBEN::dump(ostream &out) const noexcept
{
    ...
    oss << getName() << "1:"
        << " seq:" << 0.05 * seq << " left:" << (int)left << " prn:" << (int)svprn << " el:" << (int)el
        << " az:" << (int)az << " chid:" << (int)chid << " " << (ascii ? "ascii" : "bin") << endl;
