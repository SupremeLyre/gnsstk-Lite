...
double Expression::FuncOpNode::getValue()
{
    ...
    if (op == "tan")
        return ::tan(rightVal);
    if (op == "acos")
        return ::acos(rightVal);
    if (op == "asin")
        return ::asin(rightVal);
    if (op == "atan")
        return ::atan(rightVal);
    if (op == "exp")
        return ::exp(rightVal);
    if (op == "abs")
        return ::fabs(rightVal);
    if (op == "sqrt")
        return ::sqrt(rightVal);
    if (op == "log")
        return ::log(rightVal);
    if (op == "log10")
        return ::log10(rightVal);
...
Expression::Token::Token(std::string iValue, int iPriority, bool isOp = false)
    : value(iValue), isOperator(isOp), resolved(false), priority(iPriority), expNode(0), used(false)
{
}
...
Expression::Expression(const std::string &istr) : root(0)
{
    defineOperators();
    setExpression(istr);
}
...
Expression::Expression(void) : root(0)
{
    defineOperators();
    setExpression("0");
}
...
void Expression::tokenize(const std::string &istr)
{
    ...
    for (it = operatorMap.begin(); it != operatorMap.end(); it++)
    {
        ...
        while ((position = str.find(it->first, position + 1)) != string::npos)
        {
            ...
            {
                sciNotation = (((str.substr(position - 1, 1) == "E") || (str.substr(position - 1, 1) == "e")) &&
...
void Expression::buildExpressionTree(void)
{
    ...
    while (totalResolved < tList.size())
    {
        ...
        if (targetToken->getOperator())
        {
            ...
            {
                ExpNode *opNode = new FuncOpNode(targetToken->getValue(), rightArg->getNode());
                targetToken->setNode(opNode);
...
bool Expression::set(const std::string name, double value)
{
    ...
    for (t = 0, i = eList.begin(); i != eList.end(); t++, i++)
    {
        ...
        if (vnode != 0)
        {
            if (StringUtils::upperCase(vnode->name) == StringUtils::upperCase(name))
            {
...
bool Expression::setSvObsEpoch(const SvObsEpoch &soe)
{
    ...
    for (SvObsEpoch::const_iterator i = soe.begin(); i != soe.end(); i++)
    {
        ...
        case ObservationType::SNR:
            type = "S";
            break;
        case ObservationType::SSI:
            ignore = true;
            break;
        case ObservationType::LLI:
            ignore = true;
            break;
        case ObservationType::TrackLen:
            ignore = true;
            break;
        default:
