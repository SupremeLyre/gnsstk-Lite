 ...
 */
Triple OceanLoading::getOceanLoading(const string &name, const CommonTime &t)
{
    ...
    for (int i = 0; i < NUM_COMPONENTS; i++)
    {
        ...
        for (int k = 0; k < NUM_HARMONICS; k++)
        {
            ...
            temp += harmonics(i, k) * std::cos(arguments(k) - harmonics((i + 3), k) * DEG_TO_RAD);
        }
...
Vector<double> OceanLoading::getArg(const CommonTime &time)
{
    ...
    double S0((((1.9e-6 * t - 0.001133) * t + 481267.88314137) * t + 270.434358) * DEG_TO_RAD);

    // Mean longitude of lunar perigee at beginning of day
    double P0((((-1.2e-5 * t - 0.010325) * t + 4069.0340329577) * t + 334.329653) * DEG_TO_RAD);

    for (int k = 0; k < NUM_HARMONICS; k++)
    {

        double temp(sig(k) * fday + angfac(0, k) * H0 + angfac(1, k) * S0 + angfac(2, k) * P0 + angfac(3, k) * TWO_PI);
