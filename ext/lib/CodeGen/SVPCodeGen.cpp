...
SVPCodeGen::SVPCodeGen(const int SVPRNID, const gnsstk::CommonTime &dt)
{
    if (SVPRNID < 0 || SVPRNID > 210)
    {
        gnsstk::Exception e("Must provide a prn between 0 and 210");
        GNSSTK_THROW(e);
    }
    currentZTime = dt;
    PRNID = SVPRNID;
...
void SVPCodeGen::getCurrentSixSeconds(CodeBuffer &pcb)
{
    ...
    */
    if (X1count == LAST_6SEC_ZCOUNT_OF_WEEK)
        X2Seq.setEOWX2Epoch(true);
    else
        X2Seq.setEOWX2Epoch(false);
