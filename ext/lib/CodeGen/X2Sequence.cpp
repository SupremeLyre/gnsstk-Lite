...
uint32_t X2Sequence::EOWEndOfSequence[LENGTH_OF_EOW_OVERLAP] = {
    0xFA5F8298, 0xB30C04D9, 0xD5CACBCA, 0x0ED47FFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
...
X2Sequence::X2Sequence()
{
    if (isInit != true)
    {
        gnsstk::Exception e("Must call X2Sequence::allocateMemory() before instantiating a X2Sequence object.");
        GNSSTK_THROW(e);
    }
...
void X2Sequence::allocateMemory()
{
    ...
    if (isInit == true)
    {
        gnsstk::Exception e("X2Sequence::allocateMemory() called multiple times");
        GNSSTK_THROW(e);
    }
    ...
    {
        gnsstk::Exception e("X2Sequence::allocateMemory() - allocation failed.");
        GNSSTK_THROW(e);
    }
    ...
    gnsstk::GenXSequence X2A(X2A_INIT, X2A_TAPS, XA_COUNT, XA_EPOCH_DELAY + X2A_EPOCH_DELAY);
    gnsstk::GenXSequence X2B(X2B_INIT, X2B_TAPS, XB_COUNT, XB_EPOCH_DELAY + X2A_EPOCH_DELAY);
...
void X2Sequence::deAllocateMemory()
{
    ...
    {
        gnsstk::Exception e("X2Sequence::deAllocateMemory() called when no memory allocated.");
        GNSSTK_THROW(e);
    }
    delete[] X2Bits;
    delete[] X2BitsEOW;
