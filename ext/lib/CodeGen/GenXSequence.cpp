...
{
// Constructor
GenXSequence::GenXSequence(const unsigned int initialState, const unsigned int tapRegister,
                           const int initialLengthOfSequence, const int maxDelay)
{
     ...
     *  problem.
     */
    unsigned int mask12bits[12] = {0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020,
    ...
    for (i = 0; i < lengthOfSequence; ++i)
    {
        ...
        {
            if ((andBits & mask12bits[bit12cnt]) != 0)
                ++cnt;
        }
        int newBit = cnt % 2;
        reg |= newBit;
...
uint32_t GenXSequence::operator[](int ia)
{
    ...
    if ((i + MAX_BIT) <= lengthOfSequence)
    {
        if (offset == 0)
            ...
            retArg = merge(bits[ndx1], bits[ndx1 + 1], offset);
    }
    /*
