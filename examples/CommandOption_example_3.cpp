...
CommandOption3Example ::CommandOption3Example(const string &applName)
    : BasicFramework(applName, "Example application for CommandOption"),
      gpsEWSOpt('o', "ews", "%E %G %g", "\"GPSEpoch 10bitGPSweek SecondOfWeek\""),
      gpsWSOpt('f', "ws", "%F %g", "\"FullGPSWeek SecondOfWeek\""),
      gpsWZOpt('w', "wz", "%F %Z", "\"FullGPSWeek Zcount\""), gpsZ29Opt(0, "z29", "%E %c", "\"29bitZcount\""),
      gpsZ32Opt('Z', "z32", "%C", "\"32bitZcount\""), screamOpt(0, "scream", "print a message very loudly"),
...
bool CommandOption3Example ::initialize(int argc, char *argv[], bool pretty) noexcept
{
    ...
    for (unsigned i = 0; i < timeOpts.size(); i++)
    {
        ...
        {
            CommandOptionWithCommonTimeArg *opt = (CommandOptionWithCommonTimeArg *)timeOpts[i];
            ...
            cout << "Option #" << order << " " << opt->getValue()[j] << " = "
                 << printTime(opt->getTime()[j], "%04Y/%02m/%02d %02H:%02M:%02S") << endl;
        }
    }
    if (screamOpt)
    {
    ...
    if (which != nullptr)
    {
        cout << "You used " << which->getFullOptionString() << " (and possibly others)" << endl;
    }
    return true;
}

void CommandOption3Example ::process()
{
    cout << "Nothing to do" << endl;
}

void CommandOption3Example ::shutDown()
{
