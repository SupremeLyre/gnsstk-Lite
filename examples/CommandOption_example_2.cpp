...
CommandOption2Example ::CommandOption2Example(const string &applName)
    : BasicFramework(applName, "Example application for CommandOption"), numOpt('n', "num", "your choice of integer"),
      floatOpt('D', "data", "specify any number of decimal numbers"),
      timeOpt('t', "time", "%Y/%m/%d %H:%M:%S", "specify a time (%Y/%m/%d %H:%M:%S)", true), num(0)
{
    numOpt.setMaxCount(1);
    timeOpt.setMaxCount(1);
}

bool CommandOption2Example ::initialize(int argc, char *argv[], bool pretty) noexcept
{
...
void CommandOption2Example ::process()
{
    ...
    cout << "Requested time:" << printTime(beginTime, "%Y/%02m/%02d %02H:%02M:%02S") << endl;
}

void CommandOption2Example ::shutDown()
{
