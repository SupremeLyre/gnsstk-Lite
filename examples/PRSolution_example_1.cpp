...
int main(int argc, char *argv[])
{
    ...
    /** Object for void-type tropospheric model (in case no
     * meteorological RINEX is available) */
    ...
    /** Pointer to one of the two available tropospheric models. It
     * points to the void model by default */
    ...
    try
    {
        ...
        while (roffs >> rod)
        {
            ...
            while ((argc == 4) && (!rml.empty()) && (mi != rml.end()) && ((*mi).time < rod.time))
            {
                ...
                ggTropModel.setWeather((*mi).data[RinexMetHeader::TD], (*mi).data[RinexMetHeader::PR],
            ...
            if (rod.epochFlag == 0 || rod.epochFlag == 1) // Begin usable data
            {
                ...
                raimSolver.RAIMComputeUnweighted(rod.time, prnVec, rangeVec, navLib, tropModelPtr);
