...
int main()
{
    ...
    while (cin)
    {
        ...
        navFiltData.prn = gnsstk::StringUtils::asUnsigned(gnsstk::StringUtils::word(line, 2, ','));
        // note that the test file contents use enums that probably
        // don't match ObsID's enums but that's really not important
        // for this example.
        navFiltData.carrier = (CarrierBand)gnsstk::StringUtils::asInt(gnsstk::StringUtils::word(line, 3, ','));
        navFiltData.code = (gnsstk::TrackingCode)gnsstk::StringUtils::asInt(gnsstk::StringUtils::word(line, 4, ','));
        ...
        {
            cout << timeString << ", 310, " << setw(2) << (*nmli)->prn << ", " << setw(1)
                 << static_cast<int>((*nmli)->carrier) << ", " << static_cast<int>((*nmli)->code) << ", 1";
            LNavFilterData *fd = dynamic_cast<LNavFilterData *>(*nmli);
            for (unsigned sfword = 0; sfword < 10; sfword++)
            {
                cout << ", " << hex << setiosflags(ios::uppercase) << setw(8) << setfill('0') << fd->sf[sfword] << dec
                     << setfill(' ');
            }
            cout << endl;
        }
