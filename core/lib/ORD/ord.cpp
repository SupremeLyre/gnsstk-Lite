...
double IonosphereFreeRange(const std::vector<double> &frequencies, const std::vector<double> &pseudoranges)
{
    ...
    {
        gnsstk::Exception exc("Mismatch between frequency and pseudorange array size");
        GNSSTK_THROW(exc)
    }
    ...
    {
        gnsstk::Exception exc("Multiple frequency and range values are required.");
        GNSSTK_THROW(exc)
    }
    ...
    if (frequencies.size() > 2)
    {
        gnsstk::Exception exc("Only dual-frequency ionosphere correction is supported.");
        GNSSTK_THROW(exc)
    }

    // TODO(someone): Add proper gamma calculation for arbitrary
    //                number of frequencies.
    const double gamma = (frequencies[0] / frequencies[1]) * (frequencies[0] / frequencies[1]);
...
double RawRange1(const Position &rxLoc, const SatID &satId, const CommonTime &timeReceived, NavLibrary &ephemeris,
                 Xvt &svXvt)
{
    bool success;
    double range;
    ...
    std::tie(success, range, svXvt) =
        RawRange::fromReceive(rxLoc, timeReceived, ephemeris, NavSatelliteID(satId), ellipsoid);
    GNSSTK_ASSERT(success);
    return range;
}

double RawRange2(double pseudorange, const Position &rxLoc, const SatID &satId, const CommonTime &time,
                 NavLibrary &ephemeris, Xvt &svXvt)
{
    ...
    GPSEllipsoid ellipsoid;
    std::tie(success, range, svXvt) =
        RawRange::fromNominalReceiveWithObs(rxLoc, time, pseudorange, ephemeris, NavSatelliteID(satId), ellipsoid);
    GNSSTK_ASSERT(success);
    return range;
}

double RawRange3(double pseudorange, const gnsstk::Position &rxLoc, const gnsstk::SatID &satId,
                 const gnsstk::CommonTime &time, NavLibrary &ephemeris, gnsstk::Xvt &svXvt)
{
    bool success;
    double range;
    GPSEllipsoid ellipsoid;
    std::tie(success, range, svXvt) =
        RawRange::fromSvTransmitWithObs(rxLoc, pseudorange, ephemeris, NavSatelliteID(satId), time, ellipsoid, true);
    GNSSTK_ASSERT(success);
    return range;
}

double RawRange4(const gnsstk::Position &rxLoc, const gnsstk::SatID &satId, const gnsstk::CommonTime &time,
                 NavLibrary &ephemeris, gnsstk::Xvt &svXvt)
{
    ...
    std::tie(success, range, rotatedSvXvt) =
        RawRange::fromSvTransmit(rxLoc, ephemeris, NavSatelliteID(satId), time, ellipsoid, true, SVHealth::Any,
                                 NavValidityType::ValidOnly, NavSearchOrder::User, tofEstimate, 0, 2);
    GNSSTK_ASSERT(success);
    ...
    std::tie(success, range, rotatedSvXvt) = RawRange::fromNominalReceiveWithObs(
        rxLoc, time, fakePsuedorange, ephemeris, NavSatelliteID(satId), ellipsoid, false);
    GNSSTK_ASSERT(success);
    svXvt = rotatedSvXvt;
    return range;
}
...
double TroposphereCorrection(const gnsstk::TropModel &tropModel, const gnsstk::Position &rxLoc,
