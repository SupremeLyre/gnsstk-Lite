...
double getLeapSeconds(const int year, const int month, const double day)
{
    ...
    static const struct
    {
        ...
        {1960, 1, 1.4178180, 0.0012960}, {1961, 1, 1.4228180, 0.0012960},  {1961, 8, 1.3728180, 0.0012960},
        {1962, 1, 1.8458580, 0.0011232}, {1963, 11, 1.9458580, 0.0011232}, {1964, 1, 3.2401300, 0.0012960},
        {1964, 4, 3.3401300, 0.0012960}, {1964, 9, 3.4401300, 0.0012960},  {1965, 1, 3.5401300, 0.0012960},
        {1965, 3, 3.6401300, 0.0012960}, {1965, 7, 3.7401300, 0.0012960},  {1965, 9, 3.8401300, 0.0012960},
        {1966, 1, 4.3131700, 0.0025920}, {1968, 2, 4.2131700, 0.0025920}};
    ...
    static const struct
    {
        ...
        {2009, 1, 34}, {2012, 7, 35}, {2015, 7, 36}, {2017, 1, 37}, // leave the last comma!
                                                                    // add new entry here, of the form:
                                                                    // { year, month(1-12), leap_sec }, // leave the
                                                                    // last comma!
    };
    ...
    else if (year < 1972)
    {
        ...
        {
            if (preleap[i].year > year || (preleap[i].year == year && preleap[i].month > month))
            {
                continue;
            }
    ...
    { // [1972- leap seconds
        ...
        {
            if (leaps[i].year > year || (leaps[i].year == year && leaps[i].month > month))
            {
                continue;
            }
...
double getTimeSystemCorrection(const TimeSystem inTS, const TimeSystem outTS, const int year, const int month,
                               const double day)
{
    double dt(0.0);
