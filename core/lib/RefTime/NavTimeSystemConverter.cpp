...
namespace gnsstk
{
bool NavTimeSystemConverter ::getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &t, double &offs)
{
    DEBUGTRACE_FUNCTION();
    if (fromSys == toSys)
    {
    ...
    {
        DEBUGTRACE("successful direct conversion, offs=" << std::setprecision(15) << std::scientific << offs);
        return true;
    }
    double offs1, offs2;
    // that didn't work, try converting fromSys->UTC->toSys
    if (navLib->getOffset(fromSys, TimeSystem::UTC, t, offs1, SVHealth::Healthy) &&
        navLib->getOffset(TimeSystem::UTC, toSys, t, offs2, SVHealth::Healthy))
    {
        offs = offs1 + offs2;
        DEBUGTRACE("successful indirect conversion, offs=" << std::setprecision(15) << std::scientific << offs);
        return true;
    }
