...
#define CHECK_SV_HERE(itty, prn)                                                                                       \
    if (itty == almPRN.end())                                                                                          \
    {                                                                                                                  \
        SVNotPresentException exc("Attempt to get data from EngAlmanac from a SV that is not"                          \
                                  " present.");                                                                        \
        GNSSTK_THROW(exc);                                                                                             \
    }
...
namespace gnsstk
{
EngAlmanac ::EngAlmanac() noexcept
{
    for (int n = 0; n < 4; n++)
    {
        alpha[n] = beta[n] = 0.0;
...
bool EngAlmanac::addSubframe(const long subframe[10], const int gpsWeek)
{
    uint32_t tinput[10];
    for (int n = 0; n < 10; ++n)
...
bool EngAlmanac::addSubframe(const uint32_t subframe[10], const short gpsWeek)
{
    double ficked[60];
    ...
    if ((ficked[0] != 0x8b) || ((ficked[4] != 4) && (ficked[4] != 5)) || (pat < 4) || (pat > 10))
    {
        InvalidParameter exc("EngAlmanac::addSubframe: Not a valid almanac page.");
        GNSSTK_THROW(exc);
    }
    ...
    case 4:
        ...
        {
            InvalidParameter exc("EngAlmanac::addSubframe, PRN out of range " + StringUtils::asString(ficked[19]));
            GNSSTK_THROW(exc);
        }
        {
            ...
            if (prn)
            {
                ...
                almPRN[sat] = AlmOrbit(prn, ficked[7], ficked[9], ficked[10], ficked[12], ficked[13], ficked[14],
                                       ficked[15], ficked[16], ficked[17], static_cast<long>(ficked[8]),
                                       static_cast<long>(ficked[2]), gpsWeek, static_cast<short>(ficked[11]));
            }
        }
        break;
...
short EngAlmanac::get6bitHealth(SatID sat) const
{
    ...
    if (i == health.end())
    {
        SVNotPresentException svnpe("SV health not present for PRN " + StringUtils::asString(sat.id));
        GNSSTK_THROW(svnpe);
    }
...
short EngAlmanac::getSVConfig(SatID sat) const
{
    ...
    if (i == SV_config.end())
    {
        SVNotPresentException svnpe("SV Configuration not present for PRN " + StringUtils::asString(sat.id));
        GNSSTK_THROW(svnpe);
    }
...
void EngAlmanac::getIon(double a[4], double b[4]) const
{
    if (!haveUTC)
    {
        InvalidRequest exc("UTC offset (subframe 4, page 18) is not present.");
        GNSSTK_THROW(exc);
    }
    for (int n = 0; n < 4; n++)
    {
...
void EngAlmanac::getUTC(double &a0, double &a1, double &deltaTLS, long &tot, int &WNt, int &WNLSF, int &DN,
                        double &deltaTLSF) const
{
    if (!haveUTC)
    {
        InvalidRequest exc("UTC offset (subframe 4, page 18) is not present.");
...
bool EngAlmanac::check(ostream &s) const
{
    ...
    for (int prn = 1; prn <= 32; prn++)
    {
        try
        {
            ...
            if (svToa != p51Toa)
            {
                s << "Toa mis-match on prn " << prn << "  page 51 Toa=" << p51Toa << ", SV Toa=" << svToa << endl;
                good = false;
            }
        }
...
void EngAlmanac::dump(ostream &s, bool checkFlag) const
{
    ...
    s << scientific << setprecision(8) << "A0:       " << setw(15) << A0 << " sec" << endl
      << "A1:       " << setw(15) << A1 << " sec/sec" << endl
      << fixed << setprecision(1) << "dt_ls:    " << setw(15) << dt_ls << " sec" << endl
    ...
    s << "Toa:    " << setfill(' ') << setw(8) << t_oa << ", week: " << setw(5) << wn_a << " (" << alm_wk << ")" << endl
      << endl
    ...
    for (int i = 1; i <= 16; i++)
        s << setw(2) << i << "    " << bits[i] << "    " << setw(2) << i + 16 << "    " << bits[i + 16] << endl;
