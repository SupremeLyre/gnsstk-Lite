 ...
 * @note This vector corresponds to a proprietary file format and
...
static double PItab[7] = {1 / (PI * PI * PI), 1 / (PI * PI), 1 / PI, 1, PI, PI *PI, PI *PI *PI};

EngNav::EngNav() noexcept
{
}

// Retained for backward compatibility
bool EngNav ::subframeConvert(const long input[10], int gpsWeek, double output[60]) noexcept
{
    uint32_t tinput[10];
    for (int n = 0; n < 10; ++n)
...
bool EngNav ::subframeConvert(const uint32_t input[10], short gpsWeek, double output[60]) noexcept
{
    short patId = -2, i = 2;
    ...
    if (patId == 1)
    {
        ...
        output[5] = static_cast<double>(convertXBit(gpsWeek, week10Bit, BITS10));
    }
    return true;
}

// Retained for backward compatibility
bool EngNav ::convert8bit(int gpsWeek, double *output) noexcept
{
...
// Retained for backward compatibility
bool EngNav ::convert10bit(int gpsWeek, double *output) noexcept
{
    short tgpsWeek = static_cast<short>(gpsWeek);
    short toutput = static_cast<short>(*output);
...
short EngNav ::convertXBit(short fullGPSWeek, short incompleteGPSWeek, BitConvertType type)
{
...
// Retained for backward compatibility
short EngNav ::getSubframePattern(const long input[10]) noexcept
{
    uint32_t tinput[10];
    for (int n = 0; n < 10; ++n)
        tinput[n] = static_cast<uint32_t>(input[n]);
...
short EngNav ::getSubframePattern(const uint32_t input[10]) noexcept
{
    short iret, svid;
    long itemp;

    short patId[] = {5, 6, 6, 6, 10, 8, 6, 7, 7, 7, 7, 7, 9};
...
bool EngNav ::sv2page(short svpgid, short &subframe, short &page) noexcept
{
    if (svpgid < 1)
        return false;
...
bool EngNav ::sfpage2svid(short subframe, short page, short &svpgid) noexcept
{
    static const short sf4pg[25] = {57, 25, 26, 27, 28, 57, 29, 30, 31, 32, 57, 62, 52,
                                    53, 54, 57, 55, 56, 58, 59, 57, 60, 61, 62, 63};
...
bool EngNav ::zcount2page(unsigned long zcount, short &subframe, short &page) noexcept
{
...
uint32_t EngNav ::computeParity(uint32_t sfword, uint32_t psfword, bool knownUpright)
{
    /*
    ...
    */
    uint32_t bmask[6] = {0x3B1F3480L, 0x1D8F9A40L, 0x2EC7CD00L, 0x1763E680L, 0x2BB1F340L, 0x0B7A89C0L};
...
uint32_t EngNav ::fixParity(uint32_t sfword, uint32_t psfword, bool nib, bool knownUpright)
{
    uint32_t bmask[6] = {0x3B1F3480L, 0x1D8F9A40L, 0x2EC7CD00L, 0x1763E680L, 0x2BB1F340L, 0x0B7A89C0L};
...
void EngNav ::convertQuant(const uint32_t input[10], double output[60], const DecodeQuant &dq) noexcept
{
    double dval;
...
void EngNav ::dump(std::ostream &s)
{
    for (short pattern = 1; pattern <= 10; pattern++)
    {
        ...
        s << "****************************************"
          ...
          << " n   OutIndex   pow2    powPI     Scale   Signq   Start  #bits   Start  #bits" << std::endl;
        for (unsigned n = 0; n < subframeListFoo[pattern].size(); n++)
        {
            ...
            s << std::dec << std::setw(2) << n << "        " << std::setw(3) << p->outIndex << "    " << std::setw(3)
              << p->pow2 << "      " << std::setw(3) << p->powPI << "       " << std::setw(3) << p->scale << "     "
              << std::setw(3) << p->signq << "     " << std::setw(3) << p->fmt[0].startBit << "    " << std::setw(3)
              << p->fmt[0].numBits << "     " << std::setw(3) << p->fmt[1].startBit << "    " << std::setw(3)
              << p->fmt[1].numBits << std::endl;
        }
        s << std::endl;
    }
    s << std::endl;
    ...
    for (int i = 0; i < 7; i++)
    {
        s << "    " << i << "       " << std::setw(2) << x << "        " << std::setw(8) << std::setprecision(5)
          << PItab[i] << std::endl;
        x++;
    }
}

bool EngNav ::getNMCTValidity(const uint32_t sf2[10], unsigned howWeek, uint32_t &aodo, CommonTime &tnmct,
                              CommonTime &toe, CommonTime &tot)
{
