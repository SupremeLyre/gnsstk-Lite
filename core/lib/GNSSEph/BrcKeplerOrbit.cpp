...
BrcKeplerOrbit::BrcKeplerOrbit() noexcept
{
    dataLoaded = false;
    ...
    Cuc = Cus = Crc = Crs = Cic = Cis = M0 = dn = dndot = ecc = A = Ahalf = Adot = OMEGA0 = i0 = w = OMEGAdot = idot =
        0.0;
}

BrcKeplerOrbit::BrcKeplerOrbit(const std::string satSysArg, const ObsID obsIDArg, const short PRNIDArg,
                               const CommonTime beginFitArg, const CommonTime endFitArg, const CommonTime ToeArg,
                               const short URAoeArg, const bool healthyArg, const double CucArg, const double CusArg,
                               const double CrcArg, const double CrsArg, const double CicArg, const double CisArg,
                               const double M0Arg, const double dnArg, const double dndotArg, const double eccArg,
                               const double AArg, const double AhalfArg, const double AdotArg, const double OMEGA0Arg,
                               const double i0Arg, const double wArg, const double OMEGAdotARg, const double idotArg)
{
    loadData(satSysArg, obsIDArg, PRNIDArg, beginFitArg, endFitArg, ToeArg, URAoeArg, healthyArg, CucArg, CusArg,
             CrcArg, CrsArg, CicArg, CisArg, M0Arg, dnArg, dndotArg, eccArg, AArg, AhalfArg, AdotArg, OMEGA0Arg, i0Arg,
             wArg, OMEGAdotARg, idotArg);
}

/// Legacy GPS Subframe 1-3
BrcKeplerOrbit::BrcKeplerOrbit(const ObsID obsIDArg, const short PRNID, const short fullweeknum,
                               const long subframe1[10], const long subframe2[10], const long subframe3[10])
{
...
{
    return ((dataLoaded == right.dataLoaded) && (satSys == right.satSys) && (obsID == right.obsID) &&
            (PRNID == right.PRNID) && (Toe == right.Toe) && (URAoe == right.URAoe) && (healthy == right.healthy) &&
            (Cuc == right.Cuc) && (Cus == right.Cus) && (Crc == right.Crc) && (Crs == right.Crs) &&
            (Cic == right.Cic) && (Cis == right.Cis) && (M0 == right.M0) && (dn == right.dn) &&
            (dndot == right.dndot) && (ecc == right.ecc) && (A == right.A) && (Ahalf == right.Ahalf) &&
            (Adot == right.Adot) && (OMEGA0 == right.OMEGA0) && (i0 == right.i0) && (w == right.w) &&
            (OMEGAdot == right.OMEGAdot) && (idot == right.idot) && (beginFit == right.beginFit) &&
            (endFit == right.endFit));
}

void BrcKeplerOrbit::loadData(const std::string satSysArg, const ObsID obsIDArg, const short PRNIDArg,
                              const CommonTime beginFitArg, const CommonTime endFitArg, const CommonTime ToeArg,
                              const short URAoeArg, const bool healthyArg, const double CucArg, const double CusArg,
                              const double CrcArg, const double CrsArg, const double CicArg, const double CisArg,
                              const double M0Arg, const double dnArg, const double dndotArg, const double eccArg,
                              ...
                              const long subframe1[10], const long subframe2[10], const long subframe3[10])
{
    ...
    /*
      ...
      weeknum++;                 // Convert week # of transmission to week # of epoch time when Toc is forward across a
      week boundary else if (diff < -HALFWEEK) weeknum--;                 // Convert week # of transmission to week # of
      epoch time when Toc is back across a week boundary
    */
...
Xvt BrcKeplerOrbit::svXvt(const CommonTime &t) const
{
    ...
    ANLON = OMEGA0 + (OMEGAdot - ell.angVelocity()) * elapte - ell.angVelocity() * ToeSOW;
    ...
    drv = A * lecc * dek * sinea - 2.e0 * dlk * (Crc * s2al - Crs * c2al) + Adot * G;

    dxp = drv * cosu - R * sinu * duv;
    dyp = drv * sinu + R * cosu * duv;

    // Calculate velocities
    vxef = dxp * can - xip * san * domk - dyp * cinc * san + yip * (sinc * san * div - cinc * can * domk);
    vyef = dxp * san + xip * can * domk + dyp * cinc * can - yip * (sinc * can * div + cinc * san * domk);
...
void BrcKeplerOrbit::dump(ostream &s) const noexcept
{
    ...
    s << endl
      ...
      << "Right ascension:       " << setw(16) << OMEGA0 << " rad    " << setw(16) << OMEGAdot << " rad/sec" << endl
      << "Inclination:           " << setw(16) << i0 << " rad    " << setw(16) << idot << " rad/sec" << endl;

    s << endl
      << "           HARMONIC CORRECTIONS" << endl
      << endl
      << "Radial        Sine: " << setw(16) << Crs << " m    Cosine: " << setw(16) << Crc << " m" << endl
      << "Inclination   Sine: " << setw(16) << Cis << " rad  Cosine: " << setw(16) << Cic << " rad" << endl
      << "In-track      Sine: " << setw(16) << Cus << " rad  Cosine: " << setw(16) << Cuc << " rad" << endl;
