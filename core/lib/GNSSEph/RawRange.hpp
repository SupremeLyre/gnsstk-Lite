 ...
 * Angular velocity | N/A | @f$\omega@f$ | An ECEF reference frame's angular velocity, typically provided by an
 * ellipsoid model Receiver (RX) position | rxPos | @f$r^{rx}@f$ | A receiver's position in ECEF coordinates SV position
 * | svPos | @f$r^{sv}@f$ | An SV's position in ECEF coordinates Nominal receive time | receiveNominal |
 * @f$\tilde{t}_{rx}@f$ | The receive time in the time frame of the receiver. Receive time | receive | @f$t_{rx}@f$ |
 * The true receive time Nominal transmit time | transmitNominal | @f$\tilde{t}_{sv}@f$ | The transmit time in the time
 * frame of the transmitter (e.g. SV) Transmit time | transmit | @f$t_{sv}@f$ | The true transmit time SV clock offset |
 * N/A | @f$\Delta t_{sv}@f$ | An SV clock's offset from a true time frame (typically GPS) RX relativity correction |
 * @f$\delta t_{sv}^{rel}@f$ | A relativity correction due to the ellipictal orbits of SVs
 *
 *
 */
class RawRange
{
  public:
     ...
     */
    static std::tuple<bool, double, Xvt> fromSvPos(const Position &rxPos, const Xvt &svXvt,
                                                   const EllipsoidModel &ellipsoid, bool smallAngleApprox = false,
                                                   double seed = 0.07, double threshold = 1.e-13, int maxIter = 5);
     ...
     *    data was found to compute the Xvt.
     */
    static std::tuple<bool, double, Xvt> fromSvTransmit(const Position &rxPos, NavLibrary &navLib,
                                                        ...
                                                        NavSearchOrder order = NavSearchOrder::User, double seed = 0.07,
     ...
     *    data was found to compute the Xvt.
     */
    static std::tuple<bool, double, Xvt> fromReceive(const Position &rxPos, const CommonTime &receive,
    ...
    static std::tuple<bool, double, Xvt> fromNominalReceiveWithObs(
        const Position &rxPos, const CommonTime &receiveNominal, double pseudorange, NavLibrary &navLib,
        const NavSatelliteID &sat, const EllipsoidModel &ellipsoid, bool smallAngleApprox = false,
        SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly,
    ...
    static std::tuple<bool, double, Xvt> fromSvTransmitWithObs(
        const Position &rxPos, double pseudorange, NavLibrary &navLib, const NavSatelliteID &sat,
        const CommonTime &transmit, const EllipsoidModel &ellipsoid, bool smallAngleApprox = false,
        SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly,
        NavSearchOrder order = NavSearchOrder::User);
    ...
    static std::tuple<bool, double, Xvt> fromNominalReceive(
        const Position &rxPos, const CommonTime &receiveNominal, NavLibrary &navLib, const NavSatelliteID &sat,
        const EllipsoidModel &ellipsoid, bool smallAngleApprox = false, SVHealth xmitHealth = SVHealth::Any,
        NavValidityType valid = NavValidityType::ValidOnly, NavSearchOrder order = NavSearchOrder::User,
        double seed = 0.7, double threshold = 1.e-13, int maxIter = 5);
     ...
     * @f]
     * @f[
     *    \rho(\tau, t_{sv}) = \biggl|\biggl|\mathbf{R3}\bigl(\tau*\omega\bigl)\cdot r^{sv}(t_{sv}) -
     * r^{rx}\biggl|\biggl|
     * @f]
     * @f[
     *    \tau = \rho(\tau, t_{sv}) / c
    ...
    static std::tuple<bool, CommonTime> estTransmitFromReceive(
        const Position &rxPos, const CommonTime &receive, NavLibrary &navLib, const NavSatelliteID &sat,
        const EllipsoidModel &ellipsoid, SVHealth xmitHealth = SVHealth::Any,
        NavValidityType valid = NavValidityType::ValidOnly, NavSearchOrder order = NavSearchOrder::User,
        double seed = 0.07, double threshold = 1.e-13, int maxIter = 5);
     ...
     *    to compute the Xvt
     */
    static std::tuple<bool, CommonTime> estTransmitFromObs(const CommonTime &receiveNominal, double pseudorange,
                                                           NavLibrary &navLib, const NavSatelliteID &sat,
                                                           SVHealth xmitHealth = SVHealth::Any,
                                                           NavValidityType valid = NavValidityType::ValidOnly,
                                                           NavSearchOrder order = NavSearchOrder::User);
     ...
     */
    static std::tuple<double, Xvt> computeRange(const Position &rxPos, const CommonTime &receive, const Xvt &svXvt,
     ...
     */
    static std::tuple<double, Xvt> computeRange(const Position &rxPos, const Xvt &svXvt, double tof,
                                                const EllipsoidModel &ellipsoid, bool smallAngleApprox = false);
     ...
     *    coordinates (@f$\mathbf{R3}(\tau*\omega)\cdot r^{sv}@f$)
     */
    static std::tuple<double, Position> computeRange(const Position &rxPos, const CommonTime &receive,
     ...
     */
    static std::tuple<double, Position> computeRange(const Position &rxPos, const Position &svPos, double tof,
                                                     const EllipsoidModel &ellipsoid, bool smallAngleApprox = false);
     ...
     */
    static Triple rotateECEF(const Triple &vec, double dt, const EllipsoidModel &ellipsoid,
                             bool smallAngleApprox = false);
     ...
     *    as the input \a vec.
     */
    static Position rotateECEF(const Position &vec, double dt, const EllipsoidModel &ellipsoid,
                               bool smallAngleApprox = false);
