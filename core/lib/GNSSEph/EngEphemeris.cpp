...
EngEphemeris::EngEphemeris() noexcept
{
    ...
    PRNID = tracker = ASalert[0] = ASalert[1] = ASalert[2] = weeknum = codeflags = health = L2Pdata = 0;
...
bool EngEphemeris::operator==(const gnsstk::EngEphemeris &right) const noexcept
{
    ...
    return ((tlm_message[0] == right.tlm_message[0]) && (tlm_message[1] == right.tlm_message[1]) &&
            (tlm_message[2] == right.tlm_message[2]) && (satSys == right.satSys) && (PRNID == right.PRNID) &&
            (tracker == right.tracker) && (HOWtime[0] == right.HOWtime[0]) && (HOWtime[1] == right.HOWtime[1]) &&
            (HOWtime[2] == right.HOWtime[2]) && (ASalert[0] == right.ASalert[0]) && (ASalert[1] == right.ASalert[1]) &&
            (ASalert[2] == right.ASalert[2]) && (weeknum == right.weeknum) && (codeflags == right.codeflags) &&
            (health == right.health) && (L2Pdata == right.L2Pdata) && (IODC == right.IODC) && (IODE == right.IODE) &&
            (AODO == right.AODO) && (fitint == right.fitint) && (Tgd == right.Tgd) && (bcClock == right.bcClock) &&
            (orbit == right.orbit));
}
...
bool EngEphemeris::addSubframe(const uint32_t subframe[10], const int gpsWeek, const short PRN, const short track)
{
    ...
    int sfNdx = sfID - 1;
    for (int i = 0; i < 10; ++i)
        subframeStore[sfNdx][i] = subframe[i];
...
bool EngEphemeris::addSubframeNoParity(const uint32_t subframe[10], const int gpsWeek, const short PRN,
                                       const short track)
{
...
bool EngEphemeris::addIncompleteSF1Thru3(const uint32_t sf1[8], const uint32_t sf2[8], const uint32_t sf3[8],
                                         const long sf1TransmitSOW, const int gpsWeek, const short PRN,
                                         const short track)
{
 ...
 */
bool EngEphemeris::unifiedConvert(const int gpsWeek, const short PRN, const short track)
{
    double ficked[60];
    ...
    orbit.loadData(satSys, obsID, PRN, beginFit, endFit, ToeCT, accFlag, healthy, Cuc, Cus, Crc, Crs, Cic, Cis, M0, dn,
                   dnDot, ecc, A, Ahalf, Adot, OMEGA0, i0, w, OMEGAdot, idot);

    bcClock.loadData(satSys, obsID, PRNID, TocCT, accFlag, healthy, af0, af1, af2);

    return true;
}

bool EngEphemeris ::isValid() const noexcept
{
    try
    {
        return ((PRNID >= 1) && (PRNID <= 37) && (HOWtime[0] >= 0) && (HOWtime[0] <= 604784) && (HOWtime[1] >= 0) &&
                (HOWtime[1] <= 604784) && (HOWtime[2] >= 0) && (HOWtime[2] <= 604784) && (getToc() >= 0) &&
                (getToc() <= 604784) && (getToe() >= 0) && (getToe() <= 604784) && (getEcc() >= 0) &&
                (getEcc() <= 0.03));
    }
    catch (...)
    {
...
bool EngEphemeris::isData(short subframe) const
{
    ...
    {
        InvalidRequest exc("Subframe " + StringUtils::asString(subframe) + " is not a valid ephemeris subframe.");
        GNSSTK_THROW(exc);
    }
...
void EngEphemeris::setAccuracy(double acc)
{
    if (acc < 0)
    {
        InvalidParameter exc("SV Accuracy of " + StringUtils::asString(acc) + " meters is invalid.");
        GNSSTK_THROW(exc);
    }
...
short EngEphemeris ::getFitInterval(short iodc, short fiti)
{
    ...
    if ((((fiti == 0) && (iodc & 0xFF) < 240) || (iodc & 0xFF) > 255))
    {
...
unsigned EngEphemeris::getTLMMessage(short subframe) const
{
    ...
    {
        InvalidRequest exc("Subframe " + StringUtils::asString(subframe) + " not stored.");
        GNSSTK_THROW(exc);
    }
...
double EngEphemeris::getHOWTime(short subframe) const
{
    if (!haveSubframe[subframe - 1])
    {
        InvalidRequest exc("getHOWTime(): Subframe " + StringUtils::asString(subframe) + " not stored.");
        GNSSTK_THROW(exc);
    }
...
short EngEphemeris::getASAlert(short subframe) const
{
    if (!haveSubframe[subframe - 1])
    {
        InvalidRequest exc("getASAlert(): Subframe " + StringUtils::asString(subframe) + " not stored.");
        GNSSTK_THROW(exc);
    }
...
#else
    long foo = static_cast<long>(std::min(getHOWTime(1), std::min(getHOWTime(2), getHOWTime(3))));
#endif
...
EngEphemeris &EngEphemeris::loadData(const std::string satSysArg, const unsigned short tlm[3], const long how[3],
                                     const short asalert[3], const short Tracker, const short prn, const short fullweek,
                                     const short cflags, const short acc, const short svhealth, const short iodc,
                                     const short l2pdata, const long aodo, const double tgd, const double toc,
                                     const double Af2, const double Af1, const double Af0, const short iode,
                                     const double crs, const double Dn, const double m0, const double cuc,
                                     const double Ecc, const double cus, const double ahalf, const double toe,
                                     const short fitInt, const double cic, const double Omega0, const double cis,
                                     const double I0, const double crc, const double W, const double OmegaDot,
                                     const double IDot)
{
    PRNID = prn;
    tracker = Tracker;
    ...
    try
    {
        ...
        orbit.loadData(satSys, obsID, PRNID, beginFit, endFit, toeCT, accFlag, (health == 0), cuc, cus, crc, crs, cic,
                       cis, m0, Dn, dndot, Ecc, A, ahalf, Adot, Omega0, I0, W, OmegaDot, IDot);

        bcClock.loadData(satSys, obsID, PRNID, tocCT, accFlag, (health == 0), Af0, Af1, Af2);
        haveSubframe[0] = true;
        haveSubframe[1] = true;
        haveSubframe[2] = true;
    }
...
EngEphemeris &EngEphemeris::setSF1(unsigned tlm, double how, short asalert, short fullweek, short cflags, short acc,
                                   short svhealth, short iodc, short l2pdata, double tgd, double toc, double Af2,
                                   double Af1, double Af0, short Tracker, short prn)
{
    ...
    try
    {
        ...
        bcClock.loadData(satSys, obsID, PRNID, tocCT, accFlagTmp, healthy, Af0, Af1, Af2);
        haveSubframe[0] = true;
    }
    catch (Exception &exc)
    {
...
EngEphemeris &EngEphemeris::setSF2(unsigned tlm, double how, short asalert, short iode, double crs, double Dn,
                                   double m0, double cuc, double Ecc, double cus, double ahalf, double toe,
                                   short fitInt)
{
    tlm_message[1] = tlm;
    ...
    try
    {
        ...
        orbit.loadData(satSys, obsID, PRNID, beginFit, endFit, toeCT, accFlag, healthy, cuc, cus, crc, crs, cic, cis,
                       m0, Dn, dndot, Ecc, A, ahalf, Adot, Omega0, I0, W, OmegaDot, IDot);
        haveSubframe[1] = true;
    }
    catch (Exception &exc)
    {
...
EngEphemeris &EngEphemeris::setSF3(unsigned tlm, double how, short asalert, double cic, double Omega0, double cis,
                                   double I0, double crc, double W, double OmegaDot, double IDot)
{
    tlm_message[2] = tlm;
...
static void shortcut(ostream &os, const long HOW)
{
    ...
    os << ":" << setfill('0') << setw(2) << hour << ":" << setw(2) << min << ":" << setw(2) << sec << setfill(' ');
}
...
void EngEphemeris ::dump(ostream &s) const
{
    ...
    {
        s << endl
          << "          SUBFRAME OVERHEAD" << endl
          << endl
          << "               SOW    DOW:HH:MM:SS     IOD    ALERT   A-S\n";
        for (int i = 0; i < 3; i++)
        {
            s << "SF" << setw(1) << (i + 1) << " HOW:   " << setw(7) << HOWtime[i] << "  ";
    ...
    {
        s << endl;
        s << "IODC: 0x" << setfill('0') << hex;
        s << setw(3) << IODC << endl;
        s << "IODE:  0x" << setfill('0') << hex;
        s << setw(2) << IODE << endl;
    }
    ...
    s << endl
      << "           ORBIT PARAMETERS" << endl
      << endl
      << "Semi-major axis:       " << setw(16) << orbit.getAhalf() << " m**.5" << endl
      << "Motion correction:     " << setw(16) << orbit.getDn() << " rad/sec" << endl
      ...
      << "Right ascension:       " << setw(16) << orbit.getOmega0() << " rad    " << setw(16) << orbit.getOmegaDot()
      << " rad/sec" << endl
      << "Inclination:           " << setw(16) << orbit.getI0() << " rad    " << setw(16) << orbit.getIDot()
      << " rad/sec" << endl;

    s << endl
      << "           HARMONIC CORRECTIONS" << endl
      << endl
      << "Radial        Sine: " << setw(16) << orbit.getCrs() << " m    Cosine: " << setw(16) << orbit.getCrc() << " m"
      << endl
      << "Inclination   Sine: " << setw(16) << orbit.getCis() << " rad  Cosine: " << setw(16) << orbit.getCic()
      << " rad" << endl
      << "In-track      Sine: " << setw(16) << orbit.getCus() << " rad  Cosine: " << setw(16) << orbit.getCuc()
      << " rad" << endl;

    s << endl
      << "           SV STATUS" << endl
      << endl
      << "Health bits:   0x" << setfill('0') << setw(2) << hex << health << dec << "      URA index: " << setfill(' ')
