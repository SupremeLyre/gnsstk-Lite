...
AlmOrbit ::AlmOrbit(short prn, double aEcc, double ai_offset, double aOMEGAdot, double aAhalf, double aOMEGA0,
                    double aw, double aM0, double aAF0, double aAF1, long aToa, long axmit_time, short aweek,
                    short aSV_health)
    : PRN(prn), ecc(aEcc), i_offset(ai_offset), OMEGAdot(aOMEGAdot), Ahalf(aAhalf), OMEGA0(aOMEGA0), w(aw), M0(aM0),
      AF0(aAF0), AF1(aAF1), Toa(aToa), xmit_time(axmit_time), week(aweek), SV_health(aSV_health)
{
}
...
Xvt AlmOrbit ::svXvt(const CommonTime &t) const
{
    ...
    double xip, yip, can, san, cinc, sinc, xef, yef, zef, dek, dlk, div, domk, duv, drv, dxp, dyp, vxef, vyef, vzef;
    ...
    anlon = OMEGA0 + (OMEGAdot - ell.angVelocity()) * elapt - ell.angVelocity() * (double)Toa;
...
void AlmOrbit::dump(std::ostream &s, int verbosity) const
{
    ...
    {
    case 0:
        s << PRN << ", " << Toa << ", " << week << ", " << std::hex << SV_health << ", " << std::dec << AF0 << ", "
          << AF1 << ", " << ecc << ", " << w << ", " << Ahalf << ", " << M0 << ", " << OMEGA0 << ", " << OMEGAdot
          << ", " << i_offset << endl;
        break;

    case 1:
        s << "PRN:" << PRN << " Toa:" << Toa << " H:" << SV_health << " AFO:" << AF0 << " AF1:" << AF1 << " Ecc:" << ecc
          << endl
          << "   w:" << w << " Ahalf:" << Ahalf << " M0:" << M0 << endl
          << "   OMEGA0:" << OMEGA0 << " OMEGAdot:" << OMEGAdot << " Ioff:" << i_offset << endl;
        break;
