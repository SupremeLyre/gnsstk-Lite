 ...
 * @f[
 *    \rho_{rx}^{sv} = \biggl|\biggl|\mathbf{R}\bigl(-(t_r - t_t)\bigl)\cdot\mathbf{r}^{sv}(t_t) -
 * \mathbf{r}^{rx}(t_r)\biggl|\biggl|
 * @f]
 * where, <br>
 *    <center>
...
class CorrectedEphemerisRange
{
  public:
    ...
    double ComputeAtReceiveTime(const CommonTime &trNom, const Position &rx, const SatID sat, NavLibrary &navLib,
     ...
     * @param[in] ellipsoid Ellipsoid model to provide an ECEF rotation rate.
     * @return The corrected range from rx to sat at trNom. */
    double ComputeAtTransmitTime(const CommonTime &trNom, const double &pr, const Position &rx, const SatID sat,
                                 NavLibrary &navLib, NavSearchOrder order = NavSearchOrder::User,
     ...
     * @param[in] ellipsoid Ellipsoid model to provide an ECEF rotation rate.
     * @return The corrected range from rx to sat at trNom. */
    double ComputeAtTransmitTime(const CommonTime &trNom, const Position &rx, const SatID sat, NavLibrary &navLib,
                                 NavSearchOrder order = NavSearchOrder::User, SVHealth xmitHealth = SVHealth::Any,
                                 NavValidityType valid = NavValidityType::ValidOnly,
                                 const EllipsoidModel &ellipsoid = GPSEllipsoid());
    ...
    double ComputeAtTransmitSvTime(const CommonTime &ttNom, const double &pr, const Position &rx, const SatID sat,
                                   NavLibrary &navLib, NavSearchOrder order = NavSearchOrder::User,
                                   SVHealth xmitHealth = SVHealth::Any,
                                   NavValidityType valid = NavValidityType::ValidOnly,
                                   const EllipsoidModel &ellipsoid = GPSEllipsoid());
