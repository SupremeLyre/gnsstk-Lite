...
PackedNavBits::PackedNavBits()
    : transmitTime(CommonTime::BEGINNING_OF_TIME), parityStatus(psUnknown), bits(900), bits_used(0), rxID(""),
      xMitCoerced(false)
{
    transmitTime.setTimeSystem(TimeSystem::GPS);
}
PackedNavBits::PackedNavBits(const SatID &satSysArg, const ObsID &obsIDArg, const CommonTime &transmitTimeArg)
    : bits(900), parityStatus(psUnknown), bits_used(0), rxID(""), xMitCoerced(false)
{
    satSys = satSysArg;
    obsID = obsIDArg;
...
PackedNavBits::PackedNavBits(const SatID &satSysArg, const ObsID &obsIDArg, const std::string rxString,
...
PackedNavBits::PackedNavBits(const SatID &satSysArg, const ObsID &obsIDArg, const NavID &navIDArg,
                             const std::string rxString, const CommonTime &transmitTimeArg)
    : bits(900), parityStatus(psUnknown), bits_used(0), rxID(""), xMitCoerced(false)
{
    satSys = satSysArg;
    obsID = obsIDArg;
...
PackedNavBits::PackedNavBits(const SatID &satSysArg, const ObsID &obsIDArg, const NavID &navIDArg,
                             const std::string rxString, const CommonTime &transmitTimeArg, unsigned numBits,
                             bool fillValue)
    : parityStatus(psUnknown), satSys(satSysArg), obsID(obsIDArg), navID(navIDArg), rxID(rxString),
      transmitTime(transmitTimeArg), bits(numBits, fillValue), bits_used(numBits), xMitCoerced(false)
{
}
...
/***    UNPACKING FUNCTIONS *********************************/
uint64_t PackedNavBits::asUint64_t(const int startBit, const int numBits) const
{
...
unsigned long PackedNavBits::asUnsignedLong(const int startBit, const int numBits, const int scale) const
{
...
long PackedNavBits::asLong(const int startBit, const int numBits, const int scale) const
{
    int64_t s = SignExtend(startBit, numBits);
    return ((long)(s * scale));
}

double PackedNavBits::asUnsignedDouble(const int startBit, const int numBits, const int power2) const
{
...
double PackedNavBits::asSignedDouble(const int startBit, const int numBits, const int power2) const
{
    int64_t s = SignExtend(startBit, numBits);
...
double PackedNavBits::asDoubleSemiCircles(const int startBits, const int numBits, const int power2) const
{
...
long PackedNavBits::asSignMagLong(const int startBit, const int numBits, const int scale) const
{
    // Get the magnitude
...
double PackedNavBits::asSignMagDouble(const int startBit, const int numBits, const int power2) const
{
    int startBitMag = startBit + 1;
    int numBitsMag = numBits - 1;
...
double PackedNavBits::asSignMagDoubleSemiCircles(const int startBit, const int numBits, const int power2) const
{
...
unsigned long PackedNavBits::asUnsignedLong(const unsigned startBits[], const unsigned numBits[], const unsigned len,
...
unsigned long PackedNavBits::asUnsignedLong(const unsigned startBit1, const unsigned numBits1, const unsigned startBit2,
                                            const unsigned numBits2, const int scale) const
{
    uint64_t temp1 = asUint64_t(startBit1, numBits1);
    uint64_t temp2 = asUint64_t(startBit2, numBits2);
    unsigned long ulong = (unsigned long)temp1;
...
long PackedNavBits::asLong(const unsigned startBits[], const unsigned numBits[], const unsigned len,
                           const int scale) const
{
...
/* Unpack a split signed long integer */
long PackedNavBits::asLong(const unsigned startBits1, const unsigned numBits1, const unsigned startBits2,
                           const unsigned numBits2, const int scale) const
{
...
double PackedNavBits::asUnsignedDouble(const unsigned startBits[], const unsigned numBits[], const unsigned len,
...
double PackedNavBits::asUnsignedDouble(const unsigned startBit1, const unsigned numBits1, const unsigned startBit2,
                                       const unsigned numBits2, const int power2) const
{
    uint64_t temp1 = asUint64_t(startBit1, numBits1);
    uint64_t temp2 = asUint64_t(startBit2, numBits2);
    unsigned long ulong = (unsigned long)temp1;
...
double PackedNavBits::asSignedDouble(const unsigned startBits[], const unsigned numBits[], const unsigned len,
                                     const int power2) const
{
    int64_t s = SignExtend(startBits[0], numBits[0]);
    uint64_t temp;
...
double PackedNavBits ::asSignedDouble(const std::vector<unsigned> &startBits, const std::vector<unsigned> &numBits,
                                      const std::vector<unsigned> &whichSF, const std::vector<PackedNavBitsPtr> &bits,
                                      const int power2)
{
...
double PackedNavBits ::asUnsignedDouble(const std::vector<unsigned> &startBits, const std::vector<unsigned> &numBits,
                                        const std::vector<unsigned> &whichSF, const std::vector<PackedNavBitsPtr> &bits,
                                        const int power2)
{
...
double PackedNavBits::asSignedDouble(const unsigned startBit1, const unsigned numBits1, const unsigned startBit2,
                                     const unsigned numBits2, const int power2) const
{
    int64_t s = SignExtend(startBit1, numBits1);
    uint64_t temp2 = asUint64_t(startBit2, numBits2);
...
double PackedNavBits::asDoubleSemiCircles(const unsigned startBits[], const unsigned numBits[], const unsigned len,
                                          const int power2) const
{
...
double PackedNavBits::asDoubleSemiCircles(const unsigned startBit1, const unsigned numBits1, const unsigned startBit2,
                                          const unsigned numBits2, const int power2) const
{
    return PI * asSignedDouble(startBit1, numBits1, startBit2, numBits2, power2);
}
...
/***    PACKING FUNCTIONS *********************************/
void PackedNavBits::addUnsignedLong(const unsigned long value, const int numBits, const int scale)
{
...
void PackedNavBits::addUnsignedDouble(const double value, const int numBits, const int power2)
{
...
void PackedNavBits::addSignedDouble(const double value, const int numBits, const int power2)
{
    union {
        uint64_t u_out;
        int64_t out;
...
void PackedNavBits::addDoubleSemiCircles(const double Radians, const int numBits, const int power2)
{
    union {
        uint64_t u_out;
        int64_t out;
...
void PackedNavBits::addDataVec(const std::vector<uint8_t> &data, unsigned numBits)
{
 ...
 *  will be changed.
 */
void PackedNavBits::copyBits(const PackedNavBits &src, const short startBit, const short endBit)
{
...
void PackedNavBits::insertUnsignedLong(const unsigned long value, const int startBit, const int numBits,
                                       const int scale)
{
...
double PackedNavBits::ScaleValue(const double value, const int power2) const
{
    ...
    if (temp >= 0)
        temp += 0.5; // Takes care of rounding
    else
        temp -= 0.5;
    return (temp);
}

void PackedNavBits::dump(ostream &s) const noexcept
{
    ...
    s << "****************************************************************"
      ...
      << "Code: " << ObsID::tcDesc[obsID.code] << "NavID: " << navID << endl;
    if (rxID.size() > 0)
        s << " RxID: " << rxID << endl;
    s << endl << "Number Of Bits: " << dec << getNumBits() << endl << endl;
...
/*
 */
int PackedNavBits::outputPackedBits(std::ostream &s, const short numPerLine, const char delimiter,
                                    const short numBitsPerWord) const
{
    ...
    for (size_t i = 0; i < bits.size(); ++i)
    {
        ...
        if (numBitInWord >= numBitsPerWord)
        {
            ...
            if (word_count > 0 && word_count % rollover == 0 && (i + 1) < bits.size())
                s << endl;
        }
    }
...
bool PackedNavBits::match(const PackedNavBits &right, const short startBit, const short endBit,
                          const unsigned flagBits) const
{
    if (!matchMetaData(right, flagBits))
        return false;
    if (!matchBits(right, startBit, endBit))
        return false;
    return true;
}

bool PackedNavBits::matchMetaData(const PackedNavBits &right, const unsigned flagBits) const
{
    ...
    if ((flagBits & mmSAT) && satSys != right.satSys)
        return false;

    // If not the same observation types (carrier, code) return false.
    if ((flagBits & mmOBS) && obsID != right.obsID)
        return false;

    // If not the same receiver return false.
    if ((flagBits & mmRX) && rxID.compare(right.rxID) != 0)
        return false;

    if ((flagBits & mmNAV) && navID.navType != (right.navID.navType))
        return false;

    return true;
}

bool PackedNavBits::matchBits(const PackedNavBits &right, const short startBitA, const short endBitA) const
{
    // If the two objects don't have the same number of bits,
    ...
    if (endBit == -1 || endBit >= int(bits.size()))
        endBit = bits.size() - 1;
    if (startBit < 0)
        startBit = 0;
    if (startBit >= int(bits.size()))
        startBit = bits.size() - 1;
...
void PackedNavBits::rawBitInput(const std::string inString)
{
    ...
    for (int i = 0; i < numWordsExpected; ++i)
    {
        ...
        if (hexWord.substr(0, 2) != "0x" && hexWord.substr(0, 2) != "0X")
        {
