 ...
 * @throw Exception
 */
int CommandLine::ProcessCommandLine(int argc, char **argv, string PrgmDesc, string &Usage, string &Errors,
                                    vector<string> &Unrecog)
{
    try
    {
        ...
        if (ConfigureLOG::ReportingLevel() >= ConfigureLOG::Level("DEBUG"))
        {
            ...
            LOG(DEBUG) << "CommandLine sets debug to " << ConfigureLOG::ToString(ConfigureLOG::ReportingLevel());
        }
        ...
        if (!ValidateCommandLine(word))
        { // word is a dummy here
            Errors = word;
            return -3;
        }
        ...
        // pre-processing -----------------------------------------------------
        // no args means help
        if (argc == 1 && requireOneArg)
            helponly = true;

        // PreProcessArgs pulls out help, debug, verbose, file, deprecated and ignore args
        vector<string> Args;
        for (j = 1; j < argc; j++)
            PreProcessArgs(argv[j], Args, Errors);
        LOG(DEBUG) << "Return from CommandLine::PreProcessArgs: help is " << (help ? "T" : "F") << " and helponly is "
                   << (helponly ? "T" : "F");

        // PreProcessArgs may have removed all cmd line args....
        if ((requireOneArg && Args.size() == 0) || helponly)
            help = true;
...
void CommandLine::DumpConfiguration(ostream &os, string tag)
{
    try
    {
        ...
        os << "   Print extended output, including cmdline summary (--verbose) : " << (verbose ? "true" : "false")
           << endl;
        os << "   Print debug output at level DEBUGn [n=0-7] (--debug<n>) : " << debug << endl;
        os << "   Print this syntax page and quit (--help) : " << (help ? "true" : "false") << endl;
...
bool CommandLine::ValidateCommandLine(string &msg)
{
    try
    {
        ...
        for (i = 0; i < options.size(); i++)
        {
            ...
            {
                isValid = false;
                oss << tag << options[i].longOpt << " (with arg) is both valid and to be ignored.\n";
            }
            // LOG(DEBUG) << " ignore_opts_with_arg ok";
            if (vectorindex(ignore_opts_without_arg, opt) != -1)
            {
                isValid = false;
                oss << tag << options[i].longOpt << " (w/o arg) is both valid and to be ignored.\n";
            }
            // LOG(DEBUG) << " ignore_opts_without_arg ok";
            if (vectorindex(ignore_on_opts, opt) != -1)
            {
                isValid = false;
                oss << tag << options[i].longOpt << " is both valid and an 'ignore on' option.\n";
            }
            // LOG(DEBUG) << " ignore_on_opts ok";
            if (vectorindex(ignore_off_opts, opt) != -1)
            {
                isValid = false;
                oss << tag << options[i].longOpt << " is both valid and an 'ignore off' option.\n";
            }
            ...
            for (j = 0; j < i; j++)
            {
                ...
                {
                    isValid = false;
                    oss << tag << "'" << options[i].longOpt << "' short form is already used in option '"
                        << options[j].longOpt << "'.\n";
                }
            }
        }
        ...
        for (it = deprec_opts.begin(); it != deprec_opts.end(); ++it)
        {
            ...
            for (i = 0; i < options.size(); i++)
            {
                if (it->second == string("--") + options[i].longOpt || it->second == string("-") + options[i].shortOpt)
                {
                    found = true;
                    break;
                }
            ...
            {
                isValid = false;
                oss << tag << it->second << ", the replacement for deprecated option " << it->first
                    << ", is not found.\n";
            }
        }
        ...
        if (!msg.empty())
        {
            LOG(DEBUG) << "ValidateCommandLine finds " << msg;
        }
...
void CommandLine::BuildSyntaxPage(void)
{
    try
    {
        ...
        for (i = 0; i < options.size(); i++)
        {
            ...
            if (options[i].arg.length())
                option += " <" + options[i].arg + "> ";
            ...
            options[i].syntax = options[i].predesc + (options[i].predesc.size() ? "\n" : "") + option // --opt <arg>
                                + options[i].desc                                                     // desc
 ...
 * @throw Exception
 */
void CommandLine::PreProcessArgs(const char *in_arg, vector<string> &Args, string &Errors)
{
    try
    {
        ...
        // ignore these
        for (i = 0; i < ignore_opts_without_arg.size(); i++)
        ...
        for (i = 0; i < ignore_opts_with_arg.size(); i++)
        { // note that -- is included
            if (sarg == ignore_opts_with_arg[i])
            {
                LOG(DEBUG) << "CommandLine::PreProcess: ignoring option " << sarg << " and its argument";
                ignore_once = true;
                return;
            }
        }
        // handle 'ignore on' args
        for (i = 0; i < ignore_on_opts.size(); i++)
        { // note that -- is included
        ...
        for (i = 0; i < ignore_off_opts.size(); i++)
        { // note that -- is included
        ...
        else if (sarg.substr(0, 2) == string("-d") || sarg.substr(0, 7) == string("--debug"))
        {
            ...
            {
                if (debug > 0)
                    level += asString(debug);
                ConfigureLOG::ReportingLevel() = ConfigureLOG::Level(level.c_str());
                LOG(DEBUG) << "CommandLine found debug option at level " << debug << ", logging level "
...
void expand_args(vector<string> &oldvalues, vector<string> &newvalues, string &msg)
{
    try
    {
        ...
        for (size_t k = 0; k < oldvalues.size(); k++)
        { // first split values on ','
            while (!(arg = stripFirstWord(oldvalues[k], ',')).empty())
            {
                ...
                { // list file
                    ...
                    {
                        msg += "  Error - Argument list file " + filename + " could not be opened.";
                        continue;
                    }
                    else
                        LOG(DEBUG) << "Opened arg list file " << filename;
                }
                else // just a value
    ...
    try
    {
        ...
        while (i < Args.size())
        {
            ...
            for (j = 0; j < options.size(); ++j)
            {
                ...
                { // match
                    ...
                    { // find value
                        ...
                        val = string("T"); // bool 'value'

                    LOG(DEBUG) << "CommandLine::Parse found arg[" << i << "] " << arg << " = " << val;
            ...
            if (j == options.size())
            {
                ...
                LOG(DEBUG) << "CommandLine::Parse found unrecognized arg[" << i << "] " << Args[i];
            }
    ...
    try
    {
        if (syntaxPageBuilt == 2)
        {
            ...
            syntaxPage += //"# (automatic options)\n" +
                leftJustify(" --verbose", optionsize) + "Print extended output, including cmdline summary (don't)\n";
            syntaxPage += leftJustify(" --debug<n>", optionsize) + "Print debug output at LOGlevel n [n=0-7] (-1)\n";
            syntaxPage += leftJustify(" --help", optionsize) + "Print this syntax page and quit (don't)";
...
void CommandLine::Postprocess(string &Errors, vector<string> &Unrecog)
{
    try
    {
        ...
        for (i = 0; i < options.size(); i++)
        {
            ...
            if (options[i].values.size() == 0)
                continue;
            LOG(DEBUG) << "CommandLine::Postprocess parse " << options[i].longOpt << " of type " << options[i].type
                       << (options[i].doc ? "" : " undocumented");

            // bool is special b/c values are ignored
            if (options[i].type == typeBool)
            {
                *((bool *)(options[i].p_output)) = (options[i].toggle ? !(*((bool *)(options[i].p_output))) : true);
                continue;
            }
            ...
            if (options[i].expand && (options[i].type == typeVectorString || options[i].type == typeVectorSat ||
            ...
            case typeInt:
                if (!isDigitString(values[0]))
                    oss << "Error: non-integer value for --" << options[i].longOpt << ": " << values[0] << endl;
                else
            ...
            case typeVectorInt:
                for (k = 0; k < values.size(); k++)
                {
                    if (!isDigitString(values[k]))
                        oss << "Error: non-integer value for --" << options[i].longOpt << ": " << values[k] << endl;
                    else
            ...
            case typeDouble:
                if (!isScientificString(values[0]))
                    oss << "Error: invalid value for --" << options[i].longOpt << ": " << values[0] << endl;
                else
