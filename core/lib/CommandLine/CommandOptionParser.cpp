...
CommandOptionParser::CommandOptionParser(const std::string &description, const CommandOptionVec &optList)
    : hasRequiredArguments(false), hasOptionalArguments(false), text(description)
...
CommandOptionParser &CommandOptionParser::addOption(gnsstk::CommandOption &co)
{
    CommandOptionVec::const_iterator optIter = optionVec.begin();
    for (; optIter != optionVec.end(); ++optIter)
    {
        if ((co.shortOpt != 0) && ((*optIter)->shortOpt == co.shortOpt))
        {
        ...
        if ((co.longOpt.size() != 0) && ((*optIter)->longOpt.compare(co.longOpt) == 0))
        {
        ...
        if (((*optIter)->optType == CommandOption::trailingType) && (co.optType == CommandOption::trailingType))
        {
...
void CommandOptionParser::parseOptions(int argc, char *argv[])
{
    ...
    for (index = 0; index < optionVec.size(); index++)
    {
        ...
        case CommandOption::stdType:
            ...
            if (!optionVec[index]->longOpt.empty())
            {
                ...
                optArray[optArraySize - 1] = optionVec[index]->toGetoptLongOption();
    ...
    while (optind < argc)
    {
        if ((cha = getopt_long(argc, argv, shortOptString.c_str(), optArray, &optionIndex)) == -1)
        {
            if (!trailing)
        ...
        else
        {
            ...
            if (itr != com.end())
            {
                ...
                if (optarg)
                {
                    ...
                    {
                        errorStrings.push_back(string("Option ") + thisOption +
                ...
                CommandOptionHelp *helpOpt = dynamic_cast<CommandOptionHelp *>(pickedOption);
                if (helpOpt)
                {
...
std::ostream &CommandOptionParser::printHelp(std::ostream &out, bool doPretty, bool firstOnly)
{
...
ostream &CommandOptionParser::displayUsage(ostream &out, bool doPretty)
{
    ...
    for (index = 0; index < optionVec.size(); index++)
    {
        if (optionVec[index]->optType == CommandOption::trailingType)
            ...
            maxlen = std::max(maxlen, unsigned(optionVec[index]->getFullOptionString().length()));
    }
    ...
    out << endl << (doPretty ? prettyPrint(text, "\n", "", "", columns) : text);
    // << endl
    //          << endl
    ...
    for (int required = 1; required >= 0; required--)
    {
        for (index = 0; index < optionVec.size(); index++)
        {
                ...
                (optionVec[index]->optType == CommandOption::stdType))
            {
                ...
                out << "}" << endl << " * \\dicdef{" << strip(optionVec[index]->description) << "}" << endl;
            }
        }
    }
...
void CommandOptionParser::resizeOptionArray(struct option *&oldArray, unsigned long &oldSize)
{
