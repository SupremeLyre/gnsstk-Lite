...
void LinearClockModel::addEpoch(const ORDEpoch &oe)
{
    ...
    {
        cout << t << " slope=" << setw(12) << clockModel.Slope() << ", intercept=" << setw(8) << clockModel.Intercept()
             << ", est=" << setw(8) << clockModel.Slope() * deltaT + clockModel.Intercept() << ", N=" << setw(6)
             << clockModel.N() << ", stdev=" << setw(6) << clockModel.StdDevY() << endl;
        tossCount++;
        if (tossCount > 5)
        {
            reset();
...
{
    s << "base: " << baseTime << ", start: " << startTime << ", end: " << endTime << endl
      << "Clock: est(end)=" << getOffset(endTime) << ", n=" << clockModel.N() << ", b=" << clockModel.Intercept()
      << ", m=" << clockModel.Slope() << ", sigma=" << clockModel.StdDevY() << ", r=" << clockModel.Correlation()
      << endl;

    if (detail > 0)
    {
        s << "min elev: " << elvmask << ", max sigma: " << sigmam << endl;
