...
class ObsID
{
  public:
    ...
    ObsID()
        : type(ObservationType::Unknown), band(CarrierBand::Unknown), code(TrackingCode::Unknown), freqOffs(0),
          freqOffsWild(true), mcode(0), mcodeMask(0), xmitAnt(XmitAnt::Any)
    {
    }
    ...
    ObsID(ObservationType ot, CarrierBand cb, TrackingCode tc, XmitAnt transmitter = XmitAnt::Any)
        : type(ot), band(cb), code(tc), freqOffs(0), freqOffsWild(true), mcode(0), mcodeMask(0), xmitAnt(transmitter)
    {
    }
     ...
     *   match any value. */
    explicit ObsID(ObservationType ot, CarrierBand cb, TrackingCode tc, int fo, XmitAnt transmitter = XmitAnt::Any,
                   bool fw = false)
        : type(ot), band(cb), code(tc), freqOffs(fo), freqOffsWild(fw), mcode(0), mcodeMask(0), xmitAnt(transmitter)
    {
    }
     ...
     * @param[in] fw If true, fo is ignored and freqOffs will
     *   match any value. */
    explicit ObsID(ObservationType ot, CarrierBand cb, TrackingCode tc, int fo, uint32_t mc,
                   XmitAnt transmitter = XmitAnt::Standard, bool fw = false)
        : type(ot), band(cb), code(tc), freqOffs(fo), freqOffsWild(fw), mcode(mc), mcodeMask(-1), xmitAnt(transmitter)
    {
    }
