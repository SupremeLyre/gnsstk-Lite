...
{
    os << "x:" << xvt.x << ", v:" << xvt.v << ", clk bias:" << xvt.clkbias << ", clk drift:" << xvt.clkdrift
       << ", relcorr:" << xvt.relcorr << ", health:" << xvt.health << ", frame:" << xvt.frame;
    return os;
}
...
{
    relcorr =
        -2.0 * ((x[0] / C_MPS) * (v[0] / C_MPS) + (x[1] / C_MPS) * (v[1] / C_MPS) + (x[2] / C_MPS) * (v[2] / C_MPS));
    return relcorr;
}
...
double Xvt::preciseRho(const Triple &rxPos, const EllipsoidModel &ellips, double correction) const noexcept
{
    ...
    double range;
    std::tie(std::ignore, range, std::ignore) = RawRange::fromSvPos(rxPos, *this, ellips, true, tofEstimate, 0, 2);
