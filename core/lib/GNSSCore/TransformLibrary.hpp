...
class TransformLibrary
{
  public:
    ...
    bool getTransform(const RefFrame &fromFrame, const RefFrame &toFrame, TransformerPtr &ptr,
                      const CommonTime &when = gnsstk::CommonTime::END_OF_TIME) const;
    ...
    bool transform(const Position &fromPos, Position &toPos,
                   const CommonTime &when = gnsstk::CommonTime::END_OF_TIME) const noexcept;
    ...
    bool transform(const Vector<double> &fromPos, const RefFrame &srcFrame, Vector<double> &toPos,
                   const RefFrame &tgtFrame, const CommonTime &when = gnsstk::CommonTime::END_OF_TIME) const noexcept;
     ...
     * @return true on success, false on failure (typically if
     *   srcFrame doesn't match fromFrame). */
    bool transform(const Triple &fromPos, const RefFrame &srcFrame, Triple &toPos, const RefFrame &tgtFrame,
                   const CommonTime &when = gnsstk::CommonTime::END_OF_TIME) const noexcept;
     ...
     * @return true on success, false on failure (typically if
     *   srcFrame doesn't match fromFrame). */
    bool transform(double fx, double fy, double fz, const RefFrame &srcFrame, double &tx, double &ty, double &tz,
                   const RefFrame &tgtFrame, const CommonTime &when = gnsstk::CommonTime::END_OF_TIME) const noexcept;
