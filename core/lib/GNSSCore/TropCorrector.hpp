...
template <class Model> class TropCorrector : public GroupPathCorrector
{
  public:
...
TropCorrector<Model>::TropCorrector()
    : useDefault(false), defTemp(std::numeric_limits<double>::quiet_NaN()),
      defPres(std::numeric_limits<double>::quiet_NaN()), defHum(std::numeric_limits<double>::quiet_NaN())
{
    corrType = CorrectorType::Trop;
}

template <class Model>
bool TropCorrector<Model>::getCorr(const Position &rxPos, const Position &svPos, const SatID &sat, const ObsID &obs,
...
template <class Model>
bool TropCorrector<Model>::getCorr(const Position &rxPos, const Xvt &svPos, const SatID &sat, const ObsID &obs,
                                   const CommonTime &when, NavType nav, double &corrOut)
{
...
template <class Model> void TropCorrector<Model>::setDefaultWx(double temp, double pres, double hum)
{
    useDefault = true;
    defTemp = temp;
    defPres = pres;
...
template <class Model> bool TropCorrector<Model>::loadFile(const std::string &fn)
{
    useDefault = false;
    wxData.read(fn);
    return true;
...
template <class Model> void TropCorrector<Model>::setWeather(Model &model, const CommonTime &when)
{
    if (!useDefault)
    {
...
template <> inline void TropCorrector<NBTropModel>::setWeather(NBTropModel &model, const CommonTime &when)
{
    if (!useDefault)
    {
