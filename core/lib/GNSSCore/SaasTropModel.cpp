...
#define THROW_IF_INVALID_DETAILED()                                                                                    \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            InvalidTropModel e;                                                                                        \
            if (!validWeather)                                                                                         \
                e.addText("Invalid trop model: Weather");                                                              \
            if (!validRxHeight)                                                                                        \
                e.addText("Invalid trop model: Rx Height");                                                            \
            if (!validRxLatitude)                                                                                      \
                e.addText("Invalid trop model: Rx Latitude");                                                          \
            if (!validDOY)                                                                                             \
                e.addText("Invalid trop model: day of year");                                                          \
            GNSSTK_THROW(e);                                                                                           \
        }                                                                                                              \
    }
...
static const double SaasWetA[5] = {0.00058021897, 0.00056794847, 0.00058118019, 0.00059727542, 0.00061641693};
static const double SaasWetB[5] = {0.0014275268, 0.0015138625, 0.0014572752, 0.0015007428, 0.0017599082};
static const double SaasWetC[5] = {0.043472961, 0.046729510, 0.043908931, 0.044626982, 0.054736038};

// constants for dry mapping function
static const double SaasDryA[5] = {0.0012769934, 0.0012683230, 0.0012465397, 0.0012196049, 0.0012045996};
static const double SaasDryB[5] = {0.0029153695, 0.0029152299, 0.0029288445, 0.0029022565, 0.0029024912};
static const double SaasDryC[5] = {0.062610505, 0.062837393, 0.063721774, 0.063824265, 0.064258455};

static const double SaasDryA1[5] = {0.0, 0.000012709626, 0.000026523662, 0.000034000452, 0.000041202191};
static const double SaasDryB1[5] = {0.0, 0.000021414979, 0.000030160779, 0.000072562722, 0.00011723375};
static const double SaasDryC1[5] = {0.0, 0.000090128400, 0.000043497037, 0.00084795348, 0.0017037206};
...
SaasTropModel::SaasTropModel(const double &lat, const int &day)
{
    validWeather = false;
    validRxHeight = false;
...
SaasTropModel::SaasTropModel(const double &lat, const int &day, const WxObservation &wx)
{
    validRxHeight = false;
    SaasTropModel::setReceiverLatitude(lat);
    SaasTropModel::setDayOfYear(day);
...
SaasTropModel::SaasTropModel(const double &lat, const int &day, const double &T, const double &P, const double &H)
{
    validRxHeight = false;
    SaasTropModel::setReceiverLatitude(lat);
    SaasTropModel::setDayOfYear(day);
...
double SaasTropModel::correction(double elevation) const
{
    ...
    {
        if (!validWeather)
            GNSSTK_THROW(InvalidTropModel("Invalid Saastamoinen trop model: weather"));
        if (!validRxLatitude)
            GNSSTK_THROW(InvalidTropModel("Invalid Saastamoinen trop model: Rx Latitude"));
        if (!validRxHeight)
            GNSSTK_THROW(InvalidTropModel("Invalid Saastamoinen trop model: Rx Height"));
        if (!validDOY)
            GNSSTK_THROW(InvalidTropModel("Invalid Saastamoinen trop model: day of year"));
        GNSSTK_THROW(InvalidTropModel("Valid flag corrupted in Saastamoinen trop model"));
    }
...
double SaasTropModel::correction(const Position &RX, const Position &SV, const CommonTime &tt)
{
...
double SaasTropModel::correction(const Xvt &RX, const Xvt &SV, const CommonTime &tt)
{
    Position R(RX), S(SV);
...
void SaasTropModel::setWeather(const double &T, const double &P, const double &H)
{
    temp = T;
    press = P;
