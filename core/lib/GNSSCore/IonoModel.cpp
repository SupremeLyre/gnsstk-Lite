...
namespace gnsstk
{
IonoModel::IonoModel(const double a[4], const double b[4], const bool semicircle_units) noexcept
{
...
void IonoModel::setModel(const double a[4], const double b[4], const bool semicircle_units) noexcept
{
    for (int n = 0; n < 4; n++)
    {
        alpha[n] = a[n];
...
double IonoModel::getCorrection(const CommonTime &time, const Position &rxgeo, double svel, double svaz,
...
{
    s << "a = { " << alpha[0] << " , " << alpha[1] << " , " << alpha[2] << " , " << alpha[3] << " }  ";
    s << "b = { " << beta[0] << " , " << beta[1] << " , " << beta[2] << " , " << beta[3] << " }" << std::endl;
}
