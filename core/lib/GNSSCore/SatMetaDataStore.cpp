...
namespace gnsstk
{
SatMetaDataStore::SVNID ::SVNID() : system(SatelliteSystem::Unknown)
{
}

SatMetaDataStore::SVNID ::SVNID(SatelliteSystem sys, const std::string &svn) : system(sys), id(svn)
{
}

bool SatMetaDataStore::SVNID ::operator<(const SVNID &right) const
{
...
bool SatMetaDataStore ::loadData(const std::string &sourceName)
{
    bool rv = true;
    ...
    while (ins)
    {
        ...
        try
        {
            ...
            {
                cerr << "Invalid record type: " << vals[0] << " on line " << lineNo << endl;
                rv = false;
                continue;
            }
        }
        ...
        {
            cerr << "Exception while processing line " << lineNo << ":" << endl << exc << endl;
            rv = false;
        }
        catch (std::exception &exc)
        {
            cerr << "Exception while processing line " << lineNo << ": " << exc.what() << endl;
            rv = false;
        }
        catch (...)
        {
...
bool SatMetaDataStore ::addSat(const std::vector<std::string> &vals, unsigned long lineNo)
{
    ...
    {
        cerr << "Missing NORAD mapping for SVN " << svn << " on line " << lineNo << endl;
        return false;
    }
    sat.norad = noradMap[svn];
    if (launchMap.find(svn) == launchMap.end())
    {
        cerr << "Missing LAUNCH record for SVN " << svn << " on line " << lineNo << endl;
        return false;
    }
    ...
    {
        cerr << "Missing CLOCK record for " << sysBlock << " on line " << lineNo << endl;
        return false;
    }
...
bool SatMetaDataStore ::addSignal(const std::vector<std::string> &vals, unsigned long lineNo)
{
    // simple way to index the columns without having to change
    // all the numbers with every little change.
    unsigned i = 1;
    ...
    if ((sig.carrier == CarrierBand::Unknown) || (sig.carrier == CarrierBand::Any) ||
        (sig.carrier == CarrierBand::Undefined) || (sig.carrier == CarrierBand::Last))
    {
        cerr << "Invalid carrier \"" << carrier << "\" on line " << lineNo << endl;
        return false;
    }
    sig.code = StringUtils::asTrackingCode(code);
    if ((sig.code == TrackingCode::Unknown) || (sig.code == TrackingCode::Any) ||
        (sig.code == TrackingCode::Undefined) || (sig.code == TrackingCode::Last))
    {
        cerr << "Invalid code \"" << code << "\" on line " << lineNo << endl;
        return false;
    }
    sig.nav = StringUtils::asNavType(nav);
    if ((sig.nav == NavType::Unknown) || (sig.nav == NavType::Any) || (sig.nav == NavType::Last))
    {
        cerr << "Invalid nav \"" << nav << "\" on line " << lineNo << endl;
        return false;
    }
...
bool SatMetaDataStore ::addClock(const std::vector<std::string> &vals, unsigned long lineNo)
{
    // simple way to index the columns without having to change
    // all the numbers with every little change.
    unsigned i = 1;
    if (vals.size() != 7)
    {
        cerr << "Invalid CLOCK record on line " << lineNo << " size!=7" << endl;
        return false;
    }
    SystemBlock key;
    ...
    if (clkMap.find(key) != clkMap.end())
    {
        ...
        cerr << "Duplicate CLOCK " << StringUtils::asString(key.sys) << " " << key.blk << " on line " << lineNo << endl;
        return false;
    }
...
bool SatMetaDataStore ::addLaunch(const std::vector<std::string> &vals, unsigned long lineNo)
{
    // simple way to index the columns without having to change
    // all the numbers with every little change.
    unsigned i = 1;
    if (vals.size() != 8)
    {
        cerr << "Invalid LAUNCH record on line " << lineNo << " size!=8" << endl;
        return false;
    }
    SVNID svn;
...
bool SatMetaDataStore ::addNORAD(const std::vector<std::string> &vals, unsigned long lineNo)
{
    // simple way to index the columns without having to change
    // all the numbers with every little change.
    unsigned i = 1;
    if (vals.size() != 4)
    {
        cerr << "Invalid NORAD record on line " << lineNo << " size!=4" << endl;
        return false;
    }
    SVNID svn;
    svn.system = convertStringToSatelliteSystem(vals[i++]);
...
bool SatMetaDataStore ::findSat(SatelliteSystem sys, uint32_t prn, const gnsstk::CommonTime &when,
                                SatMetaData &sat) const
{
    ...
    // identification.
    for (SatSet::const_iterator rv = sysIt->second.begin(); rv != sysIt->second.end(); rv++)
    {
        if (rv->prn < prn)
        {
            // cerr << "< prn" << endl;
...
bool SatMetaDataStore ::findSatBySVN(SatelliteSystem sys, const std::string &svn, const gnsstk::CommonTime &when,
                                     SatMetaData &sat) const
{
    SatMetaMap::const_iterator sysIt = satMap.find(sys);
    if (sysIt == satMap.end())
    ...
    for (SatSet::const_iterator rv = sysIt->second.begin(); rv != sysIt->second.end(); rv++)
    {
        if ((rv->svn == svn) && (when >= rv->startTime) && (when < rv->endTime))
        {
            // std::cerr << "found it" << std::endl;
            // cerr << *rv << endl;
            sat = *rv;
...
bool SatMetaDataStore::findSatBySlotFdma(uint32_t slotID, int32_t channel, const gnsstk::CommonTime &when,
                                         SatMetaData &sat) const
{
    ...
    for (SatSet::const_iterator rv = sysIt->second.begin(); rv != sysIt->second.end(); rv++)
    {
        if ((rv->slotID == slotID) && (rv->chl == channel) && (when >= rv->startTime) && (when < rv->endTime))
        {
            // std::cerr << "found it" << std::endl;
            // cerr << *rv << endl;
            sat = *rv;
...
bool SatMetaDataStore ::getSVN(SatelliteSystem sys, uint32_t prn, const gnsstk::CommonTime &when,
                               std::string &svn) const
{
    SatMetaData sat;
...
bool SatMetaDataStore ::getNORAD(SatelliteSystem sys, uint32_t prn, const gnsstk::CommonTime &when,
                                 int32_t &norad) const
{
    SatMetaData sat;
    if (findSat(sys, prn, when, sat))
...
bool SatMetaDataStore ::getPRN(SatelliteSystem sys, const std::string &svn, const gnsstk::CommonTime &when,
                               uint32_t &prn) const
{
    SatMetaData sat;
...
std::set<std::string> SatMetaDataStore ::getSignalSet(const Signal &sig) const
{
...
std::set<std::string> SatMetaDataStore ::getSignalSet(const SignalSet &signals) const
{
    std::set<std::string> rv;
    for (const auto &smi : sigMap)
    {
...
SatMetaDataStore::SatSet SatMetaDataStore ::getSatsBySignal(const SignalSet &signals)
{
    SatSet rv;
...
SatMetaDataStore::SatSet SatMetaDataStore ::getSatsBySignal(const SignalSet &signals, const CommonTime &when)
{
...
SatMetaDataStore::SatSet SatMetaDataStore ::getSatsBySignal(const SignalSet &signals, const CommonTime &beginTime,
                                                            ...
                                                            const std::set<SatMetaData::Status> &status)
{
    ...
    for (const auto &ii : intermediate)
    {
        if ((ii.startTime <= endTime) && (ii.endTime >= beginTime) && (status.count(ii.status) > 0))
        {
            rv.insert(ii);
        }
    }
