...
namespace gnsstk
{
RefFrame ::RefFrame() : system(RefFrameSys::Unknown), realization(RefFrameRlz::Unknown)
{
}

RefFrame ::RefFrame(RefFrameRlz rlz) : realization(rlz), system(getRefFrameSys(rlz))
{
}

RefFrame ::RefFrame(RefFrameSys sys, const gnsstk::CommonTime &when)
    : system(sys), realization(getRefFrameRlz(sys, when))
{
}

RefFrame ::RefFrame(const std::string &str, const gnsstk::CommonTime &when)
{
...
bool RefFrame ::operator<(const RefFrame &right) const noexcept
{
    if (system < right.system)
        return true;
    if (right.system < system)
        return false;
    if (realization < right.realization)
        return true;
    return false;
}

bool RefFrame ::operator==(ReferenceFrame orf) const noexcept
{
    // Only compare the reference frame SYSTEMS because that's
    // how it's being used in HelmertTransform.
    return (((orf == ReferenceFrame::Unknown) && (system == RefFrameSys::Unknown)) ||
            ((orf == ReferenceFrame::WGS84) && (system == RefFrameSys::WGS84)) ||
            ((orf == ReferenceFrame::ITRF) && (system == RefFrameSys::ITRF)) ||
            ((orf == ReferenceFrame::PZ90) && (system == RefFrameSys::PZ90)) ||
            ((orf == ReferenceFrame::CGCS2000) && (system == RefFrameSys::CGCS2000)));
}

RefFrame ::RefFrame(ReferenceFrame orf, const gnsstk::CommonTime &when)
{
    switch (orf)
    {
    ...
    case ReferenceFrame::CGCS2000:
        *this = RefFrame(RefFrameSys::CGCS2000, when);
        break;
    default: {
        InvalidParameter exc("Invalid ReferenceFrame " + StringUtils::asString((int)orf));
        GNSSTK_THROW(exc);
        break;
    }
    }
