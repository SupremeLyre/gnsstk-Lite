...
WxObservation WxObsData::getMostRecent(const CommonTime &t) const noexcept
{
    if (obs.size() == 0)
...
void WxObsData::insertObservation(const WxObservation &wx) noexcept
{
    obs[wx.t] = wx;
    if (wx.t > lastTime)
        lastTime = wx.t;
...
bool WxObservation::isAllValid() const noexcept
{
    return temperatureSource != noWx && pressureSource != noWx && humiditySource != noWx;
};
...
WxObservation WxObsData::getWxObservation(const CommonTime &t, unsigned iv, bool interpolate) const
{
    if (obs.empty())
    {
        ObjectNotFound e("No WxObservation available near time " + printTime(t, "%02H:%02M:%02S on day %03j of %4Y"));
        GNSSTK_THROW(e);
    }
    ...
    if (after == obs.end())
    {
        ...
        {
            ObjectNotFound e("No WeatherData available near time " + printTime(t, "%02H:%02M:%02S on day %03j of %4Y"));
            GNSSTK_THROW(e);
        }
    }
    else
...
std::ostream &operator<<(std::ostream &s, const gnsstk::WxObservation &obs) noexcept
{
    ...
    s << obs.t << ", t=" << obs.temperature << ", p=" << obs.pressure << ", rh=" << obs.humidity;
    return s;
}
