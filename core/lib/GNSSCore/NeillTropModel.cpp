...
namespace gnsstk
{
NeillTropModel::NeillTropModel(const Position &RX, const CommonTime &time)
{
 ...
 */
static const double NeillWetA[5] = {0.00058021897, 0.00056794847, 0.00058118019, 0.00059727542, 0.00061641693};
static const double NeillWetB[5] = {0.0014275268, 0.0015138625, 0.0014572752, 0.0015007428, 0.0017599082};
static const double NeillWetC[5] = {0.043472961, 0.046729510, 0.043908931, 0.044626982, 0.054736038};

// constants for dry mapping function
static const double NeillDryA[5] = {0.0012769934, 0.0012683230, 0.0012465397, 0.0012196049, 0.0012045996};
static const double NeillDryB[5] = {0.0029153695, 0.0029152299, 0.0029288445, 0.0029022565, 0.0029024912};
static const double NeillDryC[5] = {0.062610505, 0.062837393, 0.063721774, 0.063824265, 0.064258455};

static const double NeillDryA1[5] = {0.0, 0.000012709626, 0.000026523662, 0.000034000452, 0.000041202191};
static const double NeillDryB1[5] = {0.0, 0.000021414979, 0.000030160779, 0.000072562722, 0.00011723375};
static const double NeillDryC1[5] = {0.0, 0.000090128400, 0.000043497037, 0.00084795348, 0.0017037206};
...
double NeillTropModel::correction(double elevation) const
{
    ...
    double tropDelay((NeillTropModel::dry_zenith_delay() * map_dry) + (NeillTropModel::wet_zenith_delay() * map_wet));

    return tropDelay;
}

double NeillTropModel::correction(const Position &RX, const Position &SV)
{
...
double NeillTropModel::correction(const Position &RX, const Position &SV, const CommonTime &tt)
{
    setDayOfYear(tt);
...
double NeillTropModel::correction(const Position &RX, const Position &SV, const int &doy)
{
    setDayOfYear(doy);
...
double NeillTropModel::correction(const Xvt &RX, const Xvt &SV)
{
    Position R(RX), S(SV);
    return NeillTropModel::correction(R, S);
}

double NeillTropModel::correction(const Xvt &RX, const Xvt &SV, const CommonTime &tt)
{
    setDayOfYear(tt);
    Position R(RX), S(SV);
...
double NeillTropModel::correction(const Xvt &RX, const Xvt &SV, const int &doy)
{
    setDayOfYear(doy);
    Position R(RX), S(SV);
...
void NeillTropModel::setWeather()
{
    ...
    {
        valid = false;
        InvalidTropModel e("NeillTropModel must have Rx latitude before computing weather");
        GNSSTK_THROW(e);
    }
    if (!validDOY)
    {
        valid = false;
        InvalidTropModel e("NeillTropModel must have day of year before computing weather");
        GNSSTK_THROW(e);
    }
...
void NeillTropModel::setAllParameters(const CommonTime &time, const Position &rxPos)
{
