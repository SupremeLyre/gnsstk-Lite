...
namespace gnsstk
{
HelmertTransformer ::HelmertTransformer() : scale(std::numeric_limits<double>::quiet_NaN()), description("Undefined")
{
    // rotation, translation and the RefFrame objects we just use
    // their default constructors.
}

HelmertTransformer ::HelmertTransformer(const RefFrame &from, const RefFrame &to, double irx, double iry, double irz,
                                        double itx, double ity, double itz, double sc, const std::string &desc,
                                        const CommonTime &refEpoch)
    : description(desc), scale(sc)
{
    if ((from.getRealization() == RefFrameRlz::Unknown) || (to.getRealization() == RefFrameRlz::Unknown))
    {
...
bool HelmertTransformer ::transform(const Position &fromPos, Position &toPos) const noexcept
{
    if ((fromPos.getReferenceFrame() == fromFrame) && (toPos.getReferenceFrame() == toFrame))
    {
        // transform
        toPos = fromPos;
    ...
    else if ((fromPos.getReferenceFrame() == toFrame) && (toPos.getReferenceFrame() == fromFrame))
    {
        // inverse transform
        toPos = fromPos;
        toPos.transformTo(Position::Cartesian);
...
bool HelmertTransformer ::transform(const Xvt &fromPos, Xvt &toPos) const noexcept
{
...
bool HelmertTransformer ::transform(const Vector<double> &fromPos, const RefFrame &srcFrame,
                                    Vector<double> &toPos) const noexcept
{
...
bool HelmertTransformer ::transform(const Triple &fromPos, const RefFrame &srcFrame, Triple &toPos) const noexcept
{
...
bool HelmertTransformer ::transform(double fx, double fy, double fz, const RefFrame &srcFrame, double &tx, double &ty,
