...
#define THROW_IF_INVALID_DETAILED()                                                                                    \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            if (!validRxLatitude)                                                                                      \
                e.addText("Invalid trop model: validRxLatitude");                                                      \
            if (!validRxHeight)                                                                                        \
                e.addText("Invalid trop model: validRxHeight");                                                        \
            if (!validDOY)                                                                                             \
...
static const double NBMad[5] = {1.2769934e-3, 1.2683230e-3, 1.2465397e-3, 1.2196049e-3, 1.2045996e-3};
static const double NBMbd[5] = {2.9153695e-3, 2.9152299e-3, 2.9288445e-3, 2.9022565e-3, 2.9024912e-3};
static const double NBMcd[5] = {62.610505e-3, 62.837393e-3, 63.721774e-3, 63.824265e-3, 64.258455e-3};

// mapping function, dry, amplitudes
static const double NBMaa[5] = {0.0, 1.2709626e-5, 2.6523662e-5, 3.4000452e-5, 4.1202191e-5};
static const double NBMba[5] = {0.0, 2.1414979e-5, 3.0160779e-5, 7.2562722e-5, 11.723375e-5};
static const double NBMca[5] = {0.0, 9.0128400e-5, 4.3497037e-5, 84.795348e-5, 170.37206e-5};

// mapping function, wet, averages (there are no amplitudes for wet)
static const double NBMaw[5] = {5.8021897e-4, 5.6794847e-4, 5.8118019e-4, 5.9727542e-4, 6.1641693e-4};
static const double NBMbw[5] = {1.4275268e-3, 1.5138625e-3, 1.4572752e-3, 1.5007428e-3, 1.7599082e-3};
static const double NBMcw[5] = {4.3472961e-2, 4.6729510e-2, 4.3908931e-2, 4.4626982e-2, 5.4736038e-2};
...
static double NB_Interpolate(double lat, int doy, TableEntry entry)
{
    ...
    { // mid-latitude -> regular interpolation
        ...
        if (entry < Maw)
            ret -= (pamp[i] + m * (pamp[i + 1] - pamp[i])) * std::cos(TWO_PI * (day - 28.0) / 365.25);
    }
    else
...
NBTropModel::NBTropModel() : validWeather(false), validRxLatitude(false), validDOY(false), validRxHeight(false)
{
}

NBTropModel::NBTropModel(const double &lat, const int &day)
...
NBTropModel::NBTropModel(const double &lat, const int &day, const WxObservation &wx)
    : validWeather(false), validRxLatitude(false), validDOY(false), validRxHeight(false)
{
    setReceiverLatitude(lat);
    setDayOfYear(day);
...
NBTropModel::NBTropModel(const double &lat, const int &day, const double &T, const double &P, const double &H)
    : validWeather(false), validRxLatitude(false), validDOY(false), validRxHeight(false)
{
    setReceiverLatitude(lat);
    setDayOfYear(day);
...
NBTropModel::NBTropModel(const double &ht, const double &lat, const int &day)
    : validWeather(false), validRxLatitude(false), validDOY(false), validRxHeight(false)
{
...
double NBTropModel::correction(const Position &RX, const Position &SV, const CommonTime &tt)
{
    THROW_IF_INVALID_DETAILED();
...
double NBTropModel::correction(const Xvt &RX, const Xvt &SV, const CommonTime &tt)
{
    Position R(RX), S(SV);
...
void NBTropModel::setWeather(const double &T, const double &P, const double &H)
{
