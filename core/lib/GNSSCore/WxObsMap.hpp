...
struct WxObservation
{
    ...
    WxObservation() noexcept
        : t(CommonTime::END_OF_TIME), temperatureSource(noWx), pressureSource(noWx), humiditySource(noWx)
    {
    }
    ...
    WxObservation(const CommonTime &t, float temp, float pres, float humid) noexcept
        : t(t), temperature(temp), pressure(pres), humidity(humid), temperatureSource(obsWx), pressureSource(obsWx),
          humiditySource(obsWx)
    {
    }
     ...
     */
    friend std::ostream &operator<<(std::ostream &s, const WxObservation &obs) noexcept;
};
...
{
    /// Constructor
    WxObsData() noexcept : firstTime(CommonTime::END_OF_TIME), lastTime(CommonTime::BEGINNING_OF_TIME)
    {
    }
