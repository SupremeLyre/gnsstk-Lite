...
class Transformer
{
  public:
    ...
    Transformer() : epoch(gnsstk::CommonTime::BEGINNING_OF_TIME)
    {
    }
    ...
    virtual bool transform(const Vector<double> &fromPos, const RefFrame &srcFrame,
                           Vector<double> &toPos) const noexcept = 0;
     ...
     * @return true on success, false on failure (typically if
     *   srcFrame doesn't match fromFrame). */
    virtual bool transform(double fx, double fy, double fz, const RefFrame &srcFrame, double &tx, double &ty,
                           double &tz) const noexcept = 0;
