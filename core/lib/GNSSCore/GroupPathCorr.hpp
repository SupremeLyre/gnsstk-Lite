...
class GroupPathCorr
{
  public:
    ...
    bool getCorr(const Position &rxPos, const Position &svPos, const SatID &sat, const ObsID &obs,
                 const CommonTime &when, NavType nav, CorrectionResults &corrOut,
    ...
    /// CorrectionResults&, CorrDupHandling)
    bool getCorr(const Position &rxPos, const Xvt &svPos, const SatID &sat, const ObsID &obs, const CommonTime &when,
                 NavType nav, CorrectionResults &corrOut, CorrDupHandling dups = CorrDupHandling::ComputeFirst);
    /// @copydoc getCorr(const Position&, const Position&, const SatID&, const ObsID&, const CommonTime&, NavType,
    /// CorrectionResults&, CorrDupHandling)
    bool getCorr(const Position &rxPos, const Position &svPos, const SatID &sat, const ObsID &obs,
    ...
    /// CorrectionResults&, CorrDupHandling)
    bool getCorr(const Position &rxPos, const Xvt &svPos, const SatID &sat, const ObsID &obs, const CommonTime &when,
                 NavType nav, double &corrOut, CorrDupHandling dups = CorrDupHandling::ComputeFirst);
