...
class RinexObsID : public ObsID
{
  public:
    ...
    RinexObsID() : ObsID(), rinexVersion(Rinex3ObsBase::currentVersion)
    {
    }
    ...
    RinexObsID(ObservationType ot, CarrierBand cb, TrackingCode tc, double version = Rinex3ObsBase::currentVersion)
        : ObsID(ot, cb, tc), rinexVersion(version)
    {
    }
     ...
     * @throw InvalidParameter
     */
    explicit RinexObsID(const char *id, double version) : RinexObsID(std::string(id), version)
    {
    }
     ...
     * @throw InvalidParameter
     */
    RinexObsID(const ObsID &oid, double version = Rinex3ObsBase::currentVersion) : rinexVersion(version)
    {
        type = oid.type;
        band = oid.band;
        code = oid.code;
