...
static const char *SystemNames[] = {"Unknown", "Geodetic", "Geocentric", "Cartesian", "Spherical"};

// return string giving name of coordinate system
string Position::getSystemName() noexcept
{
...
Position &Position::setTolerance(const double tol) noexcept
{
    tolerance = tol;
    return *this;
}
...
Position::Position(const double &a, const double &b, const double &c, Position::CoordinateSystem s,
                   const EllipsoidModel *ell, const RefFrame &frame)
{
    try
    {
...
Position::Position(const double ABC[3], CoordinateSystem s, const EllipsoidModel *ell, const RefFrame &frame)
{
    double a = ABC[0];
    double b = ABC[1];
    double c = ABC[2];
...
Position::Position(const Triple &ABC, CoordinateSystem s, const EllipsoidModel *ell, const RefFrame &frame)
{
    double a = ABC[0];
    double b = ABC[1];
    double c = ABC[2];
...
Position::Position(const Xvt &xvt) noexcept
{
    double a = xvt.x[0];
    double b = xvt.x[1];
    double c = xvt.x[2];
...
//
Position &Position::operator-=(const Position &right) noexcept
{
    Position r(right);
...
bool Position::operator==(const Position &right) const noexcept
{
...
// @return *this
Position &Position::transformTo(CoordinateSystem sys) noexcept
{
    if (sys == Unknown || sys == system)
        return *this;
...
const RefFrame &Position::getReferenceFrame() const noexcept
{
    return refFrame;
}

// Get X coordinate (meters)
double Position::X() const noexcept
{
    if (system == Cartesian)
        return theArray[0];
    Position t(*this);
...
double Position::Y() const noexcept
{
    if (system == Cartesian)
        return theArray[1];
    Position t(*this);
...
double Position::Z() const noexcept
{
    if (system == Cartesian)
        return theArray[2];
    Position t(*this);
...
double Position::geodeticLatitude() const noexcept
{
    if (system == Geodetic)
        return theArray[0];
    Position t(*this);
...
double Position::geocentricLatitude() const noexcept
{
...
double Position::theta() const noexcept
{
    if (system == Spherical)
        return theArray[0];
    Position t(*this);
...
double Position::phi() const noexcept
{
    if (system == Spherical)
        return theArray[1];
    Position t(*this);
...
double Position::radius() const noexcept
{
...
double Position::height() const noexcept
{
    if (system == Geodetic)
        return theArray[2];
    Position t(*this);
...
//
void Position::setReferenceFrame(const RefFrame &frame) noexcept
{
    refFrame = frame;
}
...
Position &Position::setGeodetic(const double lat, const double lon, const double ht, const EllipsoidModel *ell)
{
    if (lat > 90 || lat < -90)
    {
        GeometryException ge("Invalid latitude in setGeodetic: " + StringUtils::asString(lat));
        GNSSTK_THROW(ge);
    }
    theArray[0] = lat;
...
// @return a reference to this object.
// @throw GeometryException on invalid input
Position &Position::setGeocentric(const double lat, const double lon, const double rad)
{
    if (lat > 90 || lat < -90)
    {
        GeometryException ge("Invalid latitude in setGeocentric: " + StringUtils::asString(lat));
        GNSSTK_THROW(ge);
    }
    if (rad < 0)
    {
        GeometryException ge("Invalid radius in setGeocentric: " + StringUtils::asString(rad));
        GNSSTK_THROW(ge);
    }
    theArray[0] = lat;
    theArray[1] = lon;
...
// @return a reference to this object.
// @throw GeometryException on invalid input
Position &Position::setSpherical(const double theta, const double phi, const double rad)
{
    if (theta < 0 || theta > 180)
    {
        GeometryException ge("Invalid theta in setSpherical: " + StringUtils::asString(theta));
        GNSSTK_THROW(ge);
    }
    if (rad < 0)
    {
        GeometryException ge("Invalid radius in setSpherical: " + StringUtils::asString(rad));
        GNSSTK_THROW(ge);
    }
...
// @return a reference to this object.
Position &Position::setECEF(const double X, const double Y, const double Z) noexcept
{
    theArray[0] = X;
    theArray[1] = Y;
    theArray[2] = Z;
...
// @return a reference to this object.
Position &Position::setToString(const std::string &str, const std::string &fmt)
{
    try
    {
        ...
        // store input values
        double x = 0.0, y = 0.0, z = 0.0, glat = 0.0, lon = 0.0, ht = 0.0, clat = 0.0, rad = 0.0, theta = 0.0,
               phi = 0.0;
        ...
        while ((s.size() > 0) && (f.size() > 0))
        {
            ...
            case 'A':
                ...
                if (glat > 90. || glat < -90.)
                {
                    InvalidRequest f("Invalid geodetic latitude for setTostring: " + toBeRemoved);
                    GNSSTK_THROW(f);
                }
                hglat = true;
                break;
            case 'a':
                ...
                if (clat > 90. || clat < -90.)
                {
                    InvalidRequest f("Invalid geocentric latitude for setTostring: " + toBeRemoved);
                    GNSSTK_THROW(f);
                }
                hclat = true;
                break;
            ...
            case 't':
                ...
                {
                    InvalidRequest f("Invalid theta for setTostring: " + toBeRemoved);
                    GNSSTK_THROW(f);
                }
                htheta = true;
                break;
            ...
            case 'r':
                ...
                if (rad < 0.0)
                {
                    InvalidRequest f("Invalid radius for setTostring: " + toBeRemoved);
                    GNSSTK_THROW(f);
                }
                hrad = true;
                break;
        ...
        if (s.length() != 0)
        {
            ...
            InvalidRequest fe("Processing error - parts of strings left unread - " + s);
            GNSSTK_THROW(fe);
        }
        ...
        {
            // throw an error - something didn't get processed in the strings
            InvalidRequest fe("Processing error - parts of strings left unread - " + f);
            GNSSTK_THROW(fe);
        }

        // throw if the specification is incomplete
        if (!(hX && hY && hZ) && !(hglat && hlon && hht) && !(hclat && hlon && hrad) && !(htheta && hphi && hrad))
        {
...
{
    string rv = fmt;
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?x"), string("xf"), X());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?y"), string("yf"), Y());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?z"), string("zf"), Z());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?X"), string("Xf"), X() / 1000);
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?Y"), string("Yf"), Y() / 1000);
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?Z"), string("Zf"), Z() / 1000);

    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?A"), string("Af"), geodeticLatitude());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?a"), string("af"), geocentricLatitude());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?L"), string("Lf"), longitude());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?l"), string("lf"), longitude());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?w"), string("wf"), 360 - longitude());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?W"), string("Wf"), 360 - longitude());

    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?t"), string("tf"), theta());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?T"), string("Tf"), theta() * DEG_TO_RAD);
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?p"), string("pf"), phi());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?P"), string("Pf"), phi() * DEG_TO_RAD);
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?r"), string("rf"), radius());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?R"), string("Rf"), radius() / 1000);
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?h"), string("hf"), height());
    rv = formattedPrint(rv, string("%[ 0-]?[[:digit:]]*(\\.[[:digit:]]+)?H"), string("Hf"), height() / 1000);
    return rv;
}
...
void Position::convertSphericalToCartesian(const Triple &tpr, Triple &xyz) noexcept
{
...
// Algorithm references: standard geometry.
void Position::convertCartesianToSpherical(const Triple &xyz, Triple &tpr) noexcept
{
    ...
    tpr[0] *= RAD_TO_DEG;
    if (RSS(xyz[0], xyz[1]) < Position::POSITION_TOLERANCE / 5)
    { // pole
        tpr[1] = 0;
        return;
    }
...
void Position::convertCartesianToGeodetic(const Triple &xyz, Triple &llh, const double A, const double eccSq) noexcept
{
    double p, slat, N, htold, latold;
    p = SQRT(xyz[0] * xyz[0] + xyz[1] * xyz[1]);
    if (p < Position::POSITION_TOLERANCE / 5)
    { // pole or origin
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (::fabs(llh[0] - latold) < 1.0e-9 && fabs(llh[2] - htold) < 1.0e-9 * A)
            break;
    }
...
// @param eccSq (input) square of Earth eccentricity
// Algorithm references:
void Position::convertGeodeticToCartesian(const Triple &llh, Triple &xyz, const double A, const double eccSq) noexcept
{
...
void Position::convertCartesianToGeocentric(const Triple &xyz, Triple &llr) noexcept
{
...
void Position::convertGeocentricToCartesian(const Triple &llr, Triple &xyz) noexcept
{
    Triple llh(llr);
...
// @param A (input) Earth semi-major axis
// @param eccSq (input) square of Earth eccentricity
void Position::convertGeocentricToGeodetic(const Triple &llr, Triple &llh, const double A, const double eccSq) noexcept
{
...
// @param A (input) Earth semi-major axis
// @param eccSq (input) square of Earth eccentricity
void Position::convertGeodeticToGeocentric(const Triple &llh, Triple &llr, const double A, const double eccSq) noexcept
{
    double slat = ::sin(llh[0] * DEG_TO_RAD);
    double N = A / SQRT(1.0 - eccSq * slat * slat);
...
double Position::radiusEarth(const double geolat, const double A, const double eccSq) noexcept
{
...
double Position::elevationGeodetic(const Position &Target) const
{
    ...
    Triple kVector(::cos(latGeodetic) * ::cos(longGeodetic), ::cos(latGeodetic) * ::sin(longGeodetic),
                   ::sin(latGeodetic));
...
double Position::azimuthGeodetic(const Position &Target) const
{
    ...
    Triple iVector(-::sin(latGeodetic) * ::cos(longGeodetic), -::sin(latGeodetic) * ::sin(longGeodetic),
                   ::cos(latGeodetic));
...
Position Position::getIonosphericPiercePoint(const double elev, const double azim, const double ionoht) const noexcept
{
    Position Rx(*this);
    ...
    IPP.theArray[1] = Rx.theArray[1] * DEG_TO_RAD + ::asin(::sin(p) * ::sin(az) / ::cos(IPP.theArray[0]));
 ...
 */
double Position::getCurvMeridian() const noexcept
{
 ...
 */
double Position::getCurvPrimeVertical() const noexcept
{
...
Angle Position::getZenithAngle(const Position &target, AngleReduced &delta) const
{
...
Angle Position ::getZenithAngle(const Angle &phi1, const Angle &lambda1, const Angle &phi2, const Angle &lambda2,
                                double r1, double r2, AngleReduced &delta)
{
    DEBUGTRACE_FUNCTION();
...
Position Position::getRayPerigee(const Position &target) const
{
    ...
    {
        AngleReduced sigma((sin(lambda2 - lambda1) * cos(phi2)) / sin(delta), // eq.158
...
// @throw GeometryException on invalid input.
void Position::initialize(const double a, const double b, const double c, Position::CoordinateSystem s,
                          const EllipsoidModel *ell, const RefFrame &frame)
{
    double bb(b);
    if (s == Geodetic || s == Geocentric)
    {
        if (a > 90 || a < -90)
        {
            GeometryException ge("Invalid latitude in constructor: " + StringUtils::asString(a));
            GNSSTK_THROW(ge);
        }
        if (bb < 0)
    ...
    if (s == Geocentric || s == Spherical)
    {
        if (c < 0)
        {
            GeometryException ge("Invalid radius in constructor: " + StringUtils::asString(c));
            GNSSTK_THROW(ge);
        }
    }
    if (s == Spherical)
    {
        if (a < 0 || a > 180)
        {
            GeometryException ge("Invalid theta in constructor: " + StringUtils::asString(a));
            GNSSTK_THROW(ge);
        }
        if (bb < 0)
            bb += 360 * (1 + (unsigned long)(bb / 360));
