...
#define THROW_IF_INVALID_DETAILED()                                                                                    \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            InvalidTropModel e;                                                                                        \
            if (!validLat)                                                                                             \
                e.addText("Invalid trop model: validLat");                                                             \
            if (!validHeight)                                                                                          \
                e.addText("Invalid trop model: validHeight");                                                          \
            if (!validTime)                                                                                            \
...
MOPSTropModel::MOPSTropModel(const double &ht, const double &lat, const int &doy)
{
...
double MOPSTropModel::correction(const Position &RX, const Position &SV)
{
    try
    {
        setReceiverHeight(RX.getAltitude());
...
double MOPSTropModel::correction(const Position &RX, const Position &SV, const CommonTime &tt)
{
    setDayOfYear(tt);
...
double MOPSTropModel::correction(const Position &RX, const Position &SV, const int &doy)
{
    setDayOfYear(doy);
...
double MOPSTropModel::correction(const Xvt &RX, const Xvt &SV)
{
    Position R(RX), S(SV);
...
double MOPSTropModel::correction(const Xvt &RX, const Xvt &SV, const CommonTime &tt)
{
    setDayOfYear(tt);
    Position R(RX), S(SV);
...
double MOPSTropModel::correction(const Xvt &RX, const Xvt &SV, const int &doy)
{
    setDayOfYear(doy);
    Position R(RX), S(SV);
...
void MOPSTropModel::setWeather()
{
    ...
    {
        valid = false;
        InvalidTropModel e("MOPSTropModel must have Rx latitude before computing weather");
        GNSSTK_THROW(e);
    }
    ...
    {
        valid = false;
        InvalidTropModel e("MOPSTropModel must have day of year before computing weather");
        GNSSTK_THROW(e);
    }
...
void MOPSTropModel::setAllParameters(const CommonTime &time, const Position &rxPos)
{
    MOPSTime = (int)(static_cast<YDSTime>(time)).doy;
    validTime = true;
...
void MOPSTropModel::prepareParameters()
{
    ...
    try
    {
        ...
        if (axfi <= 15.0)
            index = 0;
        if ((axfi > 15.0) && (axfi <= 30.0))
            index = 1;
        if ((axfi > 30.0) && (axfi <= 45.0))
            index = 2;
        if ((axfi > 45.0) && (axfi <= 60.0))
            index = 3;
        if ((axfi > 60.0) && (axfi < 75.0))
            index = 4;
        if (axfi >= 75.0)
            index = 5;
        ...
        for (j = 0; j < 5; j++)
        {
            ...
            else
            {
                if (index < 5)
                {
                    avr0(j) = avr(index - 1, j) + (avr(index, j) - avr(index - 1, j)) * (axfi - fi0(index - 1)) /
                                                      (fi0(index) - fi0(index - 1));

                    svr0(j) = svr(index - 1, j) + (svr(index, j) - svr(index - 1, j)) * (axfi - fi0(index - 1)) /
                                                      (fi0(index) - fi0(index - 1));
                }
                else
                {
