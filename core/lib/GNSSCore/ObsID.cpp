...
{
    s << ObsID::cbDesc[band] << " " << ObsID::tcDesc[code] << " " << ObsID::otDesc[type];
    if (ObsID::verbose)
    {
        std::ios::fmtflags oldFlags = s.flags();
        s << " " << std::boolalpha << freqOffs << "/" << freqOffsWild << " " << std::hex << mcode << "/" << mcodeMask
...
bool ObsID::operator==(const ObsID &right) const
{
    ...
    return ((type == ObservationType::Any || right.type == ObservationType::Any || type == right.type) &&
            (band == CarrierBand::Any || right.band == CarrierBand::Any || band == right.band) &&
            (code == TrackingCode::Any || right.code == TrackingCode::Any || code == right.code) &&
            (xmitAnt == XmitAnt::Any || right.xmitAnt == XmitAnt::Any || xmitAnt == right.xmitAnt) &&
            (freqOffsWild == true || right.freqOffsWild == true || freqOffs == right.freqOffs) &&
...
#define ORDERRET(RV)                                                                                                   \
    {                                                                                                                  \
        std::cerr << "operator<() returning " << RV << " @ " << __LINE__ << std::endl;                                 \
        return RV;                                                                                                     \
    }
#else
...
bool ObsID::operator<(const ObsID &right) const
{
    ...
    if ((type != ObservationType::Any) && (right.type != ObservationType::Any))
    {
        if (type < right.type)
            ORDERRET(true);
        if (right.type < type)
    ...
    {
        if (xmitAnt < right.xmitAnt)
            ORDERRET(true);
        if (right.xmitAnt < xmitAnt)
            ORDERRET(false);
    }
    if (!freqOffsWild && !right.freqOffsWild)
    {
        if (freqOffs < right.freqOffs)
            ORDERRET(true);
        if (right.freqOffs < freqOffs)
            ORDERRET(false);
    }
    // combined mask, which basically means that a 0 in either
    // mask is a wildcard.
...
bool ObsID ::isWild() const
{
    return ((type == ObservationType::Any) || (band == CarrierBand::Any) || (code == TrackingCode::Any) ||
            (xmitAnt == XmitAnt::Any) || (mcodeMask != -1) || freqOffsWild);
}
