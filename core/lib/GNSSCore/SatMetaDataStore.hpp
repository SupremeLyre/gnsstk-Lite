...
class SatMetaDataStore
{
  public:
    ...
    struct Signal
    {
        ...
        Signal() : carrier(CarrierBand::Unknown), code(TrackingCode::Unknown), nav(NavType::Unknown)
        {
        }
        /// Initialize using complete set of specified values.
        Signal(CarrierBand c, TrackingCode r, NavType n) : carrier(c), code(r), nav(n)
        {
        }
     ...
     * @return true if the requested satellite mapping was found.
     */
    bool getNORAD(SatelliteSystem sys, uint32_t prn, const gnsstk::CommonTime &when, int32_t &norad) const;
     ...
     * @return true if the requested satellite mapping was found.
     */
    bool getNORAD(const SatID &sat, const gnsstk::CommonTime &when, int32_t &norad) const
    {
     ...
     * @return true if the requested satellite mapping was found.
     */
    bool findSatBySVN(SatelliteSystem sys, const std::string &svn, const gnsstk::CommonTime &when,
    ...
    SatSet getSatsBySignal(const SignalSet &signals, const CommonTime &beginTime, const CommonTime &endTime,
                           const std::set<SatMetaData::Status> &status = {SatMetaData::Status::Operational,
...
bool SatMetaDataStore::Signal ::operator<(const Signal &right) const
{
...
bool SatMetaDataStore::Signal ::operator==(const Signal &right) const
{
    if ((carrier != CarrierBand::Any) && (right.carrier != CarrierBand::Any) && (carrier != right.carrier))
    {
        return false;
    }
    if ((code != TrackingCode::Any) && (right.code != TrackingCode::Any) && (code != right.code))
    {
        return false;
    }
    if ((nav != NavType::Any) && (right.nav != NavType::Any) && (nav != right.nav))
    {
        return false;
    }
    return true;
...
bool SatMetaDataStore::SystemBlock ::operator<(const SatMetaDataStore::SystemBlock &right) const
{
...
inline std::ostream &operator<<(std::ostream &s, const SatMetaDataStore::SystemBlock &sblk)
{
    s << gnsstk::StringUtils::asString(sblk.sys) << " " << sblk.blk;
    return s;
}

inline std::ostream &operator<<(std::ostream &s, const SatMetaDataStore::SVNID &svn)
{
