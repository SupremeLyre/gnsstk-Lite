...
std::map<std::string, std::string> RinexObsID::map1to3sys{{"G", "GPS"}, {"R", "GLO"}, {"E", "GAL"}, {"S", "GEO"},
                                                          {"C", "BDS"}, {"J", "QZS"}, {"I", "IRN"}};

std::map<std::string, std::string> RinexObsID::map3to1sys{{"GPS", "G"}, {"GLO", "R"}, {"GAL", "E"}, {"GEO", "S"},
                                                          {"BDS", "C"}, {"QZS", "J"}, {"IRN", "I"}};
...
std::string RinexObsID::asString(double version) const
{
    ...
    if ((fabs(version - 3.02) < 0.005) && (band == CarrierBand::B1) &&
        ((code == TrackingCode::B1I) || (code == TrackingCode::B1Q) || (code == TrackingCode::B1IQ)))
    {
    ...
    try
    {
        ...
        for (size_t i = 0; i < RinexObsID::validRinexSystems.size(); i++)
        {
            ...
            for (it = RinexObsID::validRinexTrackingCodes[sys[0]].begin();
                 it != RinexObsID::validRinexTrackingCodes[sys[0]].end(); ++it)
            {
                s << "   " << system << "(" << sys << "), freq " << it->first << ", codes '" << it->second << "'"
                  << std::endl;
                ...
                for (size_t j = 0; j < codes.size(); ++j)
                {
                    ...
                    {
                        str = std::string(1, types[k]) + std::string(1, it->first) + std::string(1, codes[j]);
                        std::ostringstream oss;
                        if (!isValidRinexObsID(str, csys))
                            oss << str << " " << "-INVALID-";
                        else
                        {
                            RinexObsID robsid(sys + str, Rinex3ObsBase::currentVersion);
                            oss << str << " " << robsid;
                        }
...
bool RinexObsID ::equalIndex(const RinexObsID &right) const
{
    if (type != right.type)
        return false;
    if (type == ObservationType::Iono)
...
RinexObsID RinexObsID ::newID(const std::string &strID, const std::string &desc)
{
    if (char2ot.count(strID[0]) && char2cb.count(strID[1]) && char2tc.count(strID[2]))
    {
        GNSSTK_THROW(InvalidParameter("Identifier " + strID + " already defined."));
    }

    return idCreator(strID, desc);
}

RinexObsID RinexObsID ::idCreator(const std::string &strID, const std::string &desc)
{
    char ot = strID[0];
...
std::string RinexObsID ::getOTChars()
{
    std::string rv;
...
std::string RinexObsID ::getCBChars()
{
    std::string rv;
...
std::string RinexObsID ::getTCChars()
{
    std::string rv;
