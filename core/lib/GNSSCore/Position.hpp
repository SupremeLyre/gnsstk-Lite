...
class Position : public Triple
{
  public:
     ...
     */
    Position(const double &a, const double &b, const double &c, CoordinateSystem s = Cartesian,
             const EllipsoidModel *ell = nullptr, const RefFrame &frame = RefFrame());
     ...
     * @throw GeometryException on invalid input.
     */
    Position(const double ABC[3], CoordinateSystem s = Cartesian, const EllipsoidModel *ell = nullptr,
     ...
     * @throw GeometryException on invalid input.
     */
    Position(const Triple &ABC, CoordinateSystem s = Cartesian, const EllipsoidModel *ell = nullptr,
             const RefFrame &frame = RefFrame());
     ...
     * @param[in] eccSq square of Earth eccentricity
     */
    static void convertGeocentricToGeodetic(const Triple &llr, Triple &geodeticllh, const double A,
                                            const double eccSq) noexcept;
     ...
     * @param[in] eccSq square of Earth eccentricity
     */
    static void convertGeodeticToGeocentric(const Triple &geodeticllh, Triple &llr, const double A,
                                            const double eccSq) noexcept;
     ...
     * @return the zenith angle between this and target.
     */
    static Angle getZenithAngle(const Angle &phi1, const Angle &lambda1, const Angle &phi2, const Angle &lambda2,
  ...
  private:
     ...
     * @throw GeometryException on invalid input.
     */
    void initialize(const double a, const double b, const double c, CoordinateSystem s = Cartesian,
                    const EllipsoidModel *ell = nullptr, const RefFrame &frame = RefFrame());
