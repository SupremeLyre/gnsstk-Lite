...
double PowerSum::moment(int i) const noexcept
{
    ...
    if (i == 2 && n > 1)
        ...
        m = ni * (s[5] + ni * (-5 * s[1] * s[4] +
                               ni * (10 * s12 * s[3] + ni * (-10 * s12 * s[1] * s[2] + ni * (4 * s12 * s12 * s[1])))));
...
void PowerSum::dump(std::ostream &str) const noexcept
{
    ...
    str << "m1:" << moment(1) << " m2:" << moment(2) << " m3:" << moment(3) << " m4:" << moment(4) << std::endl;

    str << "average:" << average() << " stddev:" << sqrt(variance()) << " skew:" << skew() << " kurtosis:" << kurtosis()
        << std::endl;
}
} // namespace gnsstk
