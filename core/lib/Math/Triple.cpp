...
Triple ::Triple() : theArray(0., 3)
{
}

Triple ::Triple(const Triple &right) : theArray(right.theArray)
{
}

Triple ::Triple(double a, double b, double c) : theArray(3)
{
    theArray[0] = a;
    theArray[1] = b;
    theArray[2] = c;
...
std::vector<double> Triple::toStdVector()
{
    ...
    for (int i = 0; i < 3; i++)
        toReturn.push_back(theArray[i]);
    return toReturn;
}

// returns the dot product of the two vectors
double Triple ::dot(const Triple &right) const noexcept
{
    Triple z;
...
Triple Triple ::cross(const Triple &right) const noexcept
{
    Triple cp;
...
double Triple ::slantRange(const Triple &right) const noexcept
{
    Triple z;
 ...
 */
Triple Triple::R1(const double &angle) const noexcept
{
 ...
 */
Triple Triple::R2(const double &angle) const noexcept
{
    double ang(angle * DEG_TO_RAD);
    double sinangle(std::sin(ang));
    double cosangle(std::cos(ang));
 ...
 */
Triple Triple::R3(const double &angle) const noexcept
{
    double ang(angle * DEG_TO_RAD);
    double sinangle(std::sin(ang));
    double cosangle(std::cos(ang));
