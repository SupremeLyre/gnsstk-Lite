...
template <class T> class Stats
{
  public:
    ...
    std::string asString(std::string msg = std::string(), int w = 7, int p = 4) const
    {
        ...
        oss << "stats(con):" << (msg.empty() ? "" : " " + msg) << " N " << std::setw(w) << N() << std::fixed
            << std::setprecision(p) << "  Ave " << std::setw(w) << Average() << "  Std " << std::setw(w) << StdDev()
            << "  Var " << std::setw(w) << Variance() << "  Min " << std::setw(w) << Minimum() << "  Max "
            << std::setw(w) << Maximum() << "  P2P " << std::setw(w) << Maximum() - Minimum();
        return oss.str();
    }
    ...
    {
        std::ostringstream oss;
        oss << msg << std::fixed << std::setprecision(p) << " N " << std::setw(w) << N() << "  Ave " << std::setw(w)
            << Average() << "  Std " << std::setw(w) << StdDev();
        return oss.str();
    }

    // the rest are just accessors --------------------------------------
    /// return the sample size
    inline unsigned int N(void) const
    {
        return n;
    }
...
template <class T> class SeqStats
{
  public:
    ...
    std::string asString(std::string msg = std::string(), int w = 7, int p = 4) const
    {
        std::ostringstream oss;
        // static const int p=4, w=7;
        oss << "stats(seq):" << (msg.empty() ? "" : " " + msg) << " N " << std::setw(w) << N() << std::fixed
            << std::setprecision(p) << "  Ave " << std::setw(w) << Average() << "  Std " << std::setw(w) << StdDev()
            << "  Var " << std::setw(w) << Variance() << "  Min " << std::setw(w) << Minimum() << "  Max "
            << std::setw(w) << Maximum() << "  P2P " << std::setw(w) << Maximum() - Minimum();
        return oss.str();
...
template <class T> class WtdStats : public SeqStats<T>
{
  public:
    ...
    void Add(const T &x, const T &wt_in) // WtdStats
    {
        T wt(::fabs(wt_in));
        if (wt == T())
            return; // Don't add with zero weight
    ...
    void Subtract(const T &x, const T &wt_in) // WtdStats
    {
        ...
        T newvar = WtNorm * SeqStats<T>::var + WtNorm * SeqStats<T>::ave * SeqStats<T>::ave - wt * x * x;
        WtNorm -= wt;
    ...
    WtdStats<T> &operator+=(const WtdStats<T> &S)
    {
        ...
        if (SeqStats<T>::min > S.min)
            SeqStats<T>::min = S.min;
        if (S.max > SeqStats<T>::max)
            SeqStats<T>::max = S.max;

        T newave = WtNorm * SeqStats<T>::ave + S.WtNorm * S.ave;
        T newvar = WtNorm * SeqStats<T>::var + S.WtNorm * S.var + WtNorm * SeqStats<T>::ave * SeqStats<T>::ave +
                   S.WtNorm * S.ave * S.ave;
        WtNorm += S.WtNorm;
        SeqStats<T>::ave = newave / WtNorm;
        SeqStats<T>::var = newvar / WtNorm - SeqStats<T>::ave * SeqStats<T>::ave;
        SeqStats<T>::n += S.n;
    ...
    {
        std::ostringstream oss;
        oss << "stats(wtd):" << (msg.empty() ? "" : " " + msg) << " N " << std::setw(w) << SeqStats<T>::N()
            << std::fixed << std::setprecision(p) << "  Ave " << std::setw(w) << SeqStats<T>::Average() << "  Std "
            << std::setw(w) << SeqStats<T>::StdDev() << "  Var " << std::setw(w) << SeqStats<T>::Variance() << "  Min "
            << std::setw(w) << SeqStats<T>::Minimum() << "  Max " << std::setw(w) << SeqStats<T>::Maximum() << "  P2P "
            << std::setw(w) << SeqStats<T>::Maximum() - SeqStats<T>::Minimum() << "  Wts " << std::setw(w) << WtsSum();
        return oss.str();
    }
    ...
    {
        std::ostringstream oss;
        oss << msg << std::fixed << std::setprecision(p) << " N " << std::setw(w) << SeqStats<T>::N() << "  Ave "
            << std::setw(w) << SeqStats<T>::Average() << "  Std " << std::setw(w) << SeqStats<T>::StdDev();
        return oss.str();
    }

    /// return normalization = sum of weights
    inline T WtsSum(void) const
    {
        return WtNorm;
    } // WtdStats
...
template <class T> class TwoSampleStats
{
  public:
    ...
    void Load(std::vector<unsigned int> &vuint, std::vector<T> &vT) // TSS
    {
        ...
        for (i = 0; i < 2; i++)
            vi2.push_back(vuint[i + 2]);
        for (i = 0; i < 5; i++)
            vt2.push_back(vT[i + 5]);
        SY.Load(vi2, vt2);
    ...
    std::string asString(std::string msg = std::string(), int w = 7, int p = 4) const
    {
        ...
        oss << SX.asString(msg, w, p) << " (X)" << std::endl
            ...
            << "stats(tss):" << (msg.empty() ? "" : " " + msg) << " N " << std::setw(w) << N() << std::fixed
            << std::setprecision(p) << "  Int " << std::setw(w) << Intercept() << "  Slp " << std::setw(w) << Slope()
            << " +- " << std::setw(w) << SigmaSlope() << "  CSig " << std::setw(w) << SigmaYX() << "  Corr "
            << std::setw(w) << Correlation();
        return oss.str();
    }
    ...
    {
        std::ostringstream oss;
        oss << SX.asShortString(msg, w, p) << " (X);" << SY.asShortString(msg, w, p) << " (Y);" << msg << std::fixed
            << std::setprecision(p) << std::fixed << std::setprecision(p) << "  Int " << std::setw(w) << Intercept()
            << "  Slp " << std::setw(w) << Slope() << " +- " << std::setw(w) << SigmaSlope() << "  CSig "
            << std::setw(w) << SigmaYX() << "  Corr " << std::setw(w) << Correlation();
        return oss.str();
    }
    ...
    inline T MinimumX(void) const
    {
        return SX.Minimum();
    }
    /// return maximum X value
    inline T MaximumX(void) const
    {
        return SX.Maximum();
    }
    /// return minimum Y value
    inline T MinimumY(void) const
    {
        return SY.Minimum();
    }
    /// return maximum Y value
    inline T MaximumY(void) const
    {
        return SY.Maximum();
    }
    ...
    inline T StdDevX(void) const
    {
        return SX.StdDev();
    }
    /// return computed Y standard deviation
    inline T StdDevY(void) const
    {
        return SY.StdDev();
    }
    ...
    inline T VarianceYX(void) const
    {
        if (n > 2)
        {
            return (VarianceY() * (T(n - 1) / T(n - 2)) * (T(1) - Correlation() * Correlation()));
        }
        else
            return T();
    }
...
template <class T> std::ostream &operator<<(std::ostream &s, const TwoSampleStats<T> &TSS)
{
    std::ofstream savefmt;
    savefmt.copyfmt(s);
    ...
    if (bad)
        ...
        s << TSS.SigmaSlope();
    s << "\n Conditional uncertainty (sigma Y given X) = ";
    s.copyfmt(savefmt);
    if (bad)
        s << badmsg;
    else
