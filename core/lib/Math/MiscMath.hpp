 ...
 * @throw Exception
 */
template <class T> T SimpleLagrangeInterpolation(const std::vector<T> &X, const std::vector<T> &Y, const T x)
{
    if (Y.size() < X.size())
    {
...
template <class T> T RSS(T aa, T bb, T cc)
{
...
template <class T> T RSS(T aa, T bb)
{
    return RSS(aa, bb, T(0));
}

/// Perform the root sum square of aa, bb, cc and dd
template <class T> T RSS(T aa, T bb, T cc, T dd)
{
