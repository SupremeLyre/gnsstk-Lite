...
#define TUASSERTE(TYPE, EXP, GOT)                                                                                      \
    ...
    {                                                                                                                  \
        std::cerr << exc << std::endl;                                                                                 \
        testFramework.assert(false, "Exception evaluating " #EXP " or " #GOT, __LINE__);                               \
    }                                                                                                                  \
    catch (...)                                                                                                        \
    {                                                                                                                  \
        testFramework.assert(false, "Exception evaluating " #EXP " or " #GOT, __LINE__);                               \
...
#define TUCMPFILE(F1, F2, SKIP)                                                                                        \
    try                                                                                                                \
    {                                                                                                                  \
        testFramework.assert_files_equal(__LINE__, F1, F2, "File mismatch: " + F1 + " " + F2, SKIP);                   \
    }                                                                                                                  \
    catch (gnsstk::Exception & exc)                                                                                    \
    {                                                                                                                  \
        std::cerr << exc << std::endl;                                                                                 \
        testFramework.assert(false, "Exception comparing " #F1 " and " #F2, __LINE__);                                 \
    }                                                                                                                  \
    catch (...)                                                                                                        \
    {                                                                                                                  \
        testFramework.assert(false, "Exception comparing " #F1 " and " #F2, __LINE__);                                 \
...
class TestUtil
{
  public:
     ...
     */
    TestUtil(const std::string &sourceClassInput = "Unknown", const std::string &sourceMethodInput = "Unknown",
             const std::string &testFileInput = "Unknown", const int &testLineInput = 0, const int &verbosityInput = 1);
     ...
     */
    void assert_equals(double expected, double got, int lineNumber, const std::string &testMsg = std::string(),
                       double epsilon = -1)
    {
        assert_equals_fp<double>(expected, got, lineNumber, testMsg, epsilon);
    }

    void assert_equals(long double expected, long double got, int lineNumber,
                       const std::string &testMsg = std::string(), long double epsilon = -1)
    {
        assert_equals_fp<double>(expected, got, lineNumber, testMsg, epsilon);
    }

    void assert_equals(float expected, float got, int lineNumber, const std::string &testMsg = std::string(),
                       float epsilon = -1)
    {
        assert_equals_fp<double>(expected, got, lineNumber, testMsg, epsilon);
    }
    ...
    template <typename T>
    void assert_equals_fp(const T &expected, const T &got, int lineNumber, const std::string &testMsg = std::string(),
                          T epsilon = -1);
     ...
     */
    template <typename T>
    void assert_equals(const gnsstk::Matrix<T> &expected, const gnsstk::Matrix<T> &got, int lineNumber,
                       const std::string &testMsg = std::string(), T epsilon = -1);
    template <typename T>
    void assert_equals(const gnsstk::Vector<T> &expected, const gnsstk::Vector<T> &got, int lineNumber,
                       const std::string &testMsg = std::string(), T epsilon = -1);
     ...
     */
    void assert_files_equal(int lineNumber, const std::string &file1Name, const std::string &file2Name,
                            const std::string &testMsg, int numLinesSkip = 0, bool ignoreLeadingSpaces = false,
     ...
     */
    void assert_binary_files_equal(int lineNumber, const std::string &file1Name, const std::string &file2Name,
                                   const std::string &testMsg, unsigned long long from = 0, unsigned long long to = -1);
     ...
     */
    bool fileEqualTest(const std::string &refFile, const std::string &checkFile, int numLinesSkip = 0,
                       bool ignoreLeadingSpaces = false, bool ignoreTrailingSpaces = false,
                       std::vector<std::string> ignoreRegex = std::vector<std::string>(0));
     ...
     * @param[in] from First byte to compare.
     * @param[in] to Last byte to compare. */
    bool fileCompTest(const std::string &refFile, const std::string &checkFile, unsigned long long from = 0,
...
TestUtil ::TestUtil(const std::string &sourceClassInput, const std::string &sourceMethodInput,
                    const std::string &testFileInput, const int &testLineInput, const int &verbosityInput)
    : outputKeyword("GNSSTkTest"), sourceClass(sourceClassInput), sourceMethod(sourceMethodInput),
      testFileName(testFileInput), testFileLine("0"), testMessage("Developer is a lazy slacker"), failBit(0),
      verbosity(verbosityInput), testCount(0), subtestID(1), failCount(0)
{
...
void TestUtil ::assert(bool testExpression, const std::string &testMsg, const int lineNumber)
{
...
template <class T>
void TestUtil ::assert_equals(const T &expected, const T &got, int lineNumber, const std::string &testMsg)
{
    std::string mess(testMsg);
    if (testMsg.empty())
    {
        std::ostringstream ostr;
        ostr << std::boolalpha << "Expected:'" << expected << "' ,But got:'" << got << "'";
        mess = ostr.str();
    }
...
template <typename T>
void TestUtil ::assert_equals_fp(const T &expected, const T &got, int lineNumber, const std::string &testMsg, T epsilon)
{
    ...
    {
        std::ostringstream ostr;
        ostr << std::setprecision(20) << "abs(" << expected << " - " << got << ") = " << err;
        if (good)
            ostr << " <= ";
        else
            ostr << " > ";
...
template <typename T>
void TestUtil ::assert_equals_fps(const T &expected, const T &got, int lineNumber, const std::string &testMsg)
{
    T err = std::abs(expected - got);
...
template <typename T>
void TestUtil ::assert_equals(const gnsstk::Matrix<T> &expected, const gnsstk::Matrix<T> &got, int lineNumber,
                              const std::string &testMsg, T epsilon)
{
    if (epsilon < 0)
        epsilon = std::numeric_limits<T>::epsilon();
...
template <typename T>
void TestUtil ::assert_equals(const gnsstk::Vector<T> &expected, const gnsstk::Vector<T> &got, int lineNumber,
                              const std::string &testMsg, T epsilon)
{
    if (epsilon < 0)
        epsilon = std::numeric_limits<T>::epsilon();
...
void TestUtil ::assert_files_equal(int lineNumber, const std::string &file1Name, const std::string &file2Name,
                                   const std::string &testMsg, int numLinesSkip, bool ignoreLeadingSpaces,
                                   bool ignoreTrailingSpaces, std::vector<std::string> ignoreRegex)
{
    bool eq = fileEqualTest(file1Name, file2Name, numLinesSkip, ignoreLeadingSpaces, ignoreTrailingSpaces, ignoreRegex);
    assert(eq, testMsg, lineNumber);
}

void TestUtil ::assert_binary_files_equal(int lineNumber, const std::string &file1Name, const std::string &file2Name,
                                          const std::string &testMsg, unsigned long long from, unsigned long long to)
{
    bool eq = fileCompTest(file1Name, file2Name, from, to);
    assert(eq, testMsg, lineNumber);
}

int TestUtil ::countFails(void)
{
    return (failCount);
}

int TestUtil ::countTests(void)
{
    return (testCount);
}

void TestUtil ::changeSourceMethod(const std::string &newMethod)
{
    sourceMethod = newMethod;
}

void TestUtil ::setTestMessage(const std::string &testMsg)
{
    testMessage = testMsg;
}

void TestUtil ::setTestMessage(const std::string &testMsg, const int lineNumber)
{
    setTestMessage(testMsg);
    setTestLine(lineNumber);
}

void TestUtil ::setTestLine(const int lineNumber)
{
...
bool TestUtil ::fileEqualTest(const std::string &refFile, const std::string &checkFile, int numLinesSkip,
                              bool ignoreLeadingSpaces, bool ignoreTrailingSpaces, std::vector<std::string> ignoreRegex)
{
    int lineNumber = 0;
    ...
    if (!checkStream.eof())
    {
        filesEqual = false;
        if (verbosity > 1)
            std::cout << "refLine:" << refLine << std::endl << "checkLine:" << checkLine << std::endl;
        return (filesEqual);
    }
    else
    {
...
bool TestUtil ::fileCompTest(const std::string &refFile, const std::string &checkFile, unsigned long long from,
...
void TestUtil ::print(void)
{
    ...
    if (failBit == 1 && verbosity >= 1)
    {
        std::cout << outputKeyword << ", "
                  ...
                  << "subtest=" << subtestID << ", "
                  << "failBit=" << failBit << ", "
                  << "testMsg=" << testMessage << std::endl; // implicit conversion from int to string
    }
    else
    {
        std::cout << outputKeyword << ", "
                  ...
                  << "testLine=" << testFileLine << ", "
                  << "subtest=" << subtestID << ", "
                  << "failBit=" << failBit << std::endl; // implicit conversion from int to string
    }
}

void TestUtil ::pass(void)
{
    failBit = 0;
    testCount++;
}

void TestUtil ::fail(void)
{
    failBit = 1;
    failCount++;
    testCount++;
...
void TestUtil ::fail(const std::string &failMsg)
{
    setTestMessage(failMsg);
    fail();
}

void TestUtil ::fail(const std::string &failMsg, const int lineNumber)
{
    setTestMessage(failMsg);
    setTestLine(lineNumber);
    fail();
