...
#define FCASSERTE(TYPE, COUNTER, EXP, GOT, LINE)                                                                       \
    ...
    {                                                                                                                  \
        ...
        ostr << std::boolalpha << "Expected " << COUNTER << "=" << EXP << ", but got " << COUNTER << "=" << GOT;       \
        testFramework.assert_equals<TYPE>(EXP, GOT, LINE, ostr.str());                                                 \
    }                                                                                                                  \
    catch (gnsstk::Exception & exc)                                                                                    \
    {                                                                                                                  \
        std::cerr << exc << std::endl;                                                                                 \
        testFramework.assert(false, "Exception evaluating " #EXP " or " #GOT, LINE);                                   \
    }                                                                                                                  \
    catch (...)                                                                                                        \
    {                                                                                                                  \
        testFramework.assert(false, "Exception evaluating " #EXP " or " #GOT, LINE);                                   \
 ...
 * systems. */
template <class Alm, class Eph, class TimeOffs, class Health, class Iono, class GrpDelay> class FactoryCounter
{
  public:
    FactoryCounter(gnsstk::TestUtil &tf)
        : testFramework(tf), almInc(true), ephInc(true), toInc(true), heaInc(true), ionoInc(true), iscInc(true)
    {
        resetCount();
    }
    ...
    void validateResults(gnsstk::NavDataPtrList &navOut, unsigned lineNo, size_t totalExp = 0, unsigned almExp = 0,
                         unsigned ephExp = 0, unsigned toExp = 0, unsigned heaExp = 0, unsigned ionoExp = 0,
                         unsigned iscExp = 0, unsigned otherExp = 0)
    {
        countResults(navOut);
        // sanity check.
        FCASSERTE(unsigned, "summed total", totalExp, almExp + ephExp + toExp + heaExp + ionoExp + iscExp + otherExp,
                  lineNo);
        // Get an expected total based on what's included and what isn't.
        totalExp = ((almInc ? almExp : 0) + (ephInc ? ephExp : 0) + (toInc ? toExp : 0) + (heaInc ? heaExp : 0) +
        ...
        FCASSERTE(unsigned, "ionoCount", (ionoInc ? ionoExp : 0), ionoCount, lineNo);
    ...
    unsigned almCount, ephCount, toCount, heaCount, ionoCount, iscCount, otherCount;
