...
int PRSolution ::PreparePRSolution(const CommonTime &nominalReceive, std::vector<SatID> &sats,
                                   const std::vector<double> &pseudoranges, NavLibrary &eph, Matrix<double> &svp,
...
int PRSolution ::SimplePRSolution(const CommonTime &T, const std::vector<SatID> &Sats, const Matrix<double> &SVP,
                                  const Matrix<double> &invMC, TropModel *pTropModel, const int &niterLimit,
                                  const double &convLimit, Vector<double> &Resids, Vector<double> &Slopes)
{
    if (!pTropModel)
    {
    ...
    try
    {
        ...
        try
        {
            ...
            std::tie(tropSuccess, converge, NIterations) =
                iterativeSinglePointWLSSolution(dX, partials, G, Covariance, Resids, Solution, filteredSats, SVP,
                                                pTropModel, T, currGNSS, filteredWeights, convLimit, niterLimit);
        ...
        if (TropFlag)
        {
            LOG(DEBUG) << "Trop correction not applied at time " << printTime(T, timfmt);
        }
...
int PRSolution ::RAIMComputeUnweighted(const CommonTime &Tr, std::vector<SatID> &Sats,
                                       const std::vector<double> &Pseudorange, NavLibrary &eph, TropModel *pTropModel,
                                       NavSearchOrder order)
{
    try
    {
        ...
        return (RAIMCompute(Tr, Sats, Pseudorange, weights, eph, pTropModel, order));
    }
    catch (Exception &e)
    {
        GNSSTK_RETHROW(e);
...
int PRSolution ::RAIMCompute(const CommonTime &Tr, std::vector<SatID> &Sats, const std::vector<double> &Pseudorange,
                             const Matrix<double> &invMC, NavLibrary &eph, TropModel *pTropModel, NavSearchOrder order)
{
    try
    {
        ...
        if (LOGlevel >= ConfigureLOG::Level("DEBUG"))
        {
            ...
            oss << " SVP matrix(" << SVP.rows() << "," << SVP.cols() << ")" << std::endl;
        ...
        for (int stage = 0;; ++stage)
        {
            ...
            do
            {
                ...
                //       -4  no ephemeris
                iret = SimplePRSolution(Tr, Sats, SVP, invMC, pTropModel, MaxNIterations, ConvergenceLimit, Resids,
                                        Slopes);
            ...
            {
                LOG(DEBUG) << " RAIM: break before stage " << stage << " due to NSatsReject " << NSatsReject;
                break;
            }
        ...
        LOG(DEBUG) << " RAIM exit with ret value " << iret << " and Valid " << (Valid ? "T" : "F");
...
std::string PRSolution ::outputValidString(int iret)
{
    std::ostringstream oss;
    if (iret != -99)
    {
...
std::string PRSolution ::outputNAVString(std::string tag, int iret, const Vector<double> &Vec)
{
    std::ostringstream oss;
    ...
    oss << tag << " NAV " << printTime(currTime, gpsfmt) << std::fixed << std::setprecision(6) << " " << std::setw(16)
        << (&Vec == &PRSNullVector ? Solution(0) : Vec(0)) << " " << std::setw(16)
        << (&Vec == &PRSNullVector ? Solution(1) : Vec(1)) << " " << std::setw(16)
        << (&Vec == &PRSNullVector ? Solution(2) : Vec(2)) << std::fixed << std::setprecision(3);
...
std::string PRSolution ::outputPOSString(const std::string &tag, int iret, const Vector<double> &Vec)
{
    std::ostringstream oss;
    ...
    oss << tag << " POS " << printTime(currTime, gpsfmt) << std::fixed << std::setprecision(6) << " " << std::setw(16)
        << (&Vec == &PRSNullVector ? Solution(0) : Vec(0)) << " " << std::setw(16)
        << (&Vec == &PRSNullVector ? Solution(1) : Vec(1)) << " " << std::setw(16)
        << (&Vec == &PRSNullVector ? Solution(2) : Vec(2)) << outputValidString(iret);

    return oss.str();
}

std::string PRSolution ::outputCLKString(std::string tag, int iret)
{
    std::ostringstream oss;
    ...
    oss << tag << " CLK " << printTime(currTime, gpsfmt) << std::fixed << std::setprecision(3);
...
std::string PRSolution ::outputRMSString(const std::string &tag, int iret)
{
    std::ostringstream oss;
    ...
    oss << tag << " RMS " << printTime(currTime, gpsfmt) << " " << std::setw(2) << goodsats.size() << std::fixed
        << std::setprecision(3) << " " << std::setw(8) << RMSResidual << std::setprecision(2) << " " << std::setw(7)
        << TDOP << " " << std::setw(7) << PDOP << " " << std::setw(7) << GDOP << std::setprecision(1) << " "
        << std::setw(5) << MaxSlope << " " << std::setw(2) << NIterations << std::scientific << std::setprecision(2)
...
std::string PRSolution ::outputString(const std::string &tag, int iret, const Vector<double> &Vec)
{
    std::ostringstream oss;
...
std::string PRSolution ::errorCodeString(int iret)
{
...
std::string PRSolution ::configString(const std::string &tag)
{
    std::ostringstream oss;
    oss << tag << "\n   iterations " << MaxNIterations << "\n   convergence " << std::scientific << std::setprecision(2)
        << ConvergenceLimit << "\n   RMS residual limit " << std::fixed << RMSLimit << "\n   RAIM slope limit "
        << std::fixed << SlopeLimit << " meters"
        << "\n   Maximum number of satellites to reject is " << NSatsReject << "\n   Memory information IS "
        << (hasMemory ? "" : "NOT ") << "stored";
...
void PRSolution ::markSatellite(SatID &sat) const
{
    sat.id = -std::abs(sat.id);
}

bool PRSolution ::isMarked(const SatID &sat) const
{
    return sat.id <= 0;
}

FilteredSats PRSolution ::filterMarkedSats(std::vector<SatID> &sats) const
{
...
FilteredConstSats PRSolution ::filterMarkedSats(const std::vector<SatID> &sats) const
{
...
void PRSolution ::markDisallowedGNSS(std::vector<SatID> &sats, const std::vector<SatelliteSystem> &allowed) const
{
    for (SatID &sat : sats)
    {
...
double PRSolution ::computeRange(Position &svPos, const Position &rxPos, bool firstIteration) const
{
...
std::tuple<bool, double> PRSolution ::computeTropDelay(const Position &svPos, const Position &rxPos,
                                                       TropModel *pTropModel, const CommonTime &time) const
{
...
bool PRSolution ::getSatPVT(Xvt &svXvt, const CommonTime &nominalReceive, double pseudorange, NavLibrary &eph,
                            SatID &sat, NavSearchOrder order) const
{
    NavSatelliteID navSat(sat);

    bool success;
    CommonTime transmitTime;
    std::tie(success, transmitTime) = RawRange::estTransmitFromObs(
        nominalReceive, pseudorange, eph, navSat, SVHealth::Healthy, NavValidityType::ValidOnly, order);
    ...
    success = eph.getXvt(navSat, transmitTime, svXvt,
                         false, // useAlm
                         SVHealth::Healthy, NavValidityType::ValidOnly, order);
    return success;
}

bool PRSolution ::computePartialsAndResiduals(Matrix<double> &partials, Vector<double> &residuals,
                                              const FilteredConstSats &filteredSats, const Matrix<double> &svp,
                                              const Vector<double> &aPrioriSolution, bool firstIteration,
                                              TropModel *pTropModel, const CommonTime &nominalReceive,
                                              const std::vector<SatelliteSystem> &currGNSS) const
{
    ...
    for (size_t filteredSatIndex = 0; filteredSatIndex < filteredSats.size(); ++filteredSatIndex)
    {
        ...
        Triple dirCos((rxPos[X] - svPos[X]) / rho, (rxPos[Y] - svPos[Y]) / rho, (rxPos[Z] - svPos[Z]) / rho);
        ...
        if (!firstIteration)
        {
            double tropDelay;
            bool success;
            std::tie(success, tropDelay) = computeTropDelay(svPos, rxPos, pTropModel, nominalReceive);
            tropSuccess = tropSuccess && success;
            residual -= tropDelay;
            LOG(DEBUG) << "Trop " << satIndex << " " << sat << " " << std::fixed << std::setprecision(3) << tropDelay;
        }
...
bool PRSolution ::singlePointWLSSolution(Vector<double> &dX, Matrix<double> &partials, Matrix<double> &G,
                                         Matrix<double> &covariance, Vector<double> &residuals,
                                         const Vector<double> &aPrioriSolution, const FilteredConstSats filteredSats,
                                         const Matrix<double> &svp, TropModel *pTropModel,
                                         const CommonTime &nominalReceive, bool firstIteration,
                                         const std::vector<SatelliteSystem> &currGNSS,
                                         const Matrix<double> &weights) const
{
    bool tropSuccess = computePartialsAndResiduals(partials, residuals, filteredSats, svp, aPrioriSolution,
    ...
    LOG(DEBUG) << "Resids (" << residuals.size() << ") " << std::fixed << std::setprecision(3) << residuals;
...
std::tuple<bool, double, int> PRSolution ::iterativeSinglePointWLSSolution(
    Vector<double> &totaldX, Matrix<double> &partials, Matrix<double> &G, Matrix<double> &covariance,
    Vector<double> &residuals, const Vector<double> &aPrioriSolution, const FilteredConstSats &filteredSats,
    const Matrix<double> &svp, TropModel *pTropModel, const CommonTime &nominalReceive,
    const std::vector<SatelliteSystem> &currGNSS, const Matrix<double> &weights, double convLimit, int niterLimit) const
{
    bool tropSuccess;
    double dim = 3 + currGNSS.size();
    partials.resize(filteredSats.size(), dim, 0.0);
    ...
    tropSuccess = singlePointWLSSolution(dX, partials, G, covariance, residuals, aPrioriSolution, filteredSats, svp,
                                         pTropModel, nominalReceive, true, currGNSS, weights);
    totaldX += dX;

    tropSuccess = singlePointWLSSolution(dX, partials, G, covariance, residuals, aPrioriSolution + totaldX,
                                         filteredSats, svp, pTropModel, nominalReceive, false, currGNSS, weights);
    totaldX += dX;
...
Matrix<double> PRSolution ::createSVP(std::vector<SatID> &sats, const CommonTime &time,
                                      const std::vector<double> &pseudoranges, NavLibrary &eph,
                                      NavSearchOrder order) const
{
    ...
    for (const auto &kv : filterMarkedSats(sats))
    {
        ...
        Xvt pvt;
        bool success = getSatPVT(pvt, time, pseudoranges[satIndex], eph, sat, order);

        if (!success)
        {
            LOG(DEBUG) << "Warning - PRSolution ignores satellite (no ephemeris) " << RinexSatID(sat) << " at time "
                       << printTime(time, timfmt);
            markSatellite(sat);
            continue;
        }
        ...
        LOG(DEBUG) << "SVP: Sat " << RinexSatID(sat) << " PR " << std::fixed << std::setprecision(3)
                   << pseudoranges[satIndex] << " clkbias " << C_MPS * pvt.clkbias << " relcorr "
                   << C_MPS * pvt.relcorr;
    }
...
void PRSolution ::computeSlopes(Vector<double> &slopes, const Matrix<double> &partials, const Matrix<double> &G,
...
Matrix<double> PRSolution ::filterWeights(const Matrix<double> &weights, const FilteredConstSats &filteredSats) const
{
...
std::vector<SatelliteSystem> PRSolution ::filterGNSS(const std::vector<SatelliteSystem> &allowedGNSS,
                                                     const FilteredConstSats &filteredSats) const
{
