  ...
  public:
    ...
    void add(const Vector<double> &Sol, const Matrix<double> &Cov)
    {
        try
        {
            ...
            if (N == 0)
            { // first call: dimension and set to zero
    ...
    void dump(std::ostream &os, std::string msg = "") const
    {
        try
        {
            os << "Simple statistics on " << msg << std::endl << std::fixed << std::setprecision(3);
            if (N > 0)
            {
                os << "  " << lab[0] << " N: " << S[0].N() << std::fixed << std::setprecision(4)
                   << " Ave: " << S[0].Average() + Sbias[0] << " Std: " << S[0].StdDev()
                   << " Min: " << S[0].Minimum() + Sbias[0] << " Max: " << S[0].Maximum() + Sbias[0] << std::endl;
                os << "  " << lab[1] << " N: " << S[1].N() << std::fixed << std::setprecision(4)
                   << " Ave: " << S[1].Average() + Sbias[1] << " Std: " << S[1].StdDev()
                   << " Min: " << S[1].Minimum() + Sbias[1] << " Max: " << S[1].Maximum() + Sbias[1] << std::endl;
                os << "  " << lab[2] << " N: " << S[2].N() << std::fixed << std::setprecision(4)
                   << " Ave: " << S[2].Average() + Sbias[2] << " Std: " << S[2].StdDev()
                   << " Min: " << S[2].Minimum() + Sbias[2] << " Max: " << S[2].Maximum() + Sbias[2] << std::endl;
...
class PRSolution
{
  public:
    /// Constructor
    PRSolution()
        : RMSLimit(6.5), SlopeLimit(1000.), NSatsReject(-1), MaxNIterations(10), ConvergenceLimit(3.e-7),
          hasMemory(true), fixedAPriori(false), nsol(0), ndata(0), APV(0.0), Valid(false)
    {
        was.reset();
    ...
    int PreparePRSolution(const CommonTime &Tr, std::vector<SatID> &Sats, const std::vector<double> &Pseudorange,
                          NavLibrary &eph, Matrix<double> &SVP, NavSearchOrder order = NavSearchOrder::User) const;
    ...
    int SimplePRSolution(const CommonTime &Tr, const std::vector<SatID> &Sats, const Matrix<double> &SVP,
    ...
    int RAIMComputeUnweighted(const CommonTime &Tr, std::vector<SatID> &Satellites,
    ...
    /// -4  ephemeris not found for all the satellites
    int RAIMCompute(const CommonTime &Tr, std::vector<SatID> &Satellites, const std::vector<double> &Pseudorange,
                    const Matrix<double> &invMC, NavLibrary &eph, TropModel *pTropModel,
                    NavSearchOrder order = NavSearchOrder::User);
    ...
    std::string outputPOSString(const std::string &tag, int iret = -99, const Vector<double> &Vec = PRSNullVector);
    ...
    std::string outputString(const std::string &tag, int iret = -99, const Vector<double> &Vec = PRSNullVector);
    ...
    void dumpSolution(std::ostream &os, std::string msg = "PRS")
    {
        try
        {
            ...
            os << "APV: " << msg << std::fixed << std::setprecision(3) << " sigma = " << sig << " meters with " << ndof
               << " degrees of freedom.\n";
        }
        catch (Exception &e)
        {
    ...
    void addToMemory(const Vector<double> &Sol, const Matrix<double> &Cov, const Vector<double> &PreFitResid,
                     const Matrix<double> &Partials, const Matrix<double> &invMeasCov)
    {
        was.add(Sol, Cov);
  ...
  private:
    ...
    void markDisallowedGNSS(std::vector<SatID> &sats, const std::vector<SatelliteSystem> &allowed) const;
     ...
     */
    double computeRange(Position &svPos, const Position &rxPos, bool firstIteration) const;
     ...
     *    receiver position.
     */
    std::tuple<bool, double> computeTropDelay(const Position &svPos, const Position &rxPos, TropModel *pTropModel,
     ...
     */
    bool getSatPVT(Xvt &satXvt, const CommonTime &nominalReceive, double pseudorange, NavLibrary &eph, SatID &sat,
     ...
     *    without issues.
     */
    bool singlePointWLSSolution(Vector<double> &dX, Matrix<double> &partials, Matrix<double> &G,
                                ...
                                bool firstIteration, const std::vector<SatelliteSystem> &currGNSS,
    ...
    std::tuple<bool, double, int> iterativeSinglePointWLSSolution(
        Vector<double> &dX, Matrix<double> &partials, Matrix<double> &G, Matrix<double> &covariance,
        Vector<double> &residuals, const Vector<double> &aPrioriSolution, const FilteredConstSats &filteredSats,
        const Matrix<double> &svp, TropModel *pTropModel, const CommonTime &nominalRecieve,
        const std::vector<SatelliteSystem> &currGNSS, const Matrix<double> &weights, double convLimit,
     ...
     *    without issues.
     */
    bool computePartialsAndResiduals(Matrix<double> &partials, Vector<double> &residuals,
                                     const FilteredConstSats &filteredSats, const Matrix<double> &svp,
                                     const Vector<double> &aPrioriSolution, bool firstIteration, TropModel *pTropModel,
     ...
     */
    Matrix<double> filterWeights(const Matrix<double> &weights, const FilteredConstSats &filteredSats) const;
     ...
     *    weighted least squares solution.
     */
    void computeSlopes(Vector<double> &slopes, const Matrix<double> &partials, const Matrix<double> &G,
     ...
     */
    Matrix<double> createSVP(std::vector<SatID> &sats, const CommonTime &time, const std::vector<double> &pseudorange,
                             NavLibrary &eph, NavSearchOrder order) const;
     ...
     *    sats.
     */
    std::vector<SatelliteSystem> filterGNSS(const std::vector<SatelliteSystem> &allowedGNSS,
                                            const FilteredConstSats &filteredSats) const;
