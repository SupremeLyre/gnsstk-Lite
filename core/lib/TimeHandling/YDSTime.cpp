...
std::string YDSTime::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "j", "ju", doy);
        rv = formattedPrint(rv, getFormatPrefixFloat() + "s", "sf", sod);
...
std::string YDSTime::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "y", "ys", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixInt() + "j", "js", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixFloat() + "s", "ss", getError().c_str());
...
bool YDSTime::operator==(const YDSTime &right) const
{
    ...
    if (year == right.year && doy == right.doy && fabs(sod - right.sod) < CommonTime::eps)
    {
        return true;
    }
    return false;
