...
#define timeSystemCheck                                                                                                \
    if (timeSystem != right.timeSystem && timeSystem != TimeSystem::Any && right.timeSystem != TimeSystem::Any)        \
    GNSSTK_THROW(InvalidRequest("CommonTime objects not in same time system, cannot be compared"))
...
class Week : public TimeTag
{
  public:
    ...
    /// Constructor.
    Week(int w = 0, TimeSystem ts = TimeSystem::Unknown) : week(w)
    {
        timeSystem = ts;
    }
    ...
    inline virtual void getEpochModWeek(unsigned int &e, unsigned int &w) const
    {
        e = getEpoch();
        w = getModWeek();
    }
    ...
    inline virtual void setEpochModWeek(unsigned int e, unsigned int w)
    {
        setEpoch(e);
        setModWeek(w);
    }
