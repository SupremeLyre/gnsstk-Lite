...
// place.
static const char *weekdayAbbr[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
...
void GPSWeekZcount::convertFromCommonTime(const CommonTime &ct)
{
    ...
    zcount = static_cast<long>(day * ZCOUNT_PER_DAY) + static_cast<long>(static_cast<double>(sod + fsod) / 1.5);
}
...
std::string GPSWeekZcount::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "a", "as", weekdayAbbr[getDayOfWeek()]);
        rv = formattedPrint(rv, getFormatPrefixInt() + "A", "As", weekday[getDayOfWeek()]);
        rv = formattedPrint(rv, getFormatPrefixInt() + "w", "wu", getDayOfWeek());
        rv = formattedPrint(rv, getFormatPrefixInt() + "z", "zu", zcount);
        rv = formattedPrint(rv, getFormatPrefixInt() + "Z", "Zu", zcount);
        rv = formattedPrint(rv, getFormatPrefixInt() + "c", "cu", getZcount29());
        rv = formattedPrint(rv, getFormatPrefixInt() + "C", "Cu", getZcount32());
...
std::string GPSWeekZcount::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "w", "ws", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixInt() + "z", "zs", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixInt() + "Z", "Zs", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixInt() + "c", "cs", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixInt() + "C", "Cs", getError().c_str());
...
GPSWeekZcount &GPSWeekZcount::addZcounts(long inZcounts)
{
    ...
    catch (gnsstk::InvalidRequest &ir)
    {
        ...
        ir.addText("Did not add " + StringUtils::asString(inZcounts) + " Z-counts.");
        GNSSTK_RETHROW(ir);
    }
...
long GPSWeekZcount::operator-(const GPSWeekZcount &right) const noexcept
{
    return (((long(week) - long(right.week)) * ZCOUNT_PER_WEEK) + (long(zcount) - long(right.zcount)));
}
...
bool GPSWeekZcount::inSameTimeBlock(const GPSWeekZcount &other, unsigned long inZcountBlock,
                                    unsigned long inZcountOffset) noexcept
{
    if (inZcountBlock < ZCOUNT_PER_WEEK)
    {
        ...
        if ((week == other.week) &&
            (((zcount - inZcountOffset) / inZcountBlock) == ((other.zcount - inZcountOffset) / inZcountBlock)))
        {
            return true;
        }
    }
