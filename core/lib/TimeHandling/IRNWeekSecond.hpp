...
#ifndef GNSSTK_IRNWEEKSECOND_HPP
#define GNSSTK_IRNWEEKSECOND_HPP
...
class IRNWeekSecond : public WeekSecond
{
  public:
    /// Constructor.
    IRNWeekSecond(unsigned int w = 0, double s = 0., TimeSystem ts = TimeSystem::IRN) : WeekSecond(w, s)
    {
        timeSystem = ts;
    }
    ...
    virtual std::string printf(const std::string &fmt) const
    {
        try
        {
            ...
            std::string rv = fmt;
            rv = formattedPrint(rv, getFormatPrefixInt() + "X", "Xu", getEpoch());
            rv = formattedPrint(rv, getFormatPrefixInt() + "O", "Ou", week);
            rv = formattedPrint(rv, getFormatPrefixInt() + "o", "ou", getModWeek());
    ...
    virtual std::string printError(const std::string &fmt) const
    {
        try
        {
            ...
            rv = formattedPrint(rv, getFormatPrefixInt() + "X", "Xs", "BadIRNepoch");
            rv = formattedPrint(rv, getFormatPrefixInt() + "O", "Os", "BadIRNfweek");
            rv = formattedPrint(rv, getFormatPrefixInt() + "o", "os", "BadIRNmweek");
            rv = formattedPrint(rv, getFormatPrefixInt() + "w", "ws", "BadIRNdow");
            rv = formattedPrint(rv, getFormatPrefixFloat() + "g", "gs", "BadIRNsow");
            rv = formattedPrint(rv, getFormatPrefixInt() + "P", "Ps", "BadIRNsys");
            return rv;
        }
        catch (gnsstk::StringUtils::StringException &e)
        {
...
#endif // GNSSTK_IRNWEEKSECOND_HPP
