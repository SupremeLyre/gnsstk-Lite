...
CommonTime MJD::convertToCommonTime() const
{
    try
    {
        ...
        CommonTime ct;
        return ct.set(jday, static_cast<long>(dTmp), dTmp - static_cast<long>(dTmp), timeSystem);
    }
...
void MJD::convertFromCommonTime(const CommonTime &ct)
{
    ...
    mjd = static_cast<long double>(jday - MJD_JDAY) +
          (static_cast<long double>(sod) + static_cast<long double>(fsod)) * DAY_PER_SEC;
}
...
std::string MJD::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixFloat() + "Q", "QLf", mjd);
...
std::string MJD::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixFloat() + "Q", "Qs", getError().c_str());
