...
class YDSTime : public TimeTag
{
  public:
     ...
     */
    YDSTime(long y = 0, long d = 0, double s = 0., TimeSystem ts = TimeSystem::Unknown) : year(y), doy(d), sod(s)
    {
        timeSystem = ts;
    }
     ...
     */
    YDSTime(const YDSTime &right) : year(right.year), doy(right.doy), sod(right.sod)
    {
