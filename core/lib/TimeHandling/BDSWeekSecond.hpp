...
class BDSWeekSecond : public WeekSecond
{
  public:
    /// Constructor.
    BDSWeekSecond(unsigned int w = 0, double s = 0., TimeSystem ts = TimeSystem::BDT) : WeekSecond(w, s)
    {
        timeSystem = ts;
    }
    ...
    virtual std::string printf(const std::string &fmt) const
    {
        try
        {
            ...
            std::string rv = fmt;
            rv = formattedPrint(rv, getFormatPrefixInt() + "R", "Ru", getEpoch());
            rv = formattedPrint(rv, getFormatPrefixInt() + "D", "Du", week);
            rv = formattedPrint(rv, getFormatPrefixInt() + "e", "eu", getModWeek());
    ...
    virtual std::string printError(const std::string &fmt) const
    {
        try
        {
            ...
            rv = formattedPrint(rv, getFormatPrefixInt() + "R", "Rs", "BadBDSepoch");
            rv = formattedPrint(rv, getFormatPrefixInt() + "D", "Ds", "BadBDSfweek");
            rv = formattedPrint(rv, getFormatPrefixInt() + "e", "es", "BadBDSmweek");
            rv = formattedPrint(rv, getFormatPrefixInt() + "w", "wu", "BadBDSdow");
            rv = formattedPrint(rv, getFormatPrefixFloat() + "g", "gf", "BadBDSsow");
            rv = formattedPrint(rv, getFormatPrefixInt() + "P", "Ps", "BadBDSsys");
            return rv;
        }
        catch (gnsstk::StringUtils::StringException &e)
        {
