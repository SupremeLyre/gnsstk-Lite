...
namespace gnsstk
{
void TimeTag::scanf(const std::string &str, const std::string &fmt)
{
    try
    {
...
void TimeTag::getInfo(const std::string &str, const std::string &fmt, IdToValue &info)
{
    ...
    if (!f.empty())
    {
        gnsstk::StringUtils::StringException exc("Failed to process time string");
        GNSSTK_THROW(exc);
    }
}

bool TimeTag ::changeTimeSystem(TimeSystem timeSys, TimeSystemConverter *conv)
{
...
bool TimeTag ::changeTimeSystem(TimeSystem timeSys)
{
    // We have to add an offset and the only way to do that is to
    // change to CommonTime and back.
    CommonTime ct = convertToCommonTime();
...
void TimeTag ::checkTimeSystem(TimeSystem ts1, TimeSystem ts2)
{
    if ((ts1 != TimeSystem::Any) && (ts2 != TimeSystem::Any) && (ts1 != ts2))
    {
