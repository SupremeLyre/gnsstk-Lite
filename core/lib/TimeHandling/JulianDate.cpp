...
CommonTime JulianDate::convertToCommonTime() const
{
    try
    {
        ...
        long double sod = (temp_jd - static_cast<long double>(jday)) * SEC_PER_DAY;

        CommonTime ct;
        return ct.set(jday, static_cast<long>(sod), static_cast<double>(sod - static_cast<long>(sod)), timeSystem);
    }
...
void JulianDate::convertFromCommonTime(const CommonTime &ct)
{
    ...
    jd = static_cast<long double>(jday) +
         (static_cast<long double>(sod) + static_cast<long double>(fsod)) * DAY_PER_SEC - 0.5;
}
...
std::string JulianDate::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixFloat() + "J", "JLf", jd);
...
std::string JulianDate::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixFloat() + "J", "Js", getError().c_str());
