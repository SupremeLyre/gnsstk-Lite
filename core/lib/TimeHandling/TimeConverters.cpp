...
void convertJDtoCalendar(long jd, int &iyear, int &imonth, int &iday)
{
    ...
    if (iyear > 1599 && !(iyear % 100) && (iyear % 400) && imonth == 2 && iday == 29)
    {
        imonth = 3;
        iday = 1;
    }
...
long convertCalendarToJD(int yy, int mm, int dd)
{
    ...
    {
        jd = 1729777 + dd + 367 * yy - static_cast<long>(7 * (y + 5001 + static_cast<long>((m - 9) / 7)) / 4) +
             static_cast<long>(275 * m / 9);
    }
    else // after Oct 4, 1582
    {
        jd = 1721029 + dd + 367 * yy - static_cast<long>(7 * (y + static_cast<long>((m + 9) / 12)) / 4) -
             static_cast<long>(3 * (static_cast<long>((y + (m - 9) / 7) / 100) + 1) / 4) +
             static_cast<long>(275 * m / 9);

        // catch century/non-400 non-leap years
        if ((!(yy % 100) && (yy % 400) && mm > 2 && mm < 9) || (!((yy - 1) % 100) && ((yy - 1) % 400) && mm == 1))
        {
            --jd;
        }
    }
