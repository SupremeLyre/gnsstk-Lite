...
const CommonTime CommonTime::BEGINNING_OF_TIME(CommonTime::BEGIN_LIMIT_JDAY, 0, 0.0, TimeSystem::Any);
// latest representable CommonTime
const CommonTime CommonTime::END_OF_TIME(CommonTime::END_LIMIT_JDAY, 0, 0.0, TimeSystem::Any);

const double CommonTime::eps = 4. * std::numeric_limits<double>::epsilon();

CommonTime::CommonTime(const CommonTime &right)
    : m_day(right.m_day), m_msod(right.m_msod), m_fsod(right.m_fsod), m_timeSystem(right.m_timeSystem)
{
}
...
CommonTime &CommonTime::set(long day, long sod, double fsod, TimeSystem timeSystem)
{
    ...
    {
        InvalidParameter ip("Invalid day: " + StringUtils::asString(day));
        GNSSTK_THROW(ip);
    }

    if (sod < 0 || sod >= SEC_PER_DAY)
    {
        InvalidParameter ip("Invalid seconds of day: " + StringUtils::asString(sod));
        GNSSTK_THROW(ip);
    }

    if (fsod < 0.0 || fsod >= 1)
    {
        InvalidParameter ip("Invalid fractional-seconds: " + StringUtils::asString(fsod));
        GNSSTK_THROW(ip);
    }
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
CommonTime &CommonTime::set(long day, double sod, TimeSystem timeSystem)
{
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
CommonTime &CommonTime::set(double day, TimeSystem timeSystem)
{
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
CommonTime &CommonTime::setInternal(long day, long msod, double fsod, TimeSystem timeSystem)
{
    if (day < BEGIN_LIMIT_JDAY || day > END_LIMIT_JDAY)
    {
        InvalidParameter ip("Invalid day: " + StringUtils::asString(day));
    ...
    {
        InvalidParameter ip("Invalid milliseconds of day: " + StringUtils::asString(msod));
        GNSSTK_THROW(ip);
    }

    if (fsod < 0.0 || fsod >= SEC_PER_MS)
    {
        InvalidParameter ip("Invalid fractional-milliseconds: " + StringUtils::asString(fsod));
        GNSSTK_THROW(ip);
    }
...
bool CommonTime ::changeTimeSystem(TimeSystem timeSystem, TimeSystemConverter *conv)
{
    double offs;
...
bool CommonTime ::changeTimeSystem(TimeSystem timeSystem)
{
    if (!tsConv)
        return false;
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
void CommonTime::get(long &day, long &sod, double &fsod, TimeSystem &timeSystem) const
{
    day = m_day;
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
void CommonTime::get(long &day, long &sod, double &fsod) const
{
    TimeSystem ts;
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
void CommonTime::get(long &day, double &sod, TimeSystem &timeSystem) const
{
    day = m_day;
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
void CommonTime::get(long &day, double &sod) const
{
    TimeSystem ts;
...
// METHOD SET FOR FUTURE DEPRECATION (PRIVATIZATION)
void CommonTime::get(double &day, TimeSystem &timeSystem) const
{
    // convert everything to days
    day = static_cast<double>(m_day) + static_cast<double>(m_msod) / MS_PER_DAY + m_fsod / SEC_PER_DAY;
    timeSystem = m_timeSystem;
}
...
double CommonTime::operator-(const CommonTime &right) const
{
    ...
    // same time systems
    if ((m_timeSystem != TimeSystem::Any && right.m_timeSystem != TimeSystem::Any) &&
        m_timeSystem != right.m_timeSystem)
    {
    ...
    return (SEC_PER_DAY * static_cast<double>(m_day - right.m_day) +
            SEC_PER_MS * static_cast<double>(m_msod - right.m_msod) + m_fsod - right.m_fsod);
}
...
bool CommonTime::operator==(const CommonTime &right) const
{
    ...
    return (m_day == right.m_day && m_msod == right.m_msod && fabs(m_fsod - right.m_fsod) < eps);
}
...
std::string CommonTime::asString() const
{
    using namespace std;
    ostringstream oss;
    oss << setfill('0') << setw(7) << m_day << " " << setw(8) << m_msod << " " << fixed << setprecision(15) << setw(17)
        << m_fsod << " " << gnsstk::StringUtils::asString(m_timeSystem);
    return oss.str();
}

bool CommonTime::add(long days, long msod, double fsod)
{
    m_day += days;
    m_msod += msod;
    m_fsod += fsod;
...
bool CommonTime::normalize()
{
    ...
    return ((m_day >= BEGIN_LIMIT_JDAY) && (m_day < END_LIMIT_JDAY));
}
