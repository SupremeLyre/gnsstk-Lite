...
void scanTime(CommonTime &t, const string &str, const string &fmt)
{
    try
    {
        ...
        bool hmjd(false), hsow(false), hweek(false), hfullweek(false), hdow(false), hyear(false), hmonth(false),
            hday(false), hzcount(false), hdoy(false), hzcount29(false), hzcount32(false), hhour(false), hmin(false),
            hsec(false), hsod(false), hunixsec(false), hunixusec(false), hepoch(false), hansi(false), hjulian(false),
            hbdsw(false), hqzsw(false), hgalw(false), hirnw(false), hbdsfw(false), hqzsfw(false), hgalfw(false),
            hirnfw(false), hbdse(false), hqzse(false), hgale(false), hirne(false), hposixsec(false), hposixnsec(false);

        // These are to hold data that no one parses.
        int idow(0);
        TimeSystem ts;

        for (TimeTag::IdToValue::iterator itr = info.begin(); itr != info.end(); itr++)
        {
            switch (itr->first)
            {
            case 'P':
            ...
            case 'A': {
                ...
                lowerCase(thisDay);
                if (isLike(thisDay, "sun.*"))
                    idow = 0;
                else if (isLike(thisDay, "mon.*"))
                    idow = 1;
                else if (isLike(thisDay, "tue.*"))
                    idow = 2;
                else if (isLike(thisDay, "wed.*"))
                    idow = 3;
                else if (isLike(thisDay, "thu.*"))
                    idow = 4;
                else if (isLike(thisDay, "fri.*"))
                    idow = 5;
                else if (isLike(thisDay, "sat.*"))
                    idow = 6;
                else
                {
                    hdow = false;
            ...
            case 'D':
                hfullweek = hbdsfw = true;
                break;
            case 'e':
                hweek = hbdsw = true;
                break;
            case 'L':
                hfullweek = hgalfw = true;
                break;
            case 'l':
                hweek = hgalw = true;
                break;
            case 'h':
                hfullweek = hqzsfw = true;
                break;
            case 'i':
                hweek = hqzsw = true;
                break;
            case 'O':
                hfullweek = hirnfw = true;
                break;
            case 'o':
                hweek = hirnw = true;
                break;
        ...
        if (hyear)
        {
            if (hmonth && hday)
            {
                ...
                if (hsod)
                {
                    convertSODtoTime(asDouble(info['s']), tt.hour, tt.minute, tt.second);
                }
            ...
            else // use YDSTime as default
            {
                ...
                if (hhour && hmin && hsec)
                {
                    tt.sod = convertTimeToSOD(asInt(info['H']), asInt(info['M']), asDouble(info['S']));
                }
                t = tt.convertToCommonTime();
                return;
            }
        ...
        if (hzcount32 || (hfullweek && (hzcount || hzcount29)) || (hepoch && (hzcount29 || (hweek && hzcount))))
        {
            GPSWeekZcount tt;
            tt.setFromInfo(info);
            t = tt.convertToCommonTime();
        ...
        {
            WeekSecond *ptt;
            if (hbdse || hbdsfw || hbdsw)
                ptt = new BDSWeekSecond();
            else if (hqzse || hqzsfw || hqzsw)
                ptt = new QZSWeekSecond();
            else if (hgale || hgalfw || hgalw)
                ptt = new GALWeekSecond();
            else if (hirne || hirnfw || hirnw)
                ptt = new IRNWeekSecond();
            else
                ptt = new GPSWeekSecond();
            ...
            if (hdow && !hsow)
            {
                ...
                {
                    ptt->sow += convertTimeToSOD(asInt(info['H']), asInt(info['M']), asDouble(info['S']));
                }
            }
...
void mixedScanTime(CommonTime &t, const string &str, const string &fmt)
{
    try
    {
        ...
        // These indicate which information has been found.
        bool hsow(false), hweek(false), hfullweek(false), hdow(false), hyear(false), hmonth(false), hday(false),
            hzcount(false), hdoy(false), hzcount29(false), hhour(false), hmin(false), hsec(false), hsod(false),
            hepoch(false),
            // hunixsec( false ), hunixusec( false ),
            hbdsw(false), hqzsw(false), hgalw(false), hirnw(false), hbdsfw(false), hqzsfw(false), hgalfw(false),
            hirnfw(false), hbdse(false), hqzse(false), hgale(false), hirne(false);
        ...
        int iweek, ifullweek, idow, iyear, imonth, iday, izcount, idoy, izcount29, ihour, imin, iepoch;
        TimeSystem ts;
        ...
        for (TimeTag::IdToValue::iterator itr = info.begin(); itr != info.end(); itr++)
        {
            ...
            case 'b':
                ...
                if (imonth < 1)
                {
                    ...
                    Exception exc("Invalid month abbreviation: " + itr->second);
                    GNSSTK_THROW(exc);
                }
                break;
            ...
            case 'B':
                ...
                {
                    ///@todo use a more appropriate exception class
                    Exception exc("Invalid month name: " + itr->second);
                    GNSSTK_THROW(exc);
                }
                break;
            ...
            case 'y':
                ...
                {
                    ///@todo use a more appropriate exception class
                    Exception exc("Invalid format for %y: expected 2 digits");
                    GNSSTK_THROW(exc);
                }
                iyear = asInt(itr->second);
                if (iyear >= 69)
        ...
        if (hepoch)
        {
            WeekSecond *ptt;
            if (hbds)
                ptt = new BDSWeekSecond(ct);
            else if (hqzs)
                ptt = new QZSWeekSecond(ct);
            else if (hgal)
                ptt = new GALWeekSecond(ct);
            else if (hirn)
                ptt = new IRNWeekSecond(ct);
            else
                ptt = new GPSWeekSecond(ct);
            ptt->setEpoch(iepoch);
