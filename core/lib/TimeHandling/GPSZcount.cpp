...
GPSZcount::GPSZcount(short inWeek, long inZcount)
{
    try
    {
        setWeek(inWeek);
...
GPSZcount &GPSZcount::setWeek(short inWeek)
{
    if (inWeek < 0)
    {
        gnsstk::InvalidParameter ip("GPS Week invalid: " + asString<short>(inWeek));
        GNSSTK_THROW(ip);
    }
    week = inWeek;
    return *this;
...
GPSZcount &GPSZcount::setZcount(long inZcount)
{
    ...
    {
        gnsstk::InvalidParameter ip("GPS Z-count invalid: " + asString<long>(inZcount));
        GNSSTK_THROW(ip);
    }
    zcount = inZcount;
    return *this;
...
GPSZcount &GPSZcount::addWeeks(short inWeeks)
{
    ...
    catch (gnsstk::InvalidParameter &ip)
    {
        ...
        ir.addText("Addition of " + asString(inWeeks) + " weeks renders this object invalid.");
        GNSSTK_THROW(ir);
    }
}
...
{
    return (double(week) - right.week) * ZCOUNT_WEEK + (zcount - right.zcount);
}
...
bool GPSZcount::operator<(const GPSZcount &right) const
{
    ...
    if (week == right.week && zcount < right.zcount)
    {
        return true;
    }
    return false;
...
bool GPSZcount::inSameTimeBlock(const GPSZcount &other, unsigned long inZcountBlock, unsigned long inZcountOffset)
{
    if ((long)inZcountBlock < ZCOUNT_WEEK)
    {
        ...
        if ((getWeek() == other.getWeek()) && (((getZcount() - inZcountOffset) / inZcountBlock) ==
...
void GPSZcount::dump(std::ostream &out, short level) const
{
    switch (level)
    {
    case 0:
    ...
    case 1:
    default:
        out << "GPS Full Week: " << setw(6) << week << endl << "GPS Z-count:   " << setw(6) << zcount << endl;
        break;
    }
}
