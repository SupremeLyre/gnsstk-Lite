...
Epoch &Epoch::setTolerance(double tol) noexcept
{
    tolerance = tol;
    return *this;
}
...
Epoch::Epoch(const TimeTag &tt) : tolerance(EPOCH_TOLERANCE)
{
    set(tt);
}
Epoch::Epoch(const CommonTime &ct) noexcept : core(ct), tolerance(EPOCH_TOLERANCE)
{
}
/**
 ...
 */
Epoch::Epoch(const WeekSecond &tt, short year) : tolerance(EPOCH_TOLERANCE)
{
    set(tt, year);
}
...
Epoch::Epoch(const GPSZcount &z) noexcept : tolerance(EPOCH_TOLERANCE)
{
    set(z);
}
...
// Copy constructor
Epoch::Epoch(const Epoch &right) noexcept : core(right.core), tolerance(right.tolerance)
{
}

// Assignment operator.
Epoch &Epoch::operator=(const Epoch &right) noexcept
{
    core = right.core;
...
double Epoch::operator-(const Epoch &right) const noexcept
{
...
// Equality operator.
bool Epoch::operator==(const Epoch &right) const noexcept
{
    // use the smaller of the two tolerances for comparison
    return (ABS(operator-(right)) <= ((tolerance > right.tolerance) ? right.tolerance : tolerance));
}
...
bool Epoch::operator<(const Epoch &right) const noexcept
{
    return (operator-(right) < -((tolerance > right.tolerance) ? right.tolerance : tolerance));
}
...
Epoch::operator CommonTime() const noexcept
{
    return core;
}
...
Epoch &Epoch::set(const CommonTime &c) noexcept
{
    core = c;
    return *this;
}
...
Epoch &Epoch::setLocalTime()
{
    ...
    ltod = localtime(&t);
    return set(
        CivilTime(1900 + ltod->tm_year, ltod->tm_mon + 1, ltod->tm_mday, ltod->tm_hour, ltod->tm_min, ltod->tm_sec));
}

Epoch &Epoch::scanf(const string &str, const string &fmt)
{
    try
    {
