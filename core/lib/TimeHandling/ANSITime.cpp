...
CommonTime ANSITime::convertToCommonTime() const
{
    ...
    {
        CommonTime ct;
        return ct.set((MJD_JDAY + UNIX_MJD + time / SEC_PER_DAY), (time % SEC_PER_DAY), 0., timeSystem);
    }
...
void ANSITime::convertFromCommonTime(const CommonTime &ct)
{
    ...
    time = static_cast<time_t>((jday - MJD_JDAY - UNIX_MJD) * SEC_PER_DAY + sod);
}
...
std::string ANSITime::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "K", "Klu", time);
...
std::string ANSITime::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "K", "Ks", getError().c_str());
