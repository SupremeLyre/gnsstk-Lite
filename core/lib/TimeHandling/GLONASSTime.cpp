...
namespace gnsstk
{
GLONASSTime ::GLONASSTime(unsigned leapCount, unsigned days, double instant, TimeSystem ts)
    : TimeTag(ts), epoch(leapCount), day(days), sod(instant)
{
}

CommonTime GLONASSTime ::convertToCommonTime() const
{
    try
    {
         ...
         * then. */
        double tmp = 1461.0 * (epoch - 1.0) + day + GLO_EPOCH_JD - ((day - 3.0) / 25.0);
        CommonTime ct;
...
void GLONASSTime ::convertFromCommonTime(const CommonTime &ct)
{
    ...
    if (J > 0)
    {
        ...
        unsigned leapDay = (((leapYear % 400) == 0) || ((leapYear % 100) != 0));
...
bool GLONASSTime ::operator==(const GLONASSTime &right) const
{
    TimeTag::checkTimeSystem(timeSystem, right.timeSystem);
    return ((epoch == right.epoch) && (day == right.day) && (fabs(sod - right.sod) < CommonTime::eps));
}
