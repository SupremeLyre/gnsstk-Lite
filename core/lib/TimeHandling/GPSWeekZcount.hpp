...
class GPSWeekZcount : public GPSWeek
{
  public:
     ...
     */
    GPSWeekZcount(int w = 0, int z = 0, TimeSystem ts = TimeSystem::GPS) : GPSWeek(w), zcount(z)
    {
        timeSystem = ts;
    }
     ...
     */
    GPSWeekZcount(const GPSWeekZcount &right) : GPSWeek(right.week), zcount(right.zcount)
    {
     ...
     */
    bool inSameTimeBlock(const GPSWeekZcount &other, unsigned long inZcountBlock,
                         unsigned long inZcountOffset = 0) noexcept;
