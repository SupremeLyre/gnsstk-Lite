...
TimeRange::TimeRange()
    : start(CommonTime::BEGINNING_OF_TIME), end(CommonTime::END_OF_TIME), includeStartTime(true), includeEndTime(true)
{
}

TimeRange::TimeRange(const CommonTime &startDT, const CommonTime &endDT, const bool startInclusive,
...
TimeRange::TimeRange(DTPair dtPair, const bool startInclusive, const bool endInclusive)
{
    try
    {
        init(dtPair.first, dtPair.second, startInclusive, endInclusive);
    }
    catch (TimeRangeException tre)
    {
...
void TimeRange::init(const CommonTime &startDT, const CommonTime &endDT, const bool startInclusive,
                     const bool endInclusive)
{
    if (endDT < startDT)
    {
...
void TimeRange::set(const CommonTime &startDT, const CommonTime &endDT, const bool startInclusive,
                    const bool endInclusive)
{
    try
    {
...
bool TimeRange::inRange(const CommonTime &testDT) const
{
    ...
    if (testDT == start && includeStartTime == true)
        return true;
    if (testDT == end && includeEndTime == true)
        return true;
    return false;
}
...
{
    if (start == right.start && end == right.end && includeStartTime == right.includeStartTime &&
        includeEndTime == right.includeEndTime)
        return true;
    return false;
}
...
bool TimeRange::isPriorTo(const TimeRange &right) const
{
    ...
    if (end == right.start && (!includeEndTime || !right.includeStartTime))
        return true;
...
bool TimeRange::overlaps(const TimeRange &right) const
{
    ...
    if (start < right.end && end > right.start)
        return true;
    ...
    //    [ right ]
    //
    if (includeEndTime && right.includeStartTime && end == right.start)
        return true;

    if (includeStartTime && right.includeEndTime && start == right.end)
        return true;
...
bool TimeRange::isAfter(const TimeRange &right) const
{
    ...
    if (start == right.end && (!includeStartTime || !right.includeEndTime))
        return true;
...
// Formatted input
TimeRange &TimeRange::setToString(const string &str, const string &fmt)
{
