...
CommonTime PosixTime::convertToCommonTime() const
{
    ...
    {
        CommonTime ct;
        return ct.set((MJD_JDAY + UNIX_MJD + ts.tv_sec / SEC_PER_DAY), (ts.tv_sec % SEC_PER_DAY),
                      (static_cast<double>(ts.tv_nsec) * 1e-9), timeSystem);
    }
...
void PosixTime::convertFromCommonTime(const CommonTime &ct)
{
    ...
    static const CommonTime MAX_CT = PosixTime(2147483647, 999999, TimeSystem::Any);
...
std::string PosixTime::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "W", "Wlu", ts.tv_sec);
        rv = formattedPrint(rv, getFormatPrefixInt() + "N", "Nlu", ts.tv_nsec);
...
std::string PosixTime::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "N", "Ns", getError().c_str());
...
bool PosixTime::operator==(const PosixTime &right) const
{
    ...
    if (ts.tv_sec == right.ts.tv_sec && abs(ts.tv_nsec - right.ts.tv_nsec) < CommonTime::eps)
    {
        return true;
    }
    return false;
...
bool PosixTime::operator<(const PosixTime &right) const
{
    ...
    if ((timeSystem != TimeSystem::Any && right.timeSystem != TimeSystem::Any) && timeSystem != right.timeSystem)
    {
        gnsstk::InvalidRequest ir("CommonTime objects not in same time system,"
                                  " cannot be compared");
        GNSSTK_THROW(ir);
    }
    ...
    if (ts.tv_sec == right.ts.tv_sec && ts.tv_nsec < right.ts.tv_nsec)
    {
        return true;
    }
    return false;
