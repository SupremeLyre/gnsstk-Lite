...
class GPSWeek : public TimeTag
{
  public:
    ...
    /// Constructor.
    GPSWeek(int w = 0, TimeSystem ts = TimeSystem::Unknown) : week(w)
    {
        timeSystem = ts;
    }
    ...
    inline virtual void getEpochWeek10(unsigned int &e, unsigned int &w) const
    {
        e = getEpoch();
        w = getWeek10();
    }
    ...
    inline virtual void setEpochWeek10(unsigned int e, unsigned int w)
    {
        setEpoch(e);
        setWeek10(w);
    }
