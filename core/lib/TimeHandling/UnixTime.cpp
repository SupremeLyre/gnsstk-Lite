...
CommonTime UnixTime::convertToCommonTime() const
{
    ...
    {
        CommonTime ct;
        return ct.set((MJD_JDAY + UNIX_MJD + tv.tv_sec / SEC_PER_DAY), (tv.tv_sec % SEC_PER_DAY),
                      (static_cast<double>(tv.tv_usec) * 1e-6), timeSystem);
    }
...
std::string UnixTime::printf(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "U", "Ulu", tv.tv_sec);
        rv = formattedPrint(rv, getFormatPrefixInt() + "u", "ulu", tv.tv_usec);
...
std::string UnixTime::printError(const std::string &fmt) const
{
    try
    {
        ...
        rv = formattedPrint(rv, getFormatPrefixInt() + "U", "Us", getError().c_str());
        rv = formattedPrint(rv, getFormatPrefixInt() + "u", "us", getError().c_str());
...
bool UnixTime::operator==(const UnixTime &right) const
{
    ...
    if (tv.tv_sec == right.tv.tv_sec && abs(tv.tv_usec - right.tv.tv_usec) < CommonTime::eps)
    {
        return true;
    }
    return false;
...
bool UnixTime::operator<(const UnixTime &right) const
{
    ...
    if (tv.tv_sec == right.tv.tv_sec && tv.tv_usec < right.tv.tv_usec)
    {
        return true;
    }
    return false;
