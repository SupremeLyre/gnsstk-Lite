...
namespace gnsstk
{
LNavOrderFilter ::LNavOrderFilter(unsigned depth, unsigned epoch) : procDepth(depth), epochInSeconds(epoch)
{
}

void LNavOrderFilter ::validate(NavMsgList &msgBitsIn, NavMsgList &msgBitsOut)
{
    NavMsgList::const_iterator nmli;
    CommonTime oldestAllowed; // default to super old i.e. any data is ok
    oldestAllowed.setTimeSystem(gnsstk::TimeSystem::Any);
    if (!orderedNav.empty())
    {
...
void LNavOrderFilter ::finalize(NavMsgList &msgBitsOut)
{
    std::copy(orderedNav.begin(), orderedNav.end(), std::back_inserter(msgBitsOut));
    orderedNav.clear();
}

void LNavOrderFilter ::examineSubframes(NavMsgList &msgBitsOut)
{
    using gnsstk::printTime;
    if (orderedNav.empty())
