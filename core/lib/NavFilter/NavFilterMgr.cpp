...
namespace gnsstk
{
NavFilterMgr ::NavFilterMgr()
{
}

void NavFilterMgr ::addFilter(NavFilter *filt)
{
    filters.push_back(filt);
}

NavFilter::NavMsgList NavFilterMgr ::validate(NavFilterKey *msgBits)
{
...
NavFilter::NavMsgList NavFilterMgr ::finalize()
{
    ...
    for (fliCur = filters.begin(); fliCur != filters.end(); fliCur++)
    {
        ...
        if (!rv2.empty())
        {
            ...
            if (!rv1.empty())
            {
                std::copy(rv1.begin(), rv1.end(), std::back_insert_iterator<NavFilter::NavMsgList>(rv));
            }
        }
    }
    return rv;
...
unsigned NavFilterMgr ::processingDepth() const noexcept
{
