...
template <std::size_t WordSize = 30> class NavMsgDataWords : public NavMsgData
{
  public:
...
template <std::size_t WordSize> uint32_t NavMsgDataWords<WordSize>::getBits(unsigned start, unsigned num) const
{
    if (num == 0)
        return 0;
    // mask and shift the desired bits
    uint32_t fbmodN = (start % WordSize);
    return ((sf[start / WordSize] & (((uint32_t)-1) >> (fbmodN + 1))) >> (31 - fbmodN - num));
}

template <std::size_t WordSize> void NavMsgDataWords<WordSize>::dump(std::ostream &s, unsigned totalBits) const
{
