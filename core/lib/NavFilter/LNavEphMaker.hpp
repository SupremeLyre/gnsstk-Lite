 ...
 * rx, prn, carrier, code. */
struct LNavEphMakerSort : std::binary_function<LNavFilterData *, LNavFilterData *, bool>
{
    inline bool operator()(const LNavFilterData *const &l, const LNavFilterData *const &r) const;
};
...
bool LNavEphMakerSort ::operator()(const LNavFilterData *const &l, const LNavFilterData *const &r) const
{
    if (l->stationID < r->stationID)
        return true;
    if (l->stationID > r->stationID)
        return false;
    if (l->rxID < r->rxID)
