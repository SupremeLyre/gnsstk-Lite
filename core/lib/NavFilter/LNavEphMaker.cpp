...
namespace gnsstk
{
LNavEphMaker ::LNavEphMaker()
{
}

void LNavEphMaker ::validate(NavMsgList &msgBitsIn, NavMsgList &msgBitsOut)
{
    ...
    for (nmli = msgBitsIn.begin(); nmli != msgBitsIn.end(); nmli++)
    {
        ...
        if ((EngNav::getTOW(ephGrp[1]->sf[1]) - EngNav::getTOW(ephGrp[0]->sf[1]) != 1) ||
            (EngNav::getTOW(ephGrp[2]->sf[1]) - EngNav::getTOW(ephGrp[1]->sf[1]) != 1))
        {
            continue;
        }
...
void LNavEphMaker ::finalize(NavMsgList &msgBitsOut)
{
