...
namespace gnsstk
{
NavOrderFilter ::NavOrderFilter(unsigned depth, unsigned epoch) : procDepth(depth), epochInSeconds(epoch)
{
}

void NavOrderFilter ::validate(NavMsgList &msgBitsIn, NavMsgList &msgBitsOut)
{
...
void NavOrderFilter ::finalize(NavMsgList &msgBitsOut)
{
    std::copy(orderedNav.begin(), orderedNav.end(), std::back_inserter(msgBitsOut));
    orderedNav.clear();
}

void NavOrderFilter ::examineSubframes(NavMsgList &msgBitsOut)
{
