...
namespace gnsstk
{
GalINavISC ::GalINavISC()
    : bgdE1E5a(std::numeric_limits<double>::quiet_NaN()), bgdE1E5b(std::numeric_limits<double>::quiet_NaN())
{
    weekFmt = "%4L(%4l)";
    msgLenSec = 2.0;
}

void GalINavISC ::dumpCorrections(std::ostream &s) const
{
    const ios::fmtflags oldFlags = s.flags();
    s << "           CORRECTION" << endl
      << endl
      << scientific << setprecision(8) << setfill(' ') << setw(20) << left << "BGD(E1,E5a):" << setw(15) << bgdE1E5a
      << endl
      << setw(20) << left << "BGD(E1,E5b):" << setw(15) << bgdE1E5b << endl;
    s.flags(oldFlags);
}

bool GalINavISC ::validate() const
{
    return !(std::isnan(bgdE1E5a) || std::isnan(bgdE1E5b));
}

bool GalINavISC ::getISC(const ObsID &oid, double &corrOut) const
{
    if ((oid.band == CarrierBand::L1) && // same as E1
        ((oid.code == TrackingCode::E1B) || (oid.code == TrackingCode::E1C)))
    {
    ...
    else if ((oid.band == CarrierBand::E5b) && ((oid.code == TrackingCode::E5bI) || (oid.code == TrackingCode::E5bQ)))
    {
        if (std::isnan(bgdE1E5b))
            return false;
