...
namespace gnsstk
{
GalINavHealth ::GalINavHealth()
    : sigHealthStatus(GalHealthStatus::Unknown), dataValidityStatus(GalDataValid::Unknown), sisaIndex(255)
{
...
void GalINavHealth ::dump(std::ostream &s, DumpDetail dl) const
{
    ...
    case DumpDetail::Brief:
        ...
        s << "  sigHealthStatus = " << static_cast<unsigned>(sigHealthStatus) << "  "
          ...
          << "  dataValidityStatus = " << static_cast<unsigned>(dataValidityStatus) << "  "
    ...
    case DumpDetail::Full:
        ...
        s << "*************************************************************"
          ...
          << endl
          << "           HEALTH DATA" << endl
          << "SHS Bits           0x" << hex << setw(1) << static_cast<unsigned>(sigHealthStatus) << dec << " ("
          << StringUtils::asString(sigHealthStatus) << ")" << endl
          << "DVS Bits           0x" << hex << setw(1) << static_cast<unsigned>(dataValidityStatus) << dec << " ("
          << StringUtils::asString(dataValidityStatus) << ")" << endl
          << "SISA Index         " << dec << static_cast<unsigned>(sisaIndex) << endl
...
SVHealth GalINavHealth ::getHealth() const
{
    return galHealth(sigHealthStatus, dataValidityStatus, sisaIndex);
}

SVHealth GalINavHealth ::galHealth(GalHealthStatus shs, GalDataValid dvs, uint8_t sisa)
{
