...
GLOFNavAlm ::GLOFNavAlm()
    : healthBits(false), taunA(std::numeric_limits<double>::quiet_NaN()),
      lambdanA(std::numeric_limits<double>::quiet_NaN()), deltainA(std::numeric_limits<double>::quiet_NaN()),
      eccnA(std::numeric_limits<double>::quiet_NaN()), omeganA(std::numeric_limits<double>::quiet_NaN()),
      tLambdanA(std::numeric_limits<double>::quiet_NaN()), deltaTnA(std::numeric_limits<double>::quiet_NaN()),
      deltaTdotnA(std::numeric_limits<double>::quiet_NaN()), freqnA(-1)
{
    signal.messageType = NavMessageType::Almanac;
    msgLenSec = 4.0;
}

bool GLOFNavAlm ::validate() const
{
    /// @todo implement some checking.
    return true;
}

bool GLOFNavAlm ::getXvt(const CommonTime &when, Xvt &xvt, const ObsID &oid)
{
...
CommonTime GLOFNavAlm ::getUserTime() const
{
    CommonTime mr = std::max({timeStamp, xmit2});
    return mr + 2.0;
}

void GLOFNavAlm ::fixFit()
{
...
void GLOFNavAlm ::dump(std::ostream &s, DumpDetail dl) const
{
    ...
    s << "**************************************************************" << endl
      << " GLONASS ORB/CLK (NON-IMMEDIATE) PARAMETERS" << endl
      << endl
    ...
    s << endl
      ...
      << "Transmit   :  " << printTime(xmit, tform) << endl
      << "Orbit Epoch:  " << printTime(Toa, tform) << endl
      << endl
    ...
    s << endl
      << "M           " << setw(16) << static_cast<int>(satType) << " encoded: " << StringUtils::asString(satType)
      << endl
      << "H           " << setw(16) << freqnA << " freq. offset" << endl
      << "Transmit SV " << setw(16) << signal.xmitSat << endl
      << "l           " << setw(16) << lhealth << " Health of transmitting SV" << endl;

    s.flags(oldFlags);
}

void GLOFNavAlm ::dumpTerse(std::ostream &s) const
{
...
void GLOFNavAlm ::setSemiMajorAxisIncl()
{
    DEBUGTRACE_FUNCTION();
    math.setSemiMajorAxisIncl(deltaTnA, deltainA, omeganA, eccnA);
}

GLOFNavAlm::NumberCruncher ::NumberCruncher()
    : lambdaBar(std::numeric_limits<double>::quiet_NaN()), tau(std::numeric_limits<double>::quiet_NaN()),
      nu(std::numeric_limits<double>::quiet_NaN()), Tdeltap(std::numeric_limits<double>::quiet_NaN()),
      n(std::numeric_limits<double>::quiet_NaN()), a(std::numeric_limits<double>::quiet_NaN()),
      i(std::numeric_limits<double>::quiet_NaN()), coslambdaBar(std::numeric_limits<double>::quiet_NaN()),
      sinlambdaBar(std::numeric_limits<double>::quiet_NaN()), cos2lambdaBar(std::numeric_limits<double>::quiet_NaN()),
      sin2lambdaBar(std::numeric_limits<double>::quiet_NaN()), cos3lambdaBar(std::numeric_limits<double>::quiet_NaN()),
      sin3lambdaBar(std::numeric_limits<double>::quiet_NaN()), cos4lambdaBar(std::numeric_limits<double>::quiet_NaN()),
      sin4lambdaBar(std::numeric_limits<double>::quiet_NaN()), earthvs(std::numeric_limits<double>::quiet_NaN()),
      sini(std::numeric_limits<double>::quiet_NaN()), sini2(std::numeric_limits<double>::quiet_NaN()),
      cosi(std::numeric_limits<double>::quiet_NaN()), cosi2(std::numeric_limits<double>::quiet_NaN()),
      JTerm(std::numeric_limits<double>::quiet_NaN()), JsinTerm(std::numeric_limits<double>::quiet_NaN()),
      Jsini2Term(std::numeric_limits<double>::quiet_NaN()), Jcosi2Term(std::numeric_limits<double>::quiet_NaN()),
      h(std::numeric_limits<double>::quiet_NaN()), l(std::numeric_limits<double>::quiet_NaN()),
      ecc2(std::numeric_limits<double>::quiet_NaN()), ecc2obv(std::numeric_limits<double>::quiet_NaN())
{
}

void GLOFNavAlm::NumberCruncher ::setEccArgOfPerigee(double ecc, double omega)
{
    DEBUGTRACE_FUNCTION();
    h = ecc * std::sin(omega);
    l = ecc * std::cos(omega);
...
void GLOFNavAlm::NumberCruncher ::setSemiMajorAxisIncl(double deltaT, double deltai, double omega, double ecc)
{
    DEBUGTRACE_FUNCTION();
    ...
    DEBUGTRACE("bigTerm = " << siniTerm << " * (" << eTerm << " / " << omegaTerm << ") + (" << nuTerm << " / "
                            << ecc2obv << ")");
...
void GLOFNavAlm::NumberCruncher ::setLambdaBar(double M, double omega, double tau, double n)
{
    DEBUGTRACE_FUNCTION();
...
void GLOFNavAlm::NumberCruncher ::setDeltas(double M, double omega, double a, double dt)
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("M = " << M);
    DEBUGTRACE("omega = " << omega);
...
double GLOFNavAlm::NumberCruncher ::getomegai(double hi, double li, double epsi)
{
    DEBUGTRACE_FUNCTION();
    if (epsi == 0)
    {
...
double GLOFNavAlm::NumberCruncher ::integrateEin(double Mi, double epsi)
{
    DEBUGTRACE_FUNCTION();
...
bool GLOFNavAlm::NumberCruncher ::getXvt(const CommonTime &when, Xvt &xvt, const GLOFNavAlm &alm)
{
    DEBUGTRACE_FUNCTION();
    ...
    DEBUGTRACE("ecc2obv = " << ecc2obv);
    ///////////////////////////////////////////////////////////////////////
    double tanEdiv2 = std::sqrt((1.0 - alm.eccnA) / (1.0 + alm.eccnA)) * std::tan(nu / 2.0);
    ...
    xvt.v[0] =
        Vri * (cosui * cosOMEGAi - sinui * sinOMEGAi * cosii) - Vui * (sinui * cosOMEGAi + cosui * sinOMEGAi * cosii);
    xvt.v[1] =
        Vri * (cosui * sinOMEGAi + sinui * cosOMEGAi * cosii) - Vui * (sinui * sinOMEGAi - cosui * cosOMEGAi * cosii);
    /* debugging code
...
GLOFNavAlm::NumberCruncher::Deltas ::Deltas()
    : deltaa(std::numeric_limits<double>::quiet_NaN()), deltah(std::numeric_limits<double>::quiet_NaN()),
      deltal(std::numeric_limits<double>::quiet_NaN()), deltaOMEGA(std::numeric_limits<double>::quiet_NaN()),
      deltai(std::numeric_limits<double>::quiet_NaN()), deltalambdaBar(std::numeric_limits<double>::quiet_NaN())
{
}

GLOFNavAlm::NumberCruncher::Deltas GLOFNavAlm::NumberCruncher::Deltas ::operator-(const Deltas &right) const
{
    Deltas rv;
...
void GLOFNavAlm::NumberCruncher::Deltas ::setDeltas(const NumberCruncher &math, double a, double tau, double n)
{
    DEBUGTRACE_FUNCTION();
    ...
    double a_a =
        2.0 * math.JsinTerm * (math.l * math.coslambdaBar + math.h * math.sinlambdaBar) +
        math.Jsini2Term * (OH * math.h * math.sinlambdaBar - OH * math.l * math.coslambdaBar + math.cos2lambdaBar +
                           SH * math.l * math.cos3lambdaBar + SH * math.h * math.sin3lambdaBar);
    // The ICD defines the abbove expression as delta a(m) / a,
    // so multiply the whole thing by a to get delta a(m)
    deltaa = a_a * a;
    deltah = math.JsinTerm * (math.l * n * tau + math.sinlambdaBar + TH * math.l * math.sin2lambdaBar -
                              TH * math.h * math.cos2lambdaBar) -
             OQ * math.Jsini2Term *
                 (math.sinlambdaBar - ST * math.sin3lambdaBar + 5.0 * math.l * math.sin2lambdaBar -
                  STH * math.l * math.sin4lambdaBar + STH * math.h * math.cos4lambdaBar + math.h * math.cos2lambdaBar) +
             math.Jcosi2Term * (math.l * n * tau - OH * math.l * math.sin2lambdaBar);
    deltal = math.JsinTerm * (-math.h * n * tau + math.coslambdaBar + TH * math.l * math.cos2lambdaBar +
                              TH * math.h * math.sin2lambdaBar) -
             OQ * math.Jsini2Term *
                 (-math.coslambdaBar - ST * math.cos3lambdaBar - 5.0 * math.h * math.sin2lambdaBar -
                  STH * math.l * math.cos4lambdaBar - STH * math.h * math.sin4lambdaBar + math.l * math.cos2lambdaBar) +
             math.Jcosi2Term * (-math.h * n * tau + OH * math.h * math.sin2lambdaBar);
    deltaOMEGA = -math.JTerm * math.cosi *
                 (n * tau + SH * math.l * math.sinlambdaBar - FH * math.h * math.coslambdaBar -
                  OH * math.sin2lambdaBar - SS * math.l * math.sin3lambdaBar + SS * math.h * math.cos3lambdaBar);
    deltai = OH * math.JTerm * math.sini * math.cosi *
             (-math.l * math.coslambdaBar + math.h * math.sinlambdaBar + math.cos2lambdaBar +
              ST * math.l * math.cos3lambdaBar + ST * math.h * math.sin3lambdaBar);
    deltalambdaBar =
        (2.0 * math.JsinTerm * (n * tau + SQ * math.l * math.sinlambdaBar - SQ * math.h * math.coslambdaBar)) +
        (3.0 * math.Jsini2Term *
         (-STF * math.h * math.coslambdaBar - STF * math.l * math.sinlambdaBar - FNSS * math.h * math.cos3lambdaBar +
          FNSS * math.l * math.sin3lambdaBar + OQ * math.sin2lambdaBar)) +
        (math.Jcosi2Term *
         (n * tau + SH * math.l * math.sinlambdaBar - FH * math.h * math.coslambdaBar - OH * math.sin2lambdaBar -
          SS * math.l * math.sin3lambdaBar + SS * math.h * math.cos3lambdaBar));
