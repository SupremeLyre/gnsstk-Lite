...
namespace gnsstk
{
GPSLNavEph ::GPSLNavEph()
    : pre2(0), pre3(0), tlm2(0), tlm3(0), isf2(false), isf3(false), iodc(0), iode(0), fitIntFlag(0), healthBits(0xff),
      uraIndex(0), tgd(0.0), asFlag2(false), asFlag3(false), alert2(false), alert3(false),
      codesL2(GPSLNavL2Codes::Invalid1), aodo(-1), L2Pdata(false)
{
    signal.messageType = NavMessageType::Ephemeris;
    msgLenSec = 18.0;
}

bool GPSLNavEph ::validate() const
{
    return GPSLNavData::validate() && ((pre2 == 0) || (pre2 == 0x8b)) && ((pre3 == 0) || (pre3 == 0x8b));
}

CommonTime GPSLNavEph ::getUserTime() const
{
    CommonTime mr = std::max({xmitTime, xmit2, xmit3});
    return mr + 6.0;
}

void GPSLNavEph ::fixFit()
{
...
void GPSLNavEph ::dump(std::ostream &s, DumpDetail dl) const
{
...
void GPSLNavEph ::dumpSVStatus(std::ostream &s) const
{
    ...
    s << "           SUBFRAME OVERHEAD" << endl
      << endl
      << "               SOW    DOW:HH:MM:SS     IOD    ALERT   A-S   ISF" << endl
      << "SF1 HOW:   " << gnsstk::printTime(xmitTime + 6, "%7.0g  %3a-%1w:%02H:%02M:%02S") << "   0x" << hex << setw(3)
      << internal << setfill('0') << nouppercase << iodc << dec << setfill(' ') << "      " << noboolalpha << alert
      << "     " << (asFlag ? " on   " : "off   ") << (isf ? "enhanced" : "legacy") << endl
      << "SF2 HOW:   " << gnsstk::printTime(xmit2 + 6, "%7.0g  %3a-%1w:%02H:%02M:%02S") << "    0x" << hex << setw(2)
      << internal << setfill('0') << nouppercase << iode << dec << setfill(' ') << "      " << alert2 << "     "
      << (asFlag2 ? " on   " : "off   ") << (isf2 ? "enhanced" : "legacy") << endl
      << "SF3 HOW:   " << gnsstk::printTime(xmit3 + 6, "%7.0g  %3a-%1w:%02H:%02M:%02S") << "    0x" << hex << setw(2)
      << internal << setfill('0') << nouppercase << iode << dec << setfill(' ') << "      " << alert3 << "     "
      << (asFlag3 ? " on   " : "off   ") << (isf3 ? "enhanced" : "legacy") << endl
      << endl
      << "           SV STATUS" << endl
      << endl
      << "Health bits         :      0x" << setw(2) << internal << setfill('0') << hex << health << dec << ", "
      << setfill(' ') << health << endl
      << "Fit interval flag   :" << setw(10) << (unsigned)fitIntFlag << endl
      << "URA index           :" << setw(10) << (unsigned)uraIndex << endl
      << "Code on L2          :" << setw(10) << StringUtils::asString(codesL2) << endl
      << "L2 P Nav data       :" << "       " << (L2Pdata ? "off" : " on") << endl
      << right << "Tgd                 :" << setw(14) << setprecision(6) << scientific << uppercase << tgd << " sec"
      << endl;
    if (aodo != -1)
    {
...
bool GPSLNavEphIODCComp ::operator()(const std::shared_ptr<GPSLNavEph> lhs, const std::shared_ptr<GPSLNavEph> rhs) const
{
    if (lhs->signal < rhs->signal)
        return true;
    if (rhs->signal < lhs->signal)
        return false;
    GPSWeekSecond lws(lhs->Toe), rws(rhs->Toe);
    if (lws.week < rws.week)
        return true;
    if (rws.week < lws.week)
        return false;
...
bool GPSLNavEphCEIComp ::operator()(const std::shared_ptr<GPSLNavEph> lhs, const std::shared_ptr<GPSLNavEph> rhs) const
{
    ...
    if (lhs->healthBits < rhs->healthBits)
        return true;
    if (rhs->healthBits < lhs->healthBits)
        return false;
    if (lhs->uraIndex < rhs->uraIndex)
        return true;
    if (rhs->uraIndex < lhs->uraIndex)
        return false;
    if (lhs->tgd < rhs->tgd)
        return true;
    if (rhs->tgd < lhs->tgd)
    ...
    if (rhs->Toc < lhs->Toc)
        return false;
    if (lhs->Ahalf < rhs->Ahalf)
        return true;
    if (rhs->Ahalf < lhs->Ahalf)
        return false;
    if (lhs->dn < rhs->dn)
        return true;
    if (rhs->dn < lhs->dn)
        return false;
    if (lhs->fitIntFlag < rhs->fitIntFlag)
        return true;
    if (rhs->fitIntFlag < lhs->fitIntFlag)
        return false;
    if (lhs->ecc < rhs->ecc)
        return true;
    if (rhs->ecc < lhs->ecc)
    ...
    if (lhs->iode < rhs->iode)
        return true;
    if (rhs->iode < lhs->iode)
        return false;
    if (lhs->isf < rhs->isf)
        return true;
    if (rhs->isf < lhs->isf)
        return false;
    if (lhs->isf2 < rhs->isf2)
        return true;
    if (rhs->isf2 < lhs->isf2)
        return false;
    if (lhs->isf3 < rhs->isf3)
        return true;
    if (rhs->isf3 < lhs->isf3)
        return false;
    if (lhs->w < rhs->w)
        return true;
    if (rhs->w < lhs->w)
        return false;
    if (lhs->OMEGAdot < rhs->OMEGAdot)
        return true;
    if (rhs->OMEGAdot < lhs->OMEGAdot)
        return false;
    if (lhs->OMEGA0 < rhs->OMEGA0)
        return true;
    if (rhs->OMEGA0 < lhs->OMEGA0)
        return false;
    if (lhs->i0 < rhs->i0)
        return true;
    if (rhs->i0 < lhs->i0)
    ...
    if (lhs->alert < rhs->alert)
        return true;
    if (rhs->alert < lhs->alert)
        return false;
    if (lhs->alert2 < rhs->alert2)
        return true;
    if (rhs->alert2 < lhs->alert2)
        return false;
    if (lhs->alert3 < rhs->alert3)
        return true;
    if (rhs->alert3 < lhs->alert3)
        return false;
    return false;
}
} // namespace gnsstk
