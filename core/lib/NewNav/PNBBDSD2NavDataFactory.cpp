...
bool PNBBDSD2NavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    if ((navIn->getNavID().navType != NavType::BeiDou_D2) || (navIn->getNumBits() != 300))
    {
    ...
    {
        unsigned long pgid;
        unsigned long sfid = navIn->asUnsignedLong(fsbFraID, fnbFraID, fscFraID);
        // cerr << "sfid=" << sfid;
        unsigned long pnum;
        // double TOA;
        ...
        case 5:
            ...
            else if (((pgid >= 37) && (pgid <= 60)) || ((pgid >= 95) && (pgid <= 100)) ||
                     ((pgid >= 103) && (pgid <= 115)))
            {
...
bool PNBBDSD2NavDataFactory ::processAlm(unsigned pnum, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    if (PNBNavDataFactory::processAlm || PNBNavDataFactory::processHea)
    {
        ...
        {
            amEpIDMap[key].setValues(navIn->getTransmitTime(), navIn->asUnsignedLong(asbAmEpID, anbAmEpID, ascAmEpID));
        }
    }
    ...
    else if ((pnum >= 103) && (pnum <= 115))
    {
        ...
        case 0x03:
            ...
            break;
        default:
            GNSSTK_THROW(Exception("Invalid AmID value " + StringUtils::asString((unsigned)amID)));
            break;
        }
    }
    ...
    alm->Ahalf = navIn->asUnsignedDouble(asbAhalfm, anbAhalfm, asbAhalfl, anbAhalfl, ascAhalf);
    alm->A = alm->Ahalf * alm->Ahalf;
    alm->OMEGA0 = navIn->asSignedDouble(asbOMEGA0m, anbOMEGA0m, asbOMEGA0l, anbOMEGA0l, ascOMEGA0);
    // alm->i0 is set below
    alm->w = navIn->asSignedDouble(asbwm, anbwm, asbwl, anbwl, ascw);
    alm->OMEGAdot = navIn->asSignedDouble(asbOMEGAdotm, anbOMEGAdotm, asbOMEGAdotl, anbOMEGAdotl, ascOMEGAdot);
...
bool PNBBDSD2NavDataFactory ::processEph(unsigned long pgid, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    // Tgd are scaled by 0.1 to get nanoseconds, then x10^-9 to get seconds
    constexpr double sf = 0.1e-9;
    if ((pgid < 1) || (pgid > 10))
        return false;
    NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    if (pgid == 1)
    {
        if (PNBNavDataFactory::processISC)
        {
            ...
            isc->signal = NavMessageID(
                NavSatelliteID(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
                NavMessageType::ISC);
            isc->pre = navIn->asUnsignedLong(fsbPre, fnbPre, fscPre);
            isc->rev = navIn->asUnsignedLong(fsbRev, fnbRev, fscRev);
            isc->fraID = 1;
            isc->sow = navIn->asUnsignedLong(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
    ...
    {
        std::shared_ptr<BDSD2NavIono> iono = std::make_shared<BDSD2NavIono>();
        ...
        iono->fraID = 1;
        iono->sow = navIn->asUnsignedLong(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
        iono->alpha[0] = navIn->asSignedDouble(esbAlpha0m, enbAlpha0m, esbAlpha0l, enbAlpha0l, escAlpha0);
        iono->alpha[1] = navIn->asSignedDouble(esbAlpha1, enbAlpha1, escAlpha1);
        iono->alpha[2] = navIn->asSignedDouble(esbAlpha2, enbAlpha2, escAlpha2);
        iono->alpha[3] = navIn->asSignedDouble(esbAlpha3m, enbAlpha3m, esbAlpha3l, enbAlpha3l, escAlpha3);
        iono->beta[0] = navIn->asSignedDouble(esbBeta0, enbBeta0, escBeta0);
        iono->beta[1] = navIn->asSignedDouble(esbBeta1, enbBeta1, escBeta1);
        iono->beta[2] = navIn->asSignedDouble(esbBeta2m, enbBeta2m, esbBeta2l, enbBeta2l, escBeta2);
    ...
    for (unsigned i = 0; i < 10; i++)
    {
        ...
        if (i > 0)
        {
            ...
            if (i == pg3)
            {
                ...
                sowA = ephSF[i - 2]->asUnsignedLong(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW) + 3;
            }
            else
            {
                sowA = ephSF[i - 1]->asUnsignedLong(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
            }
            sowB = ephSF[i]->asUnsignedLong(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
    ...
    double toe = ephSF[esitoem]->asUnsignedDouble(esbtoem, enbtoem, esbtoel, enbtoel, esctoe);
    double toc = ephSF[esitocm]->asUnsignedDouble(esbtocm, enbtocm, esbtocl, enbtocl, esctoc);
    unsigned wn = ephSF[esiWN]->asUnsignedLong(esbWN, enbWN, escWN);
    eph->Toe = BDSWeekSecond(wn, toe);
    eph->Toc = BDSWeekSecond(wn, toc);
    // health is set below
    eph->Cuc = PackedNavBits::asSignedDouble({esbCucm, esbCucl}, {enbCucm, enbCucl}, {esiCucm, esiCucl}, ephSF, escCuc);
    // cout << "Cuc bits: " << hex
    //      << ephSF[esiCucm]->asUnsignedLong(esbCucm,enbCucm,esbCucl,enbCucl,
    //                                        escCuc)
    //      << dec << " " << eph->Cuc << endl;
    eph->Cus = ephSF[esiCusm]->asSignedDouble(esbCusm, enbCusm, esbCusl, enbCusl, escCus);
    // cout << "Cus bits: " << hex
    //      << ephSF[esiCus]->asUnsignedLong(esbCus,enbCus,escCus)
    //      << dec << " " << eph->Cus << endl;
    eph->Crc = ephSF[esiCrcm]->asSignedDouble(esbCrcm, enbCrcm, esbCrcl, enbCrcl, escCrc);
    eph->Crs = ephSF[esiCrs]->asSignedDouble(esbCrs, enbCrs, escCrs);
    eph->Cic = PackedNavBits::asSignedDouble({esbCicm, esbCici, esbCicl}, {enbCicm, enbCici, enbCicl},
                                             {esiCicm, esiCici, esiCicl}, ephSF, escCic);
    eph->Cis = ephSF[esiCis]->asSignedDouble(esbCis, enbCis, escCis);
    eph->M0 = PI * PackedNavBits::asSignedDouble({esbM0m, esbM0i, esbM0l}, {enbM0m, enbM0i, enbM0l},
                                                 {esiM0m, esiM0i, esiM0l}, ephSF, escM0);
    eph->dn = ephSF[esidn]->asDoubleSemiCircles(esbdn, enbdn, escdn);
    // no dndot in BDS D2NAV
    eph->ecc = PackedNavBits::asUnsignedDouble({esbEccm, esbEcci, esbEccl}, {enbEccm, enbEcci, enbEccl},
                                               {esiEccm, esiEcci, esiEccl}, ephSF, escEcc);
    eph->Ahalf = PackedNavBits::asUnsignedDouble({esbAhalfm, esbAhalfi, esbAhalfl}, {enbAhalfm, enbAhalfi, enbAhalfl},
                                                 {esiAhalfm, esiAhalfi, esiAhalfl}, ephSF, escAhalf);
    eph->A = eph->Ahalf * eph->Ahalf;
    // no Adot in BDS D2NAV
    eph->OMEGA0 =
        PI * PackedNavBits::asSignedDouble({esbOMEGA0m, esbOMEGA0i, esbOMEGA0l}, {enbOMEGA0m, enbOMEGA0i, enbOMEGA0l},
                                           {esiOMEGA0m, esiOMEGA0i, esiOMEGA0l}, ephSF, escOMEGA0);
    eph->i0 = PI * PackedNavBits::asSignedDouble({esbi0m, esbi0i1, esbi0i2, esbi0l}, {enbi0m, enbi0i1, enbi0i2, enbi0l},
                                                 {esii0m, esii0i1, esii0i2, esii0l}, ephSF, esci0);
    eph->w = PI * PackedNavBits::asSignedDouble({esbwm, esbwi, esbwl}, {enbwm, enbwi, enbwl}, {esiwm, esiwi, esiwl},
                                                ephSF, escw);
    eph->OMEGAdot = PI * PackedNavBits::asSignedDouble({esbOMEGAdotm, esbOMEGAdoti, esbOMEGAdotl},
                                                       {enbOMEGAdotm, enbOMEGAdoti, enbOMEGAdotl},
                                                       {esiOMEGAdotm, esiOMEGAdoti, esiOMEGAdotl}, ephSF, escOMEGAdot);
    eph->idot = ephSF[esiidotm]->asDoubleSemiCircles(esbidotm, enbidotm, esbidotl, enbidotl, escidot);
    eph->af0 = ephSF[esia0m]->asSignedDouble(esba0m, enba0m, esba0l, enba0l, esca0);
    eph->af1 = PackedNavBits::asSignedDouble({esba1m, esba1i, esba1l}, {enba1m, enba1i, enba1l},
                                             {esia1m, esia1i, esia1l}, ephSF, esca1);
    eph->af2 = ephSF[esia2m]->asSignedDouble(esba2m, enba2m, esba2l, enba2l, esca2);
    // BDSD2NavData
    ...
    eph->sow = ephSF[pg1]->asUnsignedLong(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
    // BDSD2NavEph
    eph->satH1 = ephSF[xesiSatH1]->asBool(xesbSatH1);
    eph->health = ((eph->satH1 == false) ? SVHealth::Healthy : SVHealth::Unhealthy); // actually in OrbitDataKepler
...
bool PNBBDSD2NavDataFactory ::processSF5Pg35(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processHea || PNBNavDataFactory::processAlm)
    {
        makeHealth(navIn, navOut, 1, h1sbHea1m, h1nbHea1m, h1sbHea1l, h1nbHea1l);
        makeHealth(navIn, navOut, 2, h1sbHea2, h1nbHea2);
        makeHealth(navIn, navOut, 3, h1sbHea3m, h1nbHea3m, h1sbHea3l, h1nbHea3l);
        makeHealth(navIn, navOut, 4, h1sbHea4, h1nbHea4);
        makeHealth(navIn, navOut, 5, h1sbHea5, h1nbHea5);
        makeHealth(navIn, navOut, 6, h1sbHea6m, h1nbHea6m, h1sbHea6l, h1nbHea6l);
        makeHealth(navIn, navOut, 7, h1sbHea7, h1nbHea7);
        makeHealth(navIn, navOut, 8, h1sbHea8m, h1nbHea8m, h1sbHea8l, h1nbHea8l);
        ...
        makeHealth(navIn, navOut, 13, h1sbHea13m, h1nbHea13m, h1sbHea13l, h1nbHea13l);
        makeHealth(navIn, navOut, 14, h1sbHea14, h1nbHea14);
        makeHealth(navIn, navOut, 15, h1sbHea15m, h1nbHea15m, h1sbHea15l, h1nbHea15l);
        makeHealth(navIn, navOut, 16, h1sbHea16, h1nbHea16);
        makeHealth(navIn, navOut, 17, h1sbHea17, h1nbHea17);
        makeHealth(navIn, navOut, 18, h1sbHea18m, h1nbHea18m, h1sbHea18l, h1nbHea18l);
...
bool PNBBDSD2NavDataFactory ::processSF5Pg36(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processHea || PNBNavDataFactory::processAlm)
    {
        makeHealth(navIn, navOut, 20, h2sbHea20m, h2nbHea20m, h2sbHea20l, h2nbHea20l);
        makeHealth(navIn, navOut, 21, h2sbHea21, h2nbHea21);
        makeHealth(navIn, navOut, 22, h2sbHea22m, h2nbHea22m, h2sbHea22l, h2nbHea22l);
        makeHealth(navIn, navOut, 23, h2sbHea23, h2nbHea23);
        makeHealth(navIn, navOut, 24, h2sbHea24, h2nbHea24);
        makeHealth(navIn, navOut, 25, h2sbHea25m, h2nbHea25m, h2sbHea25l, h2nbHea25l);
        makeHealth(navIn, navOut, 26, h2sbHea26, h2nbHea26);
        makeHealth(navIn, navOut, 27, h2sbHea27m, h2nbHea27m, h2sbHea27l, h2nbHea27l);
    ...
    if (PNBNavDataFactory::processAlm)
    {
        ...
        double toa = navIn->asUnsignedDouble(h2sbtoam, h2nbtoam, h2sbtoal, h2nbtoal, h2sctoa);
        ...
        unsigned long toaab = navIn->asUnsignedLong(h2sbtoam, h2nbtoam, h2sbtoal, h2nbtoal, 1);
...
bool PNBBDSD2NavDataFactory ::processSF5Pg101(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processTim)
    {
        ...
        ref.sow = 0;
        // BDT-GPS time offset
        std::shared_ptr<BDSD2NavTimeOffset> gps = std::make_shared<BDSD2NavTimeOffset>();
        ...
        gps->a1 = sf * navIn->asSignedDouble(csbA1GPSm, cnbA1GPSm, csbA1GPSl, cnbA1GPSl, cscA1GPS);
        gps->refTime = ref;
        ...
        std::shared_ptr<BDSD2NavTimeOffset> gal = std::make_shared<BDSD2NavTimeOffset>();
        gal->timeStamp = navIn->getTransmitTime();
        gal->signal = NavMessageID(
            NavSatelliteID(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
            NavMessageType::TimeOffset);
        gal->tgt = TimeSystem::GAL; // by definition
        gal->a0 = sf * navIn->asSignedDouble(csbA0GALm, cnbA0GALm, csbA0GALl, cnbA0GALl, cscA0GAL);
        ...
        std::shared_ptr<BDSD2NavTimeOffset> glo = std::make_shared<BDSD2NavTimeOffset>();
        ...
        glo->a1 = sf * navIn->asSignedDouble(csbA1GLOm, cnbA1GLOm, csbA1GLOl, cnbA1GLOl, cscA1GLO);
        glo->refTime = ref;
        // cerr << "add D2NAV time offset" << endl;
...
bool PNBBDSD2NavDataFactory ::processSF5Pg102(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processTim)
    {
        std::shared_ptr<BDSD2NavTimeOffset> to = std::make_shared<BDSD2NavTimeOffset>();
        to->timeStamp = navIn->getTransmitTime();
        to->signal = NavMessageID(
            NavSatelliteID(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
            NavMessageType::TimeOffset);
        to->tgt = TimeSystem::UTC; // by definition
        to->a0 = navIn->asSignedDouble(csbA0UTCm, cnbA0UTCm, csbA0UTCl, cnbA0UTCl, cscA0UTC);
        to->a1 = navIn->asSignedDouble(csbA1UTCm, cnbA1UTCm, csbA1UTCl, cnbA1UTCl, cscA1UTC);
        to->deltatLS = navIn->asLong(csbdtLSm, cnbdtLSm, csbdtLSl, cnbdtLSl, cscdtLS);
...
bool PNBBDSD2NavDataFactory ::processSF5Pg116(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processHea || PNBNavDataFactory::processAlm)
    {
        unsigned o = 0;
        ...
        makeHealth(navIn, navOut, o + 31, h3sbHea31m, h3nbHea31m, h3sbHea31l, h3nbHea31l);
        makeHealth(navIn, navOut, o + 32, h3sbHea32, h3nbHea32);
        makeHealth(navIn, navOut, o + 33, h3sbHea33m, h3nbHea33m, h3sbHea33l, h3nbHea33l);
        makeHealth(navIn, navOut, o + 34, h3sbHea34, h3nbHea34);
        makeHealth(navIn, navOut, o + 35, h3sbHea35, h3nbHea35);
        makeHealth(navIn, navOut, o + 36, h3sbHea36m, h3nbHea36m, h3sbHea36l, h3nbHea36l);
        makeHealth(navIn, navOut, o + 37, h3sbHea37, h3nbHea37);
        // AmID b11 only uses 31-37
        if (amID != 0x03)
        {
            makeHealth(navIn, navOut, o + 38, h3sbHea38m, h3nbHea38m, h3sbHea38l, h3nbHea38l);
            ...
            makeHealth(navIn, navOut, o + 43, h3sbHea43m, h3nbHea43m, h3sbHea43l, h3nbHea43l);
        }
        SatID xmitSat(navIn->getsatSys());
        NavSatelliteID key(0, xmitSat, navIn->getobsID(), navIn->getNavID());
        finishAlm(false, key, navOut);
...
void PNBBDSD2NavDataFactory ::makeHealth(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned long subjID,
                                         unsigned startBit1, unsigned numBits1, unsigned startBit2, unsigned numBits2)
{
    std::shared_ptr<BDSD2NavHealth> hea = std::make_shared<BDSD2NavHealth>();
    hea->timeStamp = navIn->getTransmitTime();
    hea->signal = NavMessageID(NavSatelliteID(subjID, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
                               NavMessageType::Health);
    // health is all "scaled" by 1, so just use the same constant.
    if (numBits2 > 0)
    {
        hea->svHealth = navIn->asUnsignedLong(startBit1, numBits1, startBit2, numBits2, h2scHea20);
    }
    else
    {
...
bool PNBBDSD2NavDataFactory ::isAlmDefault(const PackedNavBitsPtr &navIn)
{
    // Create a PNB with the same metadata, but all 0s, which is
    // how the BeiDou default pages appear (except words 1-2
    // which we ignore).
    PackedNavBits defPage(navIn->getsatSys(), navIn->getobsID(), navIn->getNavID(), navIn->getRxID(),
                          navIn->getTransmitTime(), navIn->getNumBits(), false);
...
void PNBBDSD2NavDataFactory ::resetState()
{
    fullWNaMap.clear();
    almAcc.clear();
    ephAcc.clear();
...
void PNBBDSD2NavDataFactory ::finishAlm(bool fromWNa, const NavSatelliteID &key, NavDataPtrList &navOut)
{
...
bool PNBBDSD2NavDataFactory ::finishAlm(PNBBDSD2NavDataFactory::AlmPtr &alm, bool fromWNa, const NavSatelliteID &key,
                                        NavDataPtrList &navOut)
{
    if (alm->isDefault && (heaAcc.find(alm->signal) != heaAcc.end()))
    {
        ...
        if (navValidity != NavValidityType::ValidOnly)
        {
            ...
            alm->health = ((alm->healthBits == 0) ? SVHealth::Healthy : SVHealth::Unhealthy);
            alm->fixFit();
    ...
    else if ((fullWNaMap.find(key.xmitSat.id) != fullWNaMap.end()) && (fullWNaMap[key.xmitSat.id].sow == alm->toa) &&
    ...
    else if (fromWNa && (fullWNaMap.find(key.xmitSat.id) != fullWNaMap.end()) &&
...
void PNBBDSD2NavDataFactory ::dumpState(std::ostream &s) const
{
    unsigned long total = 0;
    s << "fullWNaMap.size() = " << fullWNaMap.size() << endl << "almAcc.size() = " << almAcc.size() << endl;
    ...
    s << "almAcc total size = " << total << endl << "ephAcc.size() = " << ephAcc.size() << endl;
    for (const auto &i : ephAcc)
    {
        s << "  ephAcc[" << i.first << "].size() = " << i.second.size() << endl;
...
PNBBDSD2NavDataFactory::AmEpID ::AmEpID()
{
...
bool PNBBDSD2NavDataFactory::AmEpID ::isExpanded(const CommonTime &t) const
{
    return ((amEpID == 0x03) && (t >= mfStart) && (t < mfEnd));
}

void PNBBDSD2NavDataFactory::AmEpID ::setValues(const BDSWeekSecond &t, uint8_t id)
{
