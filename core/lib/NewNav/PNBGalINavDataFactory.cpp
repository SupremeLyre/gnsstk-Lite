...
namespace gnsstk
{
bool PNBGalINavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    ...
    try
    {
        unsigned long wordType = navFlex->asUnsignedLong(fsbType, fnbType, fscType);
...
bool PNBGalINavDataFactory ::wordFromPagePair(const PackedNavBits &navIn, PackedNavBits &navOut)
{
...
bool PNBGalINavDataFactory ::processEph(unsigned wordType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    if (!PNBNavDataFactory::processEph && !PNBNavDataFactory::processHea && !PNBNavDataFactory::processIono &&
        !PNBNavDataFactory::processISC)
    {
    ...
    if (wordType == 5)
    {
        ...
        if (PNBNavDataFactory::processHea && ephWord[wt3])
        {
            ...
            hp1->sigHealthStatus =
                static_cast<GalHealthStatus>(ephWord[isiE5bhs]->asUnsignedLong(isbE5bhs, inbE5bhs, iscE5bhs));
            hp1->dataValidityStatus =
                static_cast<GalDataValid>(ephWord[isiE5bdvs]->asUnsignedLong(isbE5bdvs, inbE5bdvs, iscE5bdvs));
            hp1->sisaIndex = ephWord[esiSISA]->asUnsignedLong(esbSISA, enbSISA, escSISA);
            ...
            *hp2 = *hp1; // copy data
            ...
            hp2->sigHealthStatus =
                static_cast<GalHealthStatus>(ephWord[isiE1Bhs]->asUnsignedLong(isbE1Bhs, inbE1Bhs, iscE1Bhs));
            hp2->dataValidityStatus =
                static_cast<GalDataValid>(ephWord[isiE1Bdvs]->asUnsignedLong(isbE1Bdvs, inbE1Bdvs, iscE1Bdvs));
    ...
    if (!ephWord[wt1] || !ephWord[wt2] || !ephWord[wt3] || !ephWord[wt4] || !ephWord[wt5] ||
        (ephWord[wt1]->getNumBits() != 128) || (ephWord[wt2]->getNumBits() != 128) ||
        (ephWord[wt3]->getNumBits() != 128) || (ephWord[wt4]->getNumBits() != 128) ||
    ...
    eph->Ahalf = ephWord[esiAhalf]->asUnsignedDouble(esbAhalf, enbAhalf, escAhalf);
    eph->A = eph->Ahalf * eph->Ahalf;
    // no Adot in I/NAV
    eph->OMEGA0 = ephWord[esiOMEGA0]->asDoubleSemiCircles(esbOMEGA0, enbOMEGA0, escOMEGA0);
    eph->i0 = ephWord[esii0]->asDoubleSemiCircles(esbi0, enbi0, esci0);
    eph->w = ephWord[esiw]->asDoubleSemiCircles(esbw, enbw, escw);
    eph->OMEGAdot = ephWord[esiOMEGAdot]->asDoubleSemiCircles(esbOMEGAdot, enbOMEGAdot, escOMEGAdot);
    eph->idot = ephWord[esiidot]->asDoubleSemiCircles(esbidot, enbidot, escidot);
    ...
    eph->sisaIndex = ephWord[esiSISA]->asUnsignedLong(esbSISA, enbSISA, escSISA);
    ...
    eph->iodnav3 = iod3;
    eph->iodnav4 = iod4;
    eph->hsE5b = static_cast<GalHealthStatus>(ephWord[isiE5bhs]->asUnsignedLong(isbE5bhs, inbE5bhs, iscE5bhs));
    eph->hsE1B = static_cast<GalHealthStatus>(ephWord[isiE1Bhs]->asUnsignedLong(isbE1Bhs, inbE1Bhs, iscE1Bhs));
    eph->dvsE5b = static_cast<GalDataValid>(ephWord[isiE5bdvs]->asUnsignedLong(isbE5bdvs, inbE5bdvs, iscE5bdvs));
    eph->dvsE1B = static_cast<GalDataValid>(ephWord[isiE1Bdvs]->asUnsignedLong(isbE1Bdvs, inbE1Bdvs, iscE1Bdvs));
    // set health using the Galileo algorithms.
    if (eph->signal.obs.band == gnsstk::CarrierBand::L1)
    {
        // E1B
        eph->health = GalINavHealth::galHealth(eph->hsE1B, eph->dvsE1B, eph->sisaIndex);
    }
    else
    {
        // E5b
        eph->health = GalINavHealth::galHealth(eph->hsE5b, eph->dvsE5b, eph->sisaIndex);
    }
    eph->fixFit();
...
bool PNBGalINavDataFactory ::processAlm(unsigned wordType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    if (!PNBNavDataFactory::processAlm && !PNBNavDataFactory::processHea && !PNBNavDataFactory::processTim)
    {
    ...
    // stop processing if we don't have three full subframes
    if (!almWord[wt7] || !almWord[wt8] || !almWord[wt9] || !almWord[wt10] || (almWord[wt7]->getNumBits() != 128) ||
        (almWord[wt8]->getNumBits() != 128) || (almWord[wt9]->getNumBits() != 128) ||
    ...
    GalINavHealth *hp1 = dynamic_cast<GalINavHealth *>(p1.get());
    GalINavHealth *hp2 = dynamic_cast<GalINavHealth *>(p2.get());
    if (processAlmOrb(almWord, alm, hp1, hp2, wt7, wt8, asiWNa_A, asit0a_A, asiSVID_1, asbSVID_1, asidAhalf_1,
                      asbdAhalf_1, asiEcc_1, asbEcc_1, asiw_1, asbw_1, asidi_1, asbdi_1, asiOMEGA0_1, asbOMEGA0_1,
                      asiOMEGAdot_1, asbOMEGAdot_1, asiM0_1, asbM0_1, asiaf0_1, asbaf0_1, asiaf1_1, asbaf1_1,
                      asiE5bhs_1, asbE5bhs_1, asiE1Bhs_1, asbE1Bhs_1))
    {
    ...
    if (processAlmOrb(almWord, alm, hp1, hp2, wt8, wt9, asiWNa_B, asit0a_B, asiSVID_2, asbSVID_2, asidAhalf_2,
                      asbdAhalf_2, asiEcc_2, asbEcc_2, asiw_2, asbw_2, asidi_2, asbdi_2, asiOMEGA0_2, asbOMEGA0_2,
                      asiOMEGAdot_2, asbOMEGAdot_2, asiM0_2, asbM0_2, asiaf0_2, asbaf0_2, asiaf1_2, asbaf1_2,
                      asiE5bhs_2, asbE5bhs_2, asiE1Bhs_2, asbE1Bhs_2))
    {
        if (PNBNavDataFactory::processAlm)
        {
    ...
    hp1 = dynamic_cast<GalINavHealth *>(p1.get());
    hp2 = dynamic_cast<GalINavHealth *>(p2.get());
    if (processAlmOrb(almWord, alm, hp1, hp2, wt9, wt10, asiWNa_B, asit0a_B, asiSVID_3, asbSVID_3, asidAhalf_3,
                      asbdAhalf_3, asiEcc_3, asbEcc_3, asiw_3, asbw_3, asidi_3, asbdi_3, asiOMEGA0_3, asbOMEGA0_3,
                      asiOMEGAdot_3, asbOMEGAdot_3, asiM0_3, asbM0_3, asiaf0_3, asbaf0_3, asiaf1_3, asbaf1_3,
                      asiE5bhs_3, asbE5bhs_3, asiE1Bhs_3, asbE1Bhs_3))
    {
        if (PNBNavDataFactory::processAlm)
        {
...
bool PNBGalINavDataFactory ::processAlmOrb(const std::vector<PackedNavBitsPtr> &almWord, GalINavAlm *alm,
                                           GalINavHealth *hp1, GalINavHealth *hp2, int wtA, int wtB, int asiWNa,
                                           int asit0a, int asiSVID, int asbSVID, int asidAhalf, int asbdAhalf,
                                           int asiEcc, int asbEcc, int asiw, int asbw, int asidi, int asbdi,
                                           int asiOMEGA0, int asbOMEGA0, int asiOMEGAdot, int asbOMEGAdot, int asiM0,
                                           int asbM0, int asiaf0, int asbaf0, int asiaf1, int asbaf1, int asiE5bhs,
                                           int asbE5bhs, int asiE1Bhs, int asbE1Bhs)
{
    // NavData
    alm->timeStamp = almWord[wtA]->getTransmitTime();
    unsigned long svid = almWord[asiSVID]->asUnsignedLong(asbSVID, anbSVID, ascSVID);
    if (svid == 0)
        return false; // empty almanac, do nothing.
    SatID xmitSat(almWord[wtA]->getsatSys());
    NavSatelliteID sat(svid, xmitSat, almWord[wtA]->getobsID(), almWord[wtA]->getNavID());
    ...
    gws.sow = alm->t0a;
    alm->Toe = alm->Toc = gws;
    // cerr << "  wna=" << alm->wna << "  tow=" << gws.sow << "  t0a=" << alm->t0a << "  Toe=" << printTime(alm->Toe,
    // "%L %g") << "  xmit=" << printTime(alm->xmitTime, "%L %g") << endl;
    alm->wna = gws.week;
    alm->M0 = almWord[asiM0]->asDoubleSemiCircles(asbM0, anbM0, ascM0);
    alm->ecc = almWord[asiEcc]->asUnsignedDouble(asbEcc, anbEcc, ascEcc);
    alm->dAhalf = almWord[asidAhalf]->asUnsignedDouble(asbdAhalf, anbdAhalf, ascdAhalf);
    ...
    alm->OMEGA0 = almWord[asiOMEGA0]->asDoubleSemiCircles(asbOMEGA0, anbOMEGA0, ascOMEGA0);
    ...
    alm->OMEGAdot = almWord[asiOMEGAdot]->asDoubleSemiCircles(asbOMEGAdot, anbOMEGAdot, ascOMEGAdot);
    alm->af0 = almWord[asiaf0]->asSignedDouble(asbaf0, anbaf0, ascaf0);
    alm->af1 = almWord[asiaf1]->asSignedDouble(asbaf1, anbaf1, ascaf1);
    // GalINavAlm
    alm->hsE5b = static_cast<GalHealthStatus>(almWord[asiE5bhs]->asUnsignedLong(asbE5bhs, anbE5bhs, ascE5bhs));
    alm->hsE1B = static_cast<GalHealthStatus>(almWord[asiE1Bhs]->asUnsignedLong(asbE1Bhs, anbE1Bhs, ascE1Bhs));
    alm->fixFit();
    alm->fixHealth();
    if (PNBNavDataFactory::processHea)
    {
        ...
        hp1->signal.obs.code = TrackingCode::E5bI;
        hp1->sigHealthStatus =
            static_cast<GalHealthStatus>(almWord[asiE5bhs]->asUnsignedLong(asbE5bhs, anbE5bhs, ascE5bhs));
        ...
        hp2->signal.obs.code = TrackingCode::E1B;
        hp2->sigHealthStatus =
            static_cast<GalHealthStatus>(almWord[asiE1Bhs]->asUnsignedLong(asbE1Bhs, anbE1Bhs, ascE1Bhs));
        /// @todo figure out what DVS and SISA should be here.
    }
    return true;
}

bool PNBGalINavDataFactory ::processOffset(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    p0->signal =
        NavMessageID(NavSatelliteID(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
                     NavMessageType::TimeOffset);
...
void PNBGalINavDataFactory ::resetState()
{
    almAcc.clear();
    ephAcc.clear();
}
