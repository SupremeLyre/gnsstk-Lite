...
namespace gnsstk
{
RinexNavDataFactory ::RinexNavDataFactory()
{
...
bool RinexNavDataFactory ::loadIntoMap(const std::string &filename, NavMessageMap &navMap,
...
bool RinexNavDataFactory ::process(const std::string &filename, NavDataFactoryCallback &cb)
{
    bool rv = true;
...
std::string RinexNavDataFactory ::getFactoryFormats() const
{
    if (procNavTypes.empty() || (procNavTypes.count(NavMessageType::Ephemeris) > 0) ||
        (procNavTypes.count(NavMessageType::Health) > 0) || (procNavTypes.count(NavMessageType::Iono) > 0) ||
...
bool RinexNavDataFactory ::convertToOrbit(const Rinex3NavData &navIn, NavDataPtr &navOut)
{
    bool rv = true;
    GPSLNavEph *gps;
    GalINavEph *galINav;
    ...
    case SatelliteSystem::QZSS:
        ...
        gps->health = ((navIn.health == 0) ? SVHealth::Healthy : SVHealth::Unhealthy);
    ...
    case SatelliteSystem::Galileo:
        if (((navIn.datasources & 0x01) == 0x01) || ((navIn.datasources & 0x04) == 0x04))
        {
            ...
            galINav->iodnav1 = galINav->iodnav2 = galINav->iodnav3 = galINav->iodnav4 = navIn.IODnav;
            galINav->dvsE1B = static_cast<GalDataValid>(navIn.health & 0x01);
            galINav->hsE1B = static_cast<GalHealthStatus>((navIn.health >> 1) & 0x03);
            ...
            galINav->dvsE5b = static_cast<GalDataValid>((navIn.health >> 6) & 0x01);
            galINav->hsE5b = static_cast<GalHealthStatus>((navIn.health >> 7) & 0x03);
            /** @note rinex can combine I/NAV ephemerides, but
             * we just assume one or the other. */
            if ((navIn.datasources & 0x01) == 0x01)
            {
                galINav->health = GalINavHealth::galHealth(galINav->hsE1B, galINav->dvsE1B, galINav->sisaIndex);
            }
            else
            {
                galINav->health = GalINavHealth::galHealth(galINav->hsE5b, galINav->dvsE5b, galINav->sisaIndex);
            }
            galINav->fixFit();
        }
        else if (navIn.datasources & 0x02)
        {
            ...
            galFNav->iodnav1 = galFNav->iodnav2 = galFNav->iodnav3 = galFNav->iodnav4 = navIn.IODnav;
            /** @note rinex includes health information for
             * three signals, but I/NAV ephemerides will only
             * include health for E5b and E1B, while F/NAV only
             * includes health for E1a */
            galFNav->dvsE5a = static_cast<GalDataValid>((navIn.health >> 3) & 0x01);
            galFNav->hsE5a = static_cast<GalHealthStatus>((navIn.health >> 4) & 0x03);
            galFNav->health = GalINavHealth::galHealth(galFNav->hsE5a, galFNav->dvsE5a, galFNav->sisaIndex);
            galFNav->fixFit();
        }
        else
        {
    ...
    case SatelliteSystem::BeiDou:
        if (isBeiDouGEO(navIn.sat))
        {
            ...
            bdsD2Nav->health = ((navIn.health == 0) ? SVHealth::Healthy : SVHealth::Unhealthy);
        ...
        else
        {
            ...
            bdsD1Nav->health = ((navIn.health == 0) ? SVHealth::Healthy : SVHealth::Unhealthy);
...
void RinexNavDataFactory ::convertToOrbitDataKepler(const Rinex3NavData &navIn, OrbitDataKepler *navOut)
{
    // OrbitDataKepler
...
bool RinexNavDataFactory ::convertToHealth(const Rinex3NavData &navIn, NavDataPtrList &healthOut)
{
    bool rv = true;
...
void RinexNavDataFactory ::convertToHealthE1B(const Rinex3NavData &navIn, NavDataPtrList &healthOut)
{
    ...
    // health info).
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("E1B=" << (navIn.datasources & 0x01) << "  E5a=" << (navIn.datasources & 0x02)
                      << "  E5b=" << (navIn.datasources & 0x04));
    if ((navIn.datasources & 0x01) || ((navIn.datasources & 0x04) == 0))
    {
        ...
        galNav->dataValidityStatus = static_cast<GalDataValid>(navIn.health & 0x01);
        galNav->sigHealthStatus = static_cast<GalHealthStatus>((navIn.health >> 1) & 0x03);
...
void RinexNavDataFactory ::convertToHealthE5a(const Rinex3NavData &navIn, NavDataPtrList &healthOut)
{
    DEBUGTRACE_FUNCTION();
    ...
    // GalFNavHealth
    // start with original health bits from RINEX, decode and shift.
    galNav->dataValidityStatus = static_cast<GalDataValid>((navIn.health >> 3) & 0x01);
    galNav->sigHealthStatus = static_cast<GalHealthStatus>((navIn.health >> 4) & 0x03);
...
void RinexNavDataFactory ::convertToHealthE5b(const Rinex3NavData &navIn, NavDataPtrList &healthOut)
{
    DEBUGTRACE_FUNCTION();
    if (navIn.datasources & 0x04)
    {
        ...
        // GalINavHealth
        // start with original health bits from RINEX, decode and shift.
        galNav->dataValidityStatus = static_cast<GalDataValid>((navIn.health >> 6) & 0x01);
        galNav->sigHealthStatus = static_cast<GalHealthStatus>((navIn.health >> 7) & 0x03);
        galNav->signal.obs.band = CarrierBand::L5;
...
bool RinexNavDataFactory ::convertToOffset(const Rinex3NavHeader &navIn, NavDataPtrList &navOut)
{
    for (const auto &mti : navIn.mapTimeCorr)
    {
        std::shared_ptr<RinexTimeOffset> rto = std::make_shared<RinexTimeOffset>(mti.second, navIn.leapSeconds);
...
bool RinexNavDataFactory ::convertToIono(const CommonTime &when, const Rinex3NavHeader &navIn, NavDataPtrList &navOut)
{
        ...
        ((bi = navIn.mapIonoCorr.find("GPSB")) != navIn.mapIonoCorr.end()))
    {
        ...
        iono->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::CA);
        ...
        std::shared_ptr<GPSLNavHealth> health(std::make_shared<GPSLNavHealth>());
        // NavData
        ...
        // we can't obtain these from RINEX NAV, so just assume L1 C/A
        health->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::CA);
    ...
    if ((ai = navIn.mapIonoCorr.find("GAL")) != navIn.mapIonoCorr.end())
    {
        ...
        iono->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::E1B);
        ...
        std::shared_ptr<GalINavHealth> health(std::make_shared<GalINavHealth>());
        // NavData
        // further kludge to set fake health time stamp to beginning of day
        YDSTime bod(when);
        bod.sod = 0;
        ...
        // we can't obtain these from RINEX NAV, so just assume L1 E1B
        health->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::E1B);
        ...
        ((bi = navIn.mapIonoCorr.find("BDSB")) != navIn.mapIonoCorr.end()))
    {
        ...
        iono->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::B1, TrackingCode::B1I);
        ...
        // the RINEX header came from a healthy satellite, and
        // stuff a fake satellite 0 health record in the data.
        std::shared_ptr<BDSD1NavHealth> health(std::make_shared<BDSD1NavHealth>());
        // NavData
        // further kludge to set fake health time stamp to beginning of day
        YDSTime bod(when);
        bod.sod = 0;
        ...
        // we can't obtain these from RINEX NAV, so just assume B1 B1I
        health->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::B1, TrackingCode::B1I);
...
bool RinexNavDataFactory ::convertToISC(const Rinex3NavData &navIn, NavDataPtr &navOut)
{
    bool rv = true;
    GPSLNavISC *gps;
    GalINavISC *galI;
...
void RinexNavDataFactory ::fillNavData(const Rinex3NavData &navIn, NavDataPtr &navOut)
{
    switch (navIn.sat.system)
    {
    case SatelliteSystem::GPS:
    case SatelliteSystem::QZSS:
        // NavData
        navOut->timeStamp = gnsstk::GPSWeekSecond(navIn.weeknum, navIn.xmitTime);
        ...
        // we can't obtain these from RINEX NAV, so just assume L1 C/A
        navOut->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::CA);
    ...
    case SatelliteSystem::Galileo:
        ...
        {
            navOut->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::E1B);
            navOut->signal.nav = NavType::GalINAV;
        }
        else if (navIn.datasources & 0x02)
        {
            navOut->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::L5, TrackingCode::E5aI);
            navOut->signal.nav = NavType::GalFNAV;
        }
        else if (navIn.datasources & 0x04)
        {
            navOut->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::E5b, TrackingCode::E5bI);
            navOut->signal.nav = NavType::GalINAV;
        }
        break;
    case SatelliteSystem::BeiDou:
        // NavData
        navOut->timeStamp = gnsstk::BDSWeekSecond(navIn.weeknum, navIn.xmitTime);
        // sat and xmitSat are always the same for ephemeris
        navOut->signal.sat = navIn.sat;
        navOut->signal.xmitSat = navIn.sat;
        navOut->signal.system = navIn.sat.system;
        // we can't obtain these from RINEX NAV, so just assume B1 B1I
        navOut->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::B1, TrackingCode::B1I);
    ...
    case SatelliteSystem::Glonass: {
        ...
        navOut->signal.obs = ObsID(ObservationType::NavMsg, CarrierBand::G1, TrackingCode::Standard, navIn.freqNum);
...
void RinexNavDataFactory ::fixTimeGPS(const Rinex3NavData &navIn, GPSLNavEph &navOut)
{
    long longToc = (long)navIn.Toc;
    long fullXmitWeekNum = navIn.weeknum;
    TimeSystem ts = (navIn.sat.system == SatelliteSystem::QZSS) ? TimeSystem::QZS : TimeSystem::GPS;

    // Case 3 check
    long adjHOWtime = navIn.xmitTime;
    if (((longToc % SEC_PER_DAY) == 0) && ((navIn.xmitTime % SEC_PER_DAY) == 0) && (longToc == navIn.xmitTime))
    {
...
void RinexNavDataFactory ::fixTimeGalileo(const Rinex3NavData &navIn, OrbitDataKepler &navOut)
{
...
void RinexNavDataFactory ::fixTimeBeiDou(const Rinex3NavData &navIn, OrbitDataKepler &navOut)
{
    /// @todo Probably need to do half week tests on the transmit time
    navOut.xmitTime = BDSWeekSecond(navIn.weeknum, navIn.xmitTime);
}

uint8_t RinexNavDataFactory ::decodeSISA(double accuracy)
{
...
double RinexNavDataFactory ::encodeSISA(uint8_t sisa)
{
    // Implementation of Galileo-OS-SIS-ICD section 5.1.11
    // (Signal-In-Space Accuracy (SISA))
    // accuracy = -1 (or less than zero anyway)
