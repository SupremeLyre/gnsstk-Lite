...
namespace gnsstk
{
bool PNBGPSCNavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    if ((navIn->getNavID().navType != NavType::GPSCNAVL2) && (navIn->getNavID().navType != NavType::GPSCNAVL5))
    {
    ...
    try
    {
        ...
        // cerr << "prn = " << prn << endl;
        unsigned long msgType = navIn->asUnsignedLong(esbMsgType, enbMsgType, escMsgType);
        ...
        // contain.
        if ((msgType == 10) || (msgType == 11) || ((msgType >= 30) && (msgType <= 37)) ||
            (isQZSS && ((msgType == 46) || (msgType == 47) || (msgType == 49) || (msgType == 51) || (msgType == 53) ||
                        (msgType == 61))))
        {
...
bool PNBGPSCNavDataFactory ::processEph(unsigned msgType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    if ((msgType == 10) && processHea)
    {
        ...
        p1L1->signal = NavMessageID(
            NavSatelliteID(prn, prn, navIn->getsatSys().system, CarrierBand::L1, TrackingCode::CA, NavType::GPSLNAV),
            NavMessageType::Health);
        if (navIn->getobsID().band == CarrierBand::L2)
        {
            p1L2->signal = NavMessageID(NavSatelliteID(prn, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
                                        NavMessageType::Health);
            p1L5->signal = NavMessageID(NavSatelliteID(prn, prn, navIn->getsatSys().system, CarrierBand::L5,
                                                       TrackingCode::L5I, NavType::GPSCNAVL5),
                                        NavMessageType::Health);
        }
        else
        {
            // assume L5
            p1L2->signal = NavMessageID(NavSatelliteID(prn, prn, navIn->getsatSys().system, CarrierBand::L2,
                                                       TrackingCode::L2CM, NavType::GPSCNAVL2),
                                        NavMessageType::Health);
            p1L5->signal = NavMessageID(NavSatelliteID(prn, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
                                        NavMessageType::Health);
        }
        dynamic_cast<GPSCNavHealth *>(p1L1.get())->health = navIn->asBool(esbHeaL1);
        dynamic_cast<GPSCNavHealth *>(p1L2.get())->health = navIn->asBool(esbHeaL2);
        dynamic_cast<GPSCNavHealth *>(p1L5.get())->health = navIn->asBool(esbHeaL5);
    ...
    if (!ephSF[ephM10] || !ephSF[ephM11] || !ephSF[ephMClk] || (ephSF[ephM10]->getNumBits() != 300) ||
        (ephSF[ephM11]->getNumBits() != 300) || (ephSF[ephMClk]->getNumBits() != 300))
    {
    ...
    eph->signal = NavMessageID(NavSatelliteID(prn, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID()),
    ...
    eph->dndot = ephSF[esidn0dot]->asDoubleSemiCircles(esbdn0dot, enbdn0dot, escdn0dot);
    eph->ecc = ephSF[esiEcc]->asUnsignedDouble(esbEcc, enbEcc, escEcc);
    eph->deltaA = ephSF[esidA]->asSignedDouble(esbdA, enbdA, escdA);
    eph->dOMEGAdot = ephSF[esidOMEGAdot]->asDoubleSemiCircles(esbdOMEGAdot, enbdOMEGAdot, escdOMEGAdot);
    ...
    eph->OMEGA0 = ephSF[esiOMEGA0]->asDoubleSemiCircles(esbOMEGA0, enbOMEGA0, escOMEGA0);
    ...
    {
    case CarrierBand::L2:
        eph->health = ((eph->healthL2 == false) ? SVHealth::Healthy : SVHealth::Unhealthy);
        break;
    case CarrierBand::L5:
        eph->health = ((eph->healthL5 == false) ? SVHealth::Healthy : SVHealth::Unhealthy);
        break;
    default:
    ...
    eph->uraNED1 = ephSF[csiURAned1]->asUnsignedLong(csbURAned1, cnbURAned1, cscURAned1);
    eph->uraNED2 = ephSF[csiURAned2]->asUnsignedLong(csbURAned2, cnbURAned2, cscURAned2);
    eph->fixFit();
...
bool PNBGPSCNavDataFactory ::processAlmOrb(unsigned msgType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    if (processHea)
    {
         ...
         * broadcasting signal status for L1 signals that don't
         * have CNAV. */
        p1L1->signal = NavMessageID(NavSatelliteID(subjSat, xmitSat,
                                                   ObsID(ObservationType::NavMsg, CarrierBand::L1, TrackingCode::CA),
                                                   NavID(NavType::GPSLNAV)),
                                    NavMessageType::Health);
        if (navIn->getobsID().band == CarrierBand::L2)
        {
            p1L2->signal = NavMessageID(NavSatelliteID(subjSat, xmitSat, navIn->getobsID(), navIn->getNavID()),
                                        NavMessageType::Health);
            p1L5->signal = NavMessageID(
                NavSatelliteID(subjSat, xmitSat, ObsID(ObservationType::NavMsg, CarrierBand::L5, TrackingCode::L5I),
        ...
        else
        {
            ...
            p1L2->signal = NavMessageID(
                NavSatelliteID(subjSat, xmitSat, ObsID(ObservationType::NavMsg, CarrierBand::L2, TrackingCode::L2CM),
                               NavID(NavType::GPSCNAVL2)),
                NavMessageType::Health);
            p1L5->signal = NavMessageID(NavSatelliteID(subjSat, xmitSat, navIn->getobsID(), navIn->getNavID()),
                                        NavMessageType::Health);
        }
        dynamic_cast<GPSCNavHealth *>(p1L1.get())->health = navIn->asBool(asbHeaL1);
        dynamic_cast<GPSCNavHealth *>(p1L2.get())->health = navIn->asBool(asbHeaL2);
        dynamic_cast<GPSCNavHealth *>(p1L5.get())->health = navIn->asBool(asbHeaL5);
    ...
    alm->signal =
        NavMessageID(NavSatelliteID(subjSat, xmitSat, navIn->getobsID(), navIn->getNavID()), NavMessageType::Almanac);
    // OrbitData = empty
    // OrbitDataKepler
    ...
    alm->OMEGAdot = navIn->asDoubleSemiCircles(asbOMEGAdot, anbOMEGAdot, ascOMEGAdot);
    ...
    {
    case CarrierBand::L2:
        alm->health = ((alm->healthL2 == false) ? SVHealth::Healthy : SVHealth::Unhealthy);
        break;
    case CarrierBand::L5:
        alm->health = ((alm->healthL5 == false) ? SVHealth::Healthy : SVHealth::Unhealthy);
        break;
    default:
        // unexpected/unsupported signal
        return false;
...
bool PNBGPSCNavDataFactory ::process12(unsigned msgType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    return processRedAlmOrb(msgType, rsb12p1, pre, alert, wna, toa, navIn, navOut) &&
...
bool PNBGPSCNavDataFactory ::process30(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
...
bool PNBGPSCNavDataFactory ::process31(unsigned msgType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    unsigned pre = navIn->asUnsignedLong(esbPre, enbPre, escPre);
    bool alert = navIn->asBool(esbAlert);
    ...
    return processRedAlmOrb(msgType, rsb31p1, pre, alert, wna, toa, navIn, navOut) &&
...
bool PNBGPSCNavDataFactory ::processRedAlmOrb(unsigned msgType, unsigned offset, unsigned pre, bool alert, unsigned wna,
                                              double toa, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    SatID xmitSat(navIn->getsatSys());
    unsigned long sprn = navIn->asUnsignedLong(offset + rsbPRNa, rnbPRNa, rscPRNa);
    SatelliteSystem subjSys = xmitSat.system;
    if (sprn == 0)
    {
        // empty almanac
    ...
    if (PNBNavDataFactory::processHea)
    {
        ...
        dynamic_cast<GPSCNavHealth *>(p1L1.get())->health = navIn->asBool(offset + rsbHeaL1);
        dynamic_cast<GPSCNavHealth *>(p1L2.get())->health = navIn->asBool(offset + rsbHeaL2);
        dynamic_cast<GPSCNavHealth *>(p1L5.get())->health = navIn->asBool(offset + rsbHeaL5);
    ...
    alm->OMEGA0 = navIn->asDoubleSemiCircles(offset + rsbOMEGA0, rnbOMEGA0, rscOMEGA0);
    // GPSCNavData
    alm->pre = pre;
    alm->alert = alert;
    // GPSCNavAlm
...
bool PNBGPSCNavDataFactory ::process33(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    GPSCNavTimeOffset *to = dynamic_cast<GPSCNavTimeOffset *>(p0.get());
...
bool PNBGPSCNavDataFactory ::process35(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (!PNBNavDataFactory::processTim)
    {
        // User doesn't want time offset data so don't do any processing.
...
void PNBGPSCNavDataFactory ::dumpState(std::ostream &s) const
{
