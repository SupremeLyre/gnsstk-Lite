...
class NeQuickIonoNavData : public IonoNavData
{
  public:
    ...
    double getIonoCorr(const CommonTime &when, const Position &rxgeo, const Position &svgeo,
  ...
  protected:
     ...
     * @return The electron density in TECU.
     */
    double getSED(double dist, const Position &rxgeo, const Position &svgeo, const MODIP &modip, CCIR &ccirData,
                  const CivilTime &when, double azu) const;
     ...
     * @return The electron density in TECU.
     */
    double getVED(double dist, const Position &rxgeo, const Position &svgeo, double modip_u, CCIR &ccirData,
                  const CivilTime &when, double azu) const;
    ...
    double integrateGaussKronrod(double heightPt1, double heightPt2, const Position &rxgeo, const Position &svgeo,
                                 const MODIP &modip, double modipSta, CCIR &ccirData, const CivilTime &when, double azu,
                                 double tolerance, bool vertical, unsigned recursionLevel = 0) const;
...
double NeQuickIonoNavData::ModelParameters ::epstein(double x, double y, double z, double w)
{
    DEBUGTRACE_FUNCTION();
...
double NeQuickIonoNavData::ModelParameters ::solarGetLongitude(const CivilTime &when)
{
    // eq.4, sort of.
