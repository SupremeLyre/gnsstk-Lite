...
namespace gnsstk
{
GPSCNav2ISC ::GPSCNav2ISC()
    : haveSF2(false), haveSF3(false), iscL1CP(std::numeric_limits<double>::quiet_NaN()),
      iscL1CD(std::numeric_limits<double>::quiet_NaN()), iscL1CA(std::numeric_limits<double>::quiet_NaN()),
...
void GPSCNav2ISC ::dumpCorrections(std::ostream &s) const
{
    using namespace std;
...
CommonTime GPSCNav2ISC ::getUserTime() const
{
...
bool GPSCNav2ISC ::validate() const
{
    /// @todo implement some checks.
    return true;
}

bool GPSCNav2ISC ::getISC(const ObsID &oid, double &corr) const
{
...
bool GPSCNav2ISC ::getISC(const ObsID &oid1, const ObsID &oid2, double &corr) const
{
    // reminder: InterSigCorr::isc is used to store Tgd
              ...
              (oid2.code == TrackingCode::L2CML)))
    {
        ...
        if ((oid1.band == CarrierBand::L1) && (oid1.code == TrackingCode::CA))
        {
            // per IS-GPS-200 30.3.3.3.1.1.2
            corr = ((iscL2C - (gamma12 * iscL1CA)) / (1 - gamma12)) - isc;
            return true;
        }
        else if ((oid1.band == CarrierBand::L1) && (oid1.code == TrackingCode::L1CP))
        {
            // per IS-GPS-800 3.5.3.9.2
            corr = ((iscL2C - (gamma12 * iscL1CP)) / (1 - gamma12)) - isc;
            return true;
        }
        else if ((oid1.band == CarrierBand::L1) && (oid1.code == TrackingCode::L1CD))
        {
            // per IS-GPS-800 3.5.3.9.2
