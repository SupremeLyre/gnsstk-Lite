...
namespace gnsstk
{
GPSCNavEph ::GPSCNavEph()
    : pre11(0), preClk(0), healthL1(true), healthL2(true), healthL5(true), uraED(-16), uraNED0(-16), uraNED1(0),
      uraNED2(0), alert11(false), alertClk(false), integStat(false), phasingL2C(false), deltaA(0.0), dOMEGAdot(0.0),
      top(gnsstk::CommonTime::BEGINNING_OF_TIME), xmit11(gnsstk::CommonTime::BEGINNING_OF_TIME),
      xmitClk(gnsstk::CommonTime::BEGINNING_OF_TIME)
{
    signal.messageType = NavMessageType::Ephemeris;
}

bool GPSCNavEph ::validate() const
{
    return GPSCNavData::validate() && ((pre11 == 0) || (pre11 == 0x8b)) && ((preClk == 0) || (preClk == 0x8b));
}

CommonTime GPSCNavEph ::getUserTime() const
{
...
void GPSCNavEph ::dumpSVStatus(std::ostream &s) const
{
    ...
    s << "           ACCURACY PARAMETERS" << endl
      ...
      << "           SV STATUS" << endl
      << endl
      << "Health bits  L1, L2, L5        :     " << setfill('0') << setw(1) << healthL1 << ",  " << healthL2 << ",  "
      << healthL5 << endl
      << "L2C Phasing                    :     " << setfill(' ') << phasingL2C << " (0=quadrature, 1=in-phase)" << endl
      << endl
      << endl
      << "           TRANSMIT TIMES" << endl
      << endl
