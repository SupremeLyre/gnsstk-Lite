...
namespace gnsstk
{
bool PNBGPSLNavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    ...
    try
    {
        ...
        case 5:
            ...
            useQZSS = ((navIn->getsatSys().system == gnsstk::SatelliteSystem::QZSS) && (dataID == dataIDQZSS));
...
bool PNBGPSLNavDataFactory ::processEph(unsigned sfid, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    DEBUGTRACE_FUNCTION();
    ...
    if (sfid == 1)
    {
        if (processHea)
        {
            ...
            dynamic_cast<GPSLNavHealth *>(p1.get())->svHealth = navIn->asUnsignedLong(esbHea, enbHea, escHea);
    ...
    eph->M0 = ephSF[esiM0]->asDoubleSemiCircles(esbM0m, enbM0m, esbM0l, enbM0l, escM0);
    eph->dn = ephSF[esidn]->asDoubleSemiCircles(esbdn, enbdn, escdn);
    // no dndot in GPS LNAV
    eph->ecc = ephSF[esiEcc]->asUnsignedDouble(esbEccm, enbEccm, esbEccl, enbEccl, escEcc);
    eph->Ahalf = ephSF[esiAhalf]->asUnsignedDouble(esbAhalfm, enbAhalfm, esbAhalfl, enbAhalfl, escAhalf);
    eph->A = eph->Ahalf * eph->Ahalf;
    // no Adot in GPS LNAV
    eph->OMEGA0 = ephSF[esiOMEGA0]->asDoubleSemiCircles(esbOMEGA0m, enbOMEGA0m, esbOMEGA0l, enbOMEGA0l, escOMEGA0);
    eph->i0 = ephSF[esii0]->asDoubleSemiCircles(esbi0m, enbi0m, esbi0l, enbi0l, esci0);
    eph->w = ephSF[esiw]->asDoubleSemiCircles(esbwm, enbwm, esbwl, enbwl, escw);
    eph->OMEGAdot = ephSF[esiOMEGAdot]->asDoubleSemiCircles(esbOMEGAdot, enbOMEGAdot, escOMEGAdot);
    ...
    eph->iodc = ephSF[esiIODC]->asUnsignedLong(esbIODCm, enbIODCm, esbIODCl, enbIODCl, escIODC);
    eph->iode = iode2; // we've already extracted it
    eph->fitIntFlag = ephSF[esiFitInt]->asUnsignedLong(esbFitInt, enbFitInt, escFitInt);
    eph->healthBits = ephSF[esiHea]->asUnsignedLong(esbHea, enbHea, escHea);
    eph->health = ((eph->healthBits == 0) ? SVHealth::Healthy : SVHealth::Unhealthy); // actually in OrbitDataKepler
    ...
    eph->codesL2 = static_cast<GPSLNavL2Codes>(ephSF[esiL2]->asUnsignedLong(esbL2, enbL2, escL2));
...
bool PNBGPSLNavDataFactory ::processAlmOrb(unsigned long prn, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    if (processHea)
    {
        ...
        dynamic_cast<GPSLNavHealth *>(p1.get())->svHealth = navIn->asUnsignedLong(asbHea, anbHea, ascHea);
    ...
    if ((fullWNaMap.find(xmitSat.id) != fullWNaMap.end()) && (fullWNaMap[xmitSat.id].sow == alm->toa))
    {
...
bool PNBGPSLNavDataFactory ::processSVID51(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    // No checks for correct svid, just assume that the input
    // data has already been checked (it will have been by
    // addData).
    ...
    for (unsigned prn = startPRN, bit = 90; prn <= endPRN; prn += 4, bit += 30)
    {
        NavDataPtr p1 = std::make_shared<GPSLNavHealth>();
        ...
        p1->timeStamp = navIn->getTransmitTime();
        p1->signal = NavMessageID(NavSatelliteID(prn + 0, xmitSat, oid, navid), NavMessageType::Health);
        dynamic_cast<GPSLNavHealth *>(p1.get())->svHealth = navIn->asUnsignedLong(bit + 0, 6, 1);
        // cerr << "add LNAV page 51 health" << endl;
        navOut.push_back(p1);
        if ((prn + 1) > endPRN)
            break;
        p2->timeStamp = navIn->getTransmitTime();
        p2->signal = NavMessageID(NavSatelliteID(prn + 1, xmitSat, oid, navid), NavMessageType::Health);
        dynamic_cast<GPSLNavHealth *>(p2.get())->svHealth = navIn->asUnsignedLong(bit + 6, 6, 1);
        // cerr << "add LNAV page 51 health" << endl;
        navOut.push_back(p2);
        if ((prn + 2) > endPRN)
            break;
        p3->timeStamp = navIn->getTransmitTime();
        p3->signal = NavMessageID(NavSatelliteID(prn + 2, xmitSat, oid, navid), NavMessageType::Health);
        dynamic_cast<GPSLNavHealth *>(p3.get())->svHealth = navIn->asUnsignedLong(bit + 12, 6, 1);
        // cerr << "add LNAV page 51 health" << endl;
        navOut.push_back(p3);
        if ((prn + 3) > endPRN)
            break;
        p4->timeStamp = navIn->getTransmitTime();
        p4->signal = NavMessageID(NavSatelliteID(prn + 3, xmitSat, oid, navid), NavMessageType::Health);
        dynamic_cast<GPSLNavHealth *>(p4.get())->svHealth = navIn->asUnsignedLong(bit + 18, 6, 1);
        // cerr << "add LNAV page 51 health" << endl;
        navOut.push_back(p4);
    }
    return true;
...
bool PNBGPSLNavDataFactory ::processSVID63(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    if (processSys)
    {
        for (unsigned prn = 1; prn <= 32; ++prn)
        {
            ...
            configPtr->signal = NavMessageID{NavSatelliteID{prn, xmitSat, oid, navid}, NavMessageType::System};
            configPtr->antispoofOn = navIn->asBool(30 * word + bitInWord);
            configPtr->svConfig = navIn->asUnsignedLong(30 * word + bitInWord + 1, 3, 1);
    ...
    if (processHea)
    {
        for (unsigned prn = 25; prn <= 32; ++prn)
        {
            ...
            healthPtr->signal = NavMessageID{NavSatelliteID{prn, xmitSat, oid, navid}, NavMessageType::Health};
            healthPtr->svHealth = navIn->asUnsignedLong(30 * word + bitInWord, 6, 1);
...
bool PNBGPSLNavDataFactory ::processSVID56(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    // No checks for correct svid, just assume that the input
    // data has already been checked (it will have been by
    // addData).
...
void PNBGPSLNavDataFactory ::resetState()
{
    fullWNaMap.clear();
    almAcc.clear();
    ephAcc.clear();
