...
GLOCNavEph ::GLOCNavEph()
    : N4(-1), NT(-1), Mj(GLOCSatType::Unknown), PS(-1), tb(-1), EjE(-1), EjT(-1), RjE(GLOCRegime::Unknown),
      RjT(GLOCRegime::Unknown), FjE(-1), FjT(-1), clkBias(std::numeric_limits<double>::quiet_NaN()),
      freqBias(std::numeric_limits<double>::quiet_NaN()), driftRate(std::numeric_limits<double>::quiet_NaN()),
      tauc(std::numeric_limits<double>::quiet_NaN()), taucdot(std::numeric_limits<double>::quiet_NaN()),
      tauDelta(std::numeric_limits<double>::quiet_NaN()), tauGPS(std::numeric_limits<double>::quiet_NaN()), step(60.0)
{
...
bool GLOCNavEph ::validate() const
{
    bool rv = (header.validate() && header11.validate() && header12.validate());
    DEBUGTRACE("GLOCNavEph::validate returning " << rv);
    return rv;
}

bool GLOCNavEph ::getXvt(const CommonTime &when, Xvt &xvt, const ObsID &oid)
{
    DEBUGTRACE_FUNCTION();
    ...
    Vector<double> initialState(6), accel(3), k1(6), k2(6), k3(6), k4(6), tempRes(6), a1(3), a23(3), a4(3);
    ...
    while (!done)
    {
        ...
        initialState = initialState + (k1 / 6.0 + k2 / 3.0 + k3 / 3.0 + k4 / 6.0) * rkStep;
...
CommonTime GLOCNavEph ::getUserTime() const
{
    ...
    if ((header.xmit.getTimeSystem() != TimeSystem::Unknown) && (header.xmit > mr))
    {
        mr = header.xmit;
    }
    if ((header11.xmit.getTimeSystem() != TimeSystem::Unknown) && (header11.xmit > mr))
    {
        mr = header11.xmit;
    }
    if ((header12.xmit.getTimeSystem() != TimeSystem::Unknown) && (header12.xmit > mr))
    {
        mr = header12.xmit;
    }
    if ((ltdmp.header31.xmit.getTimeSystem() != TimeSystem::Unknown) && (ltdmp.header31.xmit > mr))
    {
        mr = ltdmp.header31.xmit;
    }
    if ((ltdmp.header32.xmit.getTimeSystem() != TimeSystem::Unknown) && (ltdmp.header32.xmit > mr))
    {
...
void GLOCNavEph ::dump(std::ostream &s, DumpDetail dl) const
{
    ...
    s << "****************************************************************"
      << "************" << endl
      << "GLONASS ORB/CLK (IMMEDIATE) PARAMETERS" << endl
      << endl
      << "SAT : " << signal.sat << endl
      << "      " << StringUtils::asString(Mj) << endl
      << endl;
    ...
    s << "           CLOCK PARAMETERS" << endl
      << endl
      << "Bias:       " << setw(16) << clkBias << " sec (τ)" << endl
      << "Drift:      " << setw(16) << freqBias << " sec/sec (γ)" << endl
      << "Drift rate: " << setw(16) << driftRate << " sec/(sec**2) (β)" << endl
      << endl
      << "           EPHEMERIS/CLOCK STATUS" << endl
      << endl
      << "                AGE  REGIME       ACCURACY (σ, m)" << endl
      << "Ephemeris:" << setw(9) << (unsigned)EjE << "  " << setw(10) << StringUtils::asString(RjE) << "  " << setw(8)
      << (int)FjE << " (" << setw(0) << setprecision(1) << fixed << factorToSigma(FjE) << ")" << endl
      << "Clock:    " << setw(9) << (unsigned)EjT << "  " << setw(10) << StringUtils::asString(RjT) << "  " << setw(8)
      << (int)FjT << " (" << setw(0) << setprecision(1) << fixed << factorToSigma(FjT) << ")" << endl
      << endl
      << scientific << setprecision(6) << "           ORBIT PARAMETERS" << endl
      << endl
      << "                           "
      << "X                  Y                  Z" << endl
      << "Position:   " << setw(16) << pos[0] << ",  " << setw(16) << pos[1] << ",  " << setw(16) << pos[2] << " km"
      << endl
      << "Velocity:   " << setw(16) << vel[0] << ",  " << setw(16) << vel[1] << ",  " << setw(16) << vel[2] << " km/sec"
      << endl
      << "Accel:      " << setw(16) << acc[0] << ",  " << setw(16) << acc[1] << ",  " << setw(16) << acc[2]
      << " km/sec**2" << endl
      << "Ant Offset: " << setw(16) << apcOffset[0] << ",  " << setw(16) << apcOffset[1] << ",  " << setw(16)
      << apcOffset[2] << endl
      << endl
      << "Code Bias:  " << setw(16) << tauDelta << " sec (Δτ, L3OCp time to L3OCd time)" << endl
      << "Time corr:  " << setw(16) << tauc << " sec  " << setw(16) << taucdot << " sec/sec (GLONASS to MT)" << endl
    ...
    s << "t_b:        " << setw(16) << tb << " seconds (MT)" << endl
      << "Health (H): " << setw(16) << StringUtils::asString(header.health) << endl
      << endl;
...
void GLOCNavEph ::dumpTerse(std::ostream &s) const
{
    string tform("%02H:%02M:%02S");
    string tform2("%02m/%02d/%4Y %03j %02H:%02M:%02S");
    s << "  " << setw(2) << signal.sat.id << "  " << printTime(beginFit, tform) << " ! " << printTime(Toe, tform2)
      << " ! " << printTime(endFit, tform) << "       " << header.dataInvalid << "     " << header.svUnhealthy << "  "
      << endl;
}

Vector<double> GLOCNavEph ::derivative(const Vector<double> &inState, const Vector<double> &accel,
                                       const Vector<double> &lt, bool simplified) const
{
    ...
    Vector<double> dxt({inState(1), gloAx, inState(3), gloAy, inState(5), gloAz});
    return dxt;
} // derivative()

double GLOCNavEph ::factorToSigma(int8_t factor)
{
    switch (factor)
    {
    case -15:
