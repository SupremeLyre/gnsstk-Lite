...
namespace gnsstk
{
NavDataFactoryWithStore ::NavDataFactoryWithStore()
{
...
bool NavDataFactoryWithStore ::find(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navOut,
...
bool NavDataFactoryWithStore ::findUser(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navData,
                                        SVHealth xmitHealth, NavValidityType valid)
{
    ...
    {
      public:
        FindMatches() : map(nullptr), finished(true)
        {
        }
        FindMatches(NavMap *theMap, const NavMap::iterator &theIt) : map(theMap), finished(false), it(theIt)
        {
        }
        NavMap *map;
        bool finished;
    ...
    if (nmid.isWild())
    {
        ...
        for (NavSatMap::iterator sati = dataIt->second.begin(); sati != dataIt->second.end(); sati++)
        {
            if (sati->first != nmid)
            {
            ...
            DEBUGTRACE("user time : " << gnsstk::printTime(nmi->second->getUserTime(), dts));
            DEBUGTRACE("(nmi != sati->second.end()) = " << (nmi != sati->second.end()));
            DEBUGTRACE("(nmi->second->getUserTime() > when) = " << (nmi->second->getUserTime() > when));
            while ((nmi != sati->second.end()) && (nmi->second->getUserTime() > when))
            {
                DEBUGTRACE("backing up (maybe)");
                nmi = (nmi == sati->second.begin() ? sati->second.end() : std::prev(nmi));
                if (nmi != sati->second.end())
                {
                    DEBUGTRACE("user time : " << gnsstk::printTime(nmi->second->getUserTime(), dts));
                }
    ...
    while (!done)
    {
        for (auto &imi : itList)
        {
            ...
            else if ((imi.it != imi.map->end()) && (imi.it->second->getUserTime() < mostRecent))
            {
                // Data is less recent than the most recent good data, so stop
                // processing this iterator.
                imi.finished = true;
            }
            else if (((imi.it != imi.map->end()) && (imi.it->second->getUserTime() > when)) ||
                     !validityCheck(imi.it, *imi.map, valid, xmitHealth, when))
            {
                DEBUGTRACE("not end, not right time");
                imi.it = (imi.it == imi.map->begin() ? imi.map->end() : std::prev(imi.it));
                done = false;
            }
            ...
            {
                DEBUGTRACE("Found something good at " << printTime(imi.it->first, dts));
...
bool NavDataFactoryWithStore ::findNearest(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navData,
                                           SVHealth xmitHealth, NavValidityType valid)
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("class: " << getClassName());
    ...
    class FindMatches
    {
      public:
         ...
         *   from lower_bound.
         */
        FindMatches(NavNearMap *theMap, const NavNearMap::iterator &theIt, const CommonTime &t)
    ...
    {
        DEBUGTRACE("wildcard search: " << nmid);
        for (NavNearSatMap::iterator sati = dataIt->second.begin(); sati != dataIt->second.end(); sati++)
        {
            if (sati->first != nmid)
                continue; // skip non matches
            DEBUGTRACE("matches " << sati->first);
    ...
    while (!done)
    {
        for (auto &imi : itList)
        {
            ...
            if ((imi.itGT != imi.map->end()) &&
                ((imi.itLT == imi.map->end()) || (fabs(imi.itGT->first - when) < fabs(imi.itLT->first - when))))
            {
            ...
            else
            {
                ...
                imi.itLT = (imi.itLT == imi.map->begin() ? imi.map->end() : std::prev(imi.itLT));
            }
        }
    }
    return false;
...
bool NavDataFactoryWithStore ::getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when,
                                         NavDataPtr &offset, SVHealth xmitHealth, NavValidityType valid)
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("class: " << getClassName());
    ...
    if (odi == offsetData.end())
    {
        if (DebugTrace::enabled)
        {
            ...
            {
                DEBUGTRACE("fwdKey=<" << gnsstk::StringUtils::asString(x.first.first) << ","
    ...
    while (!done)
    {
        ...
        else if (oemi->first > whenny)
        {
            ...
            oemi = (oemi == odi->second.begin() ? odi->second.end() : std::prev(oemi));
            DEBUGTRACE(printTime(oemi->first, "backed up to " + dts));
        }
        else
        {
            DEBUGTRACE("looking for acceptable data " << gnsstk::StringUtils::asString(valid) << " "
...
void NavDataFactoryWithStore ::edit(const CommonTime &fromTime, const CommonTime &toTime)
{
    ...
    for (auto mti = nearestData.begin(); mti != nearestData.end();)
    {
        ...
        for (auto sati = mti->second.begin(); sati != mti->second.end();)
        {
            ...
            for (auto cti = sati->second.begin(); cti != sati->second.end();)
            {
                ...
                for (auto ndpli = cti->second.begin(); ndpli != cti->second.end();)
                {
                    if (((*ndpli)->timeStamp < toTime) && ((*ndpli)->timeStamp >= fromTime))
                    {
    ...
    for (auto ocmi = offsetData.begin(); ocmi != offsetData.end();)
    {
        ...
        for (auto cti = ocmi->second.begin(); cti != ocmi->second.end();)
        {
            ...
            {
                if ((sati->second->timeStamp < toTime) && (sati->second->timeStamp >= fromTime))
                {
...
void NavDataFactoryWithStore ::edit(const CommonTime &fromTime, const CommonTime &toTime, const NavSatelliteID &satID)
{
    // edit transmit time storage
    for (auto mti = data.begin(); mti != data.end();)
    {
...
void NavDataFactoryWithStore ::edit(const CommonTime &fromTime, const CommonTime &toTime, const NavSignalID &signal)
{
    NavSatelliteID satMatch(signal);
    return edit(fromTime, toTime, satMatch);
}

void NavDataFactoryWithStore ::clear()
{
    data.clear();
    nearestData.clear();
    offsetData.clear();
...
bool NavDataFactoryWithStore ::addNavData(const NavDataPtr &nd, NavMessageMap &navMap, NavNearMessageMap &navNearMap,
                                          OffsetCvtMap &ofsMap)
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("class: " << getClassName());
    ...
    DEBUGTRACE("addNavData user = " << nd->getUserTime() << "  nearest = " << nd->getNearTime());
    ...
    if (satID.system != SatelliteSystem::Unknown)
    {
        ...
        {
            firstLastMap[satID] = std::pair<CommonTime, CommonTime>(nd->timeStamp, nd->timeStamp);
        }
        else
        {
            ...
            // anyway.
            CommonTime anyFirst(firstLastMap[satID].first), anyLast(firstLastMap[satID].second),
    ...
    navNearMap[nd->signal.messageType][nd->signal][nd->getNearTime()].push_back(nd);
...
bool NavDataFactoryWithStore ::updateInitialFinal(const CommonTime &begin, const CommonTime &end)
{
    if (((initialTime.getTimeSystem() != begin.getTimeSystem()) && (initialTime.getTimeSystem() != TimeSystem::Any)) ||
        ((finalTime.getTimeSystem() != end.getTimeSystem()) && (finalTime.getTimeSystem() != TimeSystem::Any)))
    {
        // different time systems, convert to UTC first.
        CommonTime t0(initialTime), t1(finalTime), f0(begin), f1(end);
        BasicTimeSystemConverter btsc;
        if ((t0.getTimeSystem() != TimeSystem::Any) && !t0.changeTimeSystem(TimeSystem::UTC, &btsc))
        {
            return false;
        }
        if ((t1.getTimeSystem() != TimeSystem::Any) && !t1.changeTimeSystem(TimeSystem::UTC, &btsc))
        {
            return false;
        }
        if ((f0.getTimeSystem() != TimeSystem::Any) && !f0.changeTimeSystem(TimeSystem::UTC, &btsc))
        {
            return false;
        }
        if ((f1.getTimeSystem() != TimeSystem::Any) && !f1.changeTimeSystem(TimeSystem::UTC, &btsc))
        {
            return false;
        }
...
size_t NavDataFactoryWithStore ::size() const
{
    size_t rv = 0;
...
size_t NavDataFactoryWithStore ::count(const NavMessageID &nmid) const
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("class: " << getClassName());
    size_t rv = 0;
    ...
    {
        if ((nmid.messageType != NavMessageType::Unknown) && (nmid.messageType != i))
        {
        ...
        for (const auto &sati : dataIt->second)
        {
            ...
            if (sati.first == key)
            {
                DEBUGTRACE("matches " << sati.second.size() << " x " << sati.first);
                rv += sati.second.size();
            }
            else
            {
...
size_t NavDataFactoryWithStore ::count(SatelliteSystem sys, NavMessageType nmt) const
{
    NavMessageID key(NavSatelliteID(gnsstk::SatID(gnsstk::SatelliteSystem::Unknown),
                                    gnsstk::SatID(gnsstk::SatelliteSystem::Unknown),
                                    gnsstk::ObsID(gnsstk::ObservationType::Any, gnsstk::CarrierBand::Any,
                                                  gnsstk::TrackingCode::Any, gnsstk::XmitAnt::Any),
...
size_t NavDataFactoryWithStore ::count(const SatID &satID, NavMessageType nmt) const
{
    NavMessageID key(NavSatelliteID(satID, gnsstk::SatID(gnsstk::SatelliteSystem::Unknown),
                                    gnsstk::ObsID(gnsstk::ObservationType::Any, gnsstk::CarrierBand::Any,
                                                  gnsstk::TrackingCode::Any, gnsstk::XmitAnt::Any),
                                    gnsstk::NavID(gnsstk::NavType::Any)),
                     nmt);
...
size_t NavDataFactoryWithStore ::count(NavMessageType nmt) const
{
    NavMessageID key(NavSatelliteID(gnsstk::SatID(gnsstk::SatelliteSystem::Unknown),
                                    gnsstk::SatID(gnsstk::SatelliteSystem::Unknown),
                                    gnsstk::ObsID(gnsstk::ObservationType::Any, gnsstk::CarrierBand::Any,
...
size_t NavDataFactoryWithStore ::numSignals() const
{
...
size_t NavDataFactoryWithStore ::numSatellites() const
{
...
bool NavDataFactoryWithStore ::validityCheck(const NavMap::iterator &ti, NavMap &nm, NavValidityType valid,
                                             SVHealth xmitHealth, const CommonTime &when)
{
...
bool NavDataFactoryWithStore ::validityCheck(const NavDataPtr &ndp, NavValidityType valid, SVHealth xmitHealth,
                                             const CommonTime &when)
{
    bool rv = true;
    // One last check for fit interval validity, but it only
...
bool NavDataFactoryWithStore ::matchHealth(NavData *ndp, SVHealth xmitHealth)
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("class: " << getClassName());
    bool rv = true;
    ...
    case SVHealth::Degraded:
        DEBUGTRACE("attempting to match " << gnsstk::StringUtils::asString(xmitHealth));
        ...
        if (!rvSet)
        {
            ...
            if (!find(nmid, ndp->timeStamp, heaPtr, SVHealth::Any, NavValidityType::Any, NavSearchOrder::User))
            {
            ...
            rv = (xmitHealth == hea->getHealth());
            DEBUGTRACE("transmit health " << (rv ? "matches" : "does not match"));
        }
        break;
    default:
...
NavSatelliteIDSet NavDataFactoryWithStore ::getAvailableSats(const CommonTime &fromTime, const CommonTime &toTime) const
{
...
NavSatelliteIDSet NavDataFactoryWithStore ::getAvailableSats(NavMessageType nmt, const CommonTime &fromTime,
...
std::set<SatID> NavDataFactoryWithStore ::getIndexSet(const CommonTime &fromTime, const CommonTime &toTime) const
{
    std::set<SatID> rv;
...
std::set<SatID> NavDataFactoryWithStore ::getIndexSet(NavMessageType nmt, const CommonTime &fromTime,
                                                      const CommonTime &toTime) const
{
    std::set<SatID> rv;
    auto nmmi = data.find(nmt);
...
NavMessageIDSet NavDataFactoryWithStore ::getAvailableMsgs(const CommonTime &fromTime, const CommonTime &toTime) const
{
    NavMessageIDSet rv;
    for (const auto &nmmi : data)
    {
...
const NavMap *NavDataFactoryWithStore ::getNavMap(const NavMessageID &nmid) const
{
...
void NavDataFactoryWithStore ::dump(std::ostream &s, DumpDetail dl) const
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("class: " << getClassName());
    ...
    for (const auto &nmmi : data)
    {
        for (const auto &nsami : nmmi.second)
        {
            ...
            case DumpDetail::OneLine:
                s << StringUtils::asString(nmmi.first) << " " << StringUtils::asString(nsami.first) << " "
            ...
            case DumpDetail::Terse:
                ...
                s << "  Map for " << StringUtils::asString(nmmi.first) << " " << StringUtils::asString(nsami.first)
                  << " has " << nsami.second.size() << " entries." << std::endl
            ...
            case DumpDetail::Brief:
                ...
                {
                    s << StringUtils::asString(nmmi.first) << " " << StringUtils::asString(nsami.first) << "   "
                      << printTime(cti.first, "%Y %2m %2d %02H:%02M:%04.1f") << std::endl;
                }
                break;
...
bool NavDataFactoryWithStore::UniqueTimeOffset ::operator()(const std::shared_ptr<StdNavTimeOffset> &left,
                                                            const std::shared_ptr<StdNavTimeOffset> &right) const
{
    if (left->refTime < right->refTime)
        return true;
    if (right->refTime < left->refTime)
        return false;
    if (left->a0 < right->a0)
        return true;
    if (right->a0 < left->a0)
