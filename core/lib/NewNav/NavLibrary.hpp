...
class NavLibrary
{
  public:
     ...
     *   to compute the Xvt. */
    bool getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, bool useAlm,
     ...
     * @return true if successful, false if no nav data was found
     *   to compute the Xvt. */
    bool getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, bool useAlm, const ObsID &oid,
                SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly,
                NavSearchOrder order = NavSearchOrder::User);
     ...
     * @return true if successful, false if no nav data was found
     *   to compute the Xvt. */
    bool getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, SVHealth xmitHealth = SVHealth::Any,
     ...
     * @return true if successful, false if no nav data was found
     *   to compute the Xvt. */
    bool getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, const ObsID &oid,
                SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly,
                NavSearchOrder order = NavSearchOrder::User);
    ...
    bool getHealth(const NavSatelliteID &sat, const CommonTime &when, SVHealth &healthOut,
                   SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly,
                   NavSearchOrder order = NavSearchOrder::User);
    ...
    bool getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when, NavDataPtr &navOut,
                   SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly);
     ...
     *   or invalid messages, or both.
     * @return true if an offset is available, false if not. */
    bool getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when, double &offset,
                   SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly);
     ...
     *   ionospheric data. */
    bool getIonoCorr(SatelliteSystem sys, const CommonTime &when, const Position &rxgeo, const Position &svgeo,
                     CarrierBand band, double &corrOut, NavType nt = NavType::Any);
    ...
    bool getIonoCorr(const SatID &sat, const CommonTime &when, const Position &rxgeo, CarrierBand band, double &corrOut,
                     NavType nt = NavType::Any, int freqOffs = 0, bool freqOffsWild = true);
    ...
    bool getISC(const SatID &sat, const ObsID &oid, const CommonTime &when, double &corrOut,
                SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly,
                NavSearchOrder order = NavSearchOrder::User);
    ...
    bool find(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navOut, SVHealth xmitHealth,
     ...
     *   data set (i.e. assuming the data is going to be a day's
     *   worth when it's actually several years. */
    bool isTypePresent(NavMessageType nmt, const NavSatelliteID &satID, const CommonTime &fromTime,
