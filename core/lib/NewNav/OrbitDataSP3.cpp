...
namespace gnsstk
{
OrbitDataSP3 ::OrbitDataSP3()
    : pos(0.0, 0.0, 0.0), posSig(0.0, 0.0, 0.0), vel(0.0, 0.0, 0.0), velSig(0.0, 0.0, 0.0), acc(0.0, 0.0, 0.0),
      accSig(0.0, 0.0, 0.0), clkBias(0.0), biasSig(0.0), clkDrift(0.0), driftSig(0.0), clkDrRate(0.0), drRateSig(0.0)
{
    signal.messageType = NavMessageType::Ephemeris;
}

OrbitDataSP3 ::OrbitDataSP3(double val)
    : pos(val, val, val), posSig(val, val, val), vel(val, val, val), velSig(val, val, val), acc(val, val, val),
      accSig(val, val, val), clkBias(val), biasSig(val), clkDrift(val), driftSig(val), clkDrRate(val), drRateSig(val)
{
    signal.messageType = NavMessageType::Ephemeris;
}

void OrbitDataSP3 ::copyXV(const OrbitDataSP3 &right)
{
    pos = right.pos;
    posSig = right.posSig;
    vel = right.vel;
...
void OrbitDataSP3 ::copyT(const OrbitDataSP3 &right)
{
...
bool OrbitDataSP3 ::getXvt(const CommonTime &when, Xvt &xvt, const ObsID &oid)
{
...
void OrbitDataSP3 ::dump(std::ostream &s, DumpDetail dl) const
{
    NavData::dump(s, dl);
    ...
    for (unsigned i = 0; i < acc.size(); i++)
        s << " " << acc[i];
    s << "} drift rate: " << clkDrRate << endl << "P sigma: {";
    for (unsigned i = 0; i < posSig.size(); i++)
        s << " " << posSig[i];
    s << "} bias sigma: " << biasSig << endl << "V sigma: {";
    for (unsigned i = 0; i < velSig.size(); i++)
        s << " " << velSig[i];
    s << "} drift sigma: " << driftSig << endl << "A sigma: {";
