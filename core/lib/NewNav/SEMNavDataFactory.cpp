...
namespace gnsstk
{
SEMNavDataFactory ::SEMNavDataFactory()
{
    supportedSignals.insert(NavSignalID(SatelliteSystem::GPS, CarrierBand::L1, TrackingCode::CA, NavType::GPSLNAV));
}

bool SEMNavDataFactory ::loadIntoMap(const std::string &filename, NavMessageMap &navMap, NavNearMessageMap &navNearMap,
                                     OffsetCvtMap &ofsMap)
{
...
bool SEMNavDataFactory ::process(const std::string &filename, NavDataFactoryCallback &cb)
{
    bool rv = true;
...
std::string SEMNavDataFactory ::getFactoryFormats() const
{
    if (procNavTypes.empty() || (procNavTypes.count(NavMessageType::Almanac) > 0) ||
        (procNavTypes.count(NavMessageType::Health) > 0) || (procNavTypes.count(NavMessageType::System) > 0))
    {
        return "SEM";
    }
    return "";
...
bool SEMNavDataFactory ::convertToOrbit(const SEMData &navIn, NavDataPtr &navOut)
{
    bool rv = true;
    GPSLNavAlm *gps;
    ...
    gps->health = ((navIn.SV_health == 0) ? SVHealth::Healthy : SVHealth::Unhealthy);
    // not in almanac data.
    gps->Cuc = 0.0;
    gps->Cus = 0.0;
    gps->Crc = 0.0;
...
bool SEMNavDataFactory ::convertToHealth(const SEMData &navIn, NavDataPtr &healthOut)
{
    bool rv = true;
    GPSLNavHealth *gps;
...
bool SEMNavDataFactory ::convertToSystem(const SEMData &navIn, NavDataPtr &systemOut)
{
...
void SEMNavDataFactory ::fillNavData(const SEMData &navIn, NavDataPtr &navOut)
{
    // NavData
