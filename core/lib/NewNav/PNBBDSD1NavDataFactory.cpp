...
bool PNBBDSD1NavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    if ((navIn->getNavID().navType != NavType::BeiDou_D1) || (navIn->getNumBits() != 300))
    {
    ...
    try
    {
        ...
        case 5:
            ...
            // TOA = navIn->asUnsignedDouble(asbtoa,anbtoa,asctoa);
            // cerr << "  FAKETOA=" << TOA;
            if (((pnum >= 1) && (pnum <= 6)) || ((pnum >= 11) && (pnum <= 23)))
            {
                rv = processAlm(sfid, pnum, navIn, navOut);
            }
            else if (pnum == 7)
...
bool PNBBDSD1NavDataFactory ::processAlm(unsigned long sfid, unsigned pnum, const PackedNavBitsPtr &navIn,
                                         NavDataPtrList &navOut)
{
    ...
    if (PNBNavDataFactory::processAlm || PNBNavDataFactory::processHea)
    {
        ...
        if ((sfid == 4) && (pnum >= 11) && (pnum <= 24))
        {
            ...
            amEpIDMap[key].amEpID = navIn->asUnsignedLong(asbAmEpID, anbAmEpID, ascAmEpID);
    ...
    if ((sfid == 5) && (pnum >= 11) && (pnum <= 23))
    {
        ...
        //      << "  key:" << key << "  expanded:"
        //      << amEpIDMap[key].isExpanded();
        if (((navIn->getTransmitTime() - amEpIDMap[key].t) != 6.0) || !amEpIDMap[key].isExpanded())
        {
...
bool PNBBDSD1NavDataFactory ::processEph(unsigned long sfid, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if ((sfid < 1) || (sfid > 3))
        return false;
    ...
    if (sfid == 1)
    {
        ...
        if (PNBNavDataFactory::processIono)
        {
            ...
            iono->fraID = sfid;
            iono->sow = navIn->asSignedDouble(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
            iono->alpha[0] = navIn->asSignedDouble(esbAlpha0, enbAlpha0, escAlpha0);
            ...
            iono->alpha[3] = navIn->asSignedDouble(esbAlpha3, enbAlpha3, escAlpha3);
            iono->beta[0] = navIn->asSignedDouble(esbBeta0m, enbBeta0m, esbBeta0l, enbBeta0l, escBeta0);
            iono->beta[1] = navIn->asSignedDouble(esbBeta1, enbBeta1, escBeta1);
            iono->beta[2] = navIn->asSignedDouble(esbBeta2, enbBeta2, escBeta2);
            iono->beta[3] = navIn->asSignedDouble(esbBeta3m, enbBeta3m, esbBeta3l, enbBeta3l, escBeta3);
        ...
        if (PNBNavDataFactory::processISC)
        {
            ...
            isc->fraID = sfid;
            isc->sow = navIn->asSignedDouble(fsbSOWm, fnbSOWm, fsbSOWl, fnbSOWl, fscSOW);
            isc->tgd1 = sf * navIn->asSignedDouble(esbTGD1, enbTGD1, escTGD1);
            isc->tgd2 = sf * navIn->asSignedDouble(esbTGD2m, enbTGD2m, esbTGD2l, enbTGD2l, escTGD2);
    ...
    if (!ephSF[sf1] || !ephSF[sf2] || !ephSF[sf3] || (ephSF[sf1]->getNumBits() != 300) ||
        (ephSF[sf2]->getNumBits() != 300) || (ephSF[sf3]->getNumBits() != 300))
    {
    ...
    // health is set below
    eph->Cuc = ephSF[esiCucm]->asSignedDouble(esbCucm, enbCucm, esbCucl, enbCucl, escCuc);
    // cout << "Cuc bits: " << hex
    ...
    eph->Crs = ephSF[esiCrsm]->asSignedDouble(esbCrsm, enbCrsm, esbCrsl, enbCrsl, escCrs);
    eph->Cic = ephSF[esiCicm]->asSignedDouble(esbCicm, enbCicm, esbCicl, enbCicl, escCic);
    eph->Cis = ephSF[esiCism]->asSignedDouble(esbCism, enbCism, esbCisl, enbCisl, escCis);
    eph->M0 = ephSF[esiM0m]->asDoubleSemiCircles(esbM0m, enbM0m, esbM0l, enbM0l, escM0);
    eph->dn = ephSF[esidnm]->asDoubleSemiCircles(esbdnm, enbdnm, esbdnl, enbdnl, escdn);
    // no dndot in BDS D1NAV
    eph->ecc = ephSF[esiEccm]->asUnsignedDouble(esbEccm, enbEccm, esbEccl, enbEccl, escEcc);
    eph->Ahalf = ephSF[esiAhalfm]->asUnsignedDouble(esbAhalfm, enbAhalfm, esbAhalfl, enbAhalfl, escAhalf);
    eph->A = eph->Ahalf * eph->Ahalf;
    // no Adot in BDS D1NAV
    eph->OMEGA0 = ephSF[esiOMEGA0m]->asDoubleSemiCircles(esbOMEGA0m, enbOMEGA0m, esbOMEGA0l, enbOMEGA0l, escOMEGA0);
    eph->i0 = ephSF[esii0m]->asDoubleSemiCircles(esbi0m, enbi0m, esbi0l, enbi0l, esci0);
    eph->w = ephSF[esiwm]->asDoubleSemiCircles(esbwm, enbwm, esbwl, enbwl, escw);
    eph->OMEGAdot =
        ephSF[esiOMEGAdotm]->asDoubleSemiCircles(esbOMEGAdotm, enbOMEGAdotm, esbOMEGAdotl, enbOMEGAdotl, escOMEGAdot);
    ...
    eph->af1 = ephSF[esia1m]->asSignedDouble(esba1m, enba1m, esba1l, enba1l, esca1);
    ...
    eph->tgd2 = sf * ephSF[esiTGD2m]->asSignedDouble(esbTGD2m, enbTGD2m, esbTGD2l, enbTGD2l, escTGD2);
...
bool PNBBDSD1NavDataFactory ::processSF5Pg7(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
...
bool PNBBDSD1NavDataFactory ::processSF5Pg8(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processHea || PNBNavDataFactory::processAlm)
    {
...
bool PNBBDSD1NavDataFactory ::processSF5Pg9(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processTim)
    {
        ...
        ref.sow = 0;
        // BDT-GPS time offset
        std::shared_ptr<BDSD1NavTimeOffset> gps = std::make_shared<BDSD1NavTimeOffset>();
        ...
        if (!factControl.bdsTimeZZfilt || (gps->a0 != 0.0) || (gps->a1 != 0.0))
        {
            navOut.push_back(gps);
        }

        // BDT-Galileo time offset
        std::shared_ptr<BDSD1NavTimeOffset> gal = std::make_shared<BDSD1NavTimeOffset>();
        ...
        gal->refTime = ref;
        // cerr << "add D1NAV time offset" << endl;
        if (!factControl.bdsTimeZZfilt || (gal->a0 != 0.0) || (gal->a1 != 0.0))
        {
            navOut.push_back(gal);
        }

        // BDT-GLONASS time offset
        std::shared_ptr<BDSD1NavTimeOffset> glo = std::make_shared<BDSD1NavTimeOffset>();
        ...
        glo->refTime = ref;
        // cerr << "add D1NAV time offset" << endl;
        if (!factControl.bdsTimeZZfilt || (glo->a0 != 0.0) || (glo->a1 != 0.0))
        {
...
bool PNBBDSD1NavDataFactory ::processSF5Pg10(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processTim)
    {
        std::shared_ptr<BDSD1NavTimeOffset> to = std::make_shared<BDSD1NavTimeOffset>();
...
bool PNBBDSD1NavDataFactory ::processSF5Pg24(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (PNBNavDataFactory::processHea || PNBNavDataFactory::processAlm)
    {
        unsigned o = 0;
...
void PNBBDSD1NavDataFactory ::makeHealth(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned long subjID,
...
bool PNBBDSD1NavDataFactory ::isAlmDefault(const PackedNavBitsPtr &navIn)
{
    // Create a PNB with the same metadata, but all 0s, which is
    // how the BeiDou default pages appear (except words 1-2
    // which we ignore).
    static const PackedNavBits defPage(navIn->getsatSys(), navIn->getobsID(), navIn->getNavID(), navIn->getRxID(),
...
void PNBBDSD1NavDataFactory ::resetState()
{
    fullWNaMap.clear();
    almAcc.clear();
    ephAcc.clear();
...
void PNBBDSD1NavDataFactory ::finishAlm(bool fromWNa, const NavSatelliteID &key, NavDataPtrList &navOut)
{
...
bool PNBBDSD1NavDataFactory ::finishAlm(PNBBDSD1NavDataFactory::AlmPtr &alm, bool fromWNa, const NavSatelliteID &key,
                                        NavDataPtrList &navOut)
{
...
void PNBBDSD1NavDataFactory ::dumpState(std::ostream &s) const
{
