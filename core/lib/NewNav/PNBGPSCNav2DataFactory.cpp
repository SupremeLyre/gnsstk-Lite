...
namespace gnsstk
{
bool PNBGPSCNav2DataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    ...
    try
    {
        ...
        case nnbSF3:
            ...
            break;
        case nnbComplete:
            rv = processEph(navIn, navOut, nnbSF1) && processSF3(navIn, navOut, nnbSF1 + nnbSF2) &&
...
bool PNBGPSCNav2DataFactory ::processEph(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned offset)
{
    ...
    if (processHea)
    {
        ...
        p1->signal = NavMessageID(
            NavSatelliteID(prn, prn, navIn->getsatSys().system, CarrierBand::L1, TrackingCode::L1CD, NavType::GPSCNAV2),
            NavMessageType::Health);
        dynamic_cast<GPSCNav2Health *>(p1.get())->health = navIn->asBool(offset + esbHeaL1C);
    ...
    eph->dndot = navIn->asDoubleSemiCircles(offset + esbdn0dot, enbdn0dot, escdn0dot);
    eph->ecc = navIn->asUnsignedDouble(offset + esbEcc, enbEcc, escEcc);
    eph->deltaA = navIn->asSignedDouble(offset + esbdA, enbdA, escdA);
    eph->dOMEGAdot = navIn->asDoubleSemiCircles(offset + esbdOMEGAdot, enbdOMEGAdot, escdOMEGAdot);
    ...
    eph->OMEGA0 = navIn->asDoubleSemiCircles(offset + esbOMEGA0, enbOMEGA0, escOMEGA0);
    ...
    eph->uraNED1 = navIn->asUnsignedLong(offset + esbURAned1, enbURAned1, escURAned1);
    eph->uraNED2 = navIn->asUnsignedLong(offset + esbURAned2, enbURAned2, escURAned2);
    eph->tgd = InterSigCorr::getGPSISC(navIn, offset + esbTGD);
    eph->iscL1CP = navIn->asSignedDouble(offset + esbISCL1CP, enbISCL1CP, escISCL1CP);
    eph->iscL1CD = navIn->asSignedDouble(offset + esbISCL1CD, enbISCL1CD, escISCL1CD);
...
bool PNBGPSCNav2DataFactory ::processSF3(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned offset)
{
    unsigned long pageNum = navIn->asUnsignedLong(offset + asbPage, anbPage, ascPage);
    switch (pageNum)
    {
    case npgUTC:
...
bool PNBGPSCNav2DataFactory ::processAlmOrb(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned offset)
{
    unsigned long sprn = navIn->asUnsignedLong(offset + asbPRNa, anbPRNa, ascPRNa);
    ...
    if (processHea)
    {
        ...
        p1L1->timeStamp = p1L2->timeStamp = p1L5->timeStamp = getSF3Time(navIn->getTransmitTime());
         ...
         * have CNAV2. */
        p1L1->signal = NavMessageID(NavSatelliteID(subjSat, xmitSat, navIn->getobsID(), navIn->getNavID()),
                                    NavMessageType::Health);
        p1L2->signal = NavMessageID(NavSatelliteID(subjSat, xmitSat,
                                                   ObsID(ObservationType::NavMsg, CarrierBand::L2, TrackingCode::L2CM),
                                                   NavID(NavType::GPSCNAVL2)),
                                    NavMessageType::Health);
        p1L5->signal = NavMessageID(NavSatelliteID(subjSat, xmitSat,
                                                   ObsID(ObservationType::NavMsg, CarrierBand::L5, TrackingCode::L5I),
                                                   NavID(NavType::GPSCNAVL5)),
                                    NavMessageType::Health);
        dynamic_cast<GPSCNav2Health *>(p1L1.get())->health = navIn->asBool(offset + asbHeaL1);
        dynamic_cast<GPSCNav2Health *>(p1L2.get())->health = navIn->asBool(offset + asbHeaL2);
        dynamic_cast<GPSCNav2Health *>(p1L5.get())->health = navIn->asBool(offset + asbHeaL5);
    ...
    alm->OMEGA0 = navIn->asDoubleSemiCircles(offset + asbOMEGA0, anbOMEGA0, ascOMEGA0);
    // i0 is set below
    alm->w = navIn->asDoubleSemiCircles(offset + asbw, anbw, ascw);
    alm->OMEGAdot = navIn->asDoubleSemiCircles(offset + asbOMEGAdot, anbOMEGAdot, ascOMEGAdot);
    ...
    alm->health = ((alm->healthL1 == false) ? SVHealth::Healthy : SVHealth::Unhealthy);
...
bool PNBGPSCNav2DataFactory ::processUTCIono(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned offset)
{
    if (PNBNavDataFactory::processIono)
    {
        ...
        iono->alpha[0] = navIn->asSignedDouble(offset + csbAlpha0, cnbAlpha0, cscAlpha0);
        iono->alpha[1] = navIn->asSignedDouble(offset + csbAlpha1, cnbAlpha1, cscAlpha1);
        iono->alpha[2] = navIn->asSignedDouble(offset + csbAlpha2, cnbAlpha2, cscAlpha2);
        iono->alpha[3] = navIn->asSignedDouble(offset + csbAlpha3, cnbAlpha3, cscAlpha3);
        iono->beta[0] = navIn->asSignedDouble(offset + csbBeta0, cnbBeta0, cscBeta0);
        iono->beta[1] = navIn->asSignedDouble(offset + csbBeta1, cnbBeta1, cscBeta1);
        iono->beta[2] = navIn->asSignedDouble(offset + csbBeta2, cnbBeta2, cscBeta2);
        iono->beta[3] = navIn->asSignedDouble(offset + csbBeta3, cnbBeta3, cscBeta3);
        // GPSCNav2Iono
        navOut.push_back(p1);
    }
    ...
    GPSCNav2TimeOffset *to = dynamic_cast<GPSCNav2TimeOffset *>(p0.get());
...
bool PNBGPSCNav2DataFactory ::processGGTOEOP(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, unsigned offset)
{
    if (!PNBNavDataFactory::processTim)
    {
        // User doesn't want time offset data so don't do any processing.
...
bool PNBGPSCNav2DataFactory ::processISC(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (!PNBNavDataFactory::processISC)
        return true;
    NavSatelliteID nsid(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    ...
    case nnbComplete:
        ...
        if (isc->haveSF3 && (navIn->asUnsignedLong(asbPage, anbPage, ascPage) != 1))
        {
...
PNBGPSCNav2DataFactory::GPSCNav2ISCPtr PNBGPSCNav2DataFactory ::getISCObj(const PackedNavBitsPtr &navIn,
...
CommonTime PNBGPSCNav2DataFactory ::getSF2Time(const CommonTime &timestamp)
{
...
CommonTime PNBGPSCNav2DataFactory ::getSF3Time(const CommonTime &timestamp)
{
    // Convert to YDS which is easier to manipulate since it's
    // evenly divisible by 18 seconds, the period of a CNAV-2
    // frame.
