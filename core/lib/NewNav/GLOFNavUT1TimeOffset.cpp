...
namespace gnsstk
{
GLOFNavUT1TimeOffset ::GLOFNavUT1TimeOffset()
    : tauc(std::numeric_limits<double>::quiet_NaN()), B1(std::numeric_limits<double>::quiet_NaN()),
      B2(std::numeric_limits<double>::quiet_NaN()), KP(-1), NT(-1), NA(-1)
{
    msgLenSec = 2.0;
    weekFmt = "";
}

void GLOFNavUT1TimeOffset ::dump(std::ostream &s, DumpDetail dl) const
{
...
void GLOFNavUT1TimeOffset ::dumpTerse(ostream &s) const
{
...
bool GLOFNavUT1TimeOffset ::getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when,
                                      double &offset) const
{
...
TimeCvtSet GLOFNavUT1TimeOffset ::getConversions() const
{
...
bool GLOFNavUT1TimeOffset ::validate() const
{
