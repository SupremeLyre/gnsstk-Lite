...
namespace gnsstk
{
GalINavEph ::GalINavEph()
    : bgdE5aE1(0.0), bgdE5bE1(0.0), sisaIndex(255), svid(0), iodnav1(0), iodnav2(0), iodnav3(0), iodnav4(0),
      hsE5b(GalHealthStatus::Unknown), hsE1B(GalHealthStatus::Unknown), dvsE5b(GalDataValid::Unknown),
...
bool GalINavEph ::validate() const
{
    return true;
}

CommonTime GalINavEph ::getUserTime() const
{
...
void GalINavEph ::fixFit()
{
    beginFit = xmitTime;
    endFit = Toe + (3600 * 4); // four hours
}

void GalINavEph ::dumpSVStatus(std::ostream &s) const
{
    ...
    s << "           PAGE OVERHEAD" << endl
      ...
      << printTime(xmit5, "Word 5 " + poFmt) << endl
      << endl
      << "           SV STATUS" << endl
      << endl
