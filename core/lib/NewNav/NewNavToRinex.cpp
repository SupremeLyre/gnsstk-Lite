...
namespace gnsstk
{
bool NewNavToRinex ::translate(const NavDataPtrList &data, HealthGetter &healthGet)
{
    DEBUGTRACE_FUNCTION();
    bool rv = false, tmp;
...
bool NewNavToRinex ::write(const std::string &filename)
{
    DEBUGTRACE_FUNCTION();
    using namespace std;
...
bool NewNavToRinex ::fillHeader(const NavDataPtr &ndp, Rinex3NavHeader &h)
{
    DEBUGTRACE_FUNCTION();
    bool rv = false;
    ...
    case NavMessageType::Iono:
        kiPtr = std::dynamic_pointer_cast<KlobucharIonoNavData>(ndp);
        if (kiPtr)
        {
        ...
        {
            neqiPtr = std::dynamic_pointer_cast<NeQuickIonoNavData>(ndp);
            if (neqiPtr)
            {
...
bool NewNavToRinex ::fillData(const NavDataPtr &ndp, Rinex3NavData &rnd, HealthGetter &healthGet)
{
    DEBUGTRACE_FUNCTION();
    if (!ndp)
    {
    ...
    if (!fillDataGPSLNav(ndp, rnd) && !fillDataGalFNav(ndp, rnd, healthGet) && !fillDataGalINav(ndp, rnd, healthGet) &&
        !fillDataBDSD1Nav(ndp, rnd) && !fillDataBDSD2Nav(ndp, rnd) && !fillDataGLOFNav(ndp, rnd))
    {
...
bool NewNavToRinex ::fillDataGPSLNav(const NavDataPtr &ndp, Rinex3NavData &rnd)
{
    DEBUGTRACE_FUNCTION();
    ...
    std::shared_ptr<OrbitDataKepler> odk = std::dynamic_pointer_cast<OrbitDataKepler>(ndp);
    if (!odk)
    {
        DEBUGTRACE("not an OrbitDataKepler");
        return false;
    }
    std::shared_ptr<GPSLNavEph> eph = std::dynamic_pointer_cast<GPSLNavEph>(ndp);
    if (!eph)
    {
...
bool NewNavToRinex ::fillDataGalFNav(const NavDataPtr &ndp, Rinex3NavData &rnd, HealthGetter &healthGet)
{
    DEBUGTRACE_FUNCTION();
    ...
    std::shared_ptr<GalFNavEph> eph = std::dynamic_pointer_cast<GalFNavEph>(ndp);
    if (!eph)
    {
    ...
    GPSWeekSecond xmit(eph->timeStamp);
    rnd.xmitTime = xmit.sow;
    DEBUGTRACE("GAL F/Nav week number=" << xmit.week << printTime(eph->timeStamp, "  date=%Y/%02m/%02d"));
    rnd.weeknum = xmit.week;
    rnd.IODnav = eph->iodnav1;
    rnd.datasources = 0;
    if ((eph->signal.obs.band == CarrierBand::L5) && (eph->signal.obs.code == TrackingCode::E5aI))
    {
...
bool NewNavToRinex ::fillDataGalINav(const NavDataPtr &ndp, Rinex3NavData &rnd, HealthGetter &healthGet)
{
    DEBUGTRACE_FUNCTION();
    ...
    std::shared_ptr<GalINavEph> eph = std::dynamic_pointer_cast<GalINavEph>(ndp);
    if (!eph)
    {
    ...
    GPSWeekSecond xmit(eph->timeStamp);
    rnd.xmitTime = xmit.sow;
    DEBUGTRACE("GAL I/Nav week number=" << xmit.week << printTime(eph->timeStamp, "  date=%Y/%02m/%02d"));
    rnd.weeknum = xmit.week;
    rnd.IODnav = eph->iodnav1;
    rnd.datasources = 0;
    if ((eph->signal.obs.band == CarrierBand::L1) && (eph->signal.obs.code == TrackingCode::E1B))
    {
        rnd.datasources |= 0x01;
    }
    else if ((eph->signal.obs.band == CarrierBand::E5b) && (eph->signal.obs.code == TrackingCode::E5bI))
    {
...
bool NewNavToRinex ::fillDataBDSD1Nav(const NavDataPtr &ndp, Rinex3NavData &rnd)
{
    DEBUGTRACE_FUNCTION();
    ...
    std::shared_ptr<BDSD1NavEph> eph = std::dynamic_pointer_cast<BDSD1NavEph>(ndp);
    if (!eph)
    {
...
bool NewNavToRinex ::fillDataBDSD2Nav(const NavDataPtr &ndp, Rinex3NavData &rnd)
{
    DEBUGTRACE_FUNCTION();
    ...
    std::shared_ptr<BDSD2NavEph> eph = std::dynamic_pointer_cast<BDSD2NavEph>(ndp);
    if (!eph)
    {
...
bool NewNavToRinex ::fillDataGLOFNav(const NavDataPtr &ndp, Rinex3NavData &rnd)
{
    DEBUGTRACE_FUNCTION();
    ...
    std::shared_ptr<GLOFNavEph> eph = std::dynamic_pointer_cast<GLOFNavEph>(ndp);
    if (!eph)
    {
...
bool NewNavToRinex ::fillDataKepler(const std::shared_ptr<OrbitDataKepler> &odk, Rinex3NavData &rnd)
{
    DEBUGTRACE_FUNCTION();
    if (!odk)
    {
...
bool NewNavToRinex ::fillTimeSystemCorrection(const std::shared_ptr<StdNavTimeOffset> &toffs, TimeSystemCorrection &tsc)
{
    DEBUGTRACE_FUNCTION();
    if (!TimeSystemCorrection::convertTimeSystemToCorrType(toffs->src, toffs->tgt, tsc.type))
    {
        return false;
    }
    tsc.frTS = toffs->src;
...
bool HealthGetter ::getHealth(const NavSatelliteID &nsid, const CommonTime &when, NavDataPtr &healthOut)
{
...
NavLibraryHealthGetter ::NavLibraryHealthGetter(NavLibrary &nl)
    : navLib(nl), xmitHealth(SVHealth::Any), valid(NavValidityType::ValidOnly), order(NavSearchOrder::User)
{
}

bool NavLibraryHealthGetter ::getHealth(const NavSatelliteID &nsid, const CommonTime &when, NavDataPtr &healthOut)
{
