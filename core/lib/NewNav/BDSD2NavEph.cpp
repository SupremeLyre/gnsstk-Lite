...
namespace gnsstk
{
BDSD2NavEph ::BDSD2NavEph()
    : satH1(true), aodc(0xff), aode(0xff), uraIndex(15), tgd1(std::numeric_limits<double>::quiet_NaN()),
      tgd2(std::numeric_limits<double>::quiet_NaN())
{
    signal.messageType = NavMessageType::Ephemeris;
}

bool BDSD2NavEph ::getXvt(const CommonTime &when, Xvt &xvt, const ObsID &oid)
{
    ...
    ANLON = OMEGA0 + OMEGAdot * elapte - ell.angVelocity() * ToeSOW;
    ...
    drv = A * lecc * dek * sinea + 2.e0 * dlk * (Crs * c2al - Crc * s2al) + Adot * G;
    ...
    dIntPos(0, 0) =
        -xip * san * OMEGAdot + dxp * can - yip * (cinc * can * OMEGAdot - sinc * san * div) - dyp * cinc * san;
    dIntPos(1, 0) =
        xip * can * OMEGAdot + dxp * san - yip * (cinc * san * OMEGAdot + sinc * can * div) + dyp * cinc * can;
    dIntPos(2, 0) = yip * cinc * div + dyp * sinc;

    gnsstk::Matrix<double> vresult(3, 1);
    vresult = matZ * matX * dIntPos + dmatZ * matX * inertialPos;
...
bool BDSD2NavEph ::validate() const
{
    return BDSD2NavData::validate();
}

CommonTime BDSD2NavEph ::getUserTime() const
{
...
void BDSD2NavEph ::dumpSVStatus(std::ostream &s) const
{
    ...
    s << "           SV STATUS" << endl
      << endl
      << "Health bit (SatH1)  :    0x" << hex << (unsigned)satH1 << dec << endl
      << "AODC                : " << setw(6) << getAOD(aodc) << " hours (" << (unsigned)aodc << ")" << endl
      << "AODE                : " << setw(6) << getAOD(aode) << " hours (" << (unsigned)aode << ")" << endl
      << "URA index           : " << setw(6) << (unsigned)uraIndex << endl
      << "URA (nominal)       : " << setw(6) << fixed << SV_ACCURACY_GPS_NOMINAL_INDEX[uraIndex] << " m" << endl
      << "Health              : " << setw(9) << gnsstk::StringUtils::asString(health) << endl
      << "Tgd1                : " << setw(13) << setprecision(6) << scientific << tgd1 << " sec" << endl
      << "Tgd2                : " << setw(13) << setprecision(6) << scientific << tgd2 << " sec" << endl
      << endl
...
unsigned BDSD2NavEph ::getAOD(uint8_t aod)
{
    if (aod < 25)
        return aod;
    if (aod < 32)
