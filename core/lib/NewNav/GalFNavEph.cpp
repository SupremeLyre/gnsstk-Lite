...
namespace gnsstk
{
GalFNavEph ::GalFNavEph()
    : bgdE5aE1(0.0), sisaIndex(255), svid(0), iodnav1(0), iodnav2(0), iodnav3(0), iodnav4(0),
      hsE5a(GalHealthStatus::Unknown), dvsE5a(GalDataValid::Unknown), wn1(0), tow1(0), wn2(0), tow2(0), wn3(0), tow3(0),
      tow4(0)
{
...
bool GalFNavEph ::validate() const
{
    return true;
}

CommonTime GalFNavEph ::getUserTime() const
{
...
void GalFNavEph ::fixFit()
{
    beginFit = xmitTime;
    endFit = Toe + 14400; // four hours
}

void GalFNavEph ::dumpSVStatus(std::ostream &s) const
{
    ...
    const std::string poFmt("TOW :  " + weekFmt + "  %6.0g  %3a-%1w:%02H:%02M:%02S");
    s << "           PAGE OVERHEAD" << endl
      << endl
      << "              Week(10bt)     SOW    DOW:HH:MM:SS      IOD_NAV" << endl
      << printTime(xmitTime, "Page 1 " + poFmt) << setw(13) << iodnav1 << endl
      << printTime(xmit2, "Page 2 " + poFmt) << setw(13) << iodnav2 << endl
      << printTime(xmit3, "Page 3 " + poFmt) << setw(13) << iodnav3 << endl
      << printTime(xmit4, "Page 4 " + poFmt) << setw(13) << iodnav4 << endl
      << endl
      << "           SV STATUS" << endl
      << endl
      ...
      << setprecision(6) << "Bgd(E1-E5a)         : " << setw(13) << bgdE5aE1 << " sec" << endl;
    s.flags(oldFlags);
}
} // namespace gnsstk
