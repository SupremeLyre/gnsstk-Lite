...
bool MultiFormatNavDataFactory ::find(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navOut,
...
bool MultiFormatNavDataFactory ::getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when,
...
void MultiFormatNavDataFactory ::edit(const CommonTime &fromTime, const CommonTime &toTime)
{
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(*myFactories))
    {
...
void MultiFormatNavDataFactory ::edit(const CommonTime &fromTime, const CommonTime &toTime, const NavSatelliteID &satID)
{
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(*myFactories))
    {
...
void MultiFormatNavDataFactory ::edit(const CommonTime &fromTime, const CommonTime &toTime, const NavSignalID &signal)
{
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(*myFactories))
    {
...
void MultiFormatNavDataFactory ::clear()
{
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(*myFactories))
    {
        fi.second->clear();
...
CommonTime MultiFormatNavDataFactory ::getInitialTime() const
{
    ...
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(*myFactories))
    {
        ...
        if ((rv.getTimeSystem() == TimeSystem::Any) || (t.getTimeSystem() == TimeSystem::Any) ||
...
CommonTime MultiFormatNavDataFactory ::getFinalTime() const
{
    // always use basic time system converter because
...
NavSatelliteIDSet MultiFormatNavDataFactory ::getAvailableSats(const CommonTime &fromTime,
...
NavSatelliteIDSet MultiFormatNavDataFactory ::getAvailableSats(NavMessageType nmt, const CommonTime &fromTime,
                                                               const CommonTime &toTime) const
{
    NavSatelliteIDSet rv, tmp;
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(*myFactories))
...
NavMessageIDSet MultiFormatNavDataFactory ::getAvailableMsgs(const CommonTime &fromTime, const CommonTime &toTime) const
{
...
bool MultiFormatNavDataFactory ::isPresent(const NavMessageID &nmid, const CommonTime &fromTime,
...
size_t MultiFormatNavDataFactory ::size() const
{
    ...
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(*myFactories))
    {
        ...
        NavDataFactoryWithStore *ndfs = dynamic_cast<NavDataFactoryWithStore *>(ndfp);
        if (ndfs != nullptr)
        {
            rv += ndfs->size();
        }
...
size_t MultiFormatNavDataFactory ::count(const NavMessageID &nmid) const
{
    // this one is easy, it's just the sum of each individual
...
size_t MultiFormatNavDataFactory ::numSignals() const
{
...
size_t MultiFormatNavDataFactory ::numSatellites() const
{
...
void MultiFormatNavDataFactory ::setValidityFilter(NavValidityType nvt)
{
...
void MultiFormatNavDataFactory ::setTypeFilter(const NavMessageTypeSet &nmts)
{
...
void MultiFormatNavDataFactory ::clearTypeFilter()
{
...
void MultiFormatNavDataFactory ::addTypeFilter(NavMessageType nmt)
{
...
bool MultiFormatNavDataFactory ::addFactory(NavDataFactoryPtr &fact)
{
...
bool MultiFormatNavDataFactory ::addDataSource(const std::string &source)
{
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(*myFactories))
    {
        NavDataFactory *ptr = fi.second.get();
        NavDataFactoryWithStoreFile *fact = dynamic_cast<NavDataFactoryWithStoreFile *>(ptr);
        if (fact != nullptr)
        {
...
bool MultiFormatNavDataFactory ::process(const std::string &filename, NavDataFactoryCallback &cb)
{
...
void MultiFormatNavDataFactory ::dump(std::ostream &s, DumpDetail dl) const
{
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(*myFactories))
    {
        NavDataFactory *ptr = fi.second.get();
...
void MultiFormatNavDataFactory ::setControl(const FactoryControl &ctrl)
{
...
std::string MultiFormatNavDataFactory ::getFactoryFormats() const
{
    std::string rv;
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(*myFactories))
    {
...
std::shared_ptr<NavDataFactoryMap> MultiFormatNavDataFactory ::factories()
{
