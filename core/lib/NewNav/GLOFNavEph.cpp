...
namespace gnsstk
{
GLOFNavEph ::GLOFNavEph()
    : clkBias(std::numeric_limits<double>::quiet_NaN()), freqBias(std::numeric_limits<double>::quiet_NaN()),
      healthBits(-1), tb(-1), P1(-1), P2(-1), P3(-1), P4(-1), interval(-1), opStatus(GLOFNavPCode::Unknown),
      tauDelta(std::numeric_limits<double>::quiet_NaN()), aod(-1), accIndex(-1), dayCount(-1),
...
bool GLOFNavEph ::validate() const
{
    /// @todo implement some checking.
    return true;
}

bool GLOFNavEph ::getXvt(const CommonTime &when, Xvt &xvt, const ObsID &oid)
{
    DEBUGTRACE_FUNCTION();
    ...
    Vector<double> initialState(6), accel(3), k1(6), k2(6), k3(6), k4(6), tempRes(6);
...
CommonTime GLOFNavEph ::getUserTime() const
{
    CommonTime mr = std::max({timeStamp, xmit2, xmit3, xmit4});
    return mr + 2.0;
}

void GLOFNavEph ::fixFit()
{
    beginFit = timeStamp;
...
void GLOFNavEph ::dump(std::ostream &s, DumpDetail dl) const
{
    ...
    s << "           STRING OVERHEAD" << endl
      ...
      << left << setw(11) << "STR1" << right << printTime(timeStamp, otform) << endl
      << left << setw(11) << "STR2" << right << printTime(xmit2, otform) << endl
      << left << setw(11) << "STR3" << right << printTime(xmit3, otform) << endl
      << left << setw(11) << "STR4" << right << printTime(xmit4, otform) << endl
      << endl;
    ...
    s << "X, X', X''  " << setw(16) << pos[0] << " km, " << setw(16) << vel[0] << " km/s, " << setw(16) << acc[0]
      << " km/s**2" << endl
      << "Y, Y', Y''  " << setw(16) << pos[1] << " km, " << setw(16) << vel[1] << " km/s, " << setw(16) << acc[1]
      << " km/s**2" << endl
      << "Z, Z', Z''  " << setw(16) << pos[2] << " km, " << setw(16) << vel[2] << " km/s, " << setw(16) << acc[2]
      << " km/s**2" << endl
      << "tau         " << setw(16) << clkBias << " sec" << endl
      << "gamma       " << setw(16) << freqBias << " sec/sec" << endl
      << "tauDelta    " << setw(16) << tauDelta << " sec (interfreq bias)" << endl;

    s.setf(ios::fixed, ios::floatfield);
    s.precision(0);
    s << "t_b         " << setw(16) << tb << " increments" << endl
      << "P           " << setw(16) << static_cast<int>(opStatus) << " encoded: " << StringUtils::asString(opStatus)
      << endl
      << "P1          " << setw(16) << P1 << " encoded:"
      << " Time interval = " << interval << " minutes" << endl
    ...
    s << endl
      ...
      << "F_T         " << setw(16) << accIndex << " encoded, " << setw(5) << getAccuracy() << " m" << endl
      << "N_t         " << setw(16) << dayCount << " day of quadrennial" << endl
    ...
    s << endl << "l (String3) " << setw(16) << lhealth << " encoded: ";
    if (lhealth == 0)
    {
        s << " healthy";
    }
    ...
    s << endl << "Health (B&l)" << setw(16) << StringUtils::asString(health) << endl;
    if (!signal.obs.freqOffsWild)
    {
        s << "H           " << setw(16) << signal.obs.freqOffs << " frequency number";
    }
    else
    {
        s << "H           " << "         unknown"
...
void GLOFNavEph ::dumpTerse(std::ostream &s) const
{
    ...
    s << "  " << setw(2) << signal.sat.id << "  " << printTime(beginFit, tform) << " ! " << printTime(Toe, tform2)
      << " ! " << printTime(endFit, tform) << "       " << lhealth << "     " << healthBits << "  " << endl;
}

double GLOFNavEph ::getAccuracy() const
{
    double retVal = 0.0;
    switch (accIndex)
    {
...
double GLOFNavEph ::getSiderealTime(const CommonTime &time)
{
    ...
    double sid(24110.54841 + jc * (8640184.812866 + jc * (0.093104 - jc * 0.0000062)));
    sid = sid / 3600.0;
    sid = fmod(sid, 24.0);
    if (sid < 0.0)
    {
...
Vector<double> GLOFNavEph ::derivative(const Vector<double> &inState, const Vector<double> &accel) const
{
