...
class NavSatelliteID : public NavSignalID
{
  public:
    ...
    NavSatelliteID(unsigned long subj, unsigned long xmit, SatelliteSystem sys, CarrierBand car, TrackingCode track,
                   NavType nmt);
     ...
     * @param[in] track The tracking code of this signal.
     * @param[in] nmt The navigation message format of this signal. */
    NavSatelliteID(unsigned long subj, SatelliteSystem sys, CarrierBand car, TrackingCode track, XmitAnt ant,
                   int freqOffs, bool freqOffsWild, NavType nmt = NavType::Any);
...
{
    s << "subj:" << nsid.sat << " xmit:" << nsid.xmitSat << " " << static_cast<NavSignalID>(nsid);
    return s;
}
