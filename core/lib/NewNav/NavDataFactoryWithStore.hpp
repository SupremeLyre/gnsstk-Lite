...
class NavDataFactoryWithStore : public NavDataFactory
{
  public:
    ...
    bool getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when, NavDataPtr &offset,
                   SVHealth xmitHealth = SVHealth::Any, NavValidityType valid = NavValidityType::ValidOnly) override;
     ...
     *   data set (i.e. assuming the data is going to be a day's
     *   worth when it's actually several years. */
    NavSatelliteIDSet getAvailableSats(NavMessageType nmt, const CommonTime &fromTime,
  ...
  protected:
     ...
     *   or invalid messages, or both.
     * @return true if successful.  If false, navData will be untouched. */
    virtual bool findUser(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navData, SVHealth xmitHealth,
     ...
     *   or invalid messages, or both.
     * @return true if successful.  If false, navData will be untouched. */
    virtual bool findNearest(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navData, SVHealth xmitHealth,
                             NavValidityType valid);
     ...
     *   xmitHealth. */
    bool validityCheck(const NavMap::iterator &ti, NavMap &nm, NavValidityType valid, SVHealth xmitHealth,
