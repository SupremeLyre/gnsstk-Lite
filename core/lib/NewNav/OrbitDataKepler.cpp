...
namespace gnsstk
{
OrbitDataKepler ::OrbitDataKepler()
    : Cuc(0.0), Cus(0.0), Crc(0.0), Crs(0.0), Cic(0.0), Cis(0.0), M0(0.0), dn(0.0), dndot(0.0), ecc(0.0), A(0.0),
      Ahalf(0.0), Adot(0.0), OMEGA0(0.0), i0(0.0), w(0.0), OMEGAdot(0.0), idot(0.0), af0(0.0), af1(0.0), af2(0.0),
      health(SVHealth::Unknown), frame(RefFrameSys::WGS84)
{
}

void OrbitDataKepler ::dump(std::ostream &s, DumpDetail dl) const
{
    if (dl == DumpDetail::OneLine)
    {
        s << printTime(timeStamp, dumpTimeFmtBrief) << " " << signal
          << (signal.messageType == NavMessageType::Almanac ? " toa: " : " toe: ") << printTime(Toe, dumpTimeFmtBrief)
    ...
    s << endl
      ...
      << (signal.messageType == NavMessageType::Ephemeris ? "Eph" : "Alm") << " Epoch:    " << getDumpTime(dl, Toe)
      << endl
...
void OrbitDataKepler ::dumpClock(std::ostream &s) const
{
    s << endl
      << "           CLOCK PARAMETERS" << endl
      << endl
...
void OrbitDataKepler ::dumpOrbit(std::ostream &s) const
{
    s << endl
      << "           ORBIT PARAMETERS" << endl
      << endl
      << "Semi-major axis:       " << setw(fw) << A << " m       " << setw(fw) << Adot << "   m/sec" << endl
      << "Motion correction:     " << setw(fw) << dn << " rad/sec " << setw(fw) << dndot << " rad/(sec**2)" << endl
      << "Eccentricity:          " << setw(fw) << ecc << endl
      << "Arg of perigee:        " << setw(fw) << w << " rad" << endl
      << "Mean anomaly at epoch: " << setw(fw) << M0 << " rad" << endl
      << "Right ascension:       " << setw(fw) << OMEGA0 << " rad     " << setw(fw) << OMEGAdot << " rad/sec" << endl
      << "Inclination:           " << setw(fw) << i0 << " rad     " << setw(fw) << idot << " rad/sec" << endl;
}

void OrbitDataKepler ::dumpHarmonics(std::ostream &s) const
{
    s << endl
      << "           HARMONIC CORRECTIONS" << endl
      << endl
      << "Radial        Sine: " << setw(fw) << Crs << " m    Cosine: " << setw(fw) << Crc << " m" << endl
      << "Inclination   Sine: " << setw(fw) << Cis << " rad  Cosine: " << setw(fw) << Cic << " rad" << endl
      << "In-track      Sine: " << setw(fw) << Cus << " rad  Cosine: " << setw(fw) << Cuc << " rad" << endl;
}

bool OrbitDataKepler ::getXvt(const CommonTime &when, const EllipsoidModel &ell, Xvt &xvt, const ObsID &oid)
{
    ...
    drv = Ak * lecc * dek * sinea - 2.e0 * dlk * (Crc * s2al - Crs * c2al) + Adot * G;
...
double OrbitDataKepler ::svRelativity(const CommonTime &when, const EllipsoidModel &ell) const
{
    double twoPI = 2.0e0 * PI;
    double sqrtgm = SQRT(ell.gm());
...
double OrbitDataKepler ::svClockBias(const CommonTime &when) const
{
    double dtc, elaptc;
    elaptc = when - Toc;
...
double OrbitDataKepler ::svClockDrift(const CommonTime &when) const
{
    double drift, elaptc;
    elaptc = when - Toc;
...
bool OrbitDataKepler ::isSameData(const NavDataPtr &right) const
{
    const std::shared_ptr<OrbitDataKepler> rhs = std::dynamic_pointer_cast<OrbitDataKepler>(right);
    if (!rhs)
    {
        // not the same type.
        return false;
    }
    return (NavData::isSameData(right) && (Toe == rhs->Toe) && (Toc == rhs->Toc) && (Cuc == rhs->Cuc) &&
            (Cus == rhs->Cus) && (Crc == rhs->Crc) && (Crs == rhs->Crs) && (Cic == rhs->Cic) && (Cis == rhs->Cis) &&
            (M0 == rhs->M0) && (dn == rhs->dn) && (dndot == rhs->dndot) && (ecc == rhs->ecc) && (A == rhs->A) &&
            (Ahalf == rhs->Ahalf) && (Adot == rhs->Adot) && (OMEGA0 == rhs->OMEGA0) && (i0 == rhs->i0) &&
            (w == rhs->w) && (OMEGAdot == rhs->OMEGAdot) && (idot == rhs->idot) && (af0 == rhs->af0) &&
            (af1 == rhs->af1) && (af2 == rhs->af2));
}

std::list<std::string> OrbitDataKepler ::compare(const NavDataPtr &right) const
{
