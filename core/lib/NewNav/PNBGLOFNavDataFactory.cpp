...
namespace gnsstk
{
PNBGLOFNavDataFactory ::PNBGLOFNavDataFactory() : almDOY(CommonTime::BEGINNING_OF_TIME), pendingAlms(false)
{
}

bool PNBGLOFNavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    DEBUGTRACE_FUNCTION();
    if ((navIn->getNavID().navType != NavType::GloCivilF) || (!validSize(navIn)))
    {
        DEBUGTRACE("fail bits=" << navIn->getNumBits() << "  nav=" << StringUtils::asString(navIn->getNavID().navType));
    ...
    try
    {
        unsigned long stringID = navIn->asUnsignedLong(fsbStrNum, fnbStrNum, fscStrNum);
...
bool PNBGLOFNavDataFactory ::processEph(unsigned long stringID, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    // set of strings
    if (!ephS[str1] || !ephS[str2] || !ephS[str3] || !ephS[str4] || !validSize(ephS[str1]) || !validSize(ephS[str2]) ||
        !validSize(ephS[str3]) || !validSize(ephS[str4]) ||
    ...
    eph->ref = CivilTime(tmp.year, tmp.month, tmp.day, tk1h, tk1m, 30.0 * tk30s, TimeSystem::GLO);
    ...
    eph->freqBias = ephS[esiGamman]->asSignMagDouble(esbGamman, enbGamman, escGamman);
    eph->healthBits = ephS[esiBn]->asUnsignedLong(esbBn, enbBn, escBn);
    eph->lhealth = ephS[esiln]->asBool(esbln);
    eph->health = ((eph->lhealth != 0) || (eph->healthBits & 0x04) ? SVHealth::Unhealthy : SVHealth::Healthy);
    ...
    eph->opStatus = static_cast<GLOFNavPCode>(ephS[esiP]->asUnsignedLong(esbP, enbP, escP));
    eph->tauDelta = ephS[esidtaun]->asSignMagDouble(esbdtaun, enbdtaun, escdtaun);
    ...
    eph->satType = static_cast<GLOFNavSatType>(ephS[esiM]->asUnsignedLong(esbM, enbM, escM));
...
bool PNBGLOFNavDataFactory ::processAlm(unsigned long stringID, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
...
bool PNBGLOFNavDataFactory ::processAlmOrb(const NavSatelliteID &key, unsigned almIdx, NavDataPtrList &navOut)
{
    std::vector<PackedNavBitsPtr> &almS(almAcc[key]);
    ...
    // }
    if (!almS[almIdx] || !almS[almIdx + 1] || (almS[almIdx]->getNumBits() != 96) ||
        (almS[almIdx + 1]->getNumBits() != 96) ||
        (almS[almIdx + 1]->getTransmitTime() - almS[almIdx]->getTransmitTime() != 2))
    {
    ...
    NavSatelliteID sat(slot, xmitSat, almS[almIdx]->getobsID(), almS[almIdx]->getNavID());
    if (PNBNavDataFactory::processHea)
    {
        NavDataPtr p1 = std::make_shared<GLOFNavHealth>();
    ...
    alm->health = (alm->healthBits == 1 ? SVHealth::Healthy : SVHealth::Unhealthy);
    alm->satType = static_cast<GLOFNavSatType>(almS[almIdx + asoM]->asUnsignedLong(asbM, anbM, ascM));
    alm->taunA = almS[almIdx + asotau]->asSignMagDouble(asbtau, anbtau, asctau);
    alm->lambdanA = almS[almIdx + asolambda]->asSignMagDoubleSemiCircles(asblambda, anblambda, asclambda);
    alm->deltainA = almS[almIdx + asodeltai]->asSignMagDoubleSemiCircles(asbdeltai, anbdeltai, ascdeltai);
    alm->eccnA = almS[almIdx + asoepsilon]->asSignMagDouble(asbepsilon, anbepsilon, ascepsilon);
    alm->omeganA = almS[almIdx + asoomega]->asSignMagDoubleSemiCircles(asbomega, anbomega, ascomega);
    ...
    alm->deltaTnA = almS[almIdx + asoDeltaT]->asSignMagDouble(asbDeltaT, anbDeltaT, ascDeltaT);
    alm->deltaTdotnA = almS[almIdx + asoDeltaTdot]->asSignMagDouble(asbDeltaTdot, anbDeltaTdot, ascDeltaTdot);
...
bool PNBGLOFNavDataFactory ::processTime(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    bool rv = true;
    NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    if (PNBNavDataFactory::processHea)
    ...
    if (PNBNavDataFactory::processTim)
    {
        ...
        double offset = 0;
        bool result = btsc.getOffset(TimeSystem::GLO, TimeSystem::GPS, to->timeStamp, offset);
        rv &= result;
        if (result)
        {
            to->deltatLS = -offset;
...
bool PNBGLOFNavDataFactory ::processTimeUT1(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (!PNBNavDataFactory::processTim)
    {
...
unsigned PNBGLOFNavDataFactory ::getFrameNum(const CommonTime &xmitTime)
{
...
void PNBGLOFNavDataFactory ::resetState()
{
    timeAcc.clear();
    ephAcc.clear();
    almAcc.clear();
...
PNBGLOFNavDataFactory::TimeMeta ::operator NavDataPtr() const
{
...
void PNBGLOFNavDataFactory::TimeMeta ::setNT(unsigned v)
{
    NT = v;
    B1.set_valid(false);
    B2.set_valid(false);
...
void PNBGLOFNavDataFactory::TimeMeta ::setNA(unsigned v)
{
    NA = v;
    B1.set_valid(false);
    B2.set_valid(false);
...
bool PNBGLOFNavDataFactory::TimeMeta ::isValid()
{
    return tauc.is_valid() && B1.is_valid() && B2.is_valid() && NT.is_valid() && NA.is_valid();
}
