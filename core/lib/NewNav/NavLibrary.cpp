...
namespace gnsstk
{
bool NavLibrary ::getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, bool useAlm, SVHealth xmitHealth,
...
bool NavLibrary ::getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, bool useAlm, const ObsID &oid,
                         SVHealth xmitHealth, NavValidityType valid, NavSearchOrder order)
{
    DEBUGTRACE_FUNCTION();
    NavMessageID nmid(sat, useAlm ? NavMessageType::Almanac : NavMessageType::Ephemeris);
    NavDataPtr ndp;
...
bool NavLibrary ::getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, SVHealth xmitHealth,
                         NavValidityType valid, NavSearchOrder order)
{
    DEBUGTRACE_FUNCTION();
    // Use the standard default ObsID for standard default behavior.
...
bool NavLibrary ::getXvt(const NavSatelliteID &sat, const CommonTime &when, Xvt &xvt, const ObsID &oid,
                         SVHealth xmitHealth, NavValidityType valid, NavSearchOrder order)
{
    DEBUGTRACE_FUNCTION();
...
bool NavLibrary ::getHealth(const NavSatelliteID &sat, const CommonTime &when, SVHealth &healthOut, SVHealth xmitHealth,
                            NavValidityType valid, NavSearchOrder order)
{
    DEBUGTRACE_FUNCTION();
...
bool NavLibrary ::getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when, double &offset,
...
bool NavLibrary ::getOffset(TimeSystem fromSys, TimeSystem toSys, const CommonTime &when, NavDataPtr &offset,
                            SVHealth xmitHealth, NavValidityType valid)
{
    DEBUGTRACE_FUNCTION();
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
        if (fi.second->getOffset(fromSys, toSys, when, offset, xmitHealth, valid))
        {
            return true;
        }
    }
...
bool NavLibrary ::getIonoCorr(SatelliteSystem sys, const CommonTime &when, const Position &rxgeo, const Position &svgeo,
                              CarrierBand band, double &corrOut, NavType nt)
{
    DEBUGTRACE_FUNCTION();
    SatID sysOnly(sys);
    NavMessageID nmid(NavSatelliteID(sysOnly, sysOnly,
                                     ObsID(ObservationType::NavMsg, CarrierBand::Any, TrackingCode::Any, XmitAnt::Any),
                                     nt),
                      NavMessageType::Iono);
    NavDataPtr navOut;
    if (!find(nmid, when, navOut, SVHealth::Healthy, NavValidityType::ValidOnly, NavSearchOrder::Nearest))
    {
        return false;
    }
...
bool NavLibrary ::getIonoCorr(const SatID &sat, const CommonTime &when, const Position &rxgeo, CarrierBand band,
                              double &corrOut, NavType nt, int freqOffs, bool freqOffsWild)
{
    DEBUGTRACE_FUNCTION();
    if (sat.isWild())
    {
    ...
    NavSatelliteID nsid(sat, wildSat, ObsID(ObservationType::NavMsg, CarrierBand::Any, TrackingCode::Any, XmitAnt::Any),
                        nt);
    if (!getXvt(nsid, when, xvt, SVHealth::Any, NavValidityType::ValidOnly, NavSearchOrder::User))
    {
...
bool NavLibrary ::getISC(const SatID &sat, const ObsID &oid, const CommonTime &when, double &corrOut,
                         SVHealth xmitHealth, NavValidityType valid, NavSearchOrder order)
{
    DEBUGTRACE_FUNCTION();
...
std::list<NavMessageID> NavLibrary ::getISCNMID(const SatID &sat, const ObsID &oid)
{
    DEBUGTRACE_FUNCTION();
    std::list<NavMessageID> rv;
    const ObsID anyObs(ObservationType::NavMsg, CarrierBand::Any, TrackingCode::Any, XmitAnt::Any);
    if (sat.system == SatelliteSystem::GPS)
    {
        const SatID gpsAnySat(SatelliteSystem::GPS);
        NavMessageID nmid(NavSatelliteID(sat, gpsAnySat, anyObs, NavType::Unknown), NavMessageType::ISC);
        switch (oid.code)
        {
        case TrackingCode::CA:
            if (oid.band == CarrierBand::L1)
            {
                ...
                rv.push_back(nmid);
                rv.push_back(
                    NavMessageID(NavSatelliteID(sat, gpsAnySat, anyObs, NavType::GPSCNAVL2), NavMessageType::ISC));
                rv.push_back(
                    NavMessageID(NavSatelliteID(sat, gpsAnySat, anyObs, NavType::GPSCNAVL5), NavMessageType::ISC));
                rv.push_back(
                    NavMessageID(NavSatelliteID(sat, gpsAnySat, anyObs, NavType::GPSCNAV2), NavMessageType::ISC));
            }
            break;
        case TrackingCode::P:
        case TrackingCode::Y:
        ...
        case TrackingCode::Semicodeless:
            ...
            case CarrierBand::L2:
                rv.push_back(
                    NavMessageID(NavSatelliteID(sat, gpsAnySat, anyObs, NavType::GPSLNAV), NavMessageType::ISC));
                break;
            }
            break;
        ...
        case TrackingCode::MprimePA:
            ...
            case CarrierBand::L2:
                rv.push_back(
                    NavMessageID(NavSatelliteID(sat, gpsAnySat, anyObs, NavType::GPSMNAV), NavMessageType::ISC));
                break;
            }
            break;
...
bool NavLibrary ::find(const NavMessageID &nmid, const CommonTime &when, NavDataPtr &navOut, SVHealth xmitHealth,
                       NavValidityType valid, NavSearchOrder order)
{
    DEBUGTRACE_FUNCTION();
...
void NavLibrary ::setValidityFilter(NavValidityType nvt)
{
    DEBUGTRACE_FUNCTION();
...
void NavLibrary ::setTypeFilter(const NavMessageTypeSet &nmts)
{
    DEBUGTRACE_FUNCTION();
    for (auto &i : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
...
void NavLibrary ::clearTypeFilter()
{
    DEBUGTRACE_FUNCTION();
    for (auto &i : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
...
void NavLibrary ::addTypeFilter(NavMessageType nmt)
{
    DEBUGTRACE_FUNCTION();
    for (auto &i : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
...
void NavLibrary ::addFactory(NavDataFactoryPtr &fact)
{
    DEBUGTRACE_FUNCTION();
...
void NavLibrary ::dump(std::ostream &s, DumpDetail dl) const
{
    DEBUGTRACE_FUNCTION();
...
void NavLibrary ::edit(const CommonTime &fromTime, const CommonTime &toTime)
{
    DEBUGTRACE_FUNCTION();
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
...
void NavLibrary ::edit(const CommonTime &fromTime, const CommonTime &toTime, const NavSatelliteID &satID)
{
    DEBUGTRACE_FUNCTION();
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
...
void NavLibrary ::edit(const CommonTime &fromTime, const CommonTime &toTime, const NavSignalID &signal)
{
    DEBUGTRACE_FUNCTION();
    for (auto &fi : NDFUniqIterator<NavDataFactoryMap>(factories))
    {
...
CommonTime NavLibrary ::getInitialTime() const
{
    DEBUGTRACE_FUNCTION();
...
CommonTime NavLibrary ::getFinalTime() const
{
    DEBUGTRACE_FUNCTION();
...
NavSatelliteIDSet NavLibrary ::getAvailableSats(const CommonTime &fromTime, const CommonTime &toTime) const
{
    DEBUGTRACE_FUNCTION();
...
NavSatelliteIDSet NavLibrary ::getAvailableSats(NavMessageType nmt, const CommonTime &fromTime,
...
std::set<SatID> NavLibrary ::getIndexSet(const CommonTime &fromTime, const CommonTime &toTime) const
{
    DEBUGTRACE_FUNCTION();
...
std::set<SatID> NavLibrary ::getIndexSet(NavMessageType nmt, const CommonTime &fromTime, const CommonTime &toTime) const
{
    DEBUGTRACE_FUNCTION();
...
NavMessageIDSet NavLibrary ::getAvailableMsgs(const CommonTime &fromTime, const CommonTime &toTime) const
{
    DEBUGTRACE_FUNCTION();
...
bool NavLibrary ::isPresent(const NavMessageID &nmid, const CommonTime &fromTime, const CommonTime &toTime)
{
    DEBUGTRACE_FUNCTION();
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(factories))
    {
...
std::string NavLibrary ::getFactoryFormats() const
{
    DEBUGTRACE_FUNCTION();
    std::string rv;
    for (const auto &fi : NDFUniqConstIterator<NavDataFactoryMap>(factories))
