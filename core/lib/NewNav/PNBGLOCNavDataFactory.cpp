...
namespace gnsstk
{
PNBGLOCNavDataFactory ::PNBGLOCNavDataFactory() : almDOY(CommonTime::BEGINNING_OF_TIME), pendingAlms(false)
{
}

bool PNBGLOCNavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    DEBUGTRACE_FUNCTION();
    static bool printed = false;
    if ((navIn->getNavID().navType != NavType::GloCivilC) || (navIn->getNumBits() != 300))
    {
        DEBUGTRACE("nope, failed.  " << navIn->getNavID().navType << "  " << navIn->getNumBits());
    ...
    try
    {
        unsigned long msgType = navIn->asUnsignedLong(fsbType, fnbType, fscType);
        ...
        if (checkMsg)
        {
            ...
            unsigned long preamble = navIn->asUnsignedLong(fsbPreamble, fnbPreamble, fscPreamble);
            bool valid = (preamble == valPreamble);
            DEBUGTRACE("Checking message validity = " << valid << "  preamble = " << hex << preamble);
            if (valid != expMsg)
            {
                return true;
            }
...
bool PNBGLOCNavDataFactory ::processHeader(const PackedNavBitsPtr &navIn, GLOCNavHeader &navOut)
{
    DEBUGTRACE_FUNCTION();
    ...
    navOut.preamble = navIn->asUnsignedLong(fsbPreamble, fnbPreamble, fscPreamble);
...
bool PNBGLOCNavDataFactory ::processEarth(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    DEBUGTRACE_FUNCTION();
    ...
    if (processTim && timeAcc[key].isValid())
    {
        ...
        GLOCNavUT1TimeOffset *to = dynamic_cast<GLOCNavUT1TimeOffset *>(p0.get());
...
bool PNBGLOCNavDataFactory ::processHealth(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    if (processHea)
    {
        NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
...
bool PNBGLOCNavDataFactory ::processEph(unsigned long stringID, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    DEBUGTRACE_FUNCTION();
    NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    ...
    // set of strings
    if (!ephS[str10] || !ephS[str11] || !ephS[str12] || (ephS[str10]->getNumBits() != 300) ||
        (ephS[str11]->getNumBits() != 300) || (ephS[str12]->getNumBits() != 300) ||
    ...
    eph->Mj = static_cast<GLOCSatType>(ephS[esiMj]->asUnsignedLong(esbMj, enbMj, escMj));
    ...
    eph->RjE = static_cast<GLOCRegime>(ephS[esiRjE]->asUnsignedLong(esbRjE, enbRjE, escRjE));
    eph->RjT = static_cast<GLOCRegime>(ephS[esiRjT]->asUnsignedLong(esbRjT, enbRjT, escRjT));
    ...
    eph->freqBias = ephS[esigammaj]->asSignMagDouble(esbgammaj, enbgammaj, escgammaj);
    /// @todo Not sure if this is signed or not but it seems likely.
    eph->driftRate = ephS[esiBetaj]->asSignMagDouble(esbBetaj, enbBetaj, escBetaj);
    eph->tauc = ephS[esitauc]->asSignMagDouble(esbtauc, enbtauc, esctauc);
    eph->taucdot = ephS[esitaucdot]->asSignMagDouble(esbtaucdot, enbtaucdot, esctaucdot);
    ...
    eph->apcOffset[0] = ephS[esiDeltaxjpc]->asSignMagDouble(esbDeltaxjpc, enbDeltaxjpc, escDeltaxjpc);
    eph->apcOffset[1] = ephS[esiDeltayjpc]->asSignMagDouble(esbDeltayjpc, enbDeltayjpc, escDeltayjpc);
    eph->apcOffset[2] = ephS[esiDeltazjpc]->asSignMagDouble(esbDeltazjpc, enbDeltazjpc, escDeltazjpc);
    eph->tauDelta = ephS[esiDeltataujL3]->asSignMagDouble(esbDeltataujL3, enbDeltataujL3, escDeltataujL3);
    eph->tauGPS = ephS[esiTauGPS]->asSignMagDouble(esbTauGPS, enbTauGPS, escTauGPS);
    ...
    // consistency.
    eph->header.health = eph->header11.health = eph->header12.health =
        ((eph->header.svUnhealthy || eph->header11.svUnhealthy || eph->header12.svUnhealthy) ? SVHealth::Unhealthy
                                                                                             : SVHealth::Healthy);
...
bool PNBGLOCNavDataFactory ::processAlm(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    DEBUGTRACE_FUNCTION();
    ...
    alm->orbitType = static_cast<GLOCOrbitType>(navIn->asUnsignedLong(asbTO, anbTO, ascTO));
    ...
    alm->satType = static_cast<GLOCSatType>(navIn->asUnsignedLong(asbM, anbM, ascM));
    alm->tau = navIn->asSignMagDouble(asbtau, anbtau, asctau);
    alm->lambda = navIn->asSignMagDouble(asblambda, anblambda, asclambda);
    alm->tLambda = navIn->asUnsignedDouble(asbtlambda, anbtlambda, asctlambda);
    ...
    alm->deltaTdot = navIn->asSignMagDouble(asbDeltaTdot, anbDeltaTdot, ascDeltaTdot);
    alm->header.health = (alm->header.svUnhealthy ? SVHealth::Unhealthy : SVHealth::Healthy);
    int N4 = 0;
    ...
    alm->Toa = gnsstk::GLONASSTime(N4, alm->NA, alm->tLambda, gnsstk::TimeSystem::GLO);
    alm->fixFit();
    navOut.push_back(p0);
    return true;
}

bool PNBGLOCNavDataFactory ::processLTDMP(unsigned long stringID, const PackedNavBitsPtr &navIn)
{
    DEBUGTRACE_FUNCTION();
...
void PNBGLOCNavDataFactory ::resetState()
{
    DEBUGTRACE_FUNCTION();
    timeAcc.clear();
    ephAcc.clear();
...
void PNBGLOCNavDataFactory::TimeMeta ::setN4(unsigned v)
{
    N4 = v;
}

bool PNBGLOCNavDataFactory::TimeMeta ::isValid()
{
    return N4.is_valid();
}
