...
namespace gnsstk
{
BDSD1NavHealth ::BDSD1NavHealth() : isAlmHealth(true), satH1(true), svHealth(0xff)
{
    weekFmt = "%4D(%4e)";
    msgLenSec = 6.0;
}

SVHealth BDSD1NavHealth ::getHealth() const
{
    if (isAlmHealth)
    {
...
void BDSD1NavHealth ::dump(std::ostream &s, DumpDetail dl) const
{
    ...
    case DumpDetail::Full:
        ...
        s << "*************************************************************"
          ...
          << endl
          << "           HEALTH DATA" << endl
          << "Bits               0x" << hex << setw(3) << setfill('0') << svHealth << endl
          << endl
        ...
        s << endl
          << "B1I signal         " << (svHealth & B1ISignalWeak ? "weak" : "normal") << endl
          << "B2I signal         " << (svHealth & B2ISignalWeak ? "weak" : "normal") << endl
          << "B3I signal         " << (svHealth & B3ISignalWeak ? "weak" : "normal") << endl
          << "Reserved (5)       " << (svHealth & 0x10 ? "set" : "unset") << endl
          << "Reserved (4)       " << (svHealth & 0x08 ? "set" : "unset") << endl
          << "Reserved (3)       " << (svHealth & 0x04 ? "set" : "unset") << endl
          << "Navigation message " << (svHealth & NavMsgBad ? "invalid" : "valid") << endl
          << "Reserved (1)       " << (svHealth & 0x01 ? "set" : "unset") << endl;
        break;
    }
    s.flags(oldFlags);
}
