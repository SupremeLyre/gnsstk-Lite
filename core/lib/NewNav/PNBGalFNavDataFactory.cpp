...
namespace gnsstk
{
bool PNBGalFNavDataFactory ::addData(const PackedNavBitsPtr &navIn, NavDataPtrList &navOut, double cadence)
{
    ...
    try
    {
        unsigned long pageType = navIn->asUnsignedLong(fsbType, fnbType, fscType);
...
bool PNBGalFNavDataFactory ::processEph(unsigned pageType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    ...
    if (!PNBNavDataFactory::processEph && !PNBNavDataFactory::processHea && !PNBNavDataFactory::processIono &&
        !PNBNavDataFactory::processISC && !PNBNavDataFactory::processTim)
    {
    ...
    if (pageType == 1)
    {
        ...
        if (PNBNavDataFactory::processHea)
        {
            ...
            hp1->sigHealthStatus =
                static_cast<GalHealthStatus>(ephPage[esiE5ahs]->asUnsignedLong(esbE5ahs, enbE5ahs, escE5ahs));
            hp1->dataValidityStatus =
                static_cast<GalDataValid>(ephPage[esiE5advs]->asUnsignedLong(esbE5advs, enbE5advs, escE5advs));
            hp1->sisaIndex = ephPage[esiSISA]->asUnsignedLong(esbSISA, enbSISA, escSISA);
            navOut.push_back(p1);
        }
    ...
    if (!ephPage[pt1] || !ephPage[pt2] || !ephPage[pt3] || !ephPage[pt4] || (ephPage[pt1]->getNumBits() != 244) ||
        (ephPage[pt2]->getNumBits() != 244) || (ephPage[pt3]->getNumBits() != 244) ||
    ...
    unsigned tow_1 = ephPage[esiTOW_1]->asUnsignedLong(esbTOW_1, enbTOW_1, escTOW_1);
    GALWeekSecond xmit1(wn_1, tow_1);
    unsigned wn_3 = ephPage[esiWN_3]->asUnsignedLong(esbWN_3, enbWN_3, escWN_3);
    unsigned tow_3 = ephPage[esiTOW_3]->asUnsignedLong(esbTOW_3, enbTOW_3, escTOW_3);
    GALWeekSecond xmit3(wn_3, tow_3);
    // cerr << "  wn_1=" << wn_1 << "  tow_1=" << tow_1 << "  wn_3=" << wn_3 << "  tow_3=" << tow_3 << "  t0e=" << t0e
    // << "  t0c=" << t0c << endl;
    ...
    eph->Ahalf = ephPage[esiAhalf]->asUnsignedDouble(esbAhalf, enbAhalf, escAhalf);
    eph->A = eph->Ahalf * eph->Ahalf;
    // no Adot in F/NAV
    eph->OMEGA0 = ephPage[esiOMEGA0]->asDoubleSemiCircles(esbOMEGA0, enbOMEGA0, escOMEGA0);
    eph->i0 = ephPage[esii0]->asDoubleSemiCircles(esbi0, enbi0, esci0);
    eph->w = ephPage[esiw]->asDoubleSemiCircles(esbw, enbw, escw);
    eph->OMEGAdot = ephPage[esiOMEGAdot]->asDoubleSemiCircles(esbOMEGAdot, enbOMEGAdot, escOMEGAdot);
    eph->idot = ephPage[esiidot]->asDoubleSemiCircles(esbidot, enbidot, escidot);
    ...
    eph->sisaIndex = ephPage[esiSISA]->asUnsignedLong(esbSISA, enbSISA, escSISA);
    ...
    eph->iodnav3 = iod3;
    eph->iodnav4 = iod4;
    eph->hsE5a = static_cast<GalHealthStatus>(ephPage[esiE5ahs]->asUnsignedLong(esbE5ahs, enbE5ahs, escE5ahs));
    eph->dvsE5a = static_cast<GalDataValid>(ephPage[esiE5advs]->asUnsignedLong(esbE5advs, enbE5advs, escE5advs));
    // set health using the Galileo algorithms.
    eph->health = GalINavHealth::galHealth(eph->hsE5a, eph->dvsE5a, eph->sisaIndex);
    eph->wn1 = wn_1;
    eph->tow1 = tow_1;
...
bool PNBGalFNavDataFactory ::processAlm(unsigned pageType, const PackedNavBitsPtr &navIn, NavDataPtrList &navOut)
{
    NavSatelliteID key(navIn->getsatSys().id, navIn->getsatSys(), navIn->getobsID(), navIn->getNavID());
    ...
    // stop processing if we don't have three full subframes
    if (!almPage[pt5] || !almPage[pt6] || (almPage[pt5]->getNumBits() != 244) || (almPage[pt6]->getNumBits() != 244))
    {
    ...
    GalFNavHealth *hp1 = dynamic_cast<GalFNavHealth *>(p1.get());
    if (processAlmOrb(almPage, alm, hp1, pt5, pt5, asiSVID_1, asbSVID_1, asidAhalf_1, asbdAhalf_1, asiEcc_1, asbEcc_1,
                      asiw_1, asbw_1, asidi_1, asbdi_1, asiOMEGAdot_1, asbOMEGAdot_1, asiM0_1, asbM0_1, asiaf0_1,
                      asbaf0_1, asiaf1_1, asbaf1_1, asiE5ahs_1, asbE5ahs_1))
    {
        if (PNBNavDataFactory::processAlm)
        {
            ...
            alm->ioda5 = ioda5;
            alm->ioda6 = ioda6;
            alm->OMEGA0 = almPage[asiOMEGA0_1]->asDoubleSemiCircles(asbOMEGA0_1, anbOMEGA0, ascOMEGA0);
            navOut.push_back(p0);
        }
        if (PNBNavDataFactory::processHea)
        {
    ...
    if (processAlmOrb(almPage, alm, hp1, pt5, pt6, asiSVID_2, asbSVID_2, asidAhalf_2, asbdAhalf_2, asiEcc_2, asbEcc_2,
                      asiw_2, asbw_2, asidi_2, asbdi_2, asiOMEGAdot_2, asbOMEGAdot_2, asiM0_2, asbM0_2, asiaf0_2,
                      asbaf0_2, asiaf1_2, asbaf1_2, asiE5ahs_2, asbE5ahs_2))
    {
        if (PNBNavDataFactory::processAlm)
        {
            // cerr << "add F/NAV alm SVID1" << endl;
            ...
            long msb = almPage[asiOMEGA0m_2]->asLong(asbOMEGA0m_2, anbOMEGA0m_2, 1);
            unsigned long lsb = almPage[asiOMEGA0l_2]->asUnsignedLong(asbOMEGA0l_2, anbOMEGA0l_2, 1);
            // combine the bits
    ...
    alm = dynamic_cast<GalFNavAlm *>(p0.get());
    hp1 = dynamic_cast<GalFNavHealth *>(p1.get());
    if (processAlmOrb(almPage, alm, hp1, pt6, pt6, asiSVID_3, asbSVID_3, asidAhalf_3, asbdAhalf_3, asiEcc_3, asbEcc_3,
                      asiw_3, asbw_3, asidi_3, asbdi_3, asiOMEGAdot_3, asbOMEGAdot_3, asiM0_3, asbM0_3, asiaf0_3,
                      asbaf0_3, asiaf1_3, asbaf1_3, asiE5ahs_3, asbE5ahs_3))
    {
        if (PNBNavDataFactory::processAlm)
        {
            // cerr << "add F/NAV alm SVID1" << endl;
            alm->ioda5 = ioda5;
            alm->ioda6 = ioda6;
            alm->OMEGA0 = almPage[asiOMEGA0_3]->asDoubleSemiCircles(asbOMEGA0_3, anbOMEGA0, ascOMEGA0);
            navOut.push_back(p0);
        }
        if (PNBNavDataFactory::processHea)
        {
...
bool PNBGalFNavDataFactory ::processAlmOrb(const std::vector<PackedNavBitsPtr> &almPage, GalFNavAlm *alm,
                                           GalFNavHealth *hp1, int ptA, int ptB, int asiSVID, int asbSVID,
                                           int asidAhalf, int asbdAhalf, int asiEcc, int asbEcc, int asiw, int asbw,
                                           int asidi, int asbdi, int asiOMEGAdot, int asbOMEGAdot, int asiM0, int asbM0,
                                           int asiaf0, int asbaf0, int asiaf1, int asbaf1, int asiE5ahs, int asbE5ahs)
{
    // NavData
    alm->timeStamp = almPage[ptA]->getTransmitTime();
    unsigned long svid = almPage[asiSVID]->asUnsignedLong(asbSVID, anbSVID, ascSVID);
    if (svid == 0)
        return false; // empty almanac, do nothing.
    SatID xmitSat(almPage[ptA]->getsatSys());
    NavSatelliteID sat(svid, xmitSat, almPage[ptA]->getobsID(), almPage[ptA]->getNavID());
    ...
    alm->dAhalf = almPage[asidAhalf]->asUnsignedDouble(asbdAhalf, anbdAhalf, ascdAhalf);
    ...
    alm->OMEGAdot = almPage[asiOMEGAdot]->asDoubleSemiCircles(asbOMEGAdot, anbOMEGAdot, ascOMEGAdot);
    alm->af0 = almPage[asiaf0]->asSignedDouble(asbaf0, anbaf0, ascaf0);
    alm->af1 = almPage[asiaf1]->asSignedDouble(asbaf1, anbaf1, ascaf1);
    // GalFNavData
    alm->hsE5a = static_cast<GalHealthStatus>(almPage[asiE5ahs]->asUnsignedLong(asbE5ahs, anbE5ahs, ascE5ahs));
    alm->fixFit();
    alm->fixHealth();
    if (PNBNavDataFactory::processHea)
    {
        ...
        hp1->sigHealthStatus =
            static_cast<GalHealthStatus>(almPage[asiE5ahs]->asUnsignedLong(asbE5ahs, anbE5ahs, ascE5ahs));
...
void PNBGalFNavDataFactory ::resetState()
{
    almAcc.clear();
    ephAcc.clear();
}
