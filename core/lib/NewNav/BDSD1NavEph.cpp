...
namespace gnsstk
{
BDSD1NavEph ::BDSD1NavEph()
    : pre2(0), pre3(0), rev2(0), rev3(0), sow2(0), sow3(0), satH1(true), aodc(0xff), aode(0xff), uraIndex(15),
      xmit2(gnsstk::CommonTime::BEGINNING_OF_TIME), xmit3(gnsstk::CommonTime::BEGINNING_OF_TIME),
      tgd1(std::numeric_limits<double>::quiet_NaN()), tgd2(std::numeric_limits<double>::quiet_NaN())
{
    signal.messageType = NavMessageType::Ephemeris;
}

bool BDSD1NavEph ::validate() const
{
    return BDSD1NavData::validate() && ((pre2 == 0) || (pre2 == bds::Preamble)) &&
           ((pre2 == 0) || (pre3 == bds::Preamble));
}

CommonTime BDSD1NavEph ::getUserTime() const
{
...
void BDSD1NavEph ::dumpSVStatus(std::ostream &s) const
{
...
unsigned BDSD1NavEph ::getAOD(uint8_t aod)
{
    if (aod < 25)
        return aod;
    if (aod < 32)
