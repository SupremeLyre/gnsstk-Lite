...
namespace gnsstk
{
NeQuickIonoNavData ::NeQuickIonoNavData() : ai{0, 0, 0}, idf{false, false, false, false, false}
{
}

void NeQuickIonoNavData ::dump(std::ostream &s, DumpDetail dl) const
{
    ...
    s << endl
      ...
      << boolalpha << "IDF region 1 " << setw(16) << idf[0] << endl
...
double NeQuickIonoNavData ::getIonoCorr(const CommonTime &when, const Position &rxgeo, const Position &svgeo,
...
double NeQuickIonoNavData ::getTEC(const CommonTime &when, const Position &rxgeo, const Position &svgeo) const
{
    DEBUGTRACE_FUNCTION();
    ...
    // the station.
    bool vertical = ((fabs(svgeo.geodeticLatitude() - rxgeo.geodeticLatitude()) < ABOVE_ELEV_EPSILON) &&
    ...
    if (ip.integHeights.size() > 1)
    {
        ...
        {
            rv += integrateGaussKronrod(ip.integHeights[i - 1], ip.integHeights[i], rxgeo, svgeo, modip, modip_u, ccir,
                                        civ, azu, ip.intThresh[i - 1], vertical);
        }
    }
...
double NeQuickIonoNavData ::getEffIonoLevel(double modip_u) const
{
    DEBUGTRACE_FUNCTION();
    ...
    if ((fabs(ai[0]) < COEFF_THRESH) &&                               // eq.17
        (fabs(ai[1]) < COEFF_THRESH) && (fabs(ai[2]) < COEFF_THRESH))
    {
...
double NeQuickIonoNavData ::getSED(double dist, const Position &rxgeo, const Position &svgeo, const MODIP &modip,
                                   CCIR &ccirData, const CivilTime &when, double azu) const
{
    DEBUGTRACE_FUNCTION();
...
double NeQuickIonoNavData ::getVED(double dist, const Position &rxgeo, const Position &svgeo, double modip_u,
                                   CCIR &ccirData, const CivilTime &when, double azu) const
{
    DEBUGTRACE_FUNCTION();
    DEBUGTRACE("height_km=" << setprecision(15) << dist);
    // remember that dist is a height for VED, and that it's in km
    Position current(rxgeo.geocentricLatitude(), rxgeo.longitude(), dist * 1000, Position::Geodetic, &elModel);
...
double NeQuickIonoNavData ::neExp(double x)
{
    if (x > ExponentMax)
    {
...
NeQuickIonoNavData::ModelParameters ::ModelParameters(double modip_u, const Position &pos, double az, CCIR &ccirData,
                                                      const CivilTime &when)
    : fXeff(effSolarZenithAngle(pos, when)), ffoF1(0.0), // default to 0, see eq.37
      ccir(ccirData)
{
    DEBUGTRACE_FUNCTION();
    int seas;
    ...
    DEBUGTRACE("# pSolar_activity->effective_ionisation_level_sfu=" << scientific << az);
...
NeQuickIonoNavData::ModelParameters ::ModelParameters(CCIR &ccirData)
    : ccir(ccirData), fAzr(std::numeric_limits<double>::quiet_NaN()), ffoE(std::numeric_limits<double>::quiet_NaN()),
      fNmE(std::numeric_limits<double>::quiet_NaN()), fBEtop(std::numeric_limits<double>::quiet_NaN()),
      fA{std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN(),
         std::numeric_limits<double>::quiet_NaN()},
      ffoF1(std::numeric_limits<double>::quiet_NaN()), fNmF1(std::numeric_limits<double>::quiet_NaN()),
      fhmF1(std::numeric_limits<double>::quiet_NaN()), fB1top(std::numeric_limits<double>::quiet_NaN()),
      fB1bot(std::numeric_limits<double>::quiet_NaN()), ffoF2(std::numeric_limits<double>::quiet_NaN()),
      fNmF2(std::numeric_limits<double>::quiet_NaN()), fhmF2(std::numeric_limits<double>::quiet_NaN()),
      fB2bot(std::numeric_limits<double>::quiet_NaN()), fM3000F2(std::numeric_limits<double>::quiet_NaN()),
      fH0(std::numeric_limits<double>::quiet_NaN()), fXeff()
{
    DEBUGTRACE_FUNCTION();
}

AngleReduced NeQuickIonoNavData::ModelParameters ::solarDeclination(const CivilTime &when)
{
    DEBUGTRACE_FUNCTION();
...
Angle NeQuickIonoNavData::ModelParameters ::solarZenithAngle(const Position &pos, const CivilTime &when)
{
    DEBUGTRACE_FUNCTION();
...
Angle NeQuickIonoNavData::ModelParameters ::effSolarZenithAngle(const Position &pos, const CivilTime &when)
{
    DEBUGTRACE_FUNCTION();
    // x is really chi.
    ...
    return Angle((x.deg() + (90 - 0.24 * neExp(20 - 0.2 * x.deg())) * exp2) / (1 + exp2), AngleType::Deg);
}

void NeQuickIonoNavData::ModelParameters ::legendre(double modip_u, const Position &pos)
{
    DEBUGTRACE_FUNCTION();
    ...
    for (unsigned k = 0; k < F2LayerMODIPCoeffCount; k++)
    {
        ...
        DEBUGTRACE("CF2[" << k << "]=" << scientific << ccir.getCF2(k) << "  M[" << k << "]=" << M[k]
                          << "  parameter=" << ffoF2);
    }
    ...
    for (unsigned n = 1; n < F2LayerLongCoeffCount; n++)
    {
        for (unsigned k = 0; k < Q[n]; k++)
        {
            ffoF2 += (ccir.getCF2(K[n] + 2 * k) * C[n - 1] + // eq.67
                     ...
                     M[k] * P[n];
            DEBUGTRACE("M[" << k << "]=" << scientific << M[k] << "  lat_coeff=" << P[n] << "  Fourier_coeff["
                            << (K[n] + 2 * k) << "]=" << ccir.getCF2(K[n] + 2 * k) << "  C[" << (n - 1)
                            << "]=" << C[n - 1] << "  Fourier_coeff[" << (K[n] + 2 * k + 1)
                            << "]=" << ccir.getCF2(K[n] + 2 * k + 1) << "  S[" << (n - 1) << "]=" << S[n - 1]
                            << "  parameter=" << ffoF2);
        }
    }
    // initialize higher order terms to zero before summation
    ...
    for (unsigned k = 0; k < F2TransFactorCoeffCount; k++)
    {
        ...
        DEBUGTRACE("CM3[" << k << "]=" << scientific << ccir.getCM3(k) << "  M[" << k << "]=" << M[k]
                          << "  parameter=" << fM3000F2);
    }
    ...
    for (unsigned n = 1; n < F2TransFactorCoeffCount; n++)
    {
        for (unsigned k = 0; k < R[n]; k++)
        {
            fM3000F2 += (ccir.getCM3(H[n] + 2 * k) * C[n - 1] + // eq.75
                         ccir.getCM3(H[n] + 2 * k + 1) * S[n - 1]) *
                        M[k] * P[n];
            DEBUGTRACE("M[" << k << "]=" << scientific << M[k] << "  lat_coeff=" << P[n] << "  Fourier_coeff["
                            << (H[n] + 2 * k) << "]=" << ccir.getCM3(H[n] + 2 * k) << "  C[" << (n - 1)
                            << "]=" << C[n - 1] << "  Fourier_coeff[" << (H[n] + 2 * k + 1)
                            << "]=" << ccir.getCM3(H[n] + 2 * k + 1) << "  S[" << (n - 1) << "]=" << S[n - 1]
                            << "  parameter=" << fM3000F2);
        }
    }
}

void NeQuickIonoNavData::ModelParameters ::height()
{
    DEBUGTRACE_FUNCTION();
...
void NeQuickIonoNavData::ModelParameters ::thickness()
{
    DEBUGTRACE_FUNCTION();
    // note that the EU code uses the standard exp here too
    fB2bot = ((0.385 * fNmF2) / // eq.85
              (0.01 * exp(-3.467 + 0.857 * log(ffoF2 * ffoF2) + 2.02 * log(fM3000F2))));
    fB1top = 0.3 * (fhmF2 - fhmF1); // eq.86
    fB1bot = 0.5 * (fhmF1 - hmE);   // eq.87
    fBEtop = std::max(fB1bot, 7.0); // eq.88
}

void NeQuickIonoNavData::ModelParameters ::exosphereAdjust(unsigned month)
{
    DEBUGTRACE_FUNCTION();
    double k = 0;
    double ka = 0;
...
void NeQuickIonoNavData::ModelParameters ::peakAmplitudes()
{
    DEBUGTRACE_FUNCTION();
...
double NeQuickIonoNavData::ModelParameters ::electronDensity(const Position &pos)
{
    DEBUGTRACE_FUNCTION();
    double rv = 0;
...
double NeQuickIonoNavData::ModelParameters ::electronDensityTop(const Position &pos)
{
    DEBUGTRACE_FUNCTION();
...
double NeQuickIonoNavData::ModelParameters ::electronDensityBottom(const Position &pos)
{
    DEBUGTRACE_FUNCTION();
    double h = pos.height() / 1000.0;           // height in km
...
NeQuickIonoNavData::IntegrationParameters ::IntegrationParameters(const Position &rx, const Position &sv,
...
double NeQuickIonoNavData ::integrateGaussKronrod(double heightPt1, double heightPt2, const Position &rxgeo,
                                                  const Position &svgeo, const MODIP &modip, double modipSta,
                                                  CCIR &ccirData, const CivilTime &when, double azu, double tolerance,
                                                  bool vertical, unsigned recursionLevel) const
{
    DEBUGTRACE_FUNCTION();
    double rv = 0;
    ...
    static const double wig[] = {0.129484966168869693270611432679082, 0.279705391489276667901467771423780,
                                 0.381830050505118944950369775488975, 0.417959183673469387755102040816327,
                                 0.381830050505118944950369775488975, 0.279705391489276667901467771423780,
    ...
    static const double xi[] = {-0.991455371120812639206854697526329, -0.949107912342758524526189684047851,
    ...
    if (((fabs(intk - intg) / intk) <= tolerance) || (fabs(intk - intg) <= tolerance))
    {
    ...
    else
    {
        ...
        rv = integrateGaussKronrod(heightPt1, heightPt1 + h2, rxgeo, svgeo, modip, modipSta, ccirData, when, azu,
                                   tolerance, vertical, recursionLevel + 1);
        DEBUGTRACE("pResult(4) = " << scientific << rv);
        rv += integrateGaussKronrod(heightPt1 + h2, heightPt2, rxgeo, svgeo, modip, modipSta, ccirData, when, azu,
                                    tolerance, vertical, recursionLevel + 1);
