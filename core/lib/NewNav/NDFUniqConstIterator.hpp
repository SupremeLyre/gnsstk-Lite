 ...
 * \endcode
 */
template <typename T> class NDFUniqConstIterator
{
  public:
    NDFUniqConstIterator() : map(nullptr)
    {
    }
    NDFUniqConstIterator(const T &omap) : nuit(omap.begin()), map(&omap)
    {
    }
    NDFUniqConstIterator(const T &omap, typename T::const_iterator i) : nuit(i), map(&omap)
    {
    }
