...
// -------------------------------------------------------------------------------
/**
 Iterate over a list (vector) of SatPass using this class. NB. this class
 ignores passes that have Status less than zero, but does not change any
 Status.
*/
class SatPassIterator
{
  public:
    /**
     Explicit (only) constructor. Check the list for consistency (else
     throw) and find common time step and obs types, as well as first and
     last times. Ignore passes with Status less than zero. After building a
     vector<SatPass> (in time order), declare an iterator using this
     constructor, then call member function next(RinexObsData&) to access
     the data in time order, until it returns zero.
     ...
     @throw Exception if input list is empty, or
            if elements in the list have differing data interval or obs
            types, or if any observation type is not registered (cf.
            RinexUtilities.hpp)
    */
    explicit SatPassIterator(std::vector<SatPass> &splist, bool rev = false, bool dbug = false);
    ...
    */
    ...
    /**
     ...
     then data will NOT be added to RinexObsData. NB. This assumes that all
     the SatPasses have the same obs in the same order!!
    ...
    */
    ...
    /**
     get a map of pairs of indexes for the current epoch. call this after
     calling next() to get pairs (i,j) where the data returned by next() is
     the same as SatPassList[i].data(j,<obstype>), for each i in the map,
     and j=map[i].
    */
  ...
  private:
    ...
    /**
     time step (seconds) found in all objects in the list (constructor
     throws if they are not identical).
    */
    double DT;

    /**
     first (earliest) start time (getFirstGoodTime()) of the passes in the
     list, and last (latest) end time (getLastGoodTime()) of the passes in
     the list.
    */
    ...
    */
    ...
    /**
     map of indexes i,j, created by next(), such that data returned by
     next() is found at SatPassList[i].spdvector[j] where map[i]=j.
    */
