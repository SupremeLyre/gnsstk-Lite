...
//------------------------------------------------------------------------------------
inline long Stem(double x, double &scale)
{
    return (long(x / scale));
}

//------------------------------------------------------------------------------------
void Robust::StemLeafPlot(std::ostream &os, double *xd, long nd, const std::string &msg)
{
    ...
    for (l = 0; l < nd; l++)
    {
        ...
        if (start || s != stem || (s == 0 && pos * sign < 0.0))
        {
            ...
            { // also print stems without leaves
                ...
                if ((s == sQ3 && (s != 0 || pos * Q3 < 0.0)) || (s == sQ1 && (s != 0 || pos * Q1 < 0.0)))
                {
...
int Robust::RobustPolyFit(double *xd, const double *td, int nd, int N, double *c, double *w)
{
    try
    {
        if (!xd || !td || !c || nd < 2)
...
/* Anderson-Darling test statistic, which is a variant of the
   ...
   @param save_flag  if true (default) array xd will NOT be changed, otherwise
                        it will be sorted. */
double gnsstk::ADtest(double *xd, const int nd, double mean, double stddev, bool save_flag)
{
    if (!xd || nd < 2)
    {
        Exception e("Invalid input");
    ...
    try
    {
        ...
        for (i = 0; i < nd; i++)
        {
            ...
            //     << " " << setw(11) << cdf << endl;
            AD -= ((2.0 * double(i) - 1.0) * ::log(cdf) + (2.0 * (tn - double(i)) + 1.0) * ::log(1.0 - cdf)) / tn;
        }
