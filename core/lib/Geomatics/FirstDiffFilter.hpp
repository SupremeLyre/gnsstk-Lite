...
/** @file FirstDiffFilter.hpp
       This class implements a statistical filter that uses first-differences.
...
//---------------------------------------------------------------------------------
/**
 This class computes the first difference of the input data array. It will
 read the data, compute the first difference and store it, then analyze
 the result, returning a vector of simple results giving outliers and
 statistics on the data between outliers.
*/
template <class T> class FirstDiffFilter
{
  public:
    ...
    */
    ...
    /**
     ...
     @param npts  number of data to process, defaults to -1,
                meaning to end of data.
     @return > 0 number of points in the analysis vector == number good
     data
    ...
    */
    ...
    /**
     analyze the output of the filter(), filling the analysis array
     'analvec' also fills the 'results' vector of one or more FilterHit.
     Uses a different algorithm - sliding window ave/sigma of fdiff on
     either side of, vs fdiff at, point of interest.
    ...
    */
    ...
    {
        std::string msg;
        return analyze2(ratlim, siglim, false, msg);
    }

    /**
     Overloaded version with analysis dump to string reference; cf. other
     versions
     @param ratlim limit on |ratio fdiff to \<sigmas\>| (also fdiff > fdlimit for hit)
    ...
    */
    ...
    /**
     ...
     if the first point(s) are outliers, analyze() makes the first
     FilterHit BOD with only 1 point, then either outliers or a slip; fix
     this by making first FilterHit outliers. Called at end of analyze()
    */
    void fixUpResults();
    ...
    */
    ...
    /**
     compute stats on the first differences within the given FilterHit,
     store in Seg NB this must be called on filter.results[i] in order to
     show stats in dump()
    */
  ...
  private:
    /**
     private version of analyze2 with all arguments, cf. overloaded
     versions' doc.
     @param ratlim limit on |ratio fdiff to \<sigmas\>| (also fdiff > fdlimit for hit)
...
//---------------------------------------------------------------------------------
template <class T> int FirstDiffFilter<T>::analyze()
{
    ...
    for (i = 0; i < analvec.size(); i++)
    {
        ...
        if (::fabs(analvec[i].diff) > fdlimit)
        {
            ...
            prevIsBad = true;
            // LOG(INFO) << "bad index " << analvec[i].index << " nbad " <<
            // nbad;
        }
        else if (!prevIsBad)
        { // good 1st diff following good 1st diff
            // LOG(INFO) << "good index " << analvec[i].index;
            igood = i;
        }
        else
        { // good 1st diff following bad one(s)
            // LOG(INFO) << "good after bad index "<< analvec[i].index<< " nbad
            // " << nbad;
            results[curr].ngood -= nbad + 1; // finish the current segment

            if (::fabs(sumbad) > fdlimit)
            { // its a slip
...
//---------------------------------------------------------------------------------
// fix some potential problems in the results vector, namely:
// if the first point(s) are outliers, analyze() makes the first FilterHit
// BOD with only 1 point (because the first point is good *by definition*),
// then either outliers or a slip; fix this by making first
// FilterHit outliers.
template <class T> void FirstDiffFilter<T>::fixUpResults()
{
    if (results[0].npts > 1)
    {
        return;
...
//---------------------------------------------------------------------------------
template <class T> int FirstDiffFilter<T>::analyze2(double ratlim, double siglim, bool dump, std::string &dumpmsg)
{
    ...
    if (dump)
    {
        oss << "FirstDiff analyze2" << std::fixed << std::setprecision(3) << " fdlimit=" << fdlimit
            << " siglim=" << siglim << " ratlim=" << ratlim << "\n# index xdata data diff step5  ave sig7 rat8  "
    ...
    // try {
    // for(i=i0; i<size+N-1; i++) {                   // i=index=1,...
    // j=count=0,...
    //   j=i-i0+1;                                 // NB i0 is changed when
    //   hit found if(i<size) fstats.Add(analvec[i].diff); if(j>N)
    //   fstats.Subtract(analvec[i-N].diff); if(j>N+1)
    //   pstats.Add(analvec[i-N-1].diff); if(j>2*N+1)
    //   pstats.Subtract(analvec[i-2*N-1].diff);

    //   std::cout << "TST " << std::setw(4) << i << std::fixed<<
    //   std::setprecision(3); if(j>N+2) std::cout
    ...
    //   std::cout << std::endl;
    //}
    //} catch(gnsstk::Exception& e) { std::cout << "TST " << e.what() <<
    // std::endl; } fstats.Reset(); pstats.Reset();
    //// END TEST

    // loop over first differences
    // keep stats on the N points before, and N points after, the current
    // point NB skip the first pt b/c diff==0 and it means nothing
    for (j = 0, i = i0; i < size + N - 1; i++)
    {                          // NB i0 changes when hit
        results[curr].ngood++; // count it; only good gets into analvec
        ...
        if (j > N)
        {
            fstats.Subtract(analvec[i - N].diff);
        }
        if (j > N + 1)
        {
            pstats.Add(analvec[i - N - 1].diff);
        }
        if (j > 2 * N + 1)
        {
            pstats.Subtract(analvec[i - 2 * N - 1].diff);
        ...
        if (j > N + 2 && pstats.N() > 2)
        {
            ...
            double pr(::fabs(diff - pa) / ps),
                fr(::fabs(diff - fa) / fs); // ratios step/sigma
            ...
            if (dump)
            {
                ...
                // psig prat fave fsig frat [SLIP] [gap]"
                oss << i << std::fixed << std::setprecision(3) << " "
                    << (noxdata ? T(analvec[i - N - 1].index) : xdata[analvec[i - N - 1].index]) << " "
                    << data[analvec[i - N - 1].index] << " " << diff << " " << step << "  " << avefd << " " << sig
                    << " " << rat << "  " << pa << " " << ps << " " << pr << "  " << fa << " " << fs << " " << fr
                    << (hitslip ? " SLIP" : "") << (hisig ? " SIG" : "");
                if (!noxdata && tstep / tstepmin - 1 > 5)
                {
                    oss << " gap(" << tstepmin << "<=" << tstep << "<=" << tstepmax << ")";
                }
                oss << (::fabs(step) > fdlimit ? " step" : "") << (sig < siglim ? " sig" : "")
                    << (rat > ratlim ? " rat" : "") << (pr > ratlim ? " prat" : "") << (fr > ratlim ? " frat" : "");
                oss << std::endl;
            }
...
//---------------------------------------------------------------------------------
template <class T> void FirstDiffFilter<T>::dump(std::ostream &os, const std::string &tag)
{
    size_t i, j, k;
    os << "#" << tag << " FirstDiffFilter::dump() with limit " << std::fixed << std::setprecision(osp) << fdlimit
    ...
    for (i = 0, j = 0, k = 0; i < ilimit; i++)
    {
        if (j >= N || i != analvec[j].index)
        {
            if (dumpNA)
            {
                os << tag << std::fixed << std::setprecision(osp) << " " << std::setw(3) << i << " " << std::setw(osw)
                   << (noxdata ? T(i) : xdata[i])
                   //<< " " << std::setw(3) << (noflags ? 0 : flags[i])
                   << " " << std::setw(osw) << data[i] << " " << std::setw(osw) << 0.0 << "  NA" << std::endl;
            }
        }
        else
        {
            os << tag << std::fixed << std::setprecision(osp) << " " << std::setw(3) << i << " " << std::setw(osw)
               << (noxdata ? T(i) : xdata[i])
               //<< " " << std::setw(3) << (noflags ? 0 : flags[i])
               << " " << std::setw(osw) << data[i] << " " << std::setw(osw) << (j >= N ? 0.0 : analvec[j].diff);
            if (k < results.size() && i == results[k].index)
            {
                os << "  "
                   << (results[k].mad != T(0) ? // has getStats been called?
                           results[k].asStatsString(osp)
                                              : results[k].asString());
                k++;
            }
            os << std::endl;
...
//---------------------------------------------------------------------------------
// compute stats on the segment of first differences given by the input
// FilterHit
template <class T> void FirstDiffFilter<T>::getStats(FilterHit<T> &fe)
{
    int j(-1);
    unsigned int i, k;
    fe.min = fe.max = fe.med = fe.mad = T(0);
    for (i = 0; i < analvec.size(); i++)
        if (analvec[i].index == fe.index)
        {
            j = i;
            break;
        }
