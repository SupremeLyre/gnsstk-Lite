...
//--------------------------------------------------------------------------------
/**
 Class to determine fundamental information about a RINEX obs file, from
 both the header and by reading part or all of the file, and then load a
 requested subset of the data into a store. NB. more than one file can be
 given, but it is assumed these files are "congruent"
 - from the same site, with the same obs types, and in time order - as if
 one file. The operation of the class is as follows.
 1. Declare an object, and give it a list of (Rinex Obs) files
    [cf. ctor(filename) or ctor(filenames) or member files(filenames)].
 2. Configure the object, using e.g. saveTheData(true), excludeSat(sat),
 etc
 3. Specify which ObsIDs to save - e.g. GC1* GC2* GL1* GL2*
 4. Run loadFiles(msg) to read the files (any error messages output in
 msg)
 5. Read the output: dumpSatObsTable() or dumpData() [if saved], and
 access output
 6. Optionally write the output to vector of SatPass with
 WriteSatPassList()
...
class Rinex3ObsFileLoader
{
  private:
    ...
    /**
     input list of wanted ObsIDs, may include "*"-the "any"
     sys/type/band/trk-code thus when the header is read this list is
     expanded into wantedObsTypes
    */
    ...
    */
    ...
    /**
     map[sat]=string containing space-separated obs types to be read into
     SatPass cf. cfgEditObsTypes()
    */
  ...
  public:
    ...
    */
    ...
    /**
     Add to list of RINEX 3 ObsIDs (4-character) that are desired for
     output Must be a valid 4-char ObsID, must begin with system character,
     and must not be a duplicate. E.g. GC1* GC2* GL1* GL2* JS1Z RL2C
    ...
    */
    inline void nEpochsToRead(int n)
    {
        nepochsToRead = n;
    }
    ...
    */
    inline void saveTheData(bool b)
    {
        saveData = b;
    }
    ...
    */
    inline void setStartTime(const CommonTime &tt)
    {
        startTime = tt;
    }
    ...
    */
    inline void setStopTime(const CommonTime &tt)
    {
        stopTime = tt;
    }
    ...
    /**
     ...
     @param[in] dt decimate data to this time step (on even
     seconds-of-week)
    */
    inline void setDecimation(double dt)
    {
        dtdec = dt;
    }
    ...
    /**
     ...
     dumps
    */
    inline void setTimeFormat(std::string fmt)
    {
        timefmt = fmt;
    }
    ...
    /**
     ...
     @param[in] sat satellite to be excluded [do not exclude system this
     way]
    */
    inline void excludeSat(SatID sat)
    {
        exSats.push_back(sat);
    }
    ...
    */
    ...
    inline CommonTime getDataBeginTime() const
    {
        return begDataTime;
    }

    /// @return end time of the entire dataset
    inline CommonTime getDataEndTime() const
    {
        return endDataTime;
    }
    ...
    */
    inline std::vector<std::string> getWantedObsTypes() const
    {
    ...
    /**
     ...
     @return map<RinexSatID, vector<int>> of counts of data found for
     Sat/ObsID
    */
    inline std::map<RinexSatID, std::vector<int>> getWantedSatObsCountMap() const
    {
    ...
    */
    inline std::vector<int> getTotalObsCounts() const
    {
    ...
    */
    inline const int getStoreSize() const
    {
    ...
    */
    inline const std::vector<Rinex3ObsData> &getStore() const
    {
        return datastore;
    }
    ...
    */
    ...
    /**
     ...
     written to. SPList need not be empty; however if not empty, obstypes
     must be identical to those of existing SatPasses.
     ...
     @return >0 number of passes created,
             -1 inconsistent input,
             -2 obstypes inconsistent with existing SatPass,
             -3 Loader not configured to save data,
             -4 no data -5 obstypes not provided for all systems
    */
    int WriteSatPassList(const std::map<char, std::vector<std::string>> &obstypes,
                         const std::map<char, std::vector<int>> &indexLoadOT, std::vector<SatPass> &SPList);
...
/**
 Utility to dump a table of all valid RinexObsIDs for all systems and
 frequencies
 @param s to which to write the data
*/
