...
/** @file SRIleastSquares.hpp
    ...
    implements a general least squares algorithm that includes linear or
    linearized problems, weighting, robust estimation, and sequential
    estimation.*/
...
/**
 ...
 algorithm using SRI, including weighted, linear or linearized, robust
...
class SRIleastSquares : public SRI
{
  public:
    ...
    */
    ...
    /**
     A general least squares update, NOT the SRIF (Kalman) measurement
     update. Given data and measurement covariance, compute a solution and
     ...
     @param Cov Covariance matrix, dimension (N,N)
                   Input:  (If doWeight is true) inverse measurement
                   covariance
     ...
     @param LSF Pointer to a function which is used to define the equation
                ...
                (output)
            When doLinearize is false, LSF ignores X and returns the
            (constant) partials matrix P and zero for f(X).
     @throw MatrixException if the input is inconsistent
     Return values: 0 ok
                   -1 Problem is underdetermined (M<N) // TD -- naturalized
                   sol? -2 Problem is singular -3 Algorithm failed to
                   converge -4 Algorithm diverged
    */
    ...
    */
    ...
    /**
     Return true if the solution is valid, i.e. if the problem is
     non-singular.
    */
    bool isValid()
    {
    ...
    */
    ...
    /**
     Get the number of iterations used in last call to
    ...
    */
    ...
    /**
     Get the convergence value found in last call to
     leastSquaresEstimation()
     @return the convergence value
    */
    double convergence()
    {
        return rmsConvergence;
    }
    ...
    */
    ...
    /**
     if true, equation F(X)=D is non-linear, the algorithm will be
     iterated, and LSF must return partials matrix and F(X). - default is
     false
    */
    bool doLinearize;
  ...
  private:
    ...
    void defaults()
    {
        ...
        doSequential = false;
        doVerbose = false;
        numberIterations = numberBatches = 0;
        rmsConvergence = conditionNum = 0.0;
        valid = false;
    }
