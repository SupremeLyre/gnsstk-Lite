...
namespace gnsstk
{
/** class SRIFilter inherits SRI and implements a square root information
 * filter, which is the square root formulation of the Kalman filter
 * algorithm. SRIFilter may be used for Kalman filtering, smoothing, or for
 * simple least squares, including weighted, linear or linearized, robust
 * and/or sequential algorithms.
 *
 * At any point the state X and covariance P are related to the SRI by
 * X = inverse(R) * z , P = inverse(R) * inverse(transpose(R)), or
 * R = upper triangular square root (Cholesky decomposition) of the inverse
 * of P, and z = R * X.
 *
 * The SRIFilter implements Kalman filter algorithm, which includes
 * sequential least squares (measurement update), dynamic propagation (time
 * update), and smoothing (technically the term 'Kalman filter algorithm' is
 * reserved for the classical algorithm just as Kalman presented it, in terms
 * of a state vector and its covariance matrix).
 *
 * The SRIFilter measurment update (which is actually just linear least
 * squares) is half of the SRIFilter (Kalman filter) - there is a 'time
 * update' that propagates the SRI (and thus the state and covariance)
 * forward in time using the dynamical model of the filter. These are
 * algebraically equivalent to the classical Kalman algorithm, but are more
 * efficient and numerically stable (actually the Kalman algorithm has been
 * shown to be numerically unstable!). The SRIFilter smoothing algorithms
 * consists of a 'backwards' filter, implemented by applying a 'smoother
 * update' to the SRIFilter at each point in reverse order.
 *
...
class SRIFilter : public SRI
{
  public:
    ...
    */
    SRIFilter(const Matrix<double> &R, const Vector<double> &Z, const Namelist &NL);
    ...
    */
    ...
    /**
     SRIF (Kalman) simple linear measurement update with optional weight
     matrix
     ...
     @throw SomeUnknownException if dimension N does not match dimension of
          SRI, or if other dimensions are inconsistent, or if CM is singular.
    ...
    */
    ...
    /**
     ...
     SRIFilter state and covariance through a time step. If the existing
     SRI state is of dimension n, and the number of noise parameter is ns,
     then the inputs must be dimensioned as indicated.
     ...
     @param Rw Matrix<double>
            ...
            noise, an ns by ns upper triangular matrix. Rw is modified on
            output.
     @param G Matrix<double>
            The n by ns matrix associated with process noise, which relates
            the state to the noise variables.  The process noise covariance
            is G*Q*transpose(G) where inverse(Q) is transpose(Rw)*Rw. G is
            destroyed on output.
     @param Zw Vector<double>
            a priori 'state' associated with the process noise,
            a vector with ns elements.  Usually set to zero by
            the calling routine (for unbiased process noise). Used for
            output.
     ...
     noise, ns may be zero.  [Ref. Bierman ftnt pg 122 seems to indicate
     that variables with zero process noise can be handled by ns=n &
     setting a column of G=0.  But note that the case of the matrix G=0 is
     the same as ns=0, because the first ns columns would be zero below the
     ...
     matrix formed by concatenation of the input matricies.  Two
     preliminary steps are to form Rd = R*PhiInv (stored in PhiInv) and
     -Rd*G (stored in G) by matrix multiplication, and to set Rwx to the
     zero matrix. Then the Householder transformation is applied to the
     following matrix, dimensions are shown in ():
          _  (ns)   (n)   (1)  _          _                  _
     ...
     where perturbation in model X =
                   Sen * diagonal(a priori sigmas of parameter
                   uncertainties).
    ...
    */
    void timeUpdate(Matrix<double> &PhiInv, Matrix<double> &Rw, Matrix<double> &G, Vector<double> &Zw,
                    Matrix<double> &Rwx);
    ...
    /**
     ...
     @param G Matrix<double>
            The n by ns matrix associated with process noise, which relates
            the state to the noise variables.  The process noise covariance
            is G*Q*transpose(G) where inverse(Q) is transpose(Rw)*Rw, also
            input to the time update. G is destroyed on output.
     @param Zw Vector<double>
            A priori 'state' associated with the process noise,
            a vector with Ns elements, output of the time update.
     @param Rwx Matrix<double> An Ns by N matrix, output of the time update.

     The inputs Rw,Zw,Rwx are the output of the SRIF time update, and these
     and Phi and G are associated with the same timestep. All the inputs
     are trashed on output.
     ...
     Dyer-McReynolds SRIS algorithm, which uses less computer resources,
     but propagates the state and covariance rather than the SRI (R,z). [As
     always, at any point the state X and covariance P are related to the
     SRI by X = inverse(R) * z , P = inverse(R) * inverse(transpose(R)).]
    ...
    */
    void smootherUpdate(Matrix<double> &Phi, Matrix<double> &Rw, Matrix<double> &G, Vector<double> &Zw,
                        Matrix<double> &Rwx);

    /**
     Covariance/State version of the Kalman smoother update
     (Dyer-McReynolds). This routine implements the Dyer-McReynolds form of
     the state and covariance recursions which constitute the backward
     filter of the Square Root Information Smoother; it is equivalent to
     the SRI form implemented in SRIFilter::smootherUpdate(). NB. This
     routine does NOT use the SRIFilter object; it is implemented as a
    ...
    */
    static void DMsmootherUpdate(Matrix<double> &P, Vector<double> &X, Matrix<double> &Phinv, Matrix<double> &Rw,
                                 Matrix<double> &G, Vector<double> &Zw, Matrix<double> &Rwx);

    /** Modification for case with control vector: Xj+1 = Phi*Xj + Gwj + u
     * @throw MatrixException
     */
    static void DMsmootherUpdateWithControl(Matrix<double> &P, Vector<double> &X, Matrix<double> &Phinv,
                                            Matrix<double> &Rw, Matrix<double> &G, Vector<double> &Zw,
  ...
  private:
     ...
     */
    template <class T>
    static void SrifTU(Matrix<T> &R, Vector<T> &Z, Matrix<T> &Phi, Matrix<T> &Rw, Matrix<T> &G, Vector<T> &Zw,
                       Matrix<T> &Rwx);
     ...
     */
    template <class T>
    static void SrifSU(Matrix<T> &R, Vector<T> &Z, Matrix<T> &Phi, Matrix<T> &Rw, Matrix<T> &G, Vector<T> &Zw,
                       Matrix<T> &Rwx);
     ...
     */
    template <class T>
    static void SrifSU_DM(Matrix<T> &P, Vector<T> &X, Matrix<T> &Phinv, Matrix<T> &Rw, Matrix<T> &G, Vector<T> &Zw,
                          Matrix<T> &Rwx);
    ...
    // Matrix<double> R;
    // inherit SRI state vector, of length equal to dimension (row and col) of
    // R.
    // Vector<double> Z;
    // inherit SRI Namelist parallel to R and Z, labelling elements of state
    // vector.
    // Namelist names;

    // --------- private member data ------------
    // TD how to implement valid?
    // indicates if filter is valid - set false when inversion finds
    // singularity.
    // bool valid;
