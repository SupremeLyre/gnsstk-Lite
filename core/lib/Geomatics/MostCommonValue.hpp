...
//--------------------------------------------------------------------------------
/**
 Simple class to find the most common value, within a given tolerance, in
 a given set of values. Used to find, e.g. the time interval of a RINEX
 obs file. Given enough examples (provided to add(dt)), it should be clear
 what the nominal or most common value is. Use a tolerance to
 differentiate dt's, default 0.001.
*/
...
#define DEFAULT_TOL 0.00095 // less than a millisecond
  ...
  public:
    /**
     only ctor; argument is the maximum number of values to remember and
     count (9)
    */
    MostCommonValue(int i = DEFAULT_DIM) : tol(DEFAULT_TOL)
    {
        reset(i);
    }
    ...
    inline void add(double dt)
    {
        for (int j = 0; j < N; j++)
        {
            // first time this value has been seen, and there is room to
            // consider it
            if (j >= counts.size())
            {
                counts.push_back(1);
            ...
            if (j == N - 1)
            {
                ...
                counts[kk] = 1;
                values[kk] = dt; // replace it
            }
        }
    }

    /// return the best estimate of timestep
    inline double bestDT()
    {
        if (counts.size() == 0)
        {
            return 0.0;
    ...
    {
        os << " MostCommonValue::dump(); tolerance is " << std::fixed << std::setprecision(prec) << tol << std::endl;
        ...
        for (int j = 0; j < counts.size(); j++)
            os << " " << std::setw(3) << counts[j] << " " << std::fixed << std::setprecision(prec) << values[j]
               << std::endl;
    }
