...
/**
 * @file SRIFilter.cpp
 * Implementation of filter using class SRI.
...
SRIFilter::SRIFilter(const Matrix<double> &Rin, const Vector<double> &Zin, const Namelist &NLin)
{
    defaults();
    if (Rin.rows() != Rin.cols() || Rin.rows() != Zin.size() || Rin.rows() != NLin.size())
    {
        MatrixException me("Invalid input dimensions: R is " + asString<int>(Rin.rows()) + "x" +
                           asString<int>(Rin.cols()) + ", Z has length " + asString<int>(Zin.size()) +
                           ", and NL has length " + asString<int>(NLin.size()));
        GNSSTK_THROW(me);
    }
    R = Rin;
    Z = Zin;
...
void SRIFilter::measurementUpdate(const Matrix<double> &H, Vector<double> &D, const Matrix<double> &CM)
{
    if (H.cols() != R.cols() || H.rows() != D.size() ||
        (&CM != &SRINullMatrix && (CM.rows() != D.size() || CM.cols() != D.size())))
    {
        string msg("\nInvalid input dimensions:\n  SRI is ");
        msg += asString<int>(R.rows()) + "x" + asString<int>(R.cols()) + ",\n  Partials is " + asString<int>(H.rows()) +
               "x" + asString<int>(H.cols()) + ",\n  Data has length " + asString<int>(D.size());
        if (&CM != &SRINullMatrix)
        {
            msg += ",\n  and Cov is " + asString<int>(CM.rows()) + "x" + asString<int>(CM.cols());
        }
...
void SRIFilter::measurementUpdate(const SparseMatrix<double> &H, Vector<double> &D, const SparseMatrix<double> &CM)
{
    if (H.cols() != R.cols() || H.rows() != D.size() ||
        (&CM != &SRINullSparseMatrix && (CM.rows() != D.size() || CM.cols() != D.size())))
    {
        string msg("\nInvalid input dimensions:\n  SRI is ");
        msg += asString<int>(R.rows()) + "x" + asString<int>(R.cols()) + ",\n  Partials is " + asString<int>(H.rows()) +
               "x" + asString<int>(H.cols()) + ",\n  Data has length " + asString<int>(D.size());
...
void SRIFilter::timeUpdate(Matrix<double> &PhiInv, Matrix<double> &Rw, Matrix<double> &G, Vector<double> &Zw,
                           Matrix<double> &Rwx)
{
    try
    {
...
void SRIFilter::smootherUpdate(Matrix<double> &Phi, Matrix<double> &Rw, Matrix<double> &G, Vector<double> &Zw,
                               Matrix<double> &Rwx)
{
    try
    {
...
//---------------------------------------------------------------------------------
void SRIFilter::DMsmootherUpdate(Matrix<double> &P, Vector<double> &X, Matrix<double> &Phinv, Matrix<double> &Rw,
...
/* Kalman time update.
   ...
   ------------------------------------------------------------------- */
template <class T>
void SRIFilter::SrifTU(Matrix<T> &R, Vector<T> &Z, Matrix<T> &PhiInv, Matrix<T> &Rw, Matrix<T> &G, Vector<T> &Zw,
                       Matrix<T> &Rwx)
{
    ...
    if (PhiInv.rows() < n || PhiInv.cols() < n || G.rows() < n || G.cols() < ns || R.cols() != n || Rwx.rows() < ns ||
        Rwx.cols() < n || Z.size() < n || Zw.size() < ns)
    {
        MatrixException me(
            "Invalid input dimensions:\n  R is " + asString<int>(R.rows()) + "x" + asString<int>(R.cols()) +
            ", Z has length " + asString<int>(Z.size()) + "\n  PhiInv is " + asString<int>(PhiInv.rows()) + "x" +
            asString<int>(PhiInv.cols()) + "\n  Rw is " + asString<int>(Rw.rows()) + "x" + asString<int>(Rw.cols()) +
            "\n  G is " + asString<int>(G.rows()) + "x" + asString<int>(G.cols()) + "\n  Zw has length " +
            asString<int>(Zw.size()) + "\n  Rwx is " + asString<int>(Rwx.rows()) + "x" + asString<int>(Rwx.cols()));
        GNSSTK_THROW(me);
    }
...
template <class T>
void SRIFilter::SrifSU(Matrix<T> &R, Vector<T> &Z, Matrix<T> &Phi, Matrix<T> &Rw, Matrix<T> &G, Vector<T> &Zw,
                       Matrix<T> &Rwx)
{
    unsigned int N = R.rows(), Ns = Rw.rows();

    if (Phi.rows() < N || Phi.cols() < N || G.rows() < N || G.cols() < Ns || R.cols() != N || Rwx.rows() < Ns ||
        Rwx.cols() < N || Z.size() < N || Zw.size() < Ns)
    {
        MatrixException me(
            "Invalid input dimensions:\n  R is " + asString<int>(R.rows()) + "x" + asString<int>(R.cols()) +
            ", Z has length " + asString<int>(Z.size()) + "\n  Phi is " + asString<int>(Phi.rows()) + "x" +
...
/* Covariance/State version of the Kalman smoother update (Dyer-McReynolds).
   ...
   Ref: Bierman, G.J. "Factorization Methods for Discrete Sequential
        Estimation," Academic Press, 1977, pg 216. */
template <class T>
void SRIFilter::SrifSU_DM(Matrix<T> &P, Vector<T> &X, Matrix<T> &Phinv, Matrix<T> &Rw, Matrix<T> &G, Vector<T> &Zw,
                          Matrix<T> &Rwx)
{
    unsigned int N = P.rows(), Ns = Rw.rows();

    if (P.cols() != P.rows() || X.size() != N || Rwx.cols() != N || Zw.size() != Ns || Rwx.rows() != Ns ||
        Rwx.cols() != N || Phinv.rows() != N || Phinv.cols() != N || G.rows() != N || G.cols() != Ns)
    {
        MatrixException me(
            "Invalid input dimensions:\n  P is " + asString<int>(P.rows()) + "x" + asString<int>(P.cols()) +
            ", X has length " + asString<int>(X.size()) + "\n  Phinv is " + asString<int>(Phinv.rows()) + "x" +
...
template <class T>
void DMsmootherUpdateWithControl(Matrix<double> &P, Vector<double> &X, Matrix<double> &Phinv, Matrix<double> &Rw,
                                 Matrix<double> &G, Vector<double> &Zw, Matrix<double> &Rwx, Vector<double> &U)
{
    unsigned int N = P.rows(), Ns = Rw.rows();

    if (P.cols() != P.rows() || X.size() != N || Rwx.cols() != N || Zw.size() != Ns || Rwx.rows() != Ns ||
        Rwx.cols() != N || Phinv.rows() != N || Phinv.cols() != N || G.rows() != N || G.cols() != Ns || U.size() != N)
    {
        MatrixException me("Invalid input dimensions:\n  P is " + asString<int>(P.rows()) + "x" +
                           asString<int>(P.cols()) + ", X has length " + asString<int>(X.size()) + "\n  Phinv is " +
                           asString<int>(Phinv.rows()) + "x" + asString<int>(Phinv.cols()) + "\n  Rw is " +
                           asString<int>(Rw.rows()) + "x" + asString<int>(Rw.cols()) + "\n  G is " +
                           asString<int>(G.rows()) + "x" + asString<int>(G.cols()) + "\n  Zw has length " +
                           asString<int>(Zw.size()) + "\n  Rwx is " + asString<int>(Rwx.rows()) + "x" +
