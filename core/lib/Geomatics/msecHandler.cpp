...
void msecHandler::setObstypes(const vector<string> &ots, const vector<double> &waves)
{
...
void msecHandler::add(CommonTime ttag, const SatID sat, const string obstype, double data)
{
    if (dt == -1.0)
    {
...
int msecHandler::afterAddbeforeFix()
{
    ...
    {
        rmvClk = false;
        fixMsg = string("Warning - cannot remove gross clock with only 1 ms adjust");
    }
    ...
    {
        doPR = true;
        fixMsg += string("Adjusts applied to pseudorange, ") + string("so apply fix to the timetags.");
    }
    else
    {
...
/* edit data by removing the millisecond adjusts, and optionally a piece-wise
   ...
   ttag. */
void msecHandler::fix(CommonTime &ttag, const SatID sat, const string obstype, double &data)
{
...
string msecHandler::getFindMessage(bool verbose)
{
    ...
    findMsg += string("Millisecond adjusts: ") + StringUtils::asString(adjMsgs.size() + badMsgs.size()) +
               string(" total adjusts found, ") + StringUtils::asString(badMsgs.size()) + string(" invalid");

    for (map<string, int>::iterator tit = typesMap.begin(); tit != typesMap.end(); ++tit)
    {
        findMsg += string("\n  Found ") + StringUtils::asString(tit->second) + string(" adjusts for ") + tit->first;
    }
...
void msecHandler::compute(const CommonTime ttag)
{
    ...
    if (prevttag != CommonTime::BEGINNING_OF_TIME)
    {
            ...
            { // just phases
                ...
                for (it = curr[i].begin(); it != curr[i].end(); ++it)
                {
                    if (past[i].find(it->first) != past[i].end())
                    {
                        ...
                        del = (curr[i][it->first] - past[i][it->first]) / (Rfact / wavelengths[i]);
        ...
        if (adj)
        {
            ...
            oss.str("");
            oss << "msAdjust " << printTime(currttag, "%04Y/%02m/%02d %02H:%02M:%06.3f = %4F %14.7g")
            ...
            { // found a valid adjust
                ...
                for (i = 0; i < N; i++)
                {
                    ...
                    oss << "," << fixed << setprecision(5)
                        << -double(nadj) * (wavelengths[i] == 0.0 ? Rfact : Rfact / wavelengths[i])
