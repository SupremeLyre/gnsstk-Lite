...
Matrix<double> northEastUp(Position &pos, bool geocentric)
{
    try
    {
        ...
        double lat = (geocentric ? pos.getGeocentricLatitude() : pos.getGeodeticLatitude()) * DEG_TO_RAD; /// rad N
 ...
 */
void satelliteNadirAzimuthAngles(const Position &sv, const Position &rx, const Matrix<double> &rot, double &nadir,
                                 double &azimuth)
{
    try
    {
 ...
 */
double satelliteYawAngle(const Position &pos, const Position &vel, const Position &sun, const bool &blkIIRF,
                         double &yawrate)
{
    try
    {
