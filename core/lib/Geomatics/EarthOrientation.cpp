...
/** @file EarthOrientation.cpp
    class EarthOrientation encapsulates the Earth orientation parameters or
    EOPs. Also, this class includes several static functions that implement the
    models in the IERS Conventions, many of which involve the EOPs. Cf. classes
    EOPStore and SolarSystem.
*/
...
{
    os << " " << fixed << setw(10) << setprecision(6) << eo.xp << " " << setw(10) << setprecision(6) << eo.yp << " "
       << setw(11) << setprecision(7) << eo.UT1mUTC << " " << eo.convention;
    return os;
}
...
/* Corrections to EOPs xp,yp,UT1mUTC for diurnal and semi-diurnal variations
   due to ocean tides. Based on IERS routine ortho_eop.f from the USNO web
   site for the 2010 conventions (NB the 2003 version of ortho_eop is
   nominally different, however in fact the algorithm and constants are the
   same; the only difference is that the 2003 algorithm (FTN) is single
   precision). param mjd time (UTC) of interest param dxp correction to xp
   in arcseconds param dyp correction to yp in arcseconds param dUT
   correction to UT1mUTC in seconds */
static void correctEOPOceanTides(double mjd, double &dxp, double &dyp, double &dUT);

/* Corrections to Earth rotation due to zonal tides using USNO IERS2010
   algorithm. param args array of 6 fundamental arguments at T, units
   radians
...
/* Given parallel arrays of length four containing the values from EOPStore
   for time (int MJD) and EOPs xp, yp, and UT1mUTC, where the time of
   interest t lies within the values of the time array, interpolate and apply
...
void EarthOrientation::interpolateEOP(const EphTime &t, const vector<double> &time, const vector<double> &X,
                                      const vector<double> &Y, vector<double> &dT, const IERSConvention &in_conv)
{
    int i, j;
    ...
    for (i = 0; i < time.size(); i++)
    {
        ...
        dT[i] -= dUT;
        // LOG(INFO) << " UT " <<fixed<< setprecision(10) << time[i] << " " <<
        // dT[i];
    }

    // 1b. interpolate UT1-UTC -------------------------------------------
    UT1mUTC =
        LagrangeInterpolation(time, dT, mjdUTC, err); // seconds
                                                      // LOG(INFO) << " -> " << fixed << setprecision(10) << mjdUTC
    ...
    /* 3a. restore long period tides to UT1-UTC --------------------------
       ...
       differences between 2003 and 2010 are very small, and only in the Zonal
       tides */
    if (convention == IERSConvention::IERS2010)
    {
        correctEarthRotationZonalTides(args, dUT, dlod, domega);
    ...
    UT1mUTC += dUT;
    // LOG(INFO) << " Zonal tides UT1 correction " <<fixed<<setprecision(15)
    // << dUT;

    /* step 3b. --------------------------------------
       Corrections to EOP due to diurnal and semidiurnal effects of ocean
       tides using USNO algorithm */
    ...
    /* LOG(INFO) << " Tides corrections " << fixed << setprecision(15)
       ...
       UT1mUTC += dUT;
       LOG(INFO) << " Librations UT1 correction " << fixed<<setprecision(15)
       << dUT; */
}

//---------------------------------------------------------------------------------
void computeFundamentalArgs(double T, double args[6])
{

    /* compute arguments and their time derivatives GMST+pi, L, Lp, F, D,
       Omega units are radians and radians/day NB to compute corrections to
       length of day (LOD), remove '//LOD' everywhere
       LOD double dargs[6];
       LOD static const double AS_TO_RAD_PER_DAY=(ARCSEC_TO_RAD/36525.0);

       GMST+pi
       GMST in seconds of day. NB 3155760000.0 = 876600.0*3600.0 */
    double GMST = ::fmod(67310.54841 + T * (3155760000.0 + 8640184.812866 + T * T * (0.093104 + T * (-6.2e-6))),
                         86400.0); // seconds of day
                                   /* convert to arcsec, then radians : 360*3600 arcsec / 24*3600 sec = 15
                                      as/sec */
    ...
    // LOD        + T*(3.*0.051635 + T*(-4.*0.00024470)))) *
    // AS_TO_RAD_PER_DAY;
...
//---------------------------------------------------------------------------------
void correctEOPOceanTides(double mjd, double &dxp, double &dyp, double &dUT)
{
    ...
    static const double fact[][2] = {{0.0298, 0.0200}, {0.1408, 0.0905}, {0.0805, 0.0638},
    ...
    static const Coeff C[] = {{2, 1, -1.94, 9.0899831, 5.18688050},    {2, 1, -1.25, 8.8234208, 5.38346657},
                              {2, 1, -6.64, 12.1189598, 5.38439079},   {2, 1, -1.51, 1.4425700, 5.41398343},
                              {2, 1, -8.02, 4.7381090, 5.41490765},    {2, 1, -9.47, 4.4715466, 5.61149372},
                              {2, 1, -50.20, 7.7670857, 5.61241794},   {2, 1, -1.80, -2.9093042, 5.64201057},
                              {2, 1, -9.54, 0.3862349, 5.64293479},    {2, 1, 1.52, -3.1758666, 5.83859664},
                              {2, 1, -49.45, 0.1196725, 5.83952086},   {2, 1, -262.21, 3.4152116, 5.84044508},
                              {2, 1, 1.70, 12.8946194, 5.84433381},    {2, 1, 3.43, 5.5137686, 5.87485066},
                              {2, 1, 1.94, 6.4441883, 6.03795537},     {2, 1, 1.37, -4.2322016, 6.06754801},
                              {2, 1, 7.41, -0.9366625, 6.06847223},    {2, 1, 20.62, 8.5427453, 6.07236095},
                              {2, 1, 4.14, 11.8382843, 6.07328517},    {2, 1, 3.94, 1.1618945, 6.10287781},
                              {2, 1, -7.14, 5.9693878, 6.24878055},    {2, 1, 1.37, -1.2032249, 6.26505830},
                              {2, 1, -122.03, 2.0923141, 6.26598252},  {2, 1, 1.02, -1.7847596, 6.28318449},
                              {2, 1, 2.89, 8.0679449, 6.28318613},     {2, 1, -7.30, 0.8953321, 6.29946388},
                              {2, 1, 368.78, 4.1908712, 6.30038810},   {2, 1, 50.01, 7.4864102, 6.30131232},
                              {2, 1, -1.08, 10.7819493, 6.30223654},   {2, 1, 2.93, 0.3137975, 6.31759007},
                              {2, 1, 5.25, 6.2894282, 6.33479368},     {2, 1, 3.95, 7.2198478, 6.49789839},
                              {2, 1, 20.62, -0.1610030, 6.52841524},   {2, 1, 4.09, 3.1345361, 6.52933946},
                              {2, 1, 3.42, 2.8679737, 6.72592553},     {2, 1, 1.69, -4.5128771, 6.75644239},
                              {2, 1, 11.29, 4.9665307, 6.76033111},    {2, 1, 7.23, 8.2620698, 6.76125533},
                              {2, 1, 1.51, 11.5576089, 6.76217955},    {2, 1, 2.16, 0.6146566, 6.98835826},
                              {2, 1, 1.38, 3.9101957, 6.98928248},     {2, 2, 1.80, 20.6617051, 11.45675174},
                              {2, 2, 4.67, 13.2808543, 11.48726860},   {2, 2, 16.01, 16.3098310, 11.68477889},
                              {2, 2, 19.32, 8.9289802, 11.71529575},   {2, 2, 1.30, 5.0519065, 11.73249771},
                              {2, 2, -1.02, 15.8350306, 11.89560406},  {2, 2, -4.51, 8.6624178, 11.91188181},
                              {2, 2, 120.99, 11.9579569, 11.91280603}, {2, 2, 1.13, 8.0808832, 11.93000800},
                              {2, 2, 22.98, 4.5771061, 11.94332289},   {2, 2, 1.06, 0.7000324, 11.96052486},
                              {2, 2, -1.90, 14.9869335, 12.11031632},  {2, 2, -2.18, 11.4831564, 12.12363121},
                              {2, 2, -23.58, 4.3105437, 12.13990896},  {2, 2, 631.92, 7.6060827, 12.14083318},
                              {2, 2, 1.92, 3.7290090, 12.15803515},    {2, 2, -4.66, 10.6350594, 12.33834347},
                              {2, 2, -17.86, 3.2542086, 12.36886033},  {2, 2, 4.47, 12.7336164, 12.37274905},
                              {2, 2, 1.97, 16.0291555, 12.37367327},   {2, 2, 17.20, 10.1602590, 12.54916865},
                              {2, 2, 294.00, 6.2831853, 12.56637061},  {2, 2, -2.46, 2.4061116, 12.58357258},
                              {2, 2, -1.02, 5.0862033, 12.59985198},   {2, 2, 79.96, 8.3817423, 12.60077620},
                              {2, 2, 23.83, 11.6772814, 12.60170041},  {2, 2, 2.59, 14.9728205, 12.60262463},
                              {2, 2, 4.47, 4.0298682, 12.82880334},    {2, 2, 1.95, 7.3254073, 12.82972756},
    ...
    for (k = 0; k < 3; ++k)
    {
        ...
        for (j = 0; j < 71; ++j)
        {
            ...
            alpha = ::fmod(C[j].phase - pinm, TWOPI) + ::fmod(C[j].freq * dt60, TWOPI);
    ...
    static const double orthowts[][3] = {
        {-6.77832, 14.86283, -1.76335}, {-14.86323, -6.77846, 1.03364}, {0.47884, 1.45234, -0.27553},
        {-1.45303, 0.47888, 0.34569},   {0.16406, -0.42056, -0.12343},  {0.42030, 0.16469, -0.10146},
        {0.09398, 15.30276, -0.47119},  {25.73054, -4.30615, 1.28997},  {-4.77974, 0.07564, -0.19336},
        {0.28080, 2.28321, 0.02724},    {1.94539, -0.45717, 0.08955},   {-0.73089, -1.62010, 0.04726}};
    // for(k=0; k<12; k++)
...
/* Corrections to Earth rotation due to zonal tides using USNO algorithm
   ...
   param dld correction to length of day in seconds/day
   param dom correction to yp in radians per second */
void correctEarthRotationZonalTides(const double args[6], double &dUT, double &dld, double &dom)
{
    // constants
    static const double TWOPI(6.283185307179586476925287);

    /* luni-solar coefficients in argument multiplying L Lp F D Omega, 62
       terms and coefficients of DUT sin, cos; DLOD cos, sin; DOMEGA cos, sin */
    typedef struct
    {
        int nargs[5];
        double dutsin, dutcos, dldcos, dldsin, domcos,
            domsin; // yes its sc cs cs
    } Coeff;

    static const Coeff C[] = {{{1, 0, 2, 2, 2}, -0.0235, 0.0000, 0.2617, 0.0000, -0.2209, 0.0000},
...
//---------------------------------------------------------------------------------
/* Corrections to Earth rotation due to zonal tides using USNO IERS2003
   algorithm param args array of 6 fundamental arguments at T, units radians
    NB args[0] (=GMST+pi) is not used
   param dUT correction to UT1mUTC in seconds
   param dld correction to length of day in seconds/day
   param dom correction to yp in radians per second */
void correctEarthRotationZonalTides2003(const double args[6], double &dUT, double &dld, double &dom)
{
    // constants
    static const double TWOPI(6.283185307179586476925287);
    ...
    static const Coeff C[] = {{{1, 0, 2, 2, 2}, -0.02, 0.00, 0.26, 0.00, -0.22, 0.00},
...
void correctEarthRotationLibrations(const double args[6], double &dUT, double &dld)
{
    ...
    static const Coeff C[] = {{{2, -2, 0, -2, 0, -2}, 0.5377239, 0.05, -0.03, -0.3, -0.6},
...
{
    return ::fmod(450160.398036 + T * (-6962890.2665 // diff Omega2003 only in .2665 vs .5431
...
double EarthOrientation::Lp(double T)
{
    return ::fmod(1287104.793048                                           // 357.52910918 * 3600
                      + T * (129596581.0481 + T * (-0.5532 + T * (0.000136 // NB this has a minus
                                                                           // sign in interp.f
                                                                  + T * (-0.00001149)))),
                  ARCSEC_PER_CIRCLE) *
           ARCSEC_TO_RAD;
...
//---------------------------------------------------------------------------------
/* Generate transformation matrix (3X3 rotation) due to the EOPs (polar
...
/* Generate the full transformation matrix (3x3 rotation) relating the ECEF
   ...
   Input is the time of interest; use this object's EOPs - the polar motion
   angles xp and yp (arcseconds), and UT1-UTC (seconds) (as found in the IERS
   bulletin). throw if convention is not defined */
...
/* Compute the transformation from ECEF to the J2000 dynamical (inertial)
   ...
   Matrix<double> EarthOrientation::ECEFtoJ2000(const EphTime& t, bool
   reduced)
  {
     try {
        if(convention != IERSConvention::IERS2010) {
        ...
        Matrix<double> FB =
        rotation(-psib,3)*rotation(phib,1)*rotation(gamb,3);
...
//------------------------------------------------------------------------------
/* Compute the locator s which gives the position of the CIO on the equator
   of the CIP, given the coordinate transformation time T. Consistent with
   IAU 2000A (IERS2003) precession-nutation, and P03 precession (IERS2010).
   and the coordinates X,Y of the CIP. Derived in part from SOFA routine
   s00.c for IERS2003 and s06.c for IERS2010. param T, the coordinate
   transformation time at the time of interest param X, the X coordinate of
   the CIP (input) param Y, the Y coordinate of the CIP (input) param which,
   the IERS convention to be used (input) return S, the parameter that
   positions the CIO on the CIP equator in radians.
   */
double EarthOrientation::S(double T, double &X, double &Y, IERSConvention which)
{
    ...
    // Based on table 5.2c IERS Tech Note 32 Chap 5 (tab5.2c.txt obtained from
    // IERS) also used SOFA routine s00.c

    // coefficients of polynomial in T for IERS2003
    static const double polycoeff[6] = {94.00e-6, 3808.35e-6, -119.94e-6, -72574.09e-6, 27.70e-6, 15.61e-6};
    // same for 2010
    static const double polycoeff2010[6] = {94.00e-6, 3808.65e-6, -122.68e-6, -72574.11e-6, 27.98e-6, 15.62e-6};
    ...
    // constant terms (T^0)
    static const Coeffs C0[] = {// indexes 1-10
    ...
    static const Coeffs C1[] = {{{0, 0, 0, 0, 2, 0, 0, 0}, -0.07e-6, 3.57e-6},
    ...
    static const Coeffs C2[] = {// indexes 1-10
...
//------------------------------------------------------------------------------
/* The position of the Terrestrial Ephemeris Origin (TEO) on the equator of
   the Celestial Intermediate Pole (CIP), as given by the quantity s'. Also
   called the Terrestrial Intermediate Origin (TIO). Valid 2003 and 2010 Ref.
   IERS Tech Note 32 Chap 5 Eqn 12 and IERS Tech Note 36 Chap 5 Eqn 5.13
...
//------------------------------------------------------------------------------
/* Compute the coordinates X,Y of the Celestial Intermediate Origin (CIO)
   using a series based on IAU 2006 precession and IAU 2000A nutation (IERS
   2010). The coordinates form a unit vector that points towards the CIO;
   they include the effects of frame bias, precession and nutation. cf. sofa
   xy06 Reference IERS(2010) Section 5.5.4 param T, the coordinate
   transformation time at the time of interest param X, x coordinate of CIO
...
//---------------------------------------------------------------------------------
/* Starting with 2003 (and valid for 2010) conventions a new method for
   computing the transformation fron ITRS to GCRS is provided by the
   Celestial Ephemeris Origin (CEO) which is based on the Earth Rotation
   Angle. cf. sofa ERA00.c param t EphTime time of interest param UT1mUTC
   offset UT1-UTC return Earth rotation angle in radians throw if the
   TimeSystem conversion fails (if t.system==TimeSystem::Unknown) */
double EarthOrientation::EarthRotationAngle(const EphTime &t, double &UT1mUTC)
{
    try
    {
        EphTime tUT1(t);
...
double EarthOrientation::equationOfEquinoxes2003(EphTime t)
{
    ...
    static const Coeffs Czero[] = {// l   lp  f   d   o   lv  le  pa     c(sin)     c(cos)
                                   // 1-10
...
void EarthOrientation::UT1mUTCTidalCorrections(double T, double &UT1mUT1R, double &dlodR, double &domegaR)
{
...
double EarthOrientation::obliquity1996(double T)
{
    /* double eps; // seconds of arc
       ...
       eps *= DEG_TO_RAD;
       return eps; */
    return (84381.448 + T * (-46.8150 + T * (-0.00059 + T * (0.001813)))) * ARCSEC_TO_RAD;
}
...
double EarthOrientation::GMST1996(EphTime t, double UT1mUTC, bool reduced)
{
    try
    {
        ...
        /* this should be the same ... seconds (24060s = 6h 41min)
           ...
           G = 0.279057273264 + 100.0021390378009*T        // seconds/86400 =
           days
...
double EarthOrientation::GMST2003(EphTime t, double UT1mUTC)
{
    try
    {
        ...
        G = ::fmod(era + (0.014506 + (4612.15739966 + (1.39667721 + (-0.00009344 + 0.00001882 * T) * T) * T) * T) *
                             ARCSEC_TO_RAD,
                   TWOPI);
...
double EarthOrientation::GMST2010(EphTime t, double UT1mUTC)
{
    try
    {
        ...
        /* double G = ::fmod(era+(0.014506 + (4612.156534 + (1.3915817 +
           ...
           return G; */
        return (era +
                (0.014506 + T * (4612.156534 // NB era in radians already
...
/* Helper to compute the Greenwich hour angle of the true vernal equinox, or
   ...
   dpsi = nutation in longitude (counted in the ecliptic),
                  in seconds of arc. */
double EarthOrientation::gast1996(EphTime t, double om, double eps, double dpsi, double UT1mUTC)
{
    try
    {
        ...
        double ee = dpsi * ::cos(eps) + (0.00264 * ::sin(om) + 0.000063 * ::sin(2.0 * om)) * ARCSEC_TO_RAD;

        LOG(DEBUG7) << "\nequequinox = " << fixed << setprecision(15) << showpos << 1.e3 * ee * RAD_TO_DEG << " / 1.e3";
        LOG(DEBUG7) << "\nGMST = " << fixed << setprecision(15) << showpos << G * RAD_TO_DEG;
...
/* Compute Greenwich Apparent Sidereal Time, or the Greenwich hour angle of
   ...
   GAST = GMST + dpsi*cos(eps) + 0.00264" * sin(Omega) +0.000063" *
   sin(2*Omega)
...
double EarthOrientation::GAST2003(EphTime t, double UT1mUTC)
{
    try
    {
        ...
        LOG(DEBUG7) << "\nee = " << fixed << setprecision(15) << showpos << 1.e6 * ee * RAD_TO_DEG << " / 1.e6";
        ee = dpsi * ::cos(epsa) + ee;
...
Matrix<double> EarthOrientation::polarMotionMatrix2003(EphTime t, double xp, double yp)
{
    double sp(Sprime(t)); // parameter s' provides position of TEO on CIP
    LOG(DEBUG7) << "\nsp = " << fixed << setprecision(15) << setw(18) << sp << " with T = " << coordTransTime(t);
    xp *= ARCSEC_TO_RAD;
    yp *= ARCSEC_TO_RAD;
...
void EarthOrientation::FukushimaWilliams(double T, double &gamb, double &phib, double &psib, double &eps)
{
    ...
    gamb = (-0.052928 + (10.556378 + (0.4932044 + (-0.00031238 + (-0.000002788 + 0.0000000260 * T) * T) * T) * T) * T) *
           ARCSEC_TO_RAD;
    phib =
        (84381.412819 + (-46.811016 + (0.0511268 + (0.00053289 + (-0.000000440 - 0.0000000176 * T) * T) * T) * T) * T) *
        ARCSEC_TO_RAD;
    psib =
...
/* Generate any of B = frame bias matrix
   ...
   return 3x3 rotation matrix */
Matrix<double> EarthOrientation::FukushimaWilliams(double gamb, double phib, double psib, double epsa)
{
    Matrix<double> R;
    R = rotation(-epsa, 1) * rotation(-psib, 3) * rotation(phib, 1) * rotation(gamb, 3);
    return R;
}
...
void EarthOrientation::nutationAngles1996(double T, double &deps, double &dpsi, double &om)
{
...
#include "IERS1996NutationData.hpp"
    ...
    double el = ::fmod((485866.733 + (715922.633 + (31.310 + 0.064 * T) * T) * T) * ARCSEC_TO_RAD +
                           ::fmod(1325.0 * T, 1.0) * TWOPI,
                       TWOPI);

    // Mean longitude of Sun minus mean longitude of Sun's perigee
    double elp = ::fmod((1287099.804 + (1292581.224 + (-0.577 - 0.012 * T) * T) * T) * ARCSEC_TO_RAD +
                            ::fmod(99.0 * T, 1.0) * TWOPI,
                        TWOPI);

    // Mean longitude of Moon minus mean longitude of Moon's node
    double f = ::fmod((335778.877 + (295263.137 + (-13.257 + 0.011 * T) * T) * T) * ARCSEC_TO_RAD +
                          ::fmod(1342.0 * T, 1.0) * TWOPI,
                      TWOPI);

    // Mean elongation of Moon from Sun
    double d = ::fmod((1072261.307 + (1105601.328 + (-6.891 + 0.019 * T) * T) * T) * ARCSEC_TO_RAD +
                          ::fmod(1236.0 * T, 1.0) * TWOPI,
                      TWOPI);

    // Longitude of the mean ascending node of the lunar orbit on the
    // ecliptic, measured from the mean equinox of date
    om = ::fmod((450160.280 + (-482890.539 + (7.455 + 0.008 * T) * T) * T) * ARCSEC_TO_RAD +
    ...
    {
        // form argument
        double arg = coeff[i].nl * el + coeff[i].nlp * elp + coeff[i].nf * f + coeff[i].nd * d + coeff[i].nom * om;
...
/* Nutation of the obliquity (deps) and of the longitude (dpsi), IERS 2003 or
   IAU 2000A model (MHB2000 luni-solar and planetary nutation without free
...
#include "IERS2003NutationData.hpp"
    ...
    {
        // argument
        arg = ::fmod(LSCoeff[i].nl * l + LSCoeff[i].nlp * lp + LSCoeff[i].nf * f + LSCoeff[i].nd * d +
                         LSCoeff[i].nom * Om,
                     TWOPI);
        sina = ::sin(arg);
        cosa = ::cos(arg);
    ...
    {
        // argument
        arg = ::fmod(PCoeff[i].nl * l + PCoeff[i].nf * f + PCoeff[i].nd * d + PCoeff[i].nom * Om + PCoeff[i].nme * lme +
                         PCoeff[i].nve * lve + PCoeff[i].nea * lea + PCoeff[i].nma * lma + PCoeff[i].nju * lju +
                         PCoeff[i].nsa * lsa + PCoeff[i].nur * lur + PCoeff[i].nne * lne + PCoeff[i].npa * pa,
                     TWOPI);
        sina = ::sin(arg);
        cosa = ::cos(arg);
        // term
...
/* Nutation of the obliquity (deps) and of the longitude (dpsi), IERS 2010 or
   IAU 2000A model (MHB2000 luni-solar and planetary nutation without free
   core n) with P03 adjustments. cf. sofa nut06a.c param T,    the coordinate
...
Matrix<double> EarthOrientation::nutationMatrix(double eps, double dpsi, double deps)
{
...
Matrix<double> EarthOrientation::precessionMatrix2003(double T)
{
    ...
    LOG(DEBUG7) << "\nobliquity at J2000 eps0 = " << fixed << setprecision(15) << showpos << eps0;
    ...
    LOG(DEBUG7) << "frame bias psi = " << fixed << setprecision(15) << showpos << psibias;
    LOG(DEBUG7) << "frame bias eps = " << fixed << setprecision(15) << showpos << epsbias;
    LOG(DEBUG7) << "frame bias dra = " << fixed << setprecision(15) << showpos << raeps0;
    ...
    LOG(DEBUG7) << "\nprecession angle psi = " << fixed << setprecision(15) << showpos << psia;
    LOG(DEBUG7) << "precession angle eps = " << fixed << setprecision(15) << showpos << epsa;
    LOG(DEBUG7) << "precession angle chi = " << fixed << setprecision(15) << showpos << chia;
    ...
    LOG(DEBUG7) << "precession rate = " << fixed << setprecision(15) << showpos << dpsipr << " " << depspr;
    psia += dpsipr;
    epsa += depspr;
    ...
    LOG(DEBUG7) << "\nframe bias matrix:\n" << fixed << setprecision(15) << showpos << FrameBias;
    ...
    LOG(DEBUG7) << "\nprecession matrix:\n" << fixed << setprecision(15) << showpos << Precess;

    LOG(DEBUG7) << "\nprecession*framebias matrix:\n" << fixed << setprecision(15) << showpos << (Precess * FrameBias);
...
void EarthOrientation::precessionRateCorrections2003(double T, double &dpsi, double &deps)
{
    // Precession rate contributions with respect to IAU 2000
    // Precession and obliquity corrections (radians)
...
/* Generate the full transformation matrix (3x3 rotation) relating the ECEF
   ...
   Input is the time of interest, the polar motion angles xp and yp
   (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as found in the
   IERS bulletin; see class EarthOrientation). */
Matrix<double> EarthOrientation::ECEFtoInertial1996(EphTime t, double xp, double yp, double UT1mUTC, bool reduced)
{
    try
    {
        ...
        eps = obliquity1996(T);
        LOG(DEBUG7) << "\nmean obliquity " << fixed << setprecision(15) << showpos << eps;
        // nutation angles - om is used in gast
        nutationAngles1996(T, deps, dpsi, om);
        LOG(DEBUG7) << "\nnutation angles psi eps " << fixed << setprecision(15) << showpos << dpsi << " " << deps;
        // nutation matrix
        N = nutationMatrix(eps, dpsi, deps);
        LOG(DEBUG7) << "\nnutation matrix:\n" << fixed << setprecision(15) << setw(18) << showpos << N;

        LOG(DEBUG7) << "\nNPB matrix:\n" << fixed << setprecision(15) << setw(18) << showpos << N * P;
        ...
        LOG(DEBUG7) << "\nGAST = " << fixed << setprecision(15) << showpos << g * RAD_TO_DEG;
...
/* Generate the full transformation matrix (3x3 rotation) relating the ECEF
   ...
   (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as found in the
   IERS bulletin; see class EarthOrientation). */
Matrix<double> EarthOrientation::ECEFtoInertial2003(EphTime t, double xp, double yp, double UT1mUTC)
{
    try
    {
        ...
        if (LOGlevel >= DEBUG7)
        {
            ...
            LOG(DEBUG7) << "\nGMST = " << fixed << setprecision(15) << showpos << gmst * RAD_TO_DEG;

            double gast = GAST2003(t, UT1mUTC);
            LOG(DEBUG7) << "\nGAST = " << fixed << setprecision(15) << showpos << gast * RAD_TO_DEG;
        }
        ...
        /* Precession rate contributions with respect to IAU 2000
           Precession and obliquity corrections (radians) */
        precessionRateCorrections2003(T, dpsipr, depspr);
        LOG(DEBUG7) << "\nprecession-rate " << fixed << setprecision(15) << showpos << dpsipr << " " << depspr;
        ...
        Matrix<double> NPB(N * P);
        LOG(DEBUG7) << "\nNPB matrix:\n" << fixed << setprecision(15) << setw(18) << showpos << NPB;

        // ERA replaces GAST in the Earth rotation matrix
        double era(EarthRotationAngle(t, UT1mUTC));
        LOG(DEBUG7) << "\nERA = " << fixed << setprecision(15) << showpos << era * RAD_TO_DEG;
        R = rotation(era, 3);
...
/* Generate the full transformation matrix (3x3 rotation) relating the ECEF
   ...
   (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as found in the
   IERS bulletin; see class EarthOrientation). */
Matrix<double> EarthOrientation::ECEFtoInertial2010(EphTime t, double xp, double yp, double UT1mUTC)
{
    try
    {
        double T(coordTransTime(t));
        ...
        /* compute transformation GCRS-to-CIRS or
           inertial-to-intermediate-celestial cf. sofa c2ixys */
        ...
        LOG(DEBUG7) << "\nNPB matrix:\n" << fixed << setprecision(15) << setw(18) << showpos << GCRStoCIRS;
        ...
        // compute transf. CIRS-to-TIRS or
        // intermediate-celestial-to-terrestrial
        Matrix<double> CIRStoTIRS;
        CIRStoTIRS = rotation(era, 3);
        LOG(DEBUG7) << "\ncelestial-to-terrestrial matrix (no polar motion):\n"
                    << fixed << setprecision(15) << setw(18) << showpos << CIRStoTIRS * GCRStoCIRS;
        ...
        LOG(DEBUG7) << "\npolar motion matrix:\n" << fixed << setprecision(15) << setw(18) << showpos << polarMotion;
...
/*=============================================================================
** SOME of these routines, as noted, are based on, but not simply copied from,
*SOFA;
