...
// Default constructor
template <class T>
SMatProxy<T>::SMatProxy(SparseMatrix<T> &sm, unsigned int i, unsigned int j) : mySM(sm), irow(i), jcol(j)
{
}
...
// output
template <class T> std::ostream &operator<<(std::ostream &os, const SparseMatrix<T> &SM);
// transpose
template <class T> SparseMatrix<T> transpose(const SparseMatrix<T> &M);
// multiplies
template <class T> SparseMatrix<T> operator*(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
template <class T> SparseVector<T> operator*(const SparseMatrix<T> &L, const SparseVector<T> &V);
template <class T> SparseVector<T> operator*(const Matrix<T> &L, const SparseVector<T> &V);
template <class T> SparseVector<T> operator*(const SparseMatrix<T> &L, const Vector<T> &V);
template <class T> SparseVector<T> operator*(const SparseVector<T> &V, const SparseMatrix<T> &R);
template <class T> SparseVector<T> operator*(const SparseVector<T> &V, const Matrix<T> &R);
template <class T> SparseVector<T> operator*(const Vector<T> &V, const SparseMatrix<T> &R);
template <class T> SparseMatrix<T> operator*(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
template <class T> SparseMatrix<T> operator*(const SparseMatrix<T> &L, const Matrix<T> &R);
template <class T> SparseMatrix<T> operator*(const Matrix<T> &L, const SparseMatrix<T> &R);
// concatenation
template <class T> SparseMatrix<T> operator||(const SparseMatrix<T> &L, const Vector<T> &V);
template <class T> SparseMatrix<T> operator||(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
 ...
 * @throw Exception
 */
template <class T> SparseMatrix<T> inverseLT(const SparseMatrix<T> &LT, T *ptrSmall, T *ptrBig);
/**
 * @throw Exception
 */
template <class T> SparseMatrix<T> inverseViaCholesky(const SparseMatrix<T> &A);

// special matrices
template <class T> SparseMatrix<T> identSparse(const unsigned int dim);

/** products MT*M, M*MT, M*C*MT etc
 * MT * M
 * @throw Exception
 */
template <class T> SparseMatrix<T> transposeTimesMatrix(const SparseMatrix<T> &M);
/** M * MT
 * @throw Exception
 */
template <class T> SparseMatrix<T> matrixTimesTranspose(const SparseMatrix<T> &M);
/** diag of P * C * PT
 * @throw Exception
 */
template <class T> Vector<T> transformDiag(const SparseMatrix<T> &P, const Matrix<T> &C);
 ...
 * @throw Exception
 */
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &A, const unsigned int M);
/**
 * @throw Exception
 */
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &P, Vector<T> &D, const unsigned int M);

//---------------------------------------------------------------------------
/**
 Class SparseMatrix. This class is designed to present an interface nearly
 identical to class Matrix, but more efficiently handle sparse matrices,
 in which most of the elements are zero. The class stores only non-zero
 elements; using a map of SparseVectors, with key = row index. it also
 stores a nominal dimension - number of rows and columns. The class uses a
 proxy class, SMatProxy, to access elements; this allows rvalues and
 lvalues to be treated separately. Notes on speed. The most expensive
 parts are the Proxy::operator(), then find() and lower_bound(). Never use
 the Proxy stuff within the class, always use iterators and assign values
 to the maps directly. Never assign zeros to the maps. See timing and test
 results in the test program smtest.cpp Matrix multiply is orders of
 magnitude faster. Transpose() is much faster than the Matrix version,
 which is something of a surprise. Most time consuming is looping over
 columns; this is expected since the design stores by row. The trick is to
 re-write the algorithm in terms of the transpose of the column-loop
 matrix, and then apply a transpose(), which is cheap, either before
 starting (when col-loop matrix is input) or after returning (output).
...
template <class T> class SparseMatrix
{
  public:
    ...
    // arithmetic
    friend SparseMatrix<T> operator- <T>(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
    friend SparseMatrix<T> operator- <T>(const SparseMatrix<T> &L, const Matrix<T> &R);
    friend SparseMatrix<T> operator- <T>(const Matrix<T> &L, const SparseMatrix<T> &R);
    friend SparseMatrix<T> operator+ <T>(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
    friend SparseMatrix<T> operator+ <T>(const SparseMatrix<T> &L, const Matrix<T> &R);
    friend SparseMatrix<T> operator+ <T>(const Matrix<T> &L, const SparseMatrix<T> &R);
    // mulitply
    friend SparseMatrix<T> operator* <T>(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
    friend SparseVector<T> operator* <T>(const SparseMatrix<T> &L, const SparseVector<T> &V);
    friend SparseVector<T> operator* <T>(const Matrix<T> &L, const SparseVector<T> &V);
    friend SparseVector<T> operator* <T>(const SparseMatrix<T> &L, const Vector<T> &V);
    friend SparseVector<T> operator* <T>(const SparseVector<T> &V, const SparseMatrix<T> &R);
    friend SparseVector<T> operator* <T>(const SparseVector<T> &V, const Matrix<T> &R);
    friend SparseVector<T> operator* <T>(const Vector<T> &V, const SparseMatrix<T> &R);
    friend SparseMatrix<T> operator* <T>(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
    friend SparseMatrix<T> operator* <T>(const SparseMatrix<T> &L, const Matrix<T> &R);
    friend SparseMatrix<T> operator* <T>(const Matrix<T> &L, const SparseMatrix<T> &R);
    // concatenation
    friend SparseMatrix<T> operator|| <T>(const SparseMatrix<T> &L, const Vector<T> &V);
    friend SparseMatrix<T> operator|| <T>(const SparseMatrix<T> &L, const SparseMatrix<T> &R);
    // inverse (via Gauss-Jordan)
    ...
    friend SparseMatrix<T> inverseLT<T>(const SparseMatrix<T> &LT, T *ptrSmall, T *ptrBig);
    ...
    // diag of P * C * PT
    friend Vector<T> transformDiag<T>(const SparseMatrix<T> &P, const Matrix<T> &C);
    friend SparseMatrix<T> SparseHouseholder<T>(const SparseMatrix<T> &A);
    friend void SrifMU<T>(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &A, const unsigned int M);
    friend void SrifMU<T>(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &P, Vector<T> &D, const unsigned int M);
    ...
    */
    SparseMatrix(const SparseMatrix<T> &SM, const unsigned int &rind, const unsigned int &cind,
    ...
    /**
     ...
     NB. This routine is called only by the user - routines defined here do
     not zeroize, as there is no way to appropriately choose a tolerance.
     The default tolerance for this routine is
     SparseVector<T>::zeroTolerance. The caller may want to consider a
     tolerance related to SM.maxabs().
    */
    ...
    std::string dump(const int p = 3, bool dosci = false) const
    {
        ...
        size_t i;
        oss << "dim(" << nrows << "," << ncols << "), size " << size() << ", datasize " << datasize() << " :";
...
template <class T>
SparseMatrix<T>::SparseMatrix(const SparseMatrix<T> &SM, const unsigned int &rind, const unsigned int &cind,
                              const unsigned int &rnum, const unsigned int &cnum)
{
...
template <class T> SparseMatrix<T> &SparseMatrix<T>::operator-=(const SparseMatrix<T> &SM)
{
...
template <class T> SparseMatrix<T> &SparseMatrix<T>::operator-=(const Matrix<T> &M)
{
...
template <class T> SparseMatrix<T> &SparseMatrix<T>::operator*=(const T &value)
{
    if (value == T(0))
    {
        rowsMap.clear();
...
/**
 Matrix addition: SparseMatrix = Matrix + SparseMatrix : copy, += M in rev
 order
*/
template <class T> SparseMatrix<T> operator+(const Matrix<T> &L, const SparseMatrix<T> &R)
{
...
//---------------------------------------------------------------------------
/// return row i of this SparseMatrix as a SparseVector
template <class T> SparseVector<T> SparseMatrix<T>::rowCopy(const unsigned int i) const
{
...
/// return col j of this SparseMatrix as a SparseVector
template <class T> SparseVector<T> SparseMatrix<T>::colCopy(const unsigned int j) const
{
    SparseVector<T> retSV;
...
/// swap rows of this SparseMatrix
template <class T> void SparseMatrix<T>::swapRows(const unsigned int &ii, const unsigned int &jj)
{
...
/// swap columns of this SparseMatrix
template <class T> void SparseMatrix<T>::swapCols(const unsigned int ii, const unsigned int jj)
{
...
template <class T> SparseMatrix<T> matrixTimesTranspose(const SparseMatrix<T> &SM)
{
    try
    {
...
/**
 ...
 NB not the best numerically; for high condition number, use
 inverseViaCholesky, or cast to Matrix, use either LUD or SVD, then cast
 back to SparseMatrix.
*/
template <class T> SparseMatrix<T> inverse(const SparseMatrix<T> &A)
{
    try
...
/**
 ...
 matrix (Cholesky decomposition) Crout algorithm.
...
template <class T> SparseMatrix<T> lowerCholesky(const SparseMatrix<T> &A)
{
    ...
    for (jt = A.rowsMap.begin(), Ljt = L.rowsMap.begin(); jt != A.rowsMap.end() && Ljt != L.rowsMap.end(); ++jt, ++Ljt)
    {
        ...
        {
            std::ostringstream oss;
            oss << "Non-positive eigenvalue " << std::scientific << d << " at col " << j
...
template <class T> SparseMatrix<T> inverseLT(const SparseMatrix<T> &L, T *ptrSmall, T *ptrBig)
{
    ...
    {
        if (it == L.rowsMap.end() || it->first != i || !it->second.isFilled(i) || it->second[i] == T(0))
        {
            std::ostringstream oss;
...
//---------------------------------------------------------------------------------
/**
 Compute upper triangular square root of a symmetric positive definite
 matrix (Cholesky decomposition) Crout algorithm; that is A =
 transpose(U)*U. Note that this result will be equal to
 transpose(lowerCholesky(A)) == transpose(Ch.L from class Cholesky), NOT
 Ch.U; class Cholesky computes L,U where A = L*LT = U*UT [while A=UT*U
 here].
 @param A SparseMatrix to be decomposed; symmetric and positive definite, const
...
/**
 ...
 @throw Exception if input SparseMatrix is not square, not positive
 definite, or singular
*/
template <class T> SparseMatrix<T> inverseViaCholesky(const SparseMatrix<T> &A)
{
    try
...
/**
 Square root information measurement update, with new data in the form of
 a single SparseMatrix concatenation of H and D: A = H || D. See doc for
 the overloaded SrifMU().
*/
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &A, const unsigned int M)
{
