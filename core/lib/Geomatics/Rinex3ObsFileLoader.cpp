...
/** @file Rinex3ObsFileLoader.cpp  Read a RINEX3 file, selecting, counting and
    ...
    try
    {
        ...
        for (unsigned int nf = 0; nf < filenames.size(); nf++)
        {
            ...
            for (;;)
            {
                    ...
                    for (kt = roh.mapObsTypes.begin(); kt != roh.mapObsTypes.end(); kt++)
                    {
                        for (i = 0; i < kt->second.size(); i++)
                        {
                            ...
                            for (j = 0; j < inputWantedObsTypes.size(); j++)
                            {
                                ...
                                ossx << " Add obs type " << srot << " =~ " << inputWantedObsTypes[j] << " from "
                                     << filename << endl;
                            }
                        }
                ...
                {
                    oss << "Error - failed to read header for file " << filename << " with exception " << e.getText(0)
                        << endl;
                    strm.close();
                    break;
                }
                ...
                while (1)
                {
                    ...
                    {
                        oss << "Error - failed to read data in file " << filename << " with exception " << e.getText(0)
                            << endl;
                        break;
                    }
                    ...
                    for (it = rod.obs.begin(); it != rod.obs.end(); ++it)
                    {
                        ...
                        /* is the sat excluded?  NB it does not exclude
                           sat=(sys,-1) */
                        if (exSats.size() > 0 && find(exSats.begin(), exSats.end(), sat) != exSats.end())
                        {
                            continue;
                        }
            ...
            if (timeOrder.size() > 0)
            {
                for (i = 0; i < timeOrder.size(); i++)
                    oss << "Warning - in file " << filename << " " << nOrder[i] << " data records following epoch "
...
string Rinex3ObsFileLoader::asString()
{
    ...
    for (i = 0; i < filenames.size(); i++)
        oss << (i == 0 ? " RINEX obs file: " : "                 ") << filenames[i] << endl;
    ...
    try
    {
        ...
        for (unsigned int nds = 0; nds < datastore.size(); nds++)
        {
            ...
            for (it = datastore[nds].obs.begin(); it != datastore[nds].obs.end(); ++it)
            {
                ...
                do
                {
                    i = SPList[satit->second].addData(datastore[nds].time, obsit->second, data, lli, ssi, flag);
...
void Rinex3ObsFileLoader::dumpStoreEpoch(ostream &s, const Rinex3ObsData &rod) const
{
    s << "Dump of Rinex3ObsData"
      << " at " << printTime(rod.time, timefmt) << " epochFlag = " << rod.epochFlag << " numSVs = " << rod.numSVs
      << fixed << setprecision(9) << " clk offset = " << rod.clockOffset << endl;
    ...
    if (rod.epochFlag == 0 || rod.epochFlag == 1)
    {
        ...
        for (jt = rod.obs.begin(); jt != rod.obs.end(); jt++)
        {
            ...
            {
                s << " " << setw(13) << jt->second[j].data << "/" << jt->second[j].lli << "/" << jt->second[j].ssi
                  << "/" << wantedObsTypes[j];
            }
            s << endl;
        }
...
void dumpAllRinex3ObsTypes(ostream &os)
{
        ...
        for (CarrierBand j : CarrierBandIterator())
        {
            for (TrackingCode k : TrackingCodeIterator())
            {
                for (ObservationType i : ObservationTypeIterator())
                {
                    try
                    {
                        string tag(string(1, syss[s]) + string(1, RinexObsID::ot2char[i]) +
                                   string(1, RinexObsID::cb2char[j]) + string(1, RinexObsID::tc2char[k]));
                        ...
                        if (find(goodtags.begin(), goodtags.end(), tag) == goodtags.end())
                        {
                            ...
                            string desc(asString(RinexObsID(tag, Rinex3ObsBase::currentVersion)));
    ...
    os << " Sys " << leftJustify("Freq", len2) << " " << center("Track", len3) << "Pseudo- Carrier Dopp  Signal"
       << endl;
    os << "     " << leftJustify("    ", len2) << " " << center("     ", len3) << " range  phase        Strength"
       << endl;
        ...
        for (jt = it->second.begin(); jt != it->second.end(); ++jt)
        {
            for (kt = jt->second.begin(); kt != jt->second.end(); ++kt)
            {
                ...
                if (fr != kt->second['L'][2])
                {
                    ...
                    if (!tc.empty())
                    {
                        os << "  all codes for " << it->first << " " << jt->first << " = '" << tc << "'";
                    }
                }
                os << endl;
            }
