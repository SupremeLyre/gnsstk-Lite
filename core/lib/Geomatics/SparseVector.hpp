...
// output stream
template <class T> std::ostream &operator<<(std::ostream &os, const SparseVector<T> &SV);
// operators
...
template <class T> SparseVector<T> operator-(const SparseVector<T> &L, const SparseVector<T> &R);
template <class T> SparseVector<T> operator-(const SparseVector<T> &L, const Vector<T> &R);
template <class T> SparseVector<T> operator-(const Vector<T> &L, const SparseVector<T> &R);
template <class T> SparseVector<T> operator+(const SparseVector<T> &L, const SparseVector<T> &R);
template <class T> SparseVector<T> operator+(const SparseVector<T> &L, const Vector<T> &R);
template <class T> SparseVector<T> operator+(const Vector<T> &L, const SparseVector<T> &R);
// SparseMatrix
 ...
 * @throw Exception
 */
template <class T> SparseMatrix<T> transform(const SparseMatrix<T> &M, const SparseMatrix<T> &C);
 ...
 * @throw Exception
 */
template <class T> SparseMatrix<T> inverseLT(const SparseMatrix<T> &LT, T *ptrSmall = NULL, T *ptrBig = NULL);
 ...
 * @throw Exception
 */
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &A, const unsigned int M = 0);

/**
 * @throw Exception
 */
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, SparseMatrix<T> &P, Vector<T> &D, const unsigned int M = 0);
...
/**
 ...
 which most of the elements are zero. The class stores only non-zero
 elements in a map with key = index; it also stores a nominal length. The
 class uses a proxy class, SVecProxy, to access elements; this allows
 rvalues and lvalues to be treated separately.
*/
template <class T> class SparseVector
{
  public:
    ...
    friend T dot_lim<T>(const SparseVector<T> &SL, const SparseVector<T> &SR, const unsigned int kb,
    ...
    // arithmetic
    friend SparseVector<T> operator- <T>(const SparseVector<T> &L, const SparseVector<T> &R);
    friend SparseVector<T> operator- <T>(const SparseVector<T> &L, const Vector<T> &R);
    friend SparseVector<T> operator- <T>(const Vector<T> &L, const SparseVector<T> &R);
    friend SparseVector<T> operator+ <T>(const SparseVector<T> &L, const SparseVector<T> &R);
    friend SparseVector<T> operator+ <T>(const SparseVector<T> &L, const Vector<T> &R);
    friend SparseVector<T> operator+ <T>(const Vector<T> &L, const SparseVector<T> &R);
    // SparseMatrix
    friend SparseMatrix<T> transpose<T>(const SparseMatrix<T> &M);
    friend SparseMatrix<T> transform<T>(const SparseMatrix<T> &M, const SparseMatrix<T> &C);
    ...
    friend SparseMatrix<T> operator* <T>(const SparseMatrix<T> &L, const SparseMatrix<T> &V);
    ...
    /**
     tolerance in considering element to be zero is std::abs(elem) <
     tolerance see zeroize(), where this is the default input value
    */
    ...
    */
    ...
    /**
     truncate - removes elements, if necessary, at and beyond column index
     j same as resize(n) but does not change the nominal length len.
    */
    ...
    /**
     ...
     NB this class and SparseMatrix call this when constructing a new
     object, e.g. after matrix multiply, but ONLY with the tolerance T(0).
    */
  ...
  private:
    /**
     length of the "real" vector (not the number of data stored =
     vecMap.size())
    */
    unsigned int len;
    ...
    /**
     return a vector containing all the indexes, in order, of non-zero
     elements.
    */
...
//---------------------------------------------------------------------------
// Default constructor
template <class T> SVecProxy<T>::SVecProxy(SparseVector<T> &sv, unsigned int i) : mySV(sv), index(i)
{
}
...
template <class T>
SparseVector<T>::SparseVector(const SparseVector<T> &SV, const unsigned int &ind, const unsigned int &n)
{
    if (ind + n > SV.len)
    {
...
template <class T> SparseVector<T> &SparseVector<T>::operator-=(const SparseVector<T> &R)
{
    if (len != R.size())
    {
...
template <class T> SparseVector<T> &SparseVector<T>::operator-=(const Vector<T> &R)
{
    if (len != R.size())
    {
...
template <class T> void SparseVector<T>::addScaledSparseVector(const T &a, const SparseVector<T> &R)
{
    if (a == T(0))
    {
        return;
...
template <class T> SparseVector<T> &SparseVector<T>::operator*=(const T &value)
{
    if (value == T(0))
    {
        resize(0);
