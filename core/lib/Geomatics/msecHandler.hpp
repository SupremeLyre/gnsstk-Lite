...
{
// -------------------------------------------------------------------------------
/** Class to detect, validate and remove millisecond clock adjusts from
    pseudorange and phase data and timetags. Caller passes all the data to
    the object, in time order, and the object detects where (in time and in
    observables) there are millisecond adjusts. Caller can then pass data to
    the object again and have it remove the appropriate adjusts from
    pseudorange, phase and/or timetags. NB. adjusts should be removed from
    pseudorange and timetags together. NB. most adjusted data has jumps in
    pseudorange, but not phase (violates RINEX!) although the opposite (found
    in phase, but not pseudorange) does exist (job3326). NB. some (Ashtech)
    usually also have jumps in the time tags; Trimble is variable. */
class msecHandler
{
  private:
    // member data
    ...
    static const double Rfact; ///< one ms in m,L1cyc,L2cyc
  ...
  public:
    ...
    /** Re-define obstypes and wavelengths with parallel arrays.
        ...
        @param waves vector<double> wavelength of phases in ots or zero for
        codes. */
    ...
    /** add data at one epoch. May be repeated at the same epoch, but MUST be
        done in time order. NB assumes, as in RINEX, that data==0 means it is
        missing.
    ...
    /** edit data by removing the millisecond adjusts, and optionally a
        piece-wise linear model of the adjusts. Must be called in time order,
        as add() was. NB may call repeatedly with the same ttag, however NB
        ttag gets fixed every call, so don't keep calling with same variable
        ttag.
        @param ttag  time of the data
        @param sat   satellite of the data
        @param obstype string observation type of the data (L1 L2 C1 C2 P1 P2)
    ...
    /** get map<CommonTime,int> of valid adjusts; should parallel
        getFindMessage() */
