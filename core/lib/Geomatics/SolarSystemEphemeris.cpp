...
/** @file SolarSystemEphemeris.cpp
    ...
    the sun, the moon and the 9 planets, plus nutation and lunar libration and
...
class SSEDouble : public FormattedDouble
{
  public:
    SSEDouble(double d = 0)
        : FormattedDouble(d, StringUtils::FFLead::Decimal, 18, 2, 26, 'D', StringUtils::FFSign::NegOnly,
    ...
    try
    {
        ...
        while (1)
        {
            ...
            else if (group == 1030)
            {
                ...
                LOG(VERBOSE) << "Times JD " << fixed << setprecision(3) << startJD << " to JD " << endJD << " = "
                             << interval << " days";
            }
            ...
            while (!line.empty())
            {
                ...
                else if (group == 1041)
                {
                    if (n++ == 0)
                    {
                        ...
                        {
                            Exception e("Nconst does not match N in GROUP 1041 : " + asString(Nconst) + " != " + word);
                            GNSSTK_THROW(e);
                        }
...
int SolarSystemEphemeris::readASCIIdata(vector<string> &filenames)
{
    try
    {
        ...
        LOG(VERBOSE) << "After reading data files, store size is " << store.size()
                     << " and new start and stop times are JD " << fixed << setprecision(9) << startJD << " and JD "
                     << endJD;
        ...
        oss << "Start Epoch: JED= " << fixed << setw(10) << setprecision(1) << startJD << " = " << yy << "/" << mm
            << "/" << dd;
        ...
        oss.str("");
        oss << "Final Epoch: JED= " << fixed << setw(10) << setprecision(1) << endJD << " = " << yy << "/" << mm << "/"
            << dd;
    ...
    try
    {
        ...
        while (1)
        {
            ...
            if (n == 0)
            {
                ...
                if (ncc != Ncoeff)
                {
                    Exception e("readASCIIdata finds conflicting sizes in header (" + asString(Ncoeff) +
                                ") and data (" + asString(ncc) + ") in file " + filename + " at line #" +
                                asString(ntot));
                    GNSSTK_THROW(e);
                }
                nc = 0;
...
int SolarSystemEphemeris::writeASCIIheader(ostream &os)
{
    try
    {
        ...
        oss << fixed << setprecision(2) << setw(12) << startJD << setw(12) << endJD << setw(12) << interval << blank;
        os << leftJustify(oss.str(), 81) << endl << blank;
        oss.seekp(ios_base::beg);
...
int SolarSystemEphemeris::writeBinaryFile(const string &filename)
{
    try
    {
        ...
        {
            LOG(DEBUG) << "writeBinaryFile writes " << nrec << " " << fixed << setprecision(6) << jt->second[0];
...
void SolarSystemEphemeris::relativeInertialPositionVelocity(double MJD, SolarSystemEphemeris::Planet target,
                                                            SolarSystemEphemeris::Planet center, double pv[6],
                                                            bool kilometers)
{
    try
    {
        ...
        if (iret)
        {
            ...
            {
                Exception e(string("Requested time is ") + (iret == retEarly ? string("before") : string("after")) +
...
void SolarSystemEphemeris::readBinaryHeader(const string &filename)
{
    try
    {
        ...
        LOG(DEBUG) << "readBinaryHeader reads start JD " << fixed << setprecision(2) << startJD;

        readBinary((char *)&endJD, sizeof(double));
        LOG(DEBUG) << "readBinaryHeader reads end JD " << fixed << setprecision(2) << endJD;
        ...
        for (i = 0; i < 12; i++)
        {
            ...
            recLength += 3 * sizeof(int);
            LOG(DEBUG) << "readBinaryHeader reads " << i << " " << c_offset[i] << " " << c_ncoeff[i] << " "
                       << c_nsets[i];
        }
        ...
        recLength += 3 * sizeof(int);
        LOG(DEBUG) << "readBinaryHeader reads " << 12 << " " << c_offset[12] << " " << c_ncoeff[12] << " "
                   << c_nsets[12];
        ...
        for (i = 0; i < 400; i++)
        {
            ...
            if (i < Nconst)
            {
                ...
                LOG(DEBUG) << "readBinaryHeader reads " << consts_names[i] << " = " << fixed << setprecision(18)
                           << setw(24) << d;
            }
        }
        // pad
        ...
        {
            LOG(WARNING) << "DENUM (" << denum << ") does not equal the array value (" << constants["DENUM"] << ")";
        }
    }
    catch (Exception &e)
    {
...
int SolarSystemEphemeris ::readBinaryData(bool save)
{
    try
    {
        ...
        while (!istrm.eof() && istrm.good())
        {
            ...
            {
                ostringstream oss;
                oss << "ERROR: found gap in data at " << nrec << fixed << setprecision(6) << " : prev end = " << prev
...
//---------------------------------------------------------------------------------
// private
void SolarSystemEphemeris::inertialPositionVelocity(double MJD, SolarSystemEphemeris::computeID which, double PV[6])
{
    try
    {
