...
*/
bool AntennaStore::getSatelliteAntenna(const char sys, const int n, string &name, AntexData &data, bool inputPRN) const
{
...
int AntennaStore::addANTEXfile(const string &filename, CommonTime time)
{
    try
    {
        ...
        while (1)
        {
            ...
            // else include it
            else if (antdata.isRxAntenna || includeSats > 1 || (includeSats == 1 && antdata.systemChar == 'G'))
            {
...
*/
Triple AntennaStore::ComToPcVector(const char sys, const int n, const CommonTime &ct, const Triple &satVector,
                                   bool inputPRN) const
{
    AntexData antenna;
    string name;
    ...
    try
    {
        ...
        {
            stringstream ss;
            ss << "AntennaStore::ComToPcVector.  No satellite data found for " << sys << " PRN " << n;
            InvalidRequest ir(ss.str());
            GNSSTK_THROW(ir);
        }
    }
...
Triple AntennaStore::ComToPcVector(const SatID &sidr, const CommonTime &ct, const Triple &satVector) const
{
    char sys = ' ';
    Triple tp;
    switch (sidr.system)
...
{
    s << "Dump (" << (detail == 0 ? "low" : (detail == 1 ? "medium" : "high")) << " detail) of AntennaStore ("
      << antennaMap.size() << ") antennas\n";
    map<string, AntexData>::iterator it;
    for (it = antennaMap.begin(); it != antennaMap.end(); it++)
    {
        if (detail > 0)
