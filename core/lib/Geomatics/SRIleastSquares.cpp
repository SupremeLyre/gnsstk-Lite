...
SRIleastSquares::SRIleastSquares(const Matrix<double> &Rin, const Vector<double> &Zin, const Namelist &NLin)
{
    defaults();
...
int SRIleastSquares::dataUpdate(Vector<double> &D, Vector<double> &X, Matrix<double> &Cov,
                                void(LSF)(Vector<double> &X, Vector<double> &f, Matrix<double> &P))
{
    ...
    if (doVerbose)
    {
        cout << "\nSRIleastSquares::leastSquaresUpdate : M,N are " << M << "," << N << endl;
    }
    ...
    if (doLinearize && M < N)
    {
        MatrixException me(string("When linearizing, problem must not be underdetermined:\n") +
                           string("   data dimension is ") + asString(M) + string(" while state dimension is ") +
                           asString(N));
        GNSSTK_THROW(me);
    }
    if (doSequential && R.rows() != X.size())
    {
        MatrixException me("Sequential problem has inconsistent dimensions:\n  SRI is " + asString<int>(R.rows()) +
                           "x" + asString<int>(R.cols()) + " while X has length " + asString<int>(X.size()));
        GNSSTK_THROW(me);
    }
    ...
    try
    {
        ...
        do
        {
            ...
            if (doVerbose)
            {
                ...
                cout << LNX << endl;
                cout << " Pre-fit data residuals:  " << fixed << setprecision(6) << Res << endl;
            }
            ...
            if (doVerbose)
            {
                cout << " Condition number: " << scientific << conditionNum << fixed << endl;
                cout << " Post-fit data residuals:  " << fixed << setprecision(6) << Res << endl;
            }
            ...
            if (doLinearize)
            {
                ...
                if (doVerbose)
                {
                    cout << " RMS convergence : " << scientific << rmsConvergence << fixed << endl;
                }
            }
            ...
            if (doRobust)
            {
                ...
                if (doVerbose)
                {
                    cout << " Convergence: " << scientific << setprecision(3) << rmsConvergence << endl;
                }
            }
