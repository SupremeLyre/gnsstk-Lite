...
/** @file DiscCorr.hpp
    ...
    pass, and a configuration object (as defined herein), detect discontinuities
...
/**
 ...
 in, or let the GDC compute it from the data - if it fails it returns -6.
 ...
 const int BadInput = -5      input data does not have the required obs
 types const int NoData = -4        insufficient input data, or all data
 is bad const int FatalProblem = -3  DT is not set, or memory problem
 const int Singularity = -1   polynomial fit fails
 const int ReturnOK = 0       normal return
 @throw Exception
*/
int DiscontinuityCorrector(SatPass &SP, GDCconfiguration &config, std::vector<std::string> &EditCmds,
                           std::string &retMsg, int GLOn = -99);
