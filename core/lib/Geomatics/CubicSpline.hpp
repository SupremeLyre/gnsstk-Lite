...
/** @file CubicSpline.hpp
    Cubic spline interpolation on an array of data (x,y) not necessarily evenly
    spaced but with x strictly increasing. */
...
template <class T> class CubicSpline
{
  public:
    ...
    /**
     Initialize array of second derivatives, which is used by
     Interpolate(). Called by constructor. If the arrays are shorter than 4
     points, then linear interpolation is used. Do not fix derivatives at
     end points; if this is desired, call initialize(X,Y,deriv1,derivN).
    ...
    /**
     Initialize array of second derivatives with the values of the
     derivative at the first and last points set by input.  Cf.
     initialize(X,Y);
     @param X  Vector of data for independent variable, must parallel Y
     @param Y  Vector of data for dependent variable, must parallel X
    ...
    /**
     Determine if the given value of x is within the range of the data used
     to initialize the object; if it is not, return the value of Y which is
     closest to x (either Y[0] or Y[N-1]).
     ...
     @throw Exception if the object has not been initialized by a call to
     initialize(). */
    ...
    /**
     Compute the value of y at the given x, using interpolation by cubic
     spline. The object must have been initialized before this call, either
     by using the constructor CubicSpline(X,Y) or by calling initialize()
     with the data arrays X and Y.
     @param x  value of independent variable X at which to evaluate spline.
     @return y(x) the interpolated value
     @throw Exception if the object has not been initialized by a call to
                      initialize(), or
                      if the given x is outside the range of the data used in
                      initialize(). */
    ...
    /**
     ...
     @param x  vector of values of independent variable X at which to
                   evaluate.
     @return y  output vector of interpolated values parallel to x.
     @throw Exception if the object has not been initialized by a call to
                initialize(), or if the given x is outside the range
                of the data used in initialize(). */
    std::vector<T> evaluate(const std::vector<T> &x)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            {
                Exception e(std::string("Input value at index ") + StringUtils::asString(i) +
  ...
  private:
    /**
     initialize array of second derivatives with the values of the data
     x(N) and y(N). If the first derivative at the end points has been
     fixed, the input boolean is true. Called by all versions of
     initialize().
     @param x  Vector of data for independent variable, must parallel y,
                and must be strictly increasing.
     @param y  Vector of data for dependent variable, must parallel x.
     @param fixEnds  if true, initial and final values of derivative dy/dx
                have been set using member data fd1 and fdN.
     @throw Exception if input is invalid: arrays are empty or x not
                strictly increasing. */
    void build(const std::vector<T> &x, const std::vector<T> &y, bool fixEnds)
    {
        ...
        if (!fixEnds)
        {
            ...
            dx1 = X[1] - X[0];
            dx2 = X[2] - X[0];
            fd1 = ((Y[1] - Y[0]) / (dx1 * dx1) - (Y[2] - Y[0]) / (dx2 * dx2)) / (T(1) / dx1 - T(1) / dx2);

            dx1 = X[N - 2] - X[N - 1];
            dx2 = X[N - 3] - X[N - 1];
            fdN =
                ((Y[N - 2] - Y[N - 1]) / (dx1 * dx1) - (Y[N - 3] - Y[N - 1]) / (dx2 * dx2)) / (T(1) / dx1 - T(1) / dx2);
        }
        ...
        for (i = 1; i < N - 1; i++)
            S[i] = T(6) * (Y[i - 1] / (X[i] - X[i - 1]) - Y[i] * (T(1) / (X[i] - X[i - 1]) + T(1) / (X[i + 1] - X[i])) +
                           Y[i + 1] / (X[i + 1] - X[i]));
    ...
    T interpolate(const int k, const T x)
    {
        ...
        return ((dxl * (Y[k] - S[k] * dx * dx / T(6)) + (S[k - 1] * dxr * dxr * dxr + S[k] * dxl * dxl * dxl) / T(6) +
    ...
    /**
     Array of second derivatives, computed by build() and used in
     interpolate(). */
    std::vector<T> S;
