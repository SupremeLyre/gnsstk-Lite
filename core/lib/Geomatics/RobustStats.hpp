...
/** @file RobustStats.hpp
    Namespace Robust includes basic robust statistical computations, including
    median, median average deviation, quartiles and m-estimate, as well as
    implementation of of stem-and-leaf plots, quantile plots and robust least
...
*/
template <typename T> void insert(T *sa, int na, int (*comp)(const T &, const T &) = gnsstk::Qsort_compare)
{
    int i, j;
    T stemp;
    for (i = 1; i < na; i = i + 1)
...
/**
 ...
 @param comp (optional) the comparison function to be used.
*/
template <typename T> void QSort(T *sa, int na, int (*comp)(const T &, const T &) = gnsstk::Qsort_compare)
{
    int i, j, nr;
    T stemp, spart;
    while (1)
 ...
 * Kolmogorov-Smirnoff test, comparing the distribution of data
 * with mean m and standard deviation s to the normal
 * distribution.
 * @note If ADtest > 0.752 then normality hypothesis is rejected
 *   for 5% level test.
 ...
 * @throw Exception
 */
template <typename T> void Quartiles(const T *xd, const int nd, T &Q1, T &Q3)
{
    if (!xd || nd < 2)
    {
        Exception e("Invalid input");
...
/** Compute the median absolute deviation of a double array
 * of length nd, as well as the median (M = Median(xd,nd));
 * @note this routine will trash the array xd unless
 * save_flag is true (default).
 ...
 * @throw Exception
 */
template <typename T> T MedianAbsoluteDeviation(T *xd, int nd, T &M, bool save_flag = true)
{
    int i;
    T mad, *save;
...
/** Compute the median absolute deviation of a double array
 * of length nd; see MedianAbsoluteDeviation().
 * @throw Exception
 */
...
template <typename T> T MEstimate(const T *xd, int nd, const T &M, const T &MAD, T *w = NULL)
{
    try
    {
...
/** Fit a polynomial of degree n to data xd, with independent
 * variable td, using robust techniques. The post-fit
 * residuals are returned in the data vector, and the
 * computed weights in the result may be output as well.
 * Specifically, the equation describing the fit is
 * c0 + c[1]*t(j) + c[2]*t(j)*t(j) + ... c[n-1]*pow(t(j),n-1) = xd[j],
 * where the zero-th coefficient and the independent variable
 * are debiased by the first value; i.e. c0 = c[0]+xd[0] and
 * t(j) = td[i]-td[0].  Specifically, to evaluate the
 * polynomial at t, eval = f(t), do the following.
 * xd0 = xd[0];
 * Robust::RobustPolyFit(xd,td,nd,n,c);
 * eval = xd0+c[0]; tt = 1.0;
 * for(j=1; j<nd; j++) { tt *= (t-td[0]); eval += c[j]*tt; }
 * @param[in] xd array of data, of length nd; contains residuals on output.
 * @param[in] td array of independent variable, length nd (parallel to xd).
 * @param[in] nd length of arrays xd and td.
 * @param[in] n degree of polynomial and dimension of coefficient array.
 * @param[out] c array of coefficients (dimension n).
 * @param[out] w array of length nd to contain weights.
...
/** Print 'stem and leaf' plot of the data in the double
 * array xd of length nd, with an optional message, on the
 * given output ostream. It is assumed that the input array
 * xd is sorted in ascending order.
...
/** Generate quantiles. Given an array xd of length nd, fill
 * it with quantiles such that (xd,yd) gives a quantile
 * plot. The distribution of yd is a normal distribution to
 * the extent that this plot is a straight line, with
 * y-intercept and slope identified with mean and standard
 * deviation, respectively, of the distribution.
