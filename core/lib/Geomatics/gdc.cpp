...
/** @file gdc.cpp GNSSTk Discontinuity Corrector
    Phase discontinuity correction. Given a SatPass object containing
    ...
    and fix. Output is a list of Rinex editing commands (see EditRinex or class
    RinexEditor).
*/
...
int gdc::DiscontinuityCorrector(SatPass &SP, std::string &retMsg, std::vector<std::string> &cmds, int GLOn)
{
    try
    {
        sat = SP.getSat();
        ...
        for (unsigned int i = 0; i < SP.size(); i++)
        {
            ...
            if (!(SP.spdvector[i].flag & SatPass::OK) || SP.data(i, L1) == 0.0 || SP.data(i, L2) == 0.0 ||
        ...
        int iret = DiscontinuityCorrector(sat, SP.getDT(), beg, L1_in, L2_in, P1_in, P2_in, dt_in, flags_in, retMsg,
                                          cmds, GLOchan, outfmt);
        if (iret)
        {
            return iret;
...
int gdc::DiscontinuityCorrector(const RinexSatID &sat_in, const double &nominalDT, const Epoch &beginTime,
                                std::vector<double> dataL1, std::vector<double> dataL2, std::vector<double> dataP1,
                                std::vector<double> dataP2, std::vector<double> dt_in, std::vector<int> flags_in,
                                std::string &retMsg, std::vector<std::string> &cmds, int GLOn, std::string outfmt_in)
{
    try
    {
        int iret;
        ...
        for (i = 0; i < dt_in.size(); i++)
        {
            ...
            if (!(flags_in[i] & SatPass::OK) || dataL1[i] == 0.0 || dataL2[i] == 0.0 || dataP1[i] == 0.0 ||
                dataP2[i] == 0.0)
            {
        ...
        if (SPSstr.empty())
        {
            ...
            oss << tag << " SPS " << setw(4) << arc.npts     // ntot
                ...
                << " 0 "                                     // status flag
                << printTime(beginT, outfmt) << " " << printTime(endT, outfmt) << " " << setprecision(1) << fixed << dt
                << " L1 L2 P1 P2";
            SPSstr = oss.str();
        }
        LOG(INFO) << SPSstr;
        ...
        if (flags.size() < 2 * cfg(width))
        {
            ...
            LOG(INFO) << tag << " Pass is too short to analyze: " << flags.size()
...
int gdc::filterFirstDiff(const unsigned which, const string label, double limit, vector<FilterHit<double>> &hits)
{
    try
    {
...
/* filter using window filter
   ...
   return return value of filter() if negative, otherwise number of slips
   found */
int gdc::filterWindow(const unsigned which, const string label, double limit, vector<FilterHit<double>> &hits)
{
    try
    {
        unsigned int i;
...
int gdc::mergeFilterResultsIntoArcs(vector<FilterHit<double>> &hits, const unsigned which)
{
    try
    {
        ...
        {
            LOG(DEBUG) << "#" << tag << " merge " << LAB[which] << " hit into Arc[" << hits[i].index << "] "
                       << hits[i].asString();
            ...
            { // slip
              ...
              // accumulate the slip magnitude for following slips
                ...
                // fixSlips()
                (which == WL ? Arcs[hits[i].index].WLinfo.step : Arcs[hits[i].index].GFinfo.step) =
                    lostslip + hits[i].step;
                (which == WL ? Arcs[hits[i].index].WLinfo.sigma : Arcs[hits[i].index].GFinfo.sigma) =
...
int gdc::fixSlips(const unsigned which)
{
    try
    {
        ...
        for (ait = Arcs.begin(); ait != Arcs.end(); ++ait)
        {
            ...
            LOG(DEBUG) << "#" << tag << " fix slip for Arc[" << ait->first << "] " << ait->second.asString();
            ...
            for (k = 0, i = 0; i < gbs.size(); i++)
            {
                ...
                { // current segment is good and short
                  // can now assume gbs[i-1] and gbs[i+1] are < 0 (bad)
                    ...
                    if ((prevbb && nextbb) || prev1 || next1 || prev23 || next23 || ibeg || iend)
                    {
                        fixup = true;
                        // oss2 << " Mark:";
                        for (j = 0; j < gbs[i]; j++)
...
void gdc::dumpData(ostream &os, const string msg)
{
    ...
    for (k = 0, i = 0; i < xdata.size(); i++)
    {
        ...
        os << msg << " " << sat << " " << printTime(xtime(i), outfmt) << fixed << setprecision(3) << " " << setw(9)
           << (xtime(i) - beginT) //  xdata[i]
           << " " << setw(2) << flags[i] << fixed << setprecision(4) << " " << setw(9) << dataWL[i] * wlWL << " "
           << setw(9) << dataGF[i] * wlGF << arcmsg << endl;
    }
} // end void gdc::dumpData(ostream& os, const string msg)

//---------------------------------------------------------------------------------
void gdc::DumpHits(const vector<FilterHit<double>> &filterResults, const string &tag, const string &label, int prec)
{
    if (prec == -1)
    {
        prec = cfg(osprec);
    ...
    {
        LOG(INFO) << tag << " " << label << " Hit" << i + 1 << "[" << filterResults[i].index << "] "
...
void gdc::DumpArcs(const string &tag, const string &label, int prec)
{
    ...
    for (ait = Arcs.begin(); ait != Arcs.end(); ++ait)
    {
        LOG(INFO) << tag << (label.size() ? " " + label : "") << " Arc" << i++ << "[" << ait->first << "] "
...
string gdc::returnMessage(int prec, int wid)
{
    ...
    for (ait = Arcs.begin(); ait != Arcs.end(); ++ait)
    {
        ...
        oss << " " << setw(4) << A.index << " " << printTime(xtime(A.index), outfmt) << " " << setw(4) << A.npts << " "
            << setw(4) << A.ngood;
        if (A.WLinfo.n > 0)
        {
            oss << " WL " << setw(4) << A.WLinfo.n << " " << setw(wid) << A.WLinfo.ave << " +- " << setw(wid)
                << A.WLinfo.sig;
        }
        if (A.GFinfo.n > 0)
        {
            oss << " GF " << setw(4) << A.GFinfo.n << " " << setw(wid) << A.GFinfo.ave << " +- " << setw(wid)
                << A.GFinfo.sig;
        }
        oss << oss2.str();
    ...
    try
    {
        ...
        for (i = 0; i < SP.size(); i++)
        {
            ...
            { // at new arc
                ...
                LOG(DEBUG) << "#" << tag << " applyFix with Arc[" << ait->first << "] " << ait->second.asString();
                ...
                if (arc.mark & Arc::BEG)
                {
                    if (i != 0)
                    {
                        LOG(WARNING) << " Warning - GDC breaks pass at index " << i << " sat " << sat << " time "
...
void gdc::generateCmds(vector<string> &cmds)
{
    try
    {
        ...
        // generate commands
        ostringstream oss;
        oss << "--BD+ " << sat << "," << L1 << "," << printTime(beginT, "%Y,%m,%d,%H,%M,%S,") << N1bias
            << printTime(beginT, " # initial L1 bias at %F,%.3g");
        cmds.push_back(oss.str());
        oss.str("");
        oss << "--BD+ " << sat << "," << L2 << "," << printTime(beginT, "%Y,%m,%d,%H,%M,%S,") << N2bias
        ...
        for (ait = Arcs.begin(); ait != Arcs.end(); ++ait)
        {
            ...
            if ((arc.mark & (Arc::WLSLIP | Arc::GFSLIP)) || (arc.mark & Arc::REJ))
            {
                ...
                if (nL1)
                {
                    oss << "--BD+ " << sat << "," << L1 << "," << printTime(ttag, "%Y,%m,%d,%H,%M,%S,") << -nL1
                ...
                if (nL2)
                {
                    oss << "--BD+ " << sat << "," << L2 << "," << printTime(ttag, "%Y,%m,%d,%H,%M,%S,") << -nL2
            ...
            if (arc.mark & Arc::REJ)
            {
                ...
                {
                    oss << "--DD+ " << sat << "," << L1 << "," << printTime(tbeg, "%Y,%m,%d,%H,%M,%S # from %F,%.3g")
                        << " - delete entire segment = " << arc.npts << " epochs";
                    cmds.push_back(oss.str());
                    oss.str("");
                    oss << "--DD- " << sat << "," << L1 << "," << printTime(tend, "%Y,%m,%d,%H,%M,%S # to %F,%.3g");
                    cmds.push_back(oss.str());
                    oss.str("");
                    oss << "--DD+ " << sat << "," << L2 << "," << printTime(tbeg, "%Y,%m,%d,%H,%M,%S # from %F,%.3g");
                    cmds.push_back(oss.str());
                    oss.str("");
                    oss << "--DD- " << sat << "," << L2 << "," << printTime(tend, "%Y,%m,%d,%H,%M,%S # to %F,%.3g");
                    cmds.push_back(oss.str());
                    oss.str("");
                }
...
bool gdc::setParameter(std::string label, double value)
{
    ...
    if (label == string("debug"))
    {
        ...
        CFG["RAW"] = CFG["WLF"] = CFG["GFF"] = CFG["WL1"] = CFG["WLG"] = CFG["WLW"] = CFG["WLF"] = CFG["GF1"] =
            CFG["GFG"] = CFG["GFW"] = CFG["GFF"] = CFG["FIN"] = 0;
        if (value > -1)
        {
...
void gdc::DisplayParameterUsage(ostream &os, string tag, bool advanced)
{
    try
    {
        ...
        os << tag << "GNSSTk Discontinuity Corrector (GDC) v." << GDCVersion << " configuration:" << endl;
        ...
        if (advanced)
        {
            ...
            for (it = CFGlist.begin(); it != CFGlist.end(); it++)
            {
                ...
                stst << name              // label
                     << "=" << CFG[name]; // value
                os << tag << " " << StringUtils::leftJustify(stst.str(), adv_name_val_width) << " : "
