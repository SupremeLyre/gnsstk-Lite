...
/// @file SatPass.cpp Data for one complete satellite pass overhead.
...
SatPass::SatPass(const RinexSatID &insat, double indt, std::vector<std::string> obstypes)
{
    init(insat, indt, obstypes);
}

void SatPass::init(const RinexSatID &insat, double indt, std::vector<std::string> obstypes)
{
    sat = insat;
    dt = indt;
...
int SatPass::addData(const Epoch &tt, const std::vector<std::string> &obstypes, const std::vector<double> &data,
                     ...
                     const unsigned short flag)
{
    ...
    {
        Exception e("Dimensions do not match in addData()" + StringUtils::asString(data.size()) + "," +
                    StringUtils::asString(lli.size()) + "," + StringUtils::asString(ssi.size()));
        GNSSTK_THROW(e);
    }
    if (spdvector.size() > 0 && spdvector[0].data.size() != data.size())
    {
        Exception e("Error - addData passed different dimension that earlier!" + StringUtils::asString(data.size()) +
...
int SatPass::addData(const RinexObsData &robs)
{
    ...
    for (it = robs.obs.begin(); it != robs.obs.end(); it++)
    {
        ...
        { // sat is this->sat
            ...
            {
                if ((jt = it->second.find(RinexObsHeader::convertObsType(kt->first))) == it->second.end())
                {
...
bool SatPass::getGLOchannel(int &n, std::string &msg)
{
    try
    {
        ...
        if (indexForLabel.find("L1") == indexForLabel.end() || indexForLabel.find("L2") == indexForLabel.end() ||
            (indexForLabel.find("C1") == indexForLabel.end() && indexForLabel.find("P1") == indexForLabel.end()) ||
            (indexForLabel.find("P2") == indexForLabel.end() && indexForLabel.find("C2") == indexForLabel.end()))
        {
        ...
        { // loop over possible choices for n
            ...
            for (i = 0; i < N; i += di)
            {
                ...
                if (!first)
                {
                    ...
                    LOG(DEBUG) << "GLODMP " << sat << " " << setw(2) << n + dn << " " << printTime(time(i), outFormat)
                               << fixed << setprecision(2) << " " << setw(9) << RB1 - pRB1 << " " << setw(9)
                               << -(L1 - pL1) / 2848. << " " << setw(4) << dN1.N() << " " << setw(9) << dN1.StdDevY()
                               << " " << setw(9) << (dN1.N() > 1 ? dN1.Slope() : 0.0) << " " << setw(9)
                               << (dN1.N() > 1 ? dN1.SigmaSlope() : 0.0) << " " << setw(9) << RB2 - pRB2 << " "
                               << setw(9) << -(L2 - pL2) / 2848. << " " << setw(4) << dN2.N() << " " << setw(9)
                               << dN2.StdDevY() << " " << setw(9) << (dN1.N() > 1 ? dN2.Slope() : 0.0) << " " << setw(9)
            ...
            { // haven't found it yet
                ...
                if (::abs(dm) > 5 || n + dn + dm < -7 || n + dn + dm > 7 || dm == 0 || dN1.SigmaSlope() > testSigma)
                {
                    if (dN1.Slope() < 0)
                    {
                        ++dn;
            ...
            LOG(DEBUG) << "GETGLO " << setw(2) << n + m << " PRELIM " << sat << fixed << setprecision(2) << " "
                       << setw(2) << dN1.N() << " " << setw(9) << dN1.StdDevY() << " (" << setprecision(0) << testStdDev
                       << ")" << setprecision(2) << " " << setprecision(3) << setw(10) << dN1.Slope() << " ("
                       << testSlope << ")" << setprecision(2) << " " << setw(9) << dN1.Slope() / dN1.SigmaSlope()
                       << " (" << setprecision(0) << testRatio << ")" << setprecision(2) << " " << setw(9)
        ...
        ostringstream oss;
        oss << "FINAL" << fixed << setprecision(6) << " " << sat << " " << setw(2) << n + dn << " "
            << printTime(getFirstGoodTime(), outFormat) << " " << printTime(getLastGoodTime(), outFormat) << fixed
            << setprecision(4) << " " << setw(2) << dN1.N() << " " << setw(8) << dN1.StdDevY() << " " << setw(8)
            << dN1.Slope() << " " << setw(8) << dN1.Slope() / dN1.SigmaSlope() << " " << setw(8) << dN1.SigmaSlope()
            << " " << (dN1.SigmaSlope() < testSigma ? "" : "??")
            //<< " " << setw(2) << dN2.N()
            //<< " " << setw(8) << dN2.StdDevY()
            //<< " " << setw(8) << dN2.Slope()
...
void SatPass::smoothSF(bool smoothPR, bool debiasPH, std::string &msg, const int freq, double wl)
{
    try
    {
        if (freq != 1 && freq != 2)
        ...
        {
            GNSSTK_THROW(
                Exception(string("smoothSF() requires pseudorange and phase:" + oss.str() + string(" are missing."))));
        }
        ...
        for (first = true, i = 0; i < spdvector.size(); i++)
        {
            ...
            LOG(DEBUG) << "SMTDMP " << sat << " " << time(i).printf(outFormat) << fixed << setprecision(3) << " "
                       << setw(13) << RB << " " << setw(13) << L << " " << setw(13) << P << " " << setw(13) << RB << " "
                       << setw(13) << LB0;
        ...
        oss.str("");
        oss << "SMT" << fixed << setprecision(2) << " " << sat << " " << getFirstGoodTime().printf(outFormat) << " "
            << getLastGoodTime().printf(outFormat) << " " << setw(5) << (freq == 1 ? PB.N() : 0) << " " << setw(12)
            << (freq == 1 ? PB.Average() + dLB0 : 0) << " " << setw(5) << (freq == 1 ? PB.StdDev() : 0) << " "
            << setw(12) << (freq == 1 ? PB.Minimum() + dLB0 : 0) << " " << setw(12)
            << (freq == 1 ? PB.Maximum() + dLB0 : 0) << " " << setw(5) << (freq == 1 ? 0 : PB.N()) << " " << setw(12)
            << (freq == 1 ? 0 : PB.Average() + dLB0) << " " << setw(5) << (freq == 1 ? 0 : PB.StdDev()) << " "
            << setw(12) << (freq == 1 ? 0 : PB.Minimum() + dLB0) << " " << setw(12)
            << (freq == 1 ? 0 : PB.Maximum() + dLB0) << " " << setw(13) << (freq == 1 ? RB : 0) << " " << setw(13)
            << (freq == 1 ? 0 : RB) << " " << setw(10) << (freq == 1 ? LB : 0) << " " << setw(10)
            << (freq == 1 ? 0 : LB);
        msg = oss.str();
...
void SatPass::smooth(bool smoothPR, bool debiasPH, std::string &msg, const double &wl1, const double &wl2)
{
    try
    {
        ostringstream oss;
        ...
        {
            GNSSTK_THROW(
                Exception(string("smooth() requires obs types L1 L2 C/P1 C/P2:") + oss.str() + string(" missing.")));
        }
        ...
        for (first = true, i = 0; i < spdvector.size(); i++)
        {
            ...
            LOG(DEBUG) << "SMTDMP " << sat << " " << printTime(time(i), outFormat) << fixed << setprecision(3) << " "
                       << setw(13) << RB1 - dbL1 << " " << setw(13) << RB2 - dbL2 << " " << setw(13) << L1 << " "
                       << setw(13) << L2 << " " << setw(13) << P1 << " " << setw(13) << P2 << " " << setw(13) << RB1
                       << " " << setw(13) << RB2;
        }
        ...
        oss.str("");
        oss << "SMT" << fixed << setprecision(2) << " " << sat << " " << printTime(getFirstGoodTime(), outFormat) << " "
            << printTime(getLastGoodTime(), outFormat) << " " << setw(5) << PB1.N() << " " << setw(12)
            << PB1.Average() + dbL1 << " " << setw(5) << PB1.StdDev() << " " << setw(12) << PB1.Minimum() + dbL1 << " "
            << setw(12) << PB1.Maximum() + dbL1 << " " << setw(5) << PB2.N() << " " << setw(12) << PB2.Average() + dbL2
            << " " << setw(5) << PB2.StdDev() << " " << setw(12) << PB2.Minimum() + dbL2 << " " << setw(12)
            << PB2.Maximum() + dbL2 << " " << setw(13) << RB1 << " " << setw(13) << RB2 << " " << setw(10) << LB1 << " "
            << setw(10) << LB2;
        msg = oss.str();
        ...
        for (i = 0; i < spdvector.size(); i++)
        {
            ...
            if (smoothPR)
            {
                ...
                spdvector[i].data[indexForLabel[(useC1 ? "C1" : "P1")]] = D11 * wl1 * dbL1 + D12 * wl2 * dbL2;
                spdvector[i].data[indexForLabel[(useC2 ? "C2" : "P2")]] = D21 * wl1 * dbL1 + D22 * wl2 * dbL2;
            }
...
Epoch SatPass::getFirstTime() const
{
    return time(0);
}
...
/* these allow you to get e.g. P1 or C1. NB return double not double& as
   above: rvalue */
double SatPass::data(unsigned int i, const std::string &type1, const std::string &type2) const
{
    if (i >= spdvector.size())
    {
        Exception e("Invalid index in data() " + asString(i));
...
unsigned short SatPass::LLI(unsigned int i, const std::string &type1, const std::string &type2)
{
    if (i >= spdvector.size())
    {
        Exception e("Invalid index in LLI() " + asString(i));
...
unsigned short SatPass::SSI(unsigned int i, const std::string &type1, const std::string &type2)
{
    if (i >= spdvector.size())
    {
        Exception e("Invalid index in SSI() " + asString(i));
...
*/
bool SatPass::hasOverlap(const SatPass &that, double *pdelt, Epoch *pttb, Epoch *ptte)
{
...
void SatPass::dump(ostream &os, const std::string &msg1, const std::string &msg2)
{
    ...
    {
        tt = time(i);
        os << msg1 << " " << setw(3) << i << " " << sat << " " << setw(3) << spdvector[i].ndt << " " << setw(2)
           << spdvector[i].flag << " " << printTime(tt, SatPass::outFormat) << fixed << setprecision(6) << " "
           << setw(9) << spdvector[i].toffset << setprecision(3);
        for (j = 0; j < indexForLabel.size(); j++)
            os << " " << setw(13) << spdvector[i].data[j] << " " << spdvector[i].lli[j] << " " << spdvector[i].ssi[j];
        if (i == 0)
        {
...
{
    os << setw(4) << sp.spdvector.size() << " " << sp.sat << " " << setw(4) << sp.ngood << " " << setw(2) << sp.Status
       << " " << printTime(sp.firstTime, SatPass::outFormat) << " " << printTime(sp.lastTime, SatPass::outFormat) << " "
