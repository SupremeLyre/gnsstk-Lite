...
/// @file SatPassIterator.cpp Iterate over a vector of SatPass in time order.
    ...
    : SPList(splist), timeReverse(rev), debug(dbug)
{
    ...
    for (i = 0; i < SPList.size(); i++)
    {
        ...
        if (SPList[i].dt != DT)
        {
            Exception e("Inconsistent time intervals: " + asString(SPList[i].dt) + " != " + asString(DT));
            GNSSTK_THROW(e);
        }
...
void SatPassIterator::reset(bool rev, bool dbug)
{
    ...
    while ((timeReverse && i >= 0) || (!timeReverse && i < SPList.size()))
    {
        ...
        for (;;)
        {
            ...
            {
                currentN =
                    int((SPList[i].firstTime - FirstTime) / DT + 0.5) + SPList[i].spdvector[SPList[i].size() - 1].ndt;
            }
            ...
            if (listIndex.find(SPList[i].sat) == listIndex.end())
            {
                ...
                countOffset[SPList[i].sat] = int((SPList[i].firstTime - FirstTime) / DT + 0.5);
                LOG(DEBUG4) << "reset - define map " << i << " for sat " << SPList[i].sat << " at time "
                            << SPList[i].firstTime.printf("%4F %10.3g") << " offset " << countOffset[SPList[i].sat];
            }
            else
            {
                indexStatus[i] = -1;
                LOG(DEBUG4) << "reset - turn off pass " << i << " for sat " << SPList[i].sat << " at time "
...
int SatPassIterator::next(std::map<unsigned int, unsigned int> &indexMap)
{
    ...
    if (debug)
    {
        LOG(INFO) << "SPIterator::next(map) - time " << (FirstTime + currentN * DT).printf("%4F %10.3g")
    ...
    while (numsvs == 0)
    {
        ...
        while (kt != listIndex.end())
        {
            ...
            if (debug)
            {
                LOG(INFO) << "Loop over listIndex: " << sat << " " << i << " " << j;
            }
            ...
            if (SPList[i].Status < 0)
            {
                ...
                if (debug)
                {
                    LOG(INFO) << " Erase this pass for bad status: index " << i << " sat " << sat << " size is now "
                              << listIndex.size();
                }
                continue;
            }
            ...
            if (countOffset[sat] + SPList[i].spdvector[j].ndt == currentN)
            {
                ...
                if (debug)
                {
                    LOG(INFO) << "SPIterator::next(map) found sat " << sat << " at index " << i;
                }

                // increment data index
                if ((timeReverse && --j < 0) || (!timeReverse && ++j == SPList[i].spdvector.size()))
                {
                    if (debug)
                    {
                        LOG(INFO) << " This pass for sat " << sat << " is done ...";
                    }
                    indexStatus[i] = 1;
                    ...
                    // for(k=i+1; k<SPList.size(); k++)
                    {
                        if (debug)
                        {
                            LOG(INFO) << " ... consider next pass " << k << " " << SPList[k].sat << " "
                    ...
                    if (indexStatus[i] == 0)
                    {
                        if (debug)
                        {
                            LOG(INFO) << " ... new pass for sat " << SPList[i].sat << " at index " << i << " and time "
                                      << SPList[i].firstTime.printf("%4F %10.3g");
                        }
                    }
                }
            ...
            {                          // a new one was not found
                ...
                if (debug)
                {
                    LOG(INFO) << " Erase this pass: index " << i << " sat " << sat << " size is now "
                              << listIndex.size();
                }
            }
            else
