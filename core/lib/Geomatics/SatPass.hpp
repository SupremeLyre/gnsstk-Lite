...
class SatPass
{
  protected:
    ...
    /**
     struct SatPassData, for internal use only, stores the data and flag
     arrays (data, lli, ssi) plus a time offset, a SatPass flag, a user flag,
     and the offset n in time (n*dt) since FirstTime. class SatPass will
     include a vector of these structs.
    */
    struct SatPassData
    {
        // member data ----------------------------------
        ...
        */
        ...
        /**
         a flag for arbitrary use by the user; SatPass ONLY has
         set/getUserFlag()
        */
        ...
        */
        SatPassData(unsigned short n = 4) : flag(SatPass::OK), userflag(0), ndt(0), toffset(0.0)
        {
        ...
        SatPassData &operator=(const SatPassData &right)
        {
            if (&right != this)
            {
                ...
                int i;
                for (i = 0; i < right.data.size(); i++)
                    data[i] = right.data[i];
                for (i = 0; i < right.lli.size(); i++)
                    lli[i] = right.lli[i];
                for (i = 0; i < right.ssi.size(); i++)
                    ssi[i] = right.ssi[i];
            }
    ...
    */
    ...
    /**
     STL map relating strings identifying obs types with indexes in
     SatPassData. E.g. index = indexForLabel[label].
    */
    ...
    /**
     nominally, timetags of the first and last data points; in fact
     firstTime is used with toffset to give the full time, so these are
     actually times - toffset
    */
    ...
    /**
     ...
     @return n>=0 if data was added successfully, n is the index of the new
     data
  ...
  public:
    ...
    /**
     class SatPassIterator allows the caller to access all the data in a
     list of SatPass objects in time order.
    */
    friend class SatPassIterator;

    /**
     Read a set of RINEX observation files, filling a vector of SatPass
     objects. Create the SatPass objects using the input list of
     observation types and timestep. If there are no obs types given
     (vector obstypes has size 0), then use the obs types in the header of
     the first file read. The files are sorted on their begin time before
     processing, and the resulting list of SatPass objects is in time
     order. Data before the begin time, and after the end time, is ignored.
     The list of SatPass objects need not be empty on input; however if not
     empty, then the list must be consistent with the input timestep and
     obs type list, and it will be sorted to be in time order. If a file
     cannot be opened, or it is not RINEX observation, simply skip it.
     @param filenames vector of input RINEX observation file names
     @param obstypes  vector of observation types to include in SatPass
                      (may be empty: include all)
     @param dt        data interval of existing SPList and input files
     @param SPList    vector of SatPass objects to fill, need not be empty,
                      but if not empty, must have consistent obs types and dt.
    ...
    */
    friend int SatPassFromRinexFiles(std::vector<std::string> &filenames, std::vector<std::string> &obstypes, double dt,
                                     std::vector<SatPass> &SPList, std::vector<RinexSatID> exSats, bool lenient,
    ...
    */
    ...
    /**
     ...
     @param obstypes  a vector of strings, each string being a 2-character
                      RINEX observation type, e.g. "L1", "P2", to be
                      stored.
    */
    SatPass(const RinexSatID &sat, double dt, std::vector<std::string> obstypes);

    /** d'tor, copy c'tor are built by compiler; so is operator= but don't use
        it! */
    ...
    /**
     Add vector of data, identified by obstypes (same as used in c'tor) at
     tt, Flag is set 'good' and lli=ssi=0
     @param tt        the time tag of interest
     @param obstypes  a vector of strings, each string being a 2-character
                      RINEX observation type, e.g. "L1", "P2", to be
                      stored. This MUST match the list used in the
                      constructor.
     @param data      a vector of data values, parallel to the obstypes
                      vector
    ...
    */
    ...
    /**
     Add vector of data, identified by obstypes (same as used in c'tor) at
     tt, Flag, lli and ssi are set using input (parallel to data).
     @param tt        the time tag of interest
     @param obstypes  a vector of strings, each string being a 2-character
                      RINEX observation type, e.g. "L1", "P2", to be
                      stored. This MUST match the list used in the constructor.
     ...
     @throw Exception
    */
    int addData(const Epoch &tt, const std::vector<std::string> &obstypes, const std::vector<double> &data,
                const std::vector<unsigned short> &lli, const std::vector<unsigned short> &ssi,
                const unsigned short flag = SatPass::OK);

    /**
     Add data as found in RinexObsData. No action if this->sat is not
     found. Pull out time tag and all data in obs type list. All flags are
     set 'good'.
     @param robs  Rinex observation data from which to pull data. Only data
                   for the object's satellite is added.
     @return n>=0 if data was added successfully, n is the index of the new data
            -1 if a gap is found (no data is added),
            -2 if time tag is out of order (no data is added)
            -3 if the satellite was not found in the RinexObsData (no data
            added)
    */
    int addData(const RinexObsData &robs);

    // -------------------------- get and set routines
    // -------------------------- can change ssi, lli, data, but not
    // times,sat,dt,ngood,count get and set flag so you can update ngood
    ...
    /**
     Access the data for one obs type at one index, as either l-value or
     r-value
    ...
    */
    ...
    /**
     Access the time offset from the nominal time (i.e. timetag) at one
     index (epoch), as either l-value or r-value
     @param  i    index of the data of interest
    ...
    */
    ...
    /**
     Access the LLI for one obs type at one index, as either l-value or
     r-value
     @param  i    index of the data of interest
     @param  type observation type (e.g. "L1") of the data of interest
    ...
    */
    ...
    /**
     Access the ssi for one obs type at one index, as either l-value or
     r-value
     @param  i    index of the data of interest
     @param  type observation type (e.g. "L1") of the data of interest
    ...
    */
    ...
    /**
     @return the earliest time (full, including toffset) in this SatPass
     data
    */
    ...
    */
    ...
    /**
     Access the data for either of two obs type at one index, as r-value
     only
     @param  i     index of the data of interest
    ...
    */
    ...
    /**
     Access the LLI for either of two obs type at one index, as r-value
     only
     @param  i     index of the data of interest
     @param  type1 observation type (e.g. "P1") of the data of interest
     @param  type2 observation type (e.g. "C1") of the data of interest
    ...
    */
    ...
    /**
     Access the ssi for either of two obs type at one index, as r-value
     only
     @param  i     index of the data of interest
    ...
    */
    ...
    /**
     ...
     @return true if this obstype was passed to the c'tor (i.e. is in
     indexForLabel)
    */
    inline bool hasType(std::string type) const
    {
    ...
    */
    ...
    /**
     ...
     @param tt the time tag of interest
     @return -1 if not within the time limits of the SatPass, else index of
     tt
     @throw Exception
    */
    ...
    */
    void renameObstypes(std::map<std::string, std::string> &subst)
    {
        ...
        while (it != subst.end())
        {
            if (indexForLabel.find(it->first) == indexForLabel.end())
            {
                continue;
            }

            i = indexForLabel[it->first];
            indexForLabel.insert(std::map<std::string, unsigned int>::value_type(it->second, i));
            ...
            for (jt = labelForIndex.begin(); jt != labelForIndex.end(); ++jt)
            {
                ...
                labelForIndex.insert(std::map<unsigned int, std::string>::value_type(i, it->second));
            }
    ...
    /**
     ...
     @param tt        the time tag of interest
     @return true if the given time tag lies within the time interval
    ...
    */
    ...
    /**
     Decimate the data in the SatPass by (integer) factor N, referencing
     refTime; that is keep only epochs that satisfy time=refTime+n*N*dt
     where n is also an integer, and dt is the current time spacing of the
     time spacing of the SatPass. This routine decimates the data, reduces
     the arrays, and may change the start and stop times and ngood; time
    ...
    */
    ...
    /**
     Determine if there is common-view between this SatPass and another,
     that is if the satellites match and the time limits overlap. Return
     the time in seconds of the overlap.
    */
    ...
    /**
     ...
     both return true if successful, false if failed; also return string
     msg, which is FINAL sat n wk sow(beg) wk sow(end) npt stddev slope
     sl/std stddev(slope) [??] NB if "??" appears at end of msg, results
    ...
    */
    ...
    /**
     Smooth pseudorange and debias phase, by computing the best estimate of
     the range-minus-phase over the whole pass, and subtracting this bias
     from the raw phase. NB. remove an INTEGER (cycles) to get "debiased
     phase" but a REAL (m) to get "smoothed pseudorange"; thus smoothed
     pseudorange != debiased phase. This routine can be called ONLY if dual
     frequency range and phase data (C/P1,C/P2,L1,L2) have been stored, and
     should be called ONLY after ALL cycleslips have been removed.
     @param  smoothPR  if true, replace the pseudorange (P1 and P2) with smoothed
                       ranges (= phase + REAL estimate of range-minus-phase
                       in m).
     @param  smoothPH  if true, replace the phase (L1 and L2) with debiased phase
                       (= phase + INTEGER estimate of range-minus-phase in
                       cycles).
     ...
     @param  wl2       wavelength of L2 for this data
      This string consists of the following 18 fields separated by
      whitespace. 1) "SMT", 2) satellite id (e.g. G07), 3) start GPS week,
      4) start GPS seconds of week, 5) end GPS week, 6) end GPS seconds of
      week, (7-11 are for the L1 bias data in units of meters) 7) number,
      8) average, 9) std deviation, 10) minimum, and 11) maximum, (12-16
      are for the L2 bias data in units of meters) 12) number, 13) average,
      14) std deviation, 15) minimum, and 16) maximum, 17) the L1 bias in
      cycles, 18) the L2 bias in cycles.
     @throw Exception
    */
    void smooth(bool smoothPR, bool smoothPH, std::string &msg, const double &wl1 = L1_WAVELENGTH_GPS,
    ...
    /**
     ...
     @param  smoothPH  if true, replace the phase with debiased phase
                       ...
                       cycles).
     @param  msg       a string containing a single-line statistical summary of
                       the smoothing results, and the estimated phase
                       biases.
    ...
    */
    std::string toString(std::string msg = "", std::string fmt = "%04Y/%02m/%02d %02H:%02M:%06.3f = %04F %w %10.3g")
    {
        ...
        os << msg << " " << sat << " N " << std::setw(4) << size() << " good " << std::setw(4) << ngood << " times "
           << printTime(getFirstTime(), fmt) << " to " << printTime(getLastTime(), fmt) << " obs:";
