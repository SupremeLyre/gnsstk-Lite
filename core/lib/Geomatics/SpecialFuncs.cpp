...
double lnGamma(double x)
{
    try
    {
        static const double con[8] = {76.18009172947146,    -86.50532032941677, 24.01409824083091, -1.231739572450155,
                                      1.208650973866179e-3, -5.395239384953e-6, 1.000000000190015, 2.5066282746310005};
...
double incompleteBeta(double x, double a, double b)
{
    ...
    try
    {
        double factor = ::exp(lnGamma(a + b) - lnGamma(a) - lnGamma(b) + a * ::log(x) + b * ::log(1.0 - x));
        if (x < (a + 1.0) / (a + b + 2.0))
        {
...
double FDistCDF(double F, int n1, int n2)
{
    ...
    try
    {
        return (1.0 - incompleteBeta(double(n2) / (double(n2) + double(n1) * F), double(n2) / 2.0, double(n1) / 2.0));
    }
    catch (Exception &e)
    {
        GNSSTK_RETHROW(e);
