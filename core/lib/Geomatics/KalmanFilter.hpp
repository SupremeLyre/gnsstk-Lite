...
/// @file KalmanFilter.hpp Kalman filter implementation using the SRIFilter class.
...
/**
 ...
 class with this class as base, implement some or all the virtual
 functions to define problem.

 The derived class MUST overload:
   virtual int defineInitial() to define initial time and apriori state
   and cov. virtual int defineMeasurements() to define Partials, Data and
   MCov. virtual void defineTimestep() to define Phi, G, Rw and Control

 The derived class MAY overload this function, which is called between
 updates,
   virtual int defineInterim() to output, change the state or anything
   else.
 ...
 Note that a "time" is used by the Kalman to index the data and determine
 how to time update (TU); this need not be a real time, however; it is
 ONLY required that 0. defineInitial(T0 ...) defines the starting value of
 T (NB also timeReverse)
 ...
 3. defineMeasurements(T...) on output, T contains the time of the NEXT
 data epoch The actual value of T is used only a) it is printed in
 output(), and
   b) some implementations will use DT in the TU to compute process
   noise(s).
 ...
 2. create the filter, using either empty c'tor and Reset(Namelist), or
 c'tor(NL)
 3. initialize the filter by calling initializeFilter() (calls
 defineInitial())
 4. call ForwardFilter(finalTime,dt); which increments time by dt and NTU
 (from 0),
    until time reaches finalTime.
       ...
       KalmanMeasurementUpdate(), which calls
          defineMeasurements() to get current time, data, mcov, and
          partials NB. defineMeasurements() controls the time steps.
  ...
  5. call BackwardFilter(M); this will smooth, starting at the current
  time down to
 ...
 Several switches may be used to control the filter. For each there is
 also a 'set' routine.
 1. setDoOutput(false) turns off the output routine.
 2. setDoInvert(false) stops inversions (compute State and Cov from the
 SRI) and
       output during the forward filter. NB. cf. setSRISU for the backward
       filter.
 3. setSmoother(true) must be called before ForwardFilter() if
 BackwardFilter() is
       to be called. This causes the smoothing information to be stored
       during TUs.
 4. setSRISU(true) causes BackwardFilter() to use the SRIF form of the
 smoothing
       algorithm (which requires inversions); otherwise the DM form is
       used.
...
class KalmanFilter
{
  ...
  protected:
    ...
    bool doOutput;
    /**
     if true, invert the SRIF to get State and Covariance whenever SRIF
     changes. In general it is wise to set this false, then reset to true
     only when the State is to be used. For example if you need the State
     Vector in the MU, then set doInversions=true in defineIntermin(1),
     then set to false in defineM. Inversions are called between all the
     define...() calls.
    */
    bool doInversions;
    /**
     if true then the SRIF is currently singular (not a problem unless
     doInversions)
    */
    bool singular;
    /**
     if true use the SRIF form of the smoother update in the backward
     filter
    */
    bool doSRISU;
    /**
     if true the filter is considered extended; this will zero the state
     before MU
    */
    bool extended;
    /**
     if true the forward filter will save the data needed by the backward
     filter
    */
    bool smoother;
    ...
    bool timeReversed;
    /**
     if true, do a "dry run" calling all user-defined func, but none of the
     SRIF
    */
    bool dryRun;
    ...
    gnsstk::Matrix<double> Partials; ///< matrix defined by defineM() and used in MU
    gnsstk::Vector<double> Data;     ///< vector defined by defineM() and used in MU
    ...
    gnsstk::Matrix<double> PhiInv;   ///< SRIF matrix used internally - inv state trans
    gnsstk::Matrix<double> G;        ///< SRIF matrix used internally - noise
    gnsstk::Matrix<double> Rw;       ///< SRIF matrix used internally
                                     // SU
    gnsstk::Vector<double> SMResid;  ///< post-smoother residuals - value after SU
  ...
  public:
     ...
     */
    virtual KalmanReturn defineMeasurements(double &T, const gnsstk::Vector<double> &X, const gnsstk::Matrix<double> &C,
                                            bool useFlag) = 0;
     ...
     * @param[in] useFlag if false, State and Cov are singular - do not use
     */
    virtual void defineTimestep(double T, double DT, const gnsstk::Vector<double> &State,
                                const gnsstk::Matrix<double> &Cov, bool useFlag) = 0;
    ...
    // -----------------------------------------------------------------------------
    /** 2. create the filter, using either the empty c'tor followed by
       Reset(Namelist), or the c'tor(Namelist), giving an initial Namelist for
       the filter state.
       empty constructor; Reset() must be called before
       initializing or filtering */
    KalmanFilter()
        : NTU(0), NMU(0), NSU(0), Nstate(0), stage(Unknown), Nnoise(0), extended(false), smoother(false), doSRISU(true),
          doOutput(true), doInversions(true), singular(true), timeReversed(false), dryRun(false)
    {
    }
    ...
    */
    ...
    virtual void ForwardFilter(double finalT, double DT)
    {
        ...
        try
        {
            ...
            {
                gnsstk::Exception e(std::string("Filter time step must be ") +
    ...
    // -------------------------------------------------------------
    /**
     Backward filter (smoother) with double argument - this is a trick to
     prevent the user from calling BackwardFilter(time).
    */
    void BackwardFilter(double M)
    {
        GNSSTK_THROW(gnsstk::Exception("BackwardFilter must be called with integer NTU"));
    }
    ...
    */
    virtual void output(int N)
    {
        ...
        {
            oss << ((stage == MU || stage == Init) ? "KNL" : "KSL") << KFtag << " " << std::fixed << N << " "
    ...
    gnsstk::SRI getSRI()
    {
        return static_cast<gnsstk::SRI>(srif);
    }

    /// get the state namelist
    gnsstk::Namelist getNames()
    {
        return srif.getNames();
    }
