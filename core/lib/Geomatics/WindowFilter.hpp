...
/** @file WindowFilter.hpp
    ...
    wrapper class (StatsFilterBase, this module) that provides a single
    interface for the two statistics, allowing WindowFilter::filter() to use
    either type of filter interchangably. Two-sample stats are used when an
    xdata array ("time") is given along with the data array; this is appropriate
    for data that has systematic "time" dependence. One-sample stats are used
    when the data is ~constant; in this case the xdata can be given as well but
    will be used only in dump().
    ...
    two filters; WindowFilter::getStats computes min, max, median and mad of
    sigma = rms(sig of future and past), but not including points within one
    width of the endpoints (avoids the bump in sigma due to slip(s) at the
    segment boundaries).
...
{
/**
 A special subset of class FilterHit used for "almost slips" in
 WindowFilter
*/
...
//---------------------------------------------------------------------------------
/**
 This class computes statistics in a sliding window with two equal halves,
 'past' and 'future', at each point in the input data array(s). The
 objective is to detect discontinuities or "slips" and compute statistics
 on the data between slips. This class will read the data, apply the
 window filter, generating statistics that are stored, then analyze the
 statistics, returning a vector of simple results, and optionally dump all
 the data, analysis information and results.
*/
template <class T> class WindowFilter
{
  public:
    ...
    */
    ...
    inline void setBufferSize(int b)
    {
        buffsize = b;
    }
    inline void setTwoSample(bool b)
    {
        twoSample = b;
    }
    inline void setBalanced(bool b)
    {
        balanced = b;
    }
    inline void setFullWindows(bool b)
    {
        fullwindows = b;
    }
    inline int getWidth()
    ...
    inline bool isTwoSample()
    {
        return twoSample;
    }
    inline bool isOneSample()
    {
        return !twoSample;
    }
    ...
    inline bool isFullWindows()
    {
        return fullwindows;
    }
    // get and set analysis configuration
    inline void setMinRatio(T val)
    {
        minratio = val;
    }
    inline void setMinStep(T val)
    {
        minstep = val;
    }
    inline void setMinMargin(T val)
    {
        minmargin = val;
    }
    inline void setPFFrac(T val)
    {
        pffrac = val;
    }
    inline void setHalfWidth(int hw)
    {
        halfwidth = hw;
    }
    inline T getMinRatio()
    {
        return minratio;
    }
    inline T getMinStep()
    ...
    inline int getHalfWidth()
    {
        return halfwidth;
    }

    /**
     get and set for dump() parameters in dump(), include the little
     message at the end of line from analyze()
    */
    inline void setDumpAnalMsg(bool b)
    {
        dumpAmsg = b;
    }
    inline bool willDumpAnalMsg()
    {
        return dumpAmsg;
    }
    /// in dump(), don't dump data when there was no analysis
    inline void setDumpNoAnal(bool b)
    {
        dumpNA = b;
    }
    inline bool willDumpNoAnal()
    {
        return dumpNA;
    }
    ...
    /**
     reset the analysis information stored internally. This does not change
     the data arrays; to do so, instantiate a new filter object.
    */
    void reset()
    {
        analvec.clear();
    ...
    /**
     window filter routine that does the work. NB this routine configures
     the analysis data and therefore is critical to anlayse() and dump().
     ...
     @return > 0 number of points in analysis vector, which will be ngood
              ...
              -3 if the xdata|flags array is given but shorter than data
              array
     This routine clears the analysis vector, runs filter and generates
     analysis.
    */
    int filter(const size_t index = 0, int npts = -1);

    /**
     analyze the output of the filter(), filling the analysis array
     'analvec' also fills the 'results' vector of one or more FilterHit.
     @return the number of slips, outliers, etc (size of results
     vector<FilterHit>)
    */
    int analyze();

    /**
     dump the data and analysis, including first and last 2 pts which have
     no stats. optionally include a tag at the start of each line, and
     configure width and precision.
    */
    ...
    /**
     ...
     defined as the min, max, median and mad of sigma, the rss(future and
     past stddev). if the flag is true (default), exclude data within the
     filter width of the end points, to avoid the bump(s) due to slip(s) at
     the FilterHit boundaries.
    */
  ...
  private:
    ...
    /**
     vector of Analysis objects, holding analysis information, generated by
     filter() and used by analyze() and included in dump() output.
    */
...
#define inc(i)                                                                                                         \
    ...
    for (i = isecond; i < limm2;)
    {
        ...
        if (debug)
        {
            std::cout << "WF:FIL" << std::fixed << std::setprecision(osp) << " " << std::setw(osw) << xvec(i) << " "
                      << std::setw(osw) << dvec(i) << " " << std::setw(osw) << A.step << " " << std::setw(osw)
                      << A.sigma << " " << std::setw(3) << A.pN << " " << std::setw(osw) << A.pave << " "
                      << std::setw(osw) << A.psig << " " << std::setw(3) << A.fN << " " << std::setw(osw) << A.fave
                      << " " << std::setw(osw) << A.fsig << " " << std::setw(osw) << ::fabs(A.step / A.sigma)
                      << std::endl;
        }
...
template <class T> int WindowFilter<T>::analyze()
{
    ...
    { // loop over arrays
        ...
        {
            std::cout << "WF:ANL"
                      << " " << std::setw(3) << i << " " << std::setw(3) << analvec[i].index << std::fixed
                      << std::setprecision(osp) << std::setw(osw) << " "
                      << (noxdata ? T(analvec[i].index) : xdata[analvec[i].index]) << " " << std::setw(osw)
                      << data[analvec[i].index] << " " << std::setw(osw) << analvec[i].step << " " << std::setw(osw)
                      << analvec[i].sigma << " " << std::setw(3) << analvec[i].pN << " " << std::setw(osw)
                      << analvec[i].pave << " " << std::setw(osw) << analvec[i].psig << " " << std::setw(3)
                      << analvec[i].fN << " " << std::setw(osw) << analvec[i].fave << " " << std::setw(osw)
                      << analvec[i].fsig << " " << std::setw(osw) << ::fabs(analvec[i].step / analvec[i].sigma)
                      << ratmsg << sigmsg << fmpmsg << wtmsg << std::flush;
        }
        ...
        else if (::fabs(analvec[i].step / analvec[i].sigma) / minratio + ::fabs(analvec[i].step) / minstep - 2. <
                 minmargin)
        {
            if (debug)
            {
...
template <class T> void WindowFilter<T>::dump(std::ostream &os, std::string tag)
{
    ...
    // TD print slope?
    os << "#" << msg << " WindowFilter::dump() with " << (twoSample ? "two" : "one") << "-sample stats, minStep "
       << std::fixed << std::setprecision(osp) << getMinStep() << " minRatio " << getMinRatio() << " width " << width
       << " btwn-buff " << buffsize << (noxdata ? " (xdata is index)" : "") << std::endl;
    os << "#"
       << msg
       //<< " i xdata data  step sigma  pN pave psig  fN fave fsig  ratio
       // slope ("
       << " i xdata data  step sigma  pN pave psig  fN fave fsig  ratio (" << (balanced ? "" : "not ") << "balanced, "
    ...
    for (i = 0, j = 0, k = 0; i < data.size(); i++)
    {
        if (j >= analvec.size() || i != analvec[j].index)
        {
            if (dumpNA)
            {
                os << msg << std::fixed << std::setprecision(osp) << " " << std::setw(3) << i << " " << std::setw(osw)
                   << (noxdata ? T(i) : xdata[i])
        ...
        else
        {
            ...
            {
                res = " " + (results[k].haveStats ? results[k].asStatsString(osp) : results[k].asString());
                k++;
            }
            ...
            os << msg << std::fixed << std::setprecision(osp) << " " << std::setw(3) << i << " " << std::setw(osw)
               << (noxdata ? T(i) : xdata[i])
               //<< " " << std::setw(3) << (noflags ? 0 : flags[i])
               << " " << std::setw(osw) << data[i] << " " << std::setw(osw) << analvec[j].step << " " << std::setw(osw)
               << analvec[j].sigma << " " << std::setw(3) << analvec[j].pN // past
               << " " << std::setw(osw) << analvec[j].pave << " " << std::setw(osw) << analvec[j].psig << " "
               << std::setw(3) << analvec[j].fN // future
               << " " << std::setw(osw) << analvec[j].fave << " " << std::setw(osw)
               << analvec[j].fsig
               // ratio:
               << " " << std::setw(osw)
...
// boundaries.
template <class T> void WindowFilter<T>::getStats(FilterHit<T> &sg, bool skip)
{
    unsigned int i, j;
