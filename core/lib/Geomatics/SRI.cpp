...
SRI::SRI(const Matrix<double> &r, const Vector<double> &z, const Namelist &nl)
{
    if (r.rows() != r.cols() || r.rows() != z.size() || r.rows() != nl.size())
    {
        MatrixException me("Invalid dimensions in explicit SRI constructor:\n R is " + asString<int>(r.rows()) + "x" +
                           asString<int>(r.cols()) + ", Z has length " + asString<int>(z.size()) +
...
void SRI::setFromCovState(const Matrix<double> &Cov, const Vector<double> &State, const Namelist &NL)
{
    if (Cov.rows() != Cov.cols() || Cov.rows() != State.size() || Cov.rows() != NL.size())
    {
        MatrixException me("Invalid dimensions in SRI constructor from Cov,State:\n"
                           " Cov is " +
                           asString<int>(Cov.rows()) + "x" + asString<int>(Cov.cols()) + ", State has length " +
                           asString<int>(State.size()) + " and NL has length " + asString<int>(NL.size()));
        GNSSTK_THROW(me);
    }
...
void SRI::shift(const Vector<double> &X0)
{
    if (X0.size() != R.cols())
    {
        MatrixException me("Invalid input dimension: SRI has dimension " + asString<int>(R.rows()) +
                           " while input has length " + asString<int>(X0.size()));
        GNSSTK_THROW(me);
    }
    Z = Z - R * X0;
}
...
void SRI::shiftZ(const Vector<double> &Z0)
{
    ...
    {
        MatrixException me("Invalid input dimension: SRI has dimension " + asString<int>(R.rows()) +
                           " while input has length " + asString<int>(Z0.size()));
        GNSSTK_THROW(me);
    }
    Z = Z - Z0;
}
...
void SRI::retriangularize(const Matrix<double> &A)
{
    ...
    {
        MatrixException me("Invalid input dimension: SRI has dimension " + asString<int>(n) +
                           " while input has dimension " + asString<int>(A.rows()) + "x" + asString<int>(A.cols()));
        GNSSTK_THROW(me);
    }
...
void SRI::retriangularize(Matrix<double> RR, Vector<double> ZZ)
{
    ...
    {
        MatrixException me("Invalid input dimension: SRI has dimension " + asString<int>(n) +
                           " while input has dimension " + asString<int>(RR.rows()) + "x" + asString<int>(RR.cols()) +
...
void SRI::transform(const Matrix<double> &invT, const Namelist &NL)
{
    ...
    {
        MatrixException me("Invalid input dimension: SRI has dimension " + asString<int>(n) +
                           " while invT has dimension " + asString<int>(invT.rows()) + "x" +
...
void SRI::stateFix(const std::string &name, double value, double sigma, bool restore)
{
...
/* -----------------------------------------------------------------------------
   ...
   param value to which the state element is fixed
   param sigma (1/information) assigned to the element */
void SRI::stateFix(const unsigned int index, double value, double sigma, bool restore)
{
    if (index >= names.size())
    {
...
void SRI::addAPriori(const Matrix<double> &Cov, const Vector<double> &X)
{
    if (Cov.rows() != Cov.cols() || Cov.rows() != R.rows() || X.size() != R.rows())
    {
        MatrixException me("Invalid input dimensions:\n  SRI has dimension " + asString<int>(R.rows()) +
                           ",\n  while input is Cov(" + asString<int>(Cov.rows()) + "x" + asString<int>(Cov.cols()) +
...
// -----------------------------------------------------------------------------
void SRI::addAPrioriInformation(const Matrix<double> &InvCov, const Vector<double> &X)
{
    if (InvCov.rows() != InvCov.cols() || InvCov.rows() != R.rows() || X.size() != R.rows())
    {
        MatrixException me("Invalid input dimensions:\n  SRI has dimension " + asString<int>(R.rows()) +
                           ",\n  while input is InvCov(" + asString<int>(InvCov.rows()) + "x" +
                           asString<int>(InvCov.cols()) + ") and X(" + asString<int>(X.size()) + ").");
        GNSSTK_THROW(me);
    }
...
void SRI::getState(Vector<double> &X, int *ptr) const
{
    ...
    { // loop over rows, in reverse order
        if (R(i, i) == 0.0)
        {
            ...
            MatrixException me("Singular matrix; zero diagonal element at index " + asString<int>(i));
            GNSSTK_THROW(me);
        }
        double sum = Z(i);
...
void SRI::getStateAndCovariance(Vector<double> &X, Matrix<double> &C, double *ptrSmall, double *ptrBig) const
{
    try
    {
        double small, big;
        ...
        {
            MatrixException me("Singular matrix: condition number is " + asString<double>(big) + " / " +
