...
class WNJfilter : public KalmanFilter
{
  public:
    ...
    WNJfilter() : filterOutput(true), ptrx(NULL), ptrv(NULL), ptra(NULL), ptrs(NULL), prec(2), width(9)
    {
    }
    ...
    void Reset(int dim)
    {
        ...
        apState = gnsstk::Vector<double>(dim, 0.0); // apriori state, of length Nstate
        apNoise = gnsstk::Vector<double>(dim, 0.0); // apriori noise, of length Nstate
        ttag.clear();
        data.clear();
        msig.clear();
        psig.clear();
    ...
    int defineInitial(double &T0, gnsstk::Vector<double> &State, gnsstk::Matrix<double> &Cov)
    {
        ...
        {
            gnsstk::Exception e(std::string("Must define apState and apNoise, and they ") +
                                std::string("must be of length Nstate = ") + gnsstk::StringUtils::asString(Nstate) +
     ...
     * @throw Exception
     */
    void defineTimestep(double T, double DT, const gnsstk::Vector<double> &State, const gnsstk::Matrix<double> &Cov,
                        bool useFlag)
    {
        if (!useFlag)
        {
     ...
     * @throw Exception
     */
    KalmanReturn defineMeasurements(double &T, const gnsstk::Vector<double> &X, const gnsstk::Matrix<double> &Cov,
                                    bool useFlag)
    {
        if (!useFlag)
        {
    ...
    virtual void output(int N)
    {
        ...
        for (i = 0; i < State.size(); i++)
            oss << " " << std::fixed << std::setprecision(prec) << std::setw(width) << State(i);
        ...
        if (stage == MU)
        {
            oss << std::scientific << std::setprecision(prec) << " " << std::setw(width) << data[count - 1] << " "
                << std::setw(width) << data[count - 1] - State(0) << " " << std::setw(width) << PFResid(0);
        }
        // if SU, also output data, sol residual
        if (stage == SU)
        {
            oss << std::scientific << std::setprecision(prec) << " " << std::setw(width) << data[count] << " "
