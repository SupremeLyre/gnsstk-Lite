...
/**
 class EOPPrediction. Earth orientation parameter prediction
 Read data from EOPP###.txt file, available from NGA. The formulas are:
                       2                           2
 xp(t)= A + B(t-ta) + SUM(Cj sin[2pi(t-ta)/Pj]) + SUM(Dj
 cos[2pi(t-ta)/Pj])
                      j=1                         j=1

                       2                           2
 yp(t)= E + F(t-ta) + SUM(Gk sin[2pi(t-ta)/Qk]) + SUM(Hk
 cos[2pi(t-ta)/Qk])
                      k=1                         k=1
 ...
 cos[2pi(t-tb)/Rm])
                         m=1                         m=1
 Ref. NGA document 'Earth Orientation Parameter Prediction (EOPP)
 Description' Effective date 08 August 2004
...
class EOPPrediction
{
  ...
  public:
    /**
     the difference between TAI and UTC in seconds - not used in the
     computation
    */
    int TAIUTC;
    ...
    */
    ...
    /**
     Compute and return the Earth orientation parameters at the given
     epoch. TD how to warn if input is outside limits of validity?
