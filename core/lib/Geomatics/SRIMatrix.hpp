...
/**
 ...
 a single matrix concatenation of H and D: A = H || D. See doc for the
 overloaded SrifMU().
 @throw MatrixException
*/
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, Matrix<T> &A, unsigned int M = 0);

/**
 Square root information filter (Srif) measurement update (MU).
 Use the Householder transformation to combine the information stored in
 the square root information (SRI) covariance matrix R and state Z with
 new information in the given partials matrix and data vector to produce
 an updated SRI {R,Z}. Measurement noise associated with the new
 information (H and D) is assumed to be white with unit covariance. If
 necessary, the data may be 'whitened' by multiplying H and D by the
 inverse of the lower triangular square root of the covariance matrix;
 that is, compute L = Cholesky(Measurement covariance) and let H = L*H, D = L*D.
...
*/
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, const Matrix<T> &H, Vector<T> &D, unsigned int M = 0);
...
*/
template <class T> Matrix<T> lowerCholesky(const Matrix<T> &A, const T ztol = T(1.e-16));

/**
 Compute upper triangular square root of a symmetric positive definite
 matrix, the Cholesky decomposition; that is A = U*UT = U*transpose(U).
 @param A   Matrix to be decomposed; symmetric and positive definite, unchanged
 @return Matrix U upper triangular square root of input matrix, A=U*UT
 @throw MatrixException if input Matrix is not square
 @throw SingularMatrixException if input Matrix is not positive definite
*/
template <class T> Matrix<T> upperCholesky(const Matrix<T> &A, const T ztol = T(1.e-16));
...
/**
 Compute inverse of upper triangular matrix, returning smallest and
 largest eigenvalues.
 @param UT upper triangular matrix to be inverted
 @param ptrSmall pointer to \<T\> small, on output *ptrSmall contains
            smallest eigenvalue.
 @param ptrBig pointer to \<T\> big, on output *ptrBig contains largest
            eigenvalue.
 @return inverse of input matrix.
 @throw MatrixException if input is not square (assumed upper triangular
 also).
 @throw SingularMatrixException if input is singular.
*/
template <class T> Matrix<T> inverseUT(const Matrix<T> &UT, T *ptrSmall = NULL, T *ptrBig = NULL);
...
/**
 Compute inverse of lower triangular matrix, returning smallest and
 largest eigenvalues.
 ...
 @throw SingularMatrixException if input is singular.
*/
template <class T> Matrix<T> inverseLT(const Matrix<T> &LT, T *ptrSmall = NULL, T *ptrBig = NULL);

/**
 Compute LDL (lower-diagonal-lower) decomposition of a square positive
 definite matrix. Return the lower triangular matrix L and the diagonal as
 a Vector<T>. The decomposition is such that A = L*diag(D)*transpose(L).
...
/**
 Compute UDU (upper-diagonal-upper) decomposition of a square positive
 definite matrix. Return the upper triangular matrix U and the diagonal as
 a Vector<T>. The decomposition is such that A = U*diag(D)*transpose(U).
 @param A Matrix<T> to be decomposed, must be square and positive definite
 @param D Vector<T> output diagonal matrix stored as a Vector
...
/**
 ...
 @throw MatrixException
*/
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, Matrix<T> &A, unsigned int M)
{
...
/**
 ...
 @throw MatrixException if the input has inconsistent dimensions.
*/
template <class T> void SrifMU(Matrix<T> &R, Vector<T> &Z, const Matrix<T> &H, Vector<T> &D, unsigned int M)
{
    try
    {
        Matrix<T> A;
...
/**
 ...
 @throw SingularMatrixException if input Matrix is not positive definite
*/
template <class T> Matrix<T> lowerCholesky(const Matrix<T> &A, const T ztol)
{
...
/**
 ...
 @throw SingularMatrixException if input Matrix is not positive definite
*/
template <class T> Matrix<T> upperCholesky(const Matrix<T> &A, const T ztol)
{
    if (A.rows() != A.cols() || A.rows() == 0)
    {
        std::ostringstream oss;
...
/**
 ...
 @throw SingularMatrixException if input is singular.
*/
template <class T> Matrix<T> inverseUT(const Matrix<T> &UT, T *ptrSmall, T *ptrBig)
{
...
/**
 ...
 @throw SingularMatrixException if input is singular.
*/
template <class T> Matrix<T> inverseLT(const Matrix<T> &LT, T *ptrSmall, T *ptrBig)
{
