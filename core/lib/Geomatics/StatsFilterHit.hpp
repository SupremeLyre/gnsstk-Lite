...
/** @file StatsFilterHit.hpp
    A class used by stats filters (FirstDiffFilter, WindowFilter and
    FDiffFilter) to hold results of the filters. StatsFilterHit is a simple
    ...
    (just a delta index) and the number of good points following the event. It
    has a member function asString() that is used in the filter dump() routines.*/
