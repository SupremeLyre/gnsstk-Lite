...
/** @file SolarSystemEphemeris.hpp
    Implement JPL solar system ephemeris handling, including reading and writing
    of ASCII and binary files and the computation of position and velocity of
    the sun, moon and the 9 planets, as well as nutation and lunar libration and
    their rate. JPL ephemeris files may be obtained from
    ftp://ssd.jpl.nasa.gov/pub/eph/planets. Generally you should download the
    ASCII files and use tools based on this code to convert to your own binary
    files; this avoids compiler- and platform-dependent differences in the
    binary files. */
...
//---------------------------------------------------------------------------------
/**
 Class SolarSystemEphemeris encapsulates the information in the JPL
 ephemeris file, header and data, reading and writing of both ASCII and
 binary files, as well as the computation of position and velocity of the
 sun, moon and planets, plus nutations and lunar librations and their
 rates. The user of this class should not have to read or write new files
 except either when the class is initially installed on a platform, or
 when a new ephemeris is obtained from JPL. Then procedure is first to
 download ASCII files for the desired ephemeris from the JPL ftp site at
 ftp://ssd.jpl.nasa.gov/pub/eph/planets. This consists of an ASCII header
 file (e.g. header.403) plus one or more ephemeris data files for the same
 ...
 file, the programmer simply instantiates a SolarSystemEphemeris object,
 calls initializeWithBinaryFile(file) once, passing it the name of the
 binary file, then calling relativeInertialPositionVelocity() any number
 of times, passing it the time and Planet of interest. Time for this class
 is always Barycentric Dynamic Time (TDB), always as MJD.
*/
class SolarSystemEphemeris
{
  public:
    ...
    {
        retOK = 0,     ///< 0 ok
        retEarly = -1, ///< -1 out of range : input time is before first time in file
        retLate = -2,  ///< -2 out of range : input time is after last time in file
        retStrm = -3,  ///< -3 stream is not open and good, or EOF found prematurely
        retEphN = -4   ///< -4 EphemerisNumber is not defined
    };

    /// These are indexes used by the caller of inertialPositionVelocity().
    enum Planet
    {
        // the following are relative to the solar system barycenter, except
        // MOON
        ...
        idNutations,             ///< 14 Nutations (psi, epsilon and rates)
        idLibrations             ///< 15 Lunar Librations (3 euler angles)
    };

    /**
     Constructor. Set EphemerisNumber to -1 to indicate that nothing has
     been read yet.
    */
    SolarSystemEphemeris() : EphemerisNumber(-1) {};

    //------------------------------------------------------------------
    // reading and writing ASCII (JPL) files

    /**
     Read the header from a JPL ASCII planetary ephemeris file. Note that
     this routine clears the 'store' map and defines the 'constants' hash.
     It also sets EphemerisNumber to the constant "DENUM" if successful.
     @param filename the name of the ASCII header file.
     @throw Exception if the file cannot be opened.
     @throw Exception if the header ends prematurely or if it is not
                properly formatted.
    ...
    */
    ...
    /**
     Read one or more ASCII data files. Call only after having read the
     header, and call only with data files for the same ephemeris as the
     header (the JPL files are named 'header.NNN' and 'ascSYYYY.NNN' where
     NNN is the ephemeris number (appears inside the header file, but not
     inside the data files), S is either 'p' or 'n' as the year is positive
     or negative (AD or BC), and YYYY is the year of the first record in
     the file.
     ...
     @throw Exception if any record in any file has a 'number of
                coefficients' that differs from the header value.
    */
    ...
    /**
     ...
     @throw Exception if any record has a 'number of coefficients' that
                differs from the header value.
    */
    ...
    */
    ...
    /**
     Read header and data from a binary file, storing ALL the data in
     store. For use with copying, merging or editing data files. Closes the
    ...
    */
    ...
    /**
     Open the given binary file, read the header and prepare for reading
     data records at random using seekToJD() and computing positions and
     velocities with inertialPositionVelocity(). Does not store the data.
     @param filename  name of binary file to be read.
     @return 0 success,
            -3 input stream is not open or not valid
            -4 header has not yet been read.
    ...
    */
    ...
    /**
     Compute inertial frame position and velocity of given 'target' body,
     relative to the 'center' body, at the given time. On successful
     return, PV contains position (in components 0-2) and velocity
     (components 3-5) (units: cf param km) for regular bodies; for
     nutations and librations the units are radians and radians/day;
     nutations (components 0-3 only) are longitude or psi (component 0) and
     obliquity or epsilon (component 1) and their rates (components 2,3);
     librations (components 0-5) are the 3 euler angles in radians and
     their rates in radians/day.
     ...
     @param center Body relative to which the results apply. However,
         center may == SolarSystemEphemeris::None, in which case the
         results are relative to the solar system barycenter.
         If target == Nutations or Librations, center is ignored.
     @param PV     Double array of length 6 containing output position and
         velocity components of target relative to center, in the order
         X,Y,Z,Vx,Vy,Vz. Units are determined by parameter km.
         If target == Nutations, PV contains 4 results, psi,
         eps, psi dot, eps dot in units radians and
         radians/day. If target == Librations, PV contains 3
         euler angles in radians and their rates in radians/day.
     @param kilometers     boolean: if true (default),
         units are km, km/day; else AU, AU/day
     ...
     the input stream is not open or not valid, or EOF was found
     prematurely, or the ephemeris is not initialized; most likely the last
     two happen because initializeWithBinaryFile() has not been called, or
     reading failed.
    */
    void relativeInertialPositionVelocity(double MJD, Planet target, Planet center, double PV[6],
    ...
    */
    ...
    /**
     ...
     has not been read, return -1. Return zero if the constant is not
     found.
    */
  ...
  private:
    ...
    */
    ...
    /**
     ...
     position, into the given vector. For use by readBinaryData() and
     seekToJD().
    ...
    */
    ...
    /**
     Search the data records of the file opened by
     initializeWithBinaryFile() and read the one whose time limits include
     the given time. May be called only after initializeWithBinaryFile().
     @param JD the time (Julian Date) of interest
     @return 0 success, or
            -1 given time is before the first record in the file,
            -2 given time is after the last record, or in a gap between
            records, -3 input stream is not open or not valid, or EOF was
            found prematurely, -4 ephemeris (binary file) is not initialized
     -3 or -4 => initializeWithBinaryFile() has not been called, or reading
     failed.
    */
    ...
    /**
     These are indexes used in the actual computation, and correspond to
     indexes in the ephemeris file; for example computation for the SUN is
     done using c_offset[SUN], c_ncoeff[SUN] and c_nsets[SUN].
    */
    enum computeID
    {
        // the following are relative to the solar system barycenter, except
    ...
    /**
     Compute inertial position and velocity of given body at given time,
     relative to the solar system barycenter, using the current coefficient
     array. NB caller MUST call seekToJD(time) BEFORE calling this. On
     successful return, PV[0-2] contains the three position components, in
     km, and PV[3-5] the velocity components in km/day (for regular
     bodies), relative to the solar system barycenter, except for the moon,
     which is relative to Earth. For nutations and librations the units are
     radians and radians/day; nutations (components 0-3 only) are longitude
     and obliquity, and librations are the three euler angles.
     @param  MJD    time (Modified Julian Date) of interest (system TDB).
     @param  which  computeID of the body of interest.
     @param  PV     double(6) array containing the inertial position and
    ...
    */
    ...
    /**
     -1 if the header has not been filled; also, for binary file input, 0
     if the file position map has not yet been filled; otherwise it equals
     the number JPL assigns the ephemeris, e.g. 403, 405, which is
     identical to constants["DENUM"].
    */
    int EphemerisNumber;
    ...
    */
    ...
    int c_offset[13];     ///< starting index in the coefficients array for each
                          ///< planet
    ...
    */
    ...
    /**
     ...
     This object is not intended to store the entire dataset, except
     temporarily for the purpose of reading/writing files, NOT for
    ...
    */
    ...
    /**
     ...
     readBinaryData(), which is called by initializeWithBinaryFile(), and
     is used by seekToJD() to read records in random order.
    */
    std::map<double, long> fileposMap;

    /**
     One complete data record (Ncoeff doubles) consisting of times and
     coefficients. seekToJD() stores the current record here, and
     inertialPositionVelocity() uses it.
    */
