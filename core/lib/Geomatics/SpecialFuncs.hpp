...
/** @file SpecialFuncs.hpp
    Implementation of special functions, including log Gamma, factorial,
    binomial coefficients, beta, incomplete beta, and error functions, as well
    as probability density functions (PDFs) for various distributions, with
    their cumulative distribution (CDF) and percent point (inverse CDF)
    functions; these include the Chi square, Student t and F distributions. The
...
/**
 ...
 dt}. Also, Beta(x,y) = gamma(x)*gamma(y)/gamma(x+y).
...
/**
 ...
 @return          erf(x)
 @throw Exception           if input argument < 0
*/
...
/**
 ...
 before applying these tests.) In modeling applications, such as linear
 and non-linear regression, the error term is often assumed to follow a
 normal distribution with fixed location (mu) and scale (sig). The normal
 distribution is widely used. Part of its appeal is that it is well
 ...
 the following occur:
   ...
   The sampling distribution of the mean is centered at the population
      mean, mu, of the original variable. In addition, the standard deviation of
 ...
 @throw Exception  if math throws
*/
...
/**
 ...
 @throw Exception if prob < 0 || >= 1, if sig <= 0, if 1-prob < eps
*/
...
/**
 ...
 @throw Exception   if x < 0 or n <= 0, or math throws
*/
...
/**
 ...
 http://www.itl.nist.gov/div898/handbook/ 1.3.6.6.6
 ...
 @throw Exception if alpha < 0 || >= 1, 1-alpha < eps, or fails to converge
*/
...
/**
 ...
 poulation standard deviation, sigma, is known or is estimated from the
 data at hand. The more typical case is where the standard deviation must
 be estimated from the data, and the test statistic is
    t = (Ybar - mu0/(s/SQRT(N))
 ...
 @throw Exception if n <= 0 or math throws
*/
...
/**
 ...
 |X/sqrt(chisq/n)| will be less than a fixed constant t is
 StudentCDF(t,n); Ref http://www.itl.nist.gov/div898/handbook/ 1.3.6.6.4
...
/**
 ...
 http://www.itl.nist.gov/div898/handbook/ 1.3.6.6.4
 @param prob probability or significance level of the test, >=0 and < 1
 @param n   degrees of freedom of sample, n > 0
 @return        t the statistic at this probability
 @throw Exception if  prob < 0 || >= 1, n<=0 or 1-prob < eps
*/
...
/**
 ...
 observed samples have the same variance. If F is the ratio of the
 observed dispersion (variance) of the first sample to that of the second,
 where the first sample has n1 degrees of freedom and the second has n2
 degrees of freedom, then this function returns the probability that F
 would be as large as it is if the first sample's distribution has smaller
 variance than the second's. In other words, FDistCDF(f,n1,n2) is the
 significance level at which the hypothesis "sample 1 has smaller variance
 than sample 2" can be rejected. A small numerical value implies a
 significant rejection, in turn implying high confidence in the hypothesis
 "sample 1 has variance greater than or equal to that of sample 2". Ref
 http://www.itl.nist.gov/div898/handbook/ 1.3.6.6.5
 ...
 @throw Exception  if F<0 or n1 <=0 or n2 <= 0 or math throws
*/
...
/**
 ...
 probability (0<=alpha<=1). The hypothesis that the two standard
 deviations are equal is rejected if
 ...
 @throw Exception if math or Gamms() throw
*/
...
/**
 ...
 @throw Exception if prob < 0 || >=1 or n1<=0 or n2<=0 or 1-prob<eps
*/
