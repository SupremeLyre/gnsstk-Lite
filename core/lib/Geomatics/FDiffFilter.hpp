...
/** @file FDiffFilter.hpp Implement a statistical filter that uses first-differences.

    This class implements a statistical filter that uses first-differences, like
    FirstDiffFilter, but this is simpler, more effective and more robust, at
    least for single differenced (across satellites) phase data. Also includes
    here is a class, IterativeFDiffFilter, that applies the FDiffFilter more
    than once; this is the most effective way to use this filter.
    ...
    data. The window filter uses a 2-pane sliding window centered on the data
    point in question; statistics on the data in each to the 2 panes are
    computed and used in the analysis.
       All filters' analysis() function computes a vector of FilterHit objects
       named
    'results'that give the caller the results of the filtering.
       All filters have a getStats(FilterHit) function that computes statistics
       on
    the filter quantities (NOT the data) over the interval covered by the event,
    and stores them in the FilterHit. These stats are slightly different for the
    two filters; FirstDiffFilter::getStats computes min, max, median and mad of
    the first differences.

       The structure of these filters allows the caller to call filters
       repeatedly,
    and to call different filters on the same dataset, because none of the
    filters modify the data array(s) in any way. The arrays are passed as
    constant references to the constructor.  The xdata reference must be
    provided, but it may be empty (except for the window filter if two sample
    statistics are to be used). If xdata is not empty, values of xdata are
    included in the dump() output.
       Similarly, an integer vector of flags is also passed to the constructor
       (also
    ...
    causes the data at index i to be ignored by the filters.
       The arrays data, xdata and flags must always be parallel, and xdata and
       flags
    cannot be shorter than data unless they are empty (when they are ignored).
       The filter() function has optional input of the starting index and the
       number
    of points to process, so that segments of the data (and xdata and flags)
    array(s) can be processed in the filters.
       These features allow the user to, for example, call a filter, mark data
       in the
    flags array (e.g. outliers) and then filter again. If a slip is found, the
    caller can then filter the data again but starting at the slip, or filter
    only the segment of data before the slip [using filter(index, npts)].
       @note the caller must construct a new filter at each call - if you declare a
    Filter object, run filter(), then use the results to modify flags[] and try
    to call filter() again, it does not see the changes to flags[]. Instead you
...
//---------------------------------------------------------------------------------
/**
 This class computes the first difference of the input data array at each
 point, as well as stats on both the first difference and the data for the
 Nwind points ending at the current point. The difference, sigmaYX of the
 difference statistics, and the slope from the data statistics, are stored
 in an "analysis vector" Avec. The Avec is used to find outliers, gaps and
 slips in the data; the analysis() method returns a vector of "result"
 objects FilterHit<T>. The caller should filter and analyze iteratively,
 since a single call is often not sufficient to find and fix all the
 problems.
 @note Flags and xdata are required and critical. Outliers must be
 flagged or removed between iterations, and xdata is needed for gaps and
 to correct slip magnitudes for the slope of the data. Probably the major
 weakness of this filter is it tends to find false slips after large gaps.
*/
template <class T> class FDiffFilter
{
  private:
    ...
    // embedded class -----------------------------------------
    /**
     class used to store analysis arrays filled by the first difference
     filter
    */
    class Analysis
    {
    ...
    /**
     ...
     generated by filter(), used by analyze() and included in dump()
     output.
    */
  ...
  public:
    // member functions ---------------------------------------
    /**
     constructor with two arrays - x is used only in dump(); x and f must
     exist but may be empty
     @param x const vector<T> of 'times' values,
     @note (x,d,f) all parallel
     @param d const vector<T> of data values
     @param f const vector<int> of flags, 0 means good. this array may be
     empty.
    */
    ...
    inline bool indexingHighSigmas()
    {
        return keepSigIndex;
    }
    ...
    inline void setprecision(int p)
    {
        osp = p;
    }

    /// get results vector of FilterHit
    inline std::vector<FilterHit<T>> getResults()
    {
        return results;
    }
    ...
    inline unsigned int getNhighSigma()
    {
        return Nsig;
    }
    ...
    /**
     filter routine that computes the first difference, and uses siglim to
     mark areas of possible outlier/slips.
     @param index in data arrays at which to start, defaults to 0
     @param npts  number of data to process, defaults to -1, meaning to end
                   of data.
    ...
    */
    ...
    /**
     after filter(), and before analyze(), compute robust stats on the
     sigma of first differences, to get a suggested siglim
     @param N int output: computed number of sigmas > new limit, -1 not
                   enough data
     @param new_siglim T output: computed limit (high outlier) on sigma for
                   analyze
    */
    void computeRobustSigmaLimit(int &N, T &new_siglim);

    /**
     analyze the output of the filter(), filling the results array with
     outliers
    ...
    */
    ...
    /**
     dump the data and analysis; optionally include a tag at the start of
     each line, and configure width and precision.
    */
...
template <class T> int FDiffFilter<T>::filter(const size_t i0, int dsize)
{
    ...
    while (i < ilimit)
    {
        ...
        else
        {
            ...
            A.diff = data[i] - data[iprev] - Avec[islope].sloN * (xdata[i] - xdata[iprev]);
            // add diff to stats
    ...
    madSlope = gnsstk::Robust::MedianAbsoluteDeviation(&slopes[0], slopes.size(), medSlope, false);
...
template <class T> void FDiffFilter<T>::dump(std::ostream &os, std::string tag)
{
    size_t i, j, k, iprev;
    int w(osw > 5 ? osw + 1 : 5);
    os << "#" << tag << " FDiffFilter::dump() with limit " << std::fixed << std::setprecision(osp) << fdlim
       << " sigma limit " << siglim << std::setprecision(osp + 2) << " med,mad slope " << medSlope << " " << madSlope
    ...
    for (i = 0, j = 0, k = 0, iprev = -1; i < ilimit; i++)
    {
        ...
        os << tag << std::fixed << std::setprecision(osp) << " " << std::setw(3) << i << " " << std::setw(w)
           << (noxdata ? T(i) : xdata[i]) << " " << std::setw(3) << (noflags ? 0 : flags[i]) << " " << std::setw(w)
           << data[i] << " " << std::setw(w) << sdif << " " << std::setw(w) << ssig << " " << std::setw(w) << slop
           << " " << std::setw(w) << slou << " " << std::setw(w) << sldx << " " << std::setw(w) << sludx << " "
           << std::setprecision(2) << std::setw(5)
           << dt
           //<< " " << std::setw(w) << sigslo
           << (haveAvec && Avec[j].sigN > siglim ? " SIG" : "") << (haveAvec ? "" : " NA");
        if (k < results.size() && haveAvec && i == results[k].index)
        {
            os << " " << results[k].asString()
               << (results[k].type == FilterHit<T>::slip && results[k].score < 100 ? " SMALL" : "");
            k++;
        }
        os << std::endl;
        if (haveAvec)
...
//---------------------------------------------------------------------------------
/**
 class IterativeFDiffFilter is an iteration loop that applies FDiffFilters
 to the data, rejecting outliers and fixing slips in each iteration.
*/
template <class T> class IterativeFDiffFilter
{
  private:
  ...
  public:
    // member functions ---------------------------------------
    /**
     Constructor with three parallel arrays, xdata (~time), data, flags
     (0=good). Flags f may be empty (size 0), but on output of analysis()
     it will be filled.
     @param x const vector<T> of 'times' values,
     @note (x,d,f) all parallel
     @param d const vector<T> of data values
     ...
     empty.
    */
    IterativeFDiffFilter(const std::vector<T> &x, const std::vector<T> &d, const std::vector<int> &f,
    ...
    inline void setLabel(const std::string doit)
    {
        label = doit;
    }
    ...
    inline T getEstimatedSigmaLimit()
    {
        return Esiglim;
    }

    /// get sigma limit used in last iteration
    inline T getUsedSigmaLimit()
    {
        return siguse;
    }
    ...
    /**
     Analyze the data using FDiffFilters, optionally computing new sigma
     outlier limit, in an interative loop. Best for single differenced
     phase. If doResetSigma(true), sigma limit is reduced, if possible, to
     outlier limit. Flags array may be empty (size 0), but on output it
     will be filled. analyze the output of the filter(), filling the
     results array with outliers Return results in Nsig, Esiglim, and
     vector<FilterHit> results.
     @return the number of outliers, or -1 if not enough good data to
     analyze.
    */
    int analysis();
    ...
    /**
     ...
     @note data arrays are NOT edited by filter.
     @note must pass SAME arrays used in c'tor, after calling analysis(), (not
     const).
     @param data vector<T> of data values
     @param flags vector<int> of flags, 0 means good. this array may be
     empty.
...
/**
 Analyze the data using FDiffFilters, optionally computing new sigma
 outlier limit, in an interative loop. Best for single differenced phase.
 */
template <class T> int IterativeFDiffFilter<T>::analysis()
{
    ...
    while (iter <= itermax)
    {
        ...
        if (verbose)
        {
            logstrm << "# " << label << " Estimated sigma limit " << std::fixed << std::setprecision(osp) << Esiglim
                    << " and used sigma limit " << siguse << " (input was " << siglim << ") with " << N
                    << " hi-sigma points " << std::endl;
        }
        ...
        for (i = 0; i < results.size(); i++)
        {
            if (verbose)
            {
                logstrm << "# " << label << " Result " << ++nr << std::fixed << std::setprecision(osp) << " "
                        << xdata[results[i].index] << " " << results[i].asString(osp) << std::endl;
            }
            ...
            if (results[i].type == FilterHit<T>::slip)
            {
                ...
                if (islip)
                {
                    if (verbose)
                    {
                        logstrm << "# " << label << " Fix slip " << islip << " " << results[i].asString(osp)
                                << std::endl;
                    }
        ...
        if (verbose)
        {
            fdf.dump(logstrm, "FIX" + gnsstk::StringUtils::asString(iter - 1) + label);
        }
...
//---------------------------------------------------------------------------------
template <class T>
int IterativeFDiffFilter<T>::editArrays(std::vector<T> &data, std::vector<int> &flags, bool doInt, const int badFlag)
{
    int nedit(0);
    unsigned int i, j, n;
