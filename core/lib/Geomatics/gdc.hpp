...
/**
    ...
    containing dual-frequency pseudorange and phase for an entire satellite
    pass, detect discontinuities in the phase and, if possible, estimate their
...
/**
 ...
 to gaps, slips and the initial point, and also for the segment of the
 data from this break to the next break. It includes start index, number
 of good and total number of points, stats and a mark giving type of
 segment and history
 ("GFslip", "outlier", "bad", "WLslipfixed", etc).
 There will always be an Arc with index==0 and type BEG to begin the data.
 Note that the only thing connecting the Arc to the data is the index;
 npts and ngood are computed using the map<index,Arc> Arcs and
 recomputeArcs()
*/
class Arc
{
  ...
  public:
    ...
    class Arcinfo
    {
      public:
        ...
        int Nslip;  ///< net slip in wavelengths = accumulated integerized step
                    ///< Thus Nslip always is the total slip, while step has
                    ///< only the latest estimate, and is used to fix the data
                    ///< (with int).
        int n;      ///< number of points in stats(ave,sig) - may be > npts in Arc
        double ave; ///< average value of the data in the Arc (data units)
        double sig; ///< std dev or other sig of all the data in the Arc (data
                    ///< units)
    }; // end class Arcinfo
    ...
    Arc(const FilterHit<double> &seg)
    {
        init();
        index = seg.index;
        if (seg.type == FilterHit<double>::BOD)
        {
            mark = BEG; // NB BEG != FilterHit::BOD
        }
        else
        {
            mark = 0;
    ...
    std::string markString() const
    {
        ...
        for (it = markStr.begin(); it != markStr.end(); ++it)
            if (mark & it->first)
            {
                msg += std::string(msg.empty() ? "" : "/") + it->second;
            }
        return msg;
    }
    ...
    std::string brkString() const
    {
        ...
        os << markString() << " good/tot=" << ngood << "/" << npts << "=" << std::fixed << std::setprecision(2)
           << 100. * ngood / double(npts) << "%";
        return os.str();
    }
...
//---------------------------------------------------------------------------------
/**
 Class gdc encapsulates configuration and processing for cycle slip
 detection and fixing using class SatPass.
*/
class gdc
{
  public:
    ...
    */
    ...
    /**
     ...
     @return true if successful, otherwise the label is not a valid
     parameter
     @throw Exception
    */
    ...
    */
    ...
    /**
     Allow caller to force unique number, before call to
     DiscontinuityCorrector(), which will immediately increment it.
    ...
    */
    ...
    /**
     ...
     Find, and fix if possible, discontinuities in the GPS or GLONASS
     carrier phase data, given dual-frequency pseudorange and phase data
     for a satellite pass.

     There are two versions, one accepts the data in parallel vectors, the
     other uses class SatPass, converts the data to vectors and calls the
     first version.

     Input is the SatPass object, or a set of vectors, that hold the data.
     Part of the input is a flags array, which is defined in SatPass (ok=1,
     bad=0); a similarly defined array is required in the array-input
     version.

     Glonass satellites require a frequency channel integer; the caller may
     pass
      this in, or let the GDC compute it from the data - if it fails it
      returns -9.

     This call assumes that the GDC has already been configured.
     See DisplayParameterUsage() for list of cfg.

     The results of the process can be found in the flags array, with its
     values defined by static constants OK, SLIP, etc. NB on output OK=0,
     the OPPOSITE of input/SatPass where OK/BAD = 1/0.

     Other output depends on the two configuration parameters doFix and
     doCmds. If doFix is on (non-zero) then the SatPass is modified for
     slips, outliers and bad data. Note that the GDC never splits a SatPass
     in two, even if rejecting data has created a large gap; this is a
     function only the caller can apply and so must be done after this
     call.

     If doCmds is on, the call returns a vector of strings which are
     editing commands for EditRinex or tk-RinEdit; they are the "--IF name"
     form which is accepted by both (tk-RinEdit only takes this form).

     Also, the L1 and L2 arrays in the input SatPass are corrected.
     The routine will flag bad points in the input data using the values
     defined in this class, including OK, BAD, WLOUTLIER, GFOUTLIER,
     Arc::WLSLIP, Arc::GFSLIP Glonass satellites require a frequency
     channel integer; the caller may pass this in, or let the GDC compute
     it from the data - if this fails it returns -9.

     Output data       filter output    : use setParameter(WLF,1)
            RAW                data as read from SatPass in m
                       WL1     results of first difference filter of WL in
                       wl WLW     results of window filter of WL in wl
            WLF                data with WL slips fixed and outliers
            removed in m
                       GF1     results of first difference filter of GF in
                       wl GFW     results of window filter of GF in wl
            GFF                data with GF slips fixed and outliers
            removed in m FIN                data after final check
     ...
     @param GLOn     GLONASS frequency channel (-7<=n<7), -99 (default) :
                         UNKNOWN. if -99, try to compute from SatPass.
     @return 0 for success, otherwise return an Error code; defined as
     follows.
            -4 insufficient input data, or all data is bad   // TD this
            true? -9 GLONASS frequency channel could not be computed.
    ...
    */
    ...
    /**
     ...
     This is where the work is done; SatPass version creates arrays and
     calls this. Flags on input must be either 1(OK) or 0(BAD) (as in
     SatPass), however on output they are defined by static consts OK, BAD,
     etc
     @throw Exception
    */
    int DiscontinuityCorrector(const RinexSatID &sat, const double &nominalDT, const Epoch &beginTime,
                               std::vector<double> dataL1, std::vector<double> dataL2, std::vector<double> dataP1,
                               std::vector<double> dataP2, std::vector<double> dt, std::vector<int> flags,
                               std::string &retMsg, std::vector<std::string> &cmds, int GLOn = -99,
  ...
  protected:
    ...
    // Arcs handles BEG, SLIP/FIX, and GAP(new BEG); flags handle bad data
    // w/in Arc NB BAD != SatPass::BAD but ONLY SatPass::BAD on input => BAD
    ...
    /**
     vectors used in processing - these MUST always remain parallel and
     equal length vector of data e.g. WLC and GFP, in wavelengths
    */
    std::vector<double> dataWL, dataGF;

    /**
     vector of dt*ndt = number of steps of dt from begin point * dt; from
     spdvector
    */
    ...
    /**
     vector of flags from SatPass OR GDC processing; non-zero == ignore
     data
    */
    std::vector<int> flags;

    /**
     The list (map) of arcs used to store beginnings (BEG=first Arc and
     after gap), slips, number of points, number of good points and stats.
     The map has key=index into data arrays at which the Arc begins, and
     value = Arc. Note that Arcs[A.index] = A, always. Since Arcs is a map
     with key=index, it is sorted on index i.e. in the same order as
     data[]. The first value in this map (Arcs.begin(), at index 0) is
     always marked BEG and has index=0 = first data index, and the last Arc
     in Arcs has index+npts = last data index + 1. New Arcs are created
     whenever there is a slip, or a gap larger than MaxGap. Arcs hang
     around, but slips get fixed, e.g. Arc.mark=WLSLIP&GFSLIP&WLFIX&GFFIX.
     When a slip is fixed, it is marked with xxFIX and the data modified
     from Arc.index all the way to the end.
    */
    ...
    /**
     process one combo (WL or GF) all the way through 1st diff and window
     filters, flagging outliers, marking and fixing slips, and dumping.
     @return return value of filter() if negative, otherwise number of
     slips found
     @throw Exception
    */
    ...
    */
    ...
    /**
     ...
     @throw Exception
    */
    int filterFirstDiff(const unsigned which, const std::string label, double limit,
    ...
    */
    ...
    /**
     ...
     slips, and call fixUpArcs(), if necessary. Test with cases where there
     is huge data rejection in GF, after WL slips, etc.
    ...
    */
    ...
    /**
     Flag bad data in the flags[] array, using a filter hit object. Don't
     alter Arcs NB FixUpArcs() must be called after this routine to
     recompute ngood. Note that flags[] is changed ONLY if flags[]
     currently OK....TD use bitmap?
    ...
    */
    ...
    /// modify Arcs: recompute npts and ngood, remove empty Arcs @throw Exception
    void fixUpArcs();

    /**
     recompute the npts and ngood for each Arc using the indexes in the
     map: Arcs
     @throw Exception
    */
    void recomputeArcs();
    ...
    */
    ...
    /**
     compute stats for 'which' data (WL or GF but not both) for the given
     Arc. NB this is sneaky and goes across fixed slips.... NB do not
     confuse this with WindowFilter::getStats()
    ...
    */
    ...
    /**
     find gaps within Arc in Arcs; if gap is larger than MaxGap, break the
     Arc into two, adding a BEG Arc after the gap.
     @throw Exception
    */
    void findLargeGaps();

    /**
     Fix slips between Arcs, using info.step (NOT info.Nslip), which is
     defined by the filter in results(FilterHit). Compute an integer from
     step and ADD it to Nslip. Thus Nslip always has the total slip, while
     step has only the latest estimate, and is used to fix. In the case of
     the FirstDifferenceFilter this step is only an approximate fix; for
     the WindowFilter the step is defined by stats on the two segments
    ...
    */
    ...
    /**
     ...
     (index+nptsgap==Arc.index+Arc.npts).
     @return map with key=index of beginning of gap, value=number of points
     in gap.
     @throw Exception
    */
    ...
    /**
     find the Arc that contains a given index, starting with the given
     iterator
    ...
    */
    void findArc(const unsigned int ind, std::map<int, Arc>::iterator &ait)
    {
        ...
        while (ait != Arcs.end() && ind >= ait->second.index + ait->second.npts)
            ++ait;
    ...
    /**
     dump all Arcs, e.g.
     #GDC  1 GFW Arc1[0] BEG good/tot=772/772=100.00% dWL=0.00wl 1210 -0.38
     +-0.39 dGF=0.00wl 1210 -0.38 +-0.34
     #GDC  1 GFW Arc2[772] WLslip/GFslip/WLfix/GFfix good/tot=438/445=98.43%
                         dWL=46.54wl 438 -0.71 +-0.32 dGF=-46.37wl 438
                         -0.71 +-0.32
    */
    ...
    #GDC  1 GFW Hit1[2] BOD npts=1206 ngood=1206 step 0.00
                                                min 0.00 max 0.00 med 0.07
                                                mad 0.03
    */
    void DumpHits(const std::vector<FilterHit<double>> &filterResults, const std::string &tag, const std::string &label,
                  int prec = -1);
    ...
    /**
     ...
     @param marks   vector of indexes in SatPass SP where breaks are
     suspected
     @throw Exception
    */
    ...
    */
    ...
    /**
     do a final check on the pass. Look for isolated good points (< MinPts
     good points surrounded by N(?) bad points on each side.
     @throw Exception
    */
    int FinalCheck();
