 ...
 * class gnsstk::Namelist encapsulates a list of labels for use with classes
...
class LabeledVector
{
  public:
    ...
    LabeledVector(const Namelist &nl, const Vector<double> &v) : wid(12), prec(5), form(1), NL(nl), V(v)
    {
    }
...
class LabeledMatrix
{
  public:
    ...
    LabeledMatrix(const Namelist &nl, const Matrix<double> &m)
        : sym(false), cln(false), wid(12), prec(5), form(1), rc(0), M(m), NLrows(nl), NLcols(nl)
    {
    }
    ...
    LabeledMatrix(const Namelist &nr, const Namelist &nc, const Matrix<double> &m)
        : sym(false), cln(false), wid(12), prec(5), form(1), rc(0), M(m), NLrows(nr), NLcols(nc)
    {
    }
