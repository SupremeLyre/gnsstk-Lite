...
/**
 class EarthOrientation encapsulates the Earth orientation parameters or
 EOPs, which consist of the polar motion angle xp and yp and the time
 offset UT1-UTC. These three parameters are determined by interpolating
 measured values managed by class EOPStore. Also, this class includes
 several static functions that implement the models in the IERS
 Conventions (1996, 2003 or 2010), many of which involve the EOPs. This
 includes the frame transformation between the conventional terrestrial
 (i.e. ECEF) frame and the conventional celestial (inertial) frame. The
 class implements models of precession and nutation of Earth's axis, as
 well as the precise rotation of Earth and its 'wobble' as given by the
 EOPs. The class requires time to be in one of two systems: UTC or TT;
 class EphTime is a simple class that enforces this requirement (plus
 TDB), transformation to and from CommonTime should be automatic. (TT is
 terrestrial and TDB is barycentric dynamic time, which is the time of
 SolarSystemEphemeris.) Also cf. classes EOPStore and SolarSystem.
 References:
 ...
 IERS2010: IERS Technical Note 36, "IERS Conventions (2010),"
   Gerard Petit and Brian Luzum eds., Bureau International des Poids et
   Mesures and U.S. Naval Observatory, 2010.
*/
class EarthOrientation
{
  public:
    ...
    */
    ...
    /**
     Given parallel arrays of length four containing the values from
     EOPStore for time (int MJD) and EOPs xp, yp, and UT1mUTC, where the
     time of interest ttag lies within the values of the time array,
     interpolate and apply corrections to determine the EOPs at ttag, using
     the algorithm prescribed by the given IERS convention.
    ...
    */
    void interpolateEOP(const EphTime &ttag, const std::vector<double> &time, const std::vector<double> &X,
                        const std::vector<double> &Y, std::vector<double> &dT, const IERSConvention &conv);
    ...
    /**
     ...
     @throw Exception if the TimeSystem conversion fails (if TimeSystem is
     Unknown)
    */
    ...
    */
    ...
    /**
     ...
     the mean vernal equinox (radians), given the coordinate time of
     interest, using this->UT1mUTC (sec), which comes from the IERS
     bulletin.
     @param t epoch of the rotation.
     @param reduced true when UT1mUTC is 'reduced', meaning assumes
                     'no tides', as is the case with the NGA EOPs
                     (default=F).
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Compute Greenwich Apparent Sidereal Time, or the Greenwich hour angle
     of the true vernal equinox (radians), given the coordinate time of
     interest, and this object's UT1-UTC (sec), which comes from the IERS
     bulletin.
     @param t epoch of the rotation.
     @param reduced true when UT1mUTC is 'reduced', meaning assumes
                     'no tides', as is the case with the NGA EOPs
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Generate transformation matrix (3X3 rotation) due to this object's
     EOPs polar motion angles xp and yp (arcseconds), as found in the IERS
     bulletin;
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Generate an Earth Precession Matrix (3X3 rotation) at the given
     EphTime.
     @param t EphTime epoch of the rotation.
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Generate precise transformation matrix (3X3 rotation) for Earth motion
     due to precession, nutation and frame bias, at the given time of
     interest.
     @param t EphTime epoch of the rotation.
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Generate the full transformation matrix (3x3 rotation) relating the
     ECEF frame to the conventional inertial frame. Input is the time of
     interest; use this object's EOPs - the polar motion angles xp and yp
     (arcseconds), and UT1-UTC (seconds) (xp,yp and UT1-UTC are as found in
     the IERS bulletin).
     @param t epoch of the rotation.
     @param reduced true when UT1mUTC is 'reduced', meaning assumes
  ...
  private:
    ...
    /**
     ...
     the CIP, given the coordinate transformation time T and the
     coordinates X,Y of the CIP. Consistent with IAU 2000A (IERS2003)
     precession-nutation, and P03 precession (IERS2010). Derived in part
     from SOFA routine s00.c for IERS2003 and s06.c for IERS2010. Used for
     both IERS2003 and IERS2010, but not for IERS1996
     @param T the coordinate transformation time at the time of interest
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     The position of the Terrestrial Ephemeris Origin (TEO) on the equator
     of the Celestial Intermediate Pole (CIP), as given by the quantity s'.
    ...
    */
    ...
    /**
     ...
     a series based on IAU 2006 precession and IAU 2000A nutation (IERS
     2010). The coordinates form a unit vector that points towards the CIO;
     they include the effects of frame bias, precession and nutation. cf.
     sofa xy06
     @param T the coordinate transformation time at the time of interest
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Starting with 2003 conventions a new method for computing the
     transformation fron ITRS to GCRS is provided by the Celestial
     Ephemeris Origin (CEO) which is based on the Earth Rotation Angle,
     which depends on time(UT1). Ref. IERS Tech Note 32 Chap 5 Eqn 14
     @param t EphTime time of interest
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Zonal tide terms for corrections of UT1mUTC when that quantity does
     not include tides (e.g. NGA EOP), ref. IERS 1996 Ch. 8, table 8.1
     pg 74.
     @param T    the coordinate transformation time at the time of interest
    ...
    */
    ...
    /**
     obliquity of the ecliptic, in radians,
     given T, the coordTransTime at the time of interest, for IERS1996
     (also 03)
     @param T coordinate transformation time.
     @return eps in radians.
    */
    ...
    */
    ...
    /**
     Compute Greenwich Mean Sidereal Time, or the Greenwich hour angle of
     the mean vernal equinox (radians), given the coordinate time of
     interest, and UT1-UTC (sec), which comes from the IERS bulletin. For
     IERS1996,03
     @param t EphTime epoch of the rotation.
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Helper to compute the Greenwich hour angle of the true vernal equinox,
     or Greenwich Apparent Sidereal Time, in radians, for IERS1996, given
     the (UT) time of interest t, and, where T = coordTransTime(t), om  =
     Omega(T) = mean longitude of lunar ascending node, in radians, ep =
     obliquity(T) = the obliquity of the ecliptic, in radians, dp =
     nutation in longitude (counted in the ecliptic),
                    in seconds of arc.

     GAST = Greenwich hour angle of the true vernal equinox
          = Greenwich apparent sidereal time
          = GMST + dpsi*cos(eps) + 0.00264"*sin(Omega) +
    ...
    */
    ...
    /**
     Compute Greenwich Apparent Sidereal Time, or the Greenwich hour angle
     of the true vernal equinox (radians), given the coordinate time of
     interest, and UT1-UTC (sec), which comes from the IERS bulletin.
     @param t EphTime epoch of the rotation.
     @param UT1mUTC UT1-UTC in seconds, as found in the IERS bulletin.
     @return GAST in radians
     @throw Exception if the TimeSystem conversion fails (if TimeSystem is
    ...
    */
    ...
    /**
     Generate transformation matrix (3X3 rotation) due to the polar motion
     angles xp and yp (arcseconds), as found in the IERS bulletin;
     The returned matrix R transforms the CIP into TRS:  V(TRS) = R *
     V(CIP). see sofa pom00. Also valid for IERS2010.
     @param t EphTime epoch of the rotation.
     @param xp Earth wobble in arcseconds, as found in the IERS bulletin.
     @param yp Earth wobble in arcseconds, as found in the IERS bulletin.
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Compute Fukushima-Williams angles for computing nutation, frame bias
     and precession matrices in IERS2010; cf. FukushimaWilliams(). NB.
     fourth angle is obliquity.
    */
    ...
    */
    ...
    //------------------------------------------------------------------------------
    /**
     Nutation of the obliquity (deps) and of the longitude (dpsi), IERS
     1996 model (ref pg 26), given
     @param T    the coordinate transformation time at the time of interest
     @param deps nutation of the obliquity (output) in radians
     @param dpsi nutation of the longitude (output) in radians
     @param om longitude mean ascending node of lunar orbit, from mean
     equinox
    */
    ...
    */
    ...
    /**
     Generate the full transformation matrix (3x3 rotation) relating the
     ECEF frame to the conventional inertial frame, using IERS 1996
     conventions. Input is the time of interest, the polar motion angles xp
     and yp (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as
     found in the IERS bulletin)
     @param t EphTime epoch of the rotation.
     @param xp Earth wobble in arcseconds, as found in the IERS bulletin.
     @param yp Earth wobble in arcseconds, as found in the IERS bulletin.
     @param UT1mUTC UT1-UTC in seconds, as found in the IERS bulletin.
    ...
    */
    ...
    /**
     Generate the full transformation matrix (3x3 rotation) relating the
     ECEF frame to the conventional inertial frame, using IERS 2003
     conventions. Input is the time of interest, the polar motion angles xp
     and yp (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as
     found in the IERS bulletin)
     @param t EphTime epoch of the rotation.
    ...
    */
    ...
    /**
     Generate the full transformation matrix (3x3 rotation) relating the
     ECEF frame to the conventional inertial frame, using IERS 2010
     conventions. Input is the time of interest, the polar motion angles xp
     and yp (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as
     found in the IERS bulletin)
     @param t EphTime epoch of the rotation.
     ...
     @param UT1mUTC UT1-UTC in seconds, as found in the IERS bulletin.
     @return 3x3 rotation matrix
     @throw Exception if TimeSystem conversion fails (if TimeSystem is Unknown)
    */
