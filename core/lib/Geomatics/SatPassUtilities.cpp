...
/// @file SatPassUtilities.cpp Various utilities using SatPass
...
void dump(vector<SatPass> &SatPassList, ostream &os, bool rev, bool dbug)
{
    try
    {
        ...
        os << printTime(ttag, " to " + SatPass::longfmt) << " timeRevers is " << (rev ? "T" : "F") << " and debug is "
           << (dbug ? "T" : "F") << endl;
        ...
        while (SPit.next(indexMap))
        {
            ...
            for (kt = indexMap.begin(); kt != indexMap.end(); kt++)
            {
                ...
                os << oss.str() << " " << SatPassList[ii].status() << " " << RinexSatID(SatPassList[ii].getSat()) << " "
                   << SatPassList[ii].getFlag(jj);
                os << fixed << setprecision(3);

                for (size_t i = 0; i < obstypes.size(); i++)
                {
                    os << " " << obstypes[i] << " " << setw(13) << SatPassList[ii].data(jj, obstypes[i]) << " "
                       << SatPassList[ii].LLI(jj, obstypes[i]) << " " << SatPassList[ii].SSI(jj, obstypes[i]);
                }
                os << endl;
...
int SatPassFromRinexFiles(vector<string> &filenames, vector<string> &obstypes, double dtin, vector<SatPass> &SPList,
                          vector<RinexSatID> exSats, bool lenient, Epoch beginTime, Epoch endTime)
{
    try
    {
        if (filenames.size() == 0)
        ...
        for (int nfile = 0; nfile < filenames.size(); nfile++)
        {
            ...
            while (1)
            {
                ...
                if (prevtime != CommonTime::BEGINNING_OF_TIME)
                {
                    ...
                    { // positive dt only
                        if (::fabs(::fmod(dt, dtin)) > dttol)
                        {
                            ...
                            {
                                GNSSTK_THROW(Exception(string("Invalid time step: expected ") + asString<double>(dtin) +
                                                       string(" seconds but found ") + asString<double>(dt) +
                                                       string(" at time ") + printTime(obsdata.time, timfmt)));
                            }
                        }
                    ...
                    { // negative, and positive but tiny (< dttol)
                        ...
                        else
                        {
                            GNSSTK_THROW(Exception(string("Invalid time step: expected ") + asString<double>(dtin) +
                                                   string(" seconds but found ") + asString<double>(dt) +
                                                   string(" at time ") + printTime(obsdata.time, "%4F %10.3g")));
                        }
                    }
                }
                ...
                for (it = obsdata.obs.begin(); it != obsdata.obs.end(); ++it)
                {
                    ...
                    {
                        if ((jt = it->second.find(RinexObsHeader::convertObsType(obstypes[j]))) == it->second.end())
                        {
                            data[j] = 0.0;
                            lli[j] = ssi[j] = 0;
                    ...
                    do
                    {
                        i = SPList[satit->second].addData(obsdata.time, obstypes, data, lli, ssi, flag);
                        if (i == -1)
                        { // gap
                            SatPass newSP(sat, dtin, obstypes);
                            SPList.push_back(newSP);
                        ...
                        { // time tag out of order
                            Exception e("Timetags out of order in RINEX file " + filename + " at time " +
                ...
                if (timeShort.size() > 50 && timeShort.size() > nepochs / 2)
                {
                    for (i = 0; i < timeOrder.size(); i++)
                        LOG(WARNING) << "Warning - " << setw(4) << nOrder[i] << " data records following epoch "
                                     << printTime(timeOrder[i], timfmt) << " are out of time order";
        ...
        {
            GNSSTK_THROW(Exception("Input time step (" + asString(dtin, 2) + ") does not match computed (" +
                                   asString(dt, 2) + ")"));
        }
        ...
        if (timeShort.size() > 0)
        {
            for (i = 0; i < timeShort.size(); i++)
                LOG(WARNING) << "Warning - " << setw(4) << nShort[i] << " data records following epoch "
                             << printTime(timeShort[i], timfmt) << " have short (<" << dtin << "sec) timestep";
        }
        if (timeOrder.size() > 0)
        {
            for (i = 0; i < timeOrder.size(); i++)
        ...
        while (spit.next(indexMap))
        {
            ...
            for (kt = indexMap.begin(); kt != indexMap.end(); ++kt)
            {
                ...
                for (ngood = 0, j = 0; j < header.obsTypeList.size(); j++)
                {
                    ...
                    if (SPList[ii].getFlag(jj) != SatPass::BAD && SPList[ii].hasType(obstypes[j]))
                    {
                    ...
                    rotm.insert(map<RinexObsType, RinexDatum>::value_type(header.obsTypeList[j], rd));
                }
                if (ngood > 0)
                {
                    robs.obs.insert(map<SatID, RinexObsData::RinexObsTypeMap>::value_type(sat, rotm));
                    robs.numSvs++;
                }
            }
...
int SatPassToRinex3File(const string &filename, const Rinex3ObsHeader &headerIn,
                        const map<char, vector<string>> &sysobs, vector<SatPass> &SPList)
{
    try
    {
        if (filename.empty())
        ...
        while (spit.next(indexMap))
        {
            ...
            for (kt = indexMap.begin(); kt != indexMap.end(); ++kt)
            {
                ...
                if (ngood > 0)
                {
                    robs.obs.insert(map<RinexSatID, vector<RinexDatum>>::value_type(sat, vRD));
                    robs.numSVs++;
                }
            }
