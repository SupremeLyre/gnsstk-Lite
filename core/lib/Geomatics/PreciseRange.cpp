...
namespace gnsstk
{
double PreciseRange::ComputeAtTransmitTime(const CommonTime &nomRecTime, const double pr, const Position &rxPos,
                                           const SatID sat, const AntexData &antenna, const std::string &freq1,
                                           const std::string &freq2, SolarSystem &solSys, NavLibrary &eph, bool isCOM,
                                           const EllipsoidModel &ellipsoid)
{
    try
    {
        ...
        std::tie(success, transmit) = RawRange::estTransmitFromObs(nomRecTime, pr, eph, NavSatelliteID(sat));
        if (!success)
        {
        ...
        double dr;
        std::tie(dr, std::ignore) = RawRange::computeRange(rxPos, SatR, 0, ellipsoid);
        ...
        std::tie(rawrange, std::ignore) = RawRange::computeRange(rxPos, SatR, 0, ellipsoid);
        double dt = rawrange / ellipsoid.c();
        std::tie(rawrange, svPosVel) = RawRange::computeRange(rxPos, svPosVel, dt, ellipsoid);
