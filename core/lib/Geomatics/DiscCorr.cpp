...
/** @file DiscCorr.cpp GPS phase discontinuity correction. Given a SatPass
    object containing dual-frequency pseudorange and phase for an entire
    satellite pass, and a configuration object (as defined herein), detect
    discontinuities in the phase and, if possible, estimate their size. Output
    is in the form of Rinex editing commands (see class RinexEditor).
*/
...
void GDCconfiguration::setParameter(const std::string &label, double value)
{
    try
    {
        ...
        else
        {
            ...
            if (CFG["Debug"] > 0)
            {
                *(p_oflog) << "GDCconfiguration::setParameter sets " << label << " to " << value << endl;
            }
            CFG[label] = value;
        }
    }
...
void GDCconfiguration::DisplayParameterUsage(ostream &os, bool advanced)
{
    try
    {
        ...
        for (it = CFG.begin(); it != CFG.end(); it++)
        {
            ...
            stst << it->first                                                              // label
                 << "=" << it->second;                                                     // value
            os << " " << leftJustify(stst.str(), 18) << " : " << CFGdescription[it->first] // description
               << endl;
        }
        if (advanced)
        {
            ...
            for (it = CFG.begin(); it != CFG.end(); it++)
            {
                ...
                stst << it->first                                                                        // label
                     << "=" << it->second;                                                               // value
                os << " " << leftJustify(stst.str(), 25) << " : " << CFGdescription[it->first].substr(2) // description
                   << endl;
            }
        }
    }
...
void GDCconfiguration::initialize()
{
    try
    {
        ...
        setcfg(OutputGPSTime, 0,
               ...
               "SatPass fmt)");
        setcfg(OutputDeletes, 1, "if non-zero, include delete commands in the output cmd list");
        ...
        setcfg(WLNWindows, 2.5, "* minimum segment size for WL small slip search (WLWindowWidth)");
        setcfg(WLobviousLimit, 3, "* minimum delta(WL) that produces an obvious slip (WLSigma)");
        setcfg(WLNSigmaStrip, 3.5, "* delete points with WL > this * computed sigma ()");
        setcfg(WLNptsOutlierStats, 200, "* maximum segment size to use robust outlier detection (pts)");
        setcfg(WLRobustWeightLimit, 0.35, "* minimum good weight in robust outlier detection (0<wt<=1)");
        // WL small slips
        setcfg(WLSlipEdge, 3, "* minimum separating WL slips and end of segment, else edit (pts)");
        setcfg(WLSlipSize, 0.9, "* minimum WL slip size (WL wavelengths)");
        setcfg(WLSlipExcess, 0.1, "* minimum amount WL slip must exceed noise (WL wavelengths)");
        setcfg(WLSlipSeparation, 2.5, "* minimum excess/noise ratio of WL slip ()");
        // GF small slips
        setcfg(GFSlipWidth, 5, "* minimum segment length for GF small slip detection (pts)");
        setcfg(GFSlipEdge, 3, "* minimum separating GF slips and end of segment, else edit (pts)");
        setcfg(GFobviousLimit, 1, "* minimum delta(GF) that produces an obvious slip (GFVariation)");
        ...
        // GF fix
        setcfg(GFFixNpts, 15, "* maximum number of points on each side to fix GF slips ()");
        setcfg(GFFixDegree, 3, "* degree of polynomial used to fix GF slips ()");
        setcfg(GFFixMaxRMS, 100, "* limit on RMS fit residuals to fix GF slips, else delete (5.4cm)");
        setcfg(GFSkipSmall, 1, "* if non-zero, skip small GF slips unless there is a WL slip");
    }
    catch (Exception &e)
    {
        GNSSTK_RETHROW(e);
...
class GDCPass : public SatPass, public GDCconfiguration
{
  public:
    ...
    /// detect obvious slips by computing the first difference (of either WL or
    /// GFP) and looking for outliers. create new segments where there are slips
    /// which is either 'WL' or 'GF'.
    /// @throw Exception
    int detectObviousSlips(string which);

    /// compute first differences of data arrays for WL and GF gross slips
    /// detection. for WL difference the WLbias; for GF, the GFP and the residual
    /// GFP-GFR Store results in temporary array A1 and A2
    /// @throw Exception
    ...
    /// also compute conventional stats for each Segment, store in
    /// Segment.WLStats
    /// @throw Exception
    ...
    /// estimate slips in the WL bias and adjust biases appropriately - ie fix WL
    /// slips also compute stats for WL for the whole pass
    /// @throw Exception
    ...
    /// gfp+fit(gfr); divide both P1(gfp) and P2(residual) by wlgf to convert to
    /// cycles also place the residual gfp+gfr(cycles) in L1
    /// @throw Exception
    int prepareGFdata();
    ...
    /// residual of fit; at the same time, compute stats on the first difference
    /// of GF
    /// @throw Exception
    int GFphaseResiduals(list<Segment>::iterator &it);

    /// detect small slips in the geometry-free phase using its first difference
    /// compute statistics in two windows of fixed width on either side of the
    /// point of interest and use these to find slips and outliers
    /// @throw Exception
    ...
    /// @throw Exception
    bool foundGFsmallSlip(int i, int nseg, int iend, int ibeg, deque<int> &pastIn, deque<int> &futureIn,
    ...
    /// using points from indexes nb to ne; n1 is the initial estimate of the
    /// slip called by GFslipFix()
    /// @throw Exception
    long EstimateGFslipFix(list<Segment>::iterator &left, list<Segment>::iterator &right, unsigned int nb,
  ...
  private:
    ...
    /// keep count of various results: slips, deletions, etc.; print to log in
    /// finish()
...
int gnsstk::DiscontinuityCorrector(SatPass &svp, GDCconfiguration &gdc, std::vector<std::string> &editCmds,
...
//---------------------------------------------------------------------------------
GDCPass::GDCPass(SatPass &sp, const GDCconfiguration &gdc) : SatPass(sp.getSat(), sp.getDT(), sp.getObsTypes())
{
    int i, j;
    Status = sp.status();
    dt = sp.getDT();
    ...
    try
    {
        ...
        if (cfg(Debug) >= 2)
        {
            ...
            log << "\n======== Beg GNSSTK Discontinuity Corrector " << GDCUnique
                << " ================================================\n";
            log << "GNSSTK Discontinuity Corrector Ver. " << GDCVersion << " Run " << CurrentTime << endl;
        }
        ...
        for (Ngood = 0, it = SegList.begin(); it != SegList.end(); it++)
        {
            ...
            for (i = it->nbeg; i <= it->nend; i++)
            {
                ...
                if (dbias > cfg(RawBiasLimit))
                {
                    if (cfg(Debug) >= 2)
                    {
                        log << "BEFresetL1 " << GDCUnique << " " << sat << " " << printTime(time(i), outFormat) << " "
                ...
                if (dbias > cfg(RawBiasLimit))
                {
                    if (cfg(Debug) >= 2)
                    {
                        log << "BEFresetL2 " << GDCUnique << " " << sat << " " << printTime(time(i), outFormat) << " "
                ...
                spdvector[i].data[A1] = spdvector[i].data[P1] - wl1 * spdvector[i].data[L1] - biasL1;
                spdvector[i].data[A2] = spdvector[i].data[P2] - wl2 * spdvector[i].data[L2] - biasL2;
        ...
        for (it = SegList.begin(); it != SegList.end(); it++)
        {
            ...
            if (cfg(Debug) >= 1 && it->npts >= static_cast<unsigned int>(cfg(MinPts)))
            {
                log << "WLSIG " << GDCUnique << " " << sat << " " << it->nseg << " "
                    << printTime(time(it->nbeg), outFormat) << fixed << setprecision(3) << " " << it->WLStats.StdDev()
                    << " " << it->WLStats.Average() << " " << it->WLStats.Minimum() << " " << it->WLStats.Maximum()
                    << " " << it->npts << " " << it->nbeg << " - " << it->nend << " " << it->bias1 << " " << it->bias2
                    << endl;
            }
    ...
    try
    {
        ...
        for (i = 0; i < static_cast<int>(size()); i++)
        {
            ...
            { // change segments
                ...
                while (it->nbeg < it->nend && it->nbeg < static_cast<int>(size()) && !(spdvector[it->nbeg].flag & OK))
                {
                    it->nbeg++;
                }

                while (it->nend > it->nbeg && it->nend > 0 && !(spdvector[it->nend].flag & OK))
                {
                    it->nend--;
                }
                it++;
                ...
                for (unsigned int j = igood + 1; j < ibad; j++)
                {
                    ...
                    if (spdvector[j].flag & DETECT)
                    {
                        log << "Warning - found an obvious slip, "
                            << "but marking BAD a point already marked with slip " << GDCUnique << " " << sat << " "
    ...
    try
    {
        ...
        for (i = 0; i < static_cast<int>(size()); i++)
        {
            ...
            if (which == string("WL"))
            {
                ...
                {
                    spdvector[i].data[A1] = (spdvector[i].data[P1] - spdvector[iprev].data[P1]);
                }
            }
            else if (which == string("GF"))
            {
                ...
                else
                {
                    ...
                    spdvector[i].data[A1] = (spdvector[i].data[L1] - spdvector[iprev].data[L1]);
                    // compute first difference of L2 = GFP
                    spdvector[i].data[A2] = (spdvector[i].data[L2] - spdvector[iprev].data[L2]);
                }
            }
    ...
    try
    {
        ...
        {   // robust
            ...
            for (j = i = it->nbeg; i <= it->nend; i++)
            {
                ...
                if (fabs(wlbias - ave) > nsigma || spdvector[j].data[A2] < cfg(WLRobustWeightLimit))
                {
                    outlier = true;
                }
                else
                ...
                if (cfg(Debug) >= 6)
                {
                    log << "DSCWLR " << GDCUnique << " " << sat << " " << it->nseg << " "
                        << printTime(time(i), outFormat) << fixed << setprecision(3) << " " << setw(3)
                        << spdvector[i].flag << " " << setw(13) << spdvector[j].data[A1] // wlbias
                        << " " << setw(13) << fabs(wlbias - ave) << " " << setw(5)
                        << spdvector[j].data[A2] // 0 <= weight <= 1
                        << " " << setw(3) << i << (outlier ? " outlier" : "");
                    if (i == it->nbeg)
                    {
                        log << " " << setw(13) << it->bias1 << " " << setw(13) << it->bias2;
                    }
                    log << endl;
                }
...
int GDCPass::WLstatSweep(list<Segment>::iterator &it, int width)
{
    try
    {
        ...
        for (i = it->nbeg; i <= it->nend; i++)
        {
            ...
            if (cfg(Debug) >= 6)
            {
                log << "WLS " << GDCUnique << " " << sat << " " << it->nseg << " " << printTime(time(i), outFormat)
                    << fixed << setprecision(3) << " " << setw(3) << pastStats.N() << " " << setw(7)
                    << pastStats.Average() << " " << setw(7) << pastStats.StdDev() << " " << setw(3) << futureStats.N()
                    << " " << setw(7) << futureStats.Average() << " " << setw(7) << futureStats.StdDev() << " "
                    << setw(9) << spdvector[i].data[A1] << " " << setw(9) << spdvector[i].data[A2] << " " << setw(9)
                    << wlbias << " " << setw(3) << i << endl;
            }
        ...
        while (i < size())
        {
            ...
            if (spdvector[i].flag & OK)
            {
                ...
                if (nok < halfwidth || (it->npts - nok) < halfwidth)
                {
                    ...
                    if (cfg(Debug) >= 6)
                    {
                        log << "too near end " << GDCUnique << " " << i << " " << nok << " " << it->npts - nok << " "
                            << printTime(time(i), outFormat) << " " << spdvector[i].data[A1] << " "
                            << spdvector[i].data[A2] << endl;
                    }
                }
...
bool GDCPass::foundWLsmallSlip(list<Segment>::iterator &it, int i)
{
    try
    {
        ...
        if (cfg(Debug) >= 6)
        {
            oss << "WLslip " << GDCUnique << " " << sat << " " << setw(2) << it->nseg << " " << setw(3) << i << " "
                << printTime(time(i), outFormat)
                //<< " " << it->npts << "pt"
                << fixed << setprecision(2) << " step=" << step << " lim=" << lim << " (1)" << spdvector[i].data[A1]
                << (spdvector[i].data[A1] > cfg(WLSlipSize) ? ">" : "<=") << cfg(WLSlipSize) << " (2)"
        ...
        if (cfg(Debug) >= 6)
        {
            oss << " (6)" << ratio << (ratio > cfg(WLSlipSeparation) ? ">" : "<=") << cfg(WLSlipSeparation);
        }
        ...
        if (cfg(Debug) >= 6)
        {
            oss << " (4)" << pass4 << (pass4 >= 2 * minMaxWidth - 1 ? ">" : "<=") << 2 * minMaxWidth - 2;
        }
        ...
        if (cfg(Debug) >= 6)
        {
            oss << " (5)" << pass5 << (pass5 >= 2 * minMaxWidth - 1 ? ">" : "<=") << 2 * minMaxWidth - 2;
        }
        ...
        {
            log << "WLslip " << GDCUnique << " " << sat << " " << setw(2) << it->nseg << " " << setw(3) << i << " "
                << printTime(time(i), outFormat) << " Warning - possible half-cycle slip of " << j
                << " WL half-cycles\n";
        }
...
void GDCPass::WLslipFix(list<Segment>::iterator &left, list<Segment>::iterator &right)
{
    try
    {
        unsigned int i;
        ...
        // full slip
        double dwl = right->bias1 + right->WLStats.Average() - (left->bias1 + left->WLStats.Average());
        long nwl = long(dwl + (dwl > 0 ? 0.5 : -0.5));

        if (cfg(Debug) >= 6)
        {
            log << "Fix " << GDCUnique << " " << sat << " " << GDCUniqueFix << " WL "
                << printTime(time(right->nbeg), outFormat) << " " << nwl // put integer fix after time, all 'Fix'
                << " " << left->nseg << "-" << right->nseg << fixed << setprecision(2) << " right: " << right->bias1
                << " + " << right->WLStats.Average() << " - left: " << left->bias1 << " + " << left->WLStats.Average()
                << " = " << dwl << " " << nwl << endl;
        }
...
/* fix one slip in the geometry-free phase
   called by fixOneSlip */
void GDCPass::GFslipFix(list<Segment>::iterator &left, list<Segment>::iterator &right)
{
    try
    {
        ...
        dn1 = spdvector[right->nbeg].data[L2] - right->bias2 - (spdvector[ilast].data[L2] - left->bias2);
        ...
        if (fabs(n1 + nadj - dnGFR) > 10. * (Rstats.StdDev() + Lstats.StdDev()))
        {
            if (cfg(Debug) >= 6)
            {
                log << "GFRadjust " << GDCUnique << " " << sat << " " << GDCUniqueFix << " GF "
                    << printTime(time(right->nbeg), outFormat) << fixed << setprecision(2) << " dbias(GFR): " << dnGFR
                    << " n1+nadj: " << n1 + nadj;
            }
        ...
        if (cfg(Debug) >= 6)
        {
            log << "Fix " << GDCUnique << " " << sat << " " << GDCUniqueFix << " GF "
                << printTime(time(right->nbeg), outFormat) << " " << nadj // put integer fix after time, all 'Fix'
                << fixed << setprecision(2) << " dbias: " << right->bias2 - left->bias2 << ", dn1: " << dn1
                << ", n1: " << n1 << ", adj: " << nadj << " indexes " << nb << " " << ne << " " << nl << " " << nr
                << " segs " << left->nseg << " " << right->nseg << " GFR-GFP:L: " << Lstats.N() << " "
                << Lstats.Average() << " " << Lstats.StdDev() << "    R: " << Rstats.N() << " " << Rstats.Average()
                << " " << Rstats.StdDev() << " tests " << n1 + nadj - dnGFR << " " << Rstats.StdDev() + Lstats.StdDev()
                << endl;
        }
...
long GDCPass::EstimateGFslipFix(list<Segment>::iterator &left, list<Segment>::iterator &right, unsigned int nb,
                                unsigned int ne, long n1)
{
    try
    {
        ...
        while (1)
        {
            ...
            for (k = 0; k < 3; k++)
            {
                ...
                for (i = nb; i <= ne; i++)
                {
                    ...
                    rof = // data minus fit
                        spdvector[i].data[L2] - (i < right->nbeg ? left->bias2 - n1 - (nadj + k - 1) : right->bias2) -
            ...
            for (quit = false, k = 0; k < 3; k++)
                ...
                {
                    log << "Warning - large RMS ROF in GF slip fix at in,k = " << in[k] << " " << k << " "
                        << rmsrof[in[k]] << " abort.\n";
                    quit = true;
                }
            if (quit)
        ...
        if (cfg(Debug) >= 4)
        {
            ...
            for (i = nb; i <= ne; i++)
            {
                ...
                log << "GFE " << GDCUnique << " " << sat << " " << GDCUniqueFix << " " << printTime(time(i), outFormat)
...
int GDCPass::GFphaseResiduals(list<Segment>::iterator &it)
{
    try
    {
        ...
        { // this should never happen
            log << "Polynomial fit to GF range is singular in segment " << it->nseg << "! .. abort." << endl;
            return Singular;
        }
...
int GDCPass::detectGFsmallSlips()
{
    try
    {
        ...
        for (it = SegList.begin(); it != SegList.end(); it++)
        {
            ...
            for (iplus = static_cast<int>(it->nbeg); iplus <= static_cast<int>(it->nend + width); iplus++)
            {
                // ignore bad points
                ...
                if (static_cast<int>(futureIndex.size()) == width || iplus > static_cast<int>(it->nend))
                {
                ...
                if (foundGFsmallSlip(i, it->nseg, it->nend, it->nbeg, pastIndex, futureIndex, pastStats, futureStats))
                {
...
//------------------------------------------------------------------------------------
bool GDCPass::foundGFoutlier(int i, int inew, Stats<double> &pastSt, Stats<double> &futureSt)
{
    try
    {
        if (i < 0 || inew < 0)
        ...
        if (cfg(Debug) >= 6)
        {
            oss << "GFoutlier " << GDCUnique << " " << sat << " " << setw(3) << inew << " "
                << printTime(time(inew), outFormat) << fixed << setprecision(3) << " p,fave=" << fabs(pmag) << ","
                << fabs(fmag) << " var=" << var << " snr=" << fabs(pmag) / var << "," << fabs(fmag) / var;
        }
        ...
        for (;;)
        {
            ...
            if (cfg(Debug) >= 6)
            {
                oss << " (2)" << fabs(pmag) / var << "or" << fabs(fmag) / var << (isOut ? ">=" : "<")
                    << cfg(GFSlipOutlier);
            }
            if (!isOut)
            {
...
bool GDCPass::foundGFsmallSlip(int i, int nseg, int iend, int ibeg, deque<int> &pastIn, deque<int> &futureIn,
                               Stats<double> &pastSt, Stats<double> &futureSt)
{
    try
    {
        ...
        if (cfg(Debug) >= 6)
        {
            log << "GFS " << GDCUnique << " " << sat << " " << nseg << " "
                ...
                << fixed << setprecision(3) << " " << setw(3) << pastSt.N() << " " << setw(7) << pastSt.Average() << " "
                << setw(7) << pastSt.StdDev() << " " << setw(3) << futureSt.N() << " " << setw(7) << futureSt.Average()
                << " " << setw(7) << futureSt.StdDev() << " " << setw(7) << mag << " " << setw(7) << ::sqrt(pvar + fvar)
                << " " << setw(9) << spdvector[i].data[A1] << " " << setw(7) << pmag << " " << setw(7) << pvar << " "
                << setw(7) << fmag << " " << setw(7) << fvar << " " << setw(3) << i << endl;
        }
        ...
        for (;;)
        {
            ...
            if (cfg(Debug) >= 6)
            {
                oss << "GFslip " << GDCUnique << " " << sat << " " << nseg << " " << setw(3) << i << " "
                    << printTime(time(i), outFormat) << fixed << setprecision(3) << " mag=" << mag
                    << " snr=" << snr; // no endl
            }
            ...
            if (cfg(Debug) >= 6)
            {
                oss << " (3)" << fabs(mag / (pmag - fmag)) << (isSlip ? ">" : "<=") << MTS;
            }
            if (!isSlip)
            {
                break;
            ...
            if (cfg(Debug) >= 6)
            {
                oss << " (4)" << fabs(mag) / ::sqrt(pvar + fvar) << (isSlip ? ">" : "<=") << MTN;
            }
            if (!isSlip)
            {
                break;
            ...
            if (static_cast<int>(pastSt.N()) < Edge || static_cast<int>(futureSt.N()) < Edge + 1)
            {
                isSlip = false;
            }
            if (cfg(Debug) >= 6)
            {
                oss << " (5)" << pastSt.N() << "," << futureSt.N() << (isSlip ? ">" : "<") << Edge;
            }
            if (!isSlip)
            {
                break;
            ...
            if (fabs(mag) > RangeCheckLimit)
            {
                ...
                if (cfg(Debug) >= 6)
                {
                    oss << "; GFR-GFP has mag: " << magGFR << ", |dmag|: " << fabs(mag - magGFR) << " and mag/noise "
                        << mtnGFR;
                }
                ...
                if (cfg(Debug) >= 6)
                {
                    oss << " (6a)" << fabs(mag - magGFR) << (isSlip ? "<=" : ">") << fabs(magGFR);
                }
                if (!isSlip)
                {
                    break;
                ...
                if (cfg(Debug) >= 6)
                {
                    oss << " (6b)" << mtnGFR << "><3:can" << (isSlip ? "" : "not") << "_see_in_GFR";
                }
                if (!isSlip)
                {
                    break;
            ...
            { // if(fabs(mag) <= RangeCheckLimit)
                ...
                if (cfg(Debug) >= 6)
                {
                    oss << " (7)1stD(GFP)mag=" << magFD << ",noise=" << fdStats.StdDev()
                        << ",snr=" << fabs(magFD) / fdStats.StdDev() << ",maxima=" << fdStats.Minimum() << ","
                        << fdStats.Maximum();
                }
            }
...
int GDCPass::WLconsistencyCheck()
{
    try
    {
        ...
        for (i = 0; i < static_cast<int>(size()); i++)
        {
            ...
            if (absmag > cfg(WLSlipSize) && absmag > pastStats.StdDev() && absmag > futureStats.StdDev())
            {
                long nwl;
                ...
                if (cfg(Debug) >= 7)
                {
                    log << "CHECK " << GDCUnique << " " << sat << " " << i << " " << printTime(time(i), outFormat)
                        ...
                        << futureStats.N()
                        //<< " " << futureStats.Average()
                        << " " << futureStats.StdDev() << "  " << futureStats.Average() - pastStats.Average() << " "
                        << nwl << endl;
                }
            }
        }
...
string GDCPass::finish(int iret, SatPass &svp, vector<string> &editCmds)
{
    try
    {
        ...
        for (i = 0; i < static_cast<int>(size()); i++)
        {
            ...
            if (ok)
            {
                ...
                if (i - ilast > 2 && cfg(OutputDeletes) != 0)
                {
                    ...
                    if (i - ilast > 3)
                    {
                        stst1 << " # begin delete of " << asString(i + 1 - ilast) << " points";
                    }
                    ...
                    if (i - ilast > 3)
                    {
                        stst2 << " # end delete of " << asString(i + 1 - ilast) << " points";
                    }
        ...
        for (i = 0; i < static_cast<int>(size()); i++)
        {
            ...
            if (spdvector[i].flag & OK)
            {
                if (((spdvector[i].flag & DETECT) == 0 && (spdvector[i].flag & FIX) != 0) || i == ifirst)
                {
        ...
        for (list<Segment>::iterator it = SegList.begin(); it != SegList.end(); it++)
        {
            ...
            oss << GDCtag << " " << GDCUnique << " " << sat << " #" << setw(2) << it->nseg << ": " << setw(4)
                << it->npts << "/" << setw(4) << i << " pts, # " << setw(4) << it->nbeg << "-" << setw(4) << it->nend
                << " (" << printTime(time(it->nbeg), outFormat) << " - " << printTime(time(it->nend), outFormat) << ")";
            if (it->npts > 0)
            {
                oss << fixed << setprecision(3) << " bias(wl)=" << setw(13) << it->bias1 // biaswl
                    << " bias(gf)=" << setw(13) << it->bias2;                            // biasgf;
                if (ilast > -1)
                {
                    ifirst = static_cast<int>(it->nbeg);
                    while (ifirst <= static_cast<int>(it->nend) && !(spdvector[ifirst].flag & OK))
                    {
                        ifirst++;
                    }
                    i = spdvector[ifirst].ndt - spdvector[ilast].ndt;
                    oss << " gap_segs " << setprecision(1) << setw(5) << cfg(DT) * i << " s = " << i << " pts.";
                }
        ...
        oss << GDCtag << " " << GDCUnique << " " << sat << fixed << setprecision(2) << " DT " << fixed
            << setprecision(2) << cfg(DT) << " wavelengths " << wl1 * 100.0 << " " << wl2 * 100.0 << " " << wlwl * 100.0
            << " " << wlgf * 100.0;
        if (sat.system == SatelliteSystem::Glonass)
        {
            oss << " GLOn " << GLOn;
        ...
        if (WLPassStats.N() > 2)
        {
            oss << GDCtag << " " << GDCUnique << " " << sat << " " << fixed << setprecision(3) << WLPassStats.StdDev()
                << " WL sigma in cycles"
                << " N=" << WLPassStats.N() << " Min=" << WLPassStats.Minimum() << " Max=" << WLPassStats.Maximum()
        ...
        if (GFPassStats.N() > 2)
        {
            oss << GDCtag << " " << GDCUnique << " " << sat << " " << fixed << setprecision(3) << GFPassStats.StdDev()
                << " sigma GF variation in meters per DT"
                << " N=" << GFPassStats.N() << " Min=" << GFPassStats.Minimum() << " Max=" << GFPassStats.Maximum()
                << " Ave=" << GFPassStats.Average() << endl;
            oss << GDCtag << " " << GDCUnique << " " << sat << " " << fixed << setprecision(3)
                << (fabs(GFPassStats.Minimum()) > fabs(GFPassStats.Maximum()) ? fabs(GFPassStats.Minimum())
                                                                              : fabs(GFPassStats.Maximum()))
                << " maximum GF variation in meters per DT"
                << " N=" << GFPassStats.N() << " Ave=" << GFPassStats.Average() << " Std=" << GFPassStats.StdDev()
                << endl;
        }
        ...
        for (kt = learn.begin(); kt != learn.end(); kt++)
            oss << GDCtag << " " << GDCUnique << " " << sat << " " << setw(3) << kt->second << " " << kt->first << endl;
        // if(sat.system == SatelliteSystem::Glonass)
        ...
        if (cfg(Debug) > 0)
        {
            oss << GDCtag << "# " << setw(3) << GDCUnique << ", SAT " << sat << ", Pts: " << setw(4) << n << " total "
                << setw(4) << ngood << " good " << fixed << setprecision(1) << setw(5) << percent << "%"
                << ", start " << printTime(firstTime, outFormat) << endl;
        }

        if (iret)
        {
            oss << GDCtag << " " << setw(3) << GDCUnique << " " << sat << " " << printTime(firstTime, outFormat)
                        ...
                        : (iret == BadInput ? "required obs types L1,L2,P1/C1,P2 not found"
                                            : (iret == Singular ? "singularity in polynomial fit"
                                                                : (iret == FatalProblem
                                                                       ...
                                                                       : (iret == PrematureEnd ? "premature end"
                                                                                               : "unknown problem")))))
                << endl;
        }
...
/* create a new segment from the given one, starting at index ibeg,
   ...
   Return an iterator pointing to the new segment. String msg is for debug
   output */
list<Segment>::iterator GDCPass::createSegment(list<Segment>::iterator sit, int ibeg, string msg)
{
    try
    {
        Segment s;
        ...
        if (cfg(Debug) >= 6)
        {
            log << "SEG " << GDCUnique << " " << sat << " " << msg << " " << printTime(time(ibeg), outFormat) << " "
                << s.nbeg << " - " << s.nend << " biases " << fixed << setprecision(3) << s.bias1 << " " << s.bias2
                << endl;
        }
...
string GDCPass::dumpSegments(string label, int level, bool extra)
{
    try
    {
        ...
        // summary of SegList
        oss << label << " " << GDCUnique << " list of Segments (" << SegList.size() << "):" << endl;
        ...
        for (it = SegList.begin(); it != SegList.end(); it++)
        {
            ...
            oss << label << " " << GDCUnique << " " << sat << " #" << setw(2) << it->nseg << ": " << setw(4) << it->npts
                << "/" << setw(4) << i << " pts, # " << setw(4) << it->nbeg << "-" << setw(4) << it->nend << " ("
                << printTime(time(it->nbeg), outFormat) << " - " << printTime(time(it->nend), outFormat) << ")";
            ...
            if (it->npts > 0)
            {
                ...
                if (ilast > -1)
                {
                    ...
                    i = spdvector[ifirst].ndt - spdvector[ilast].ndt;
                    oss << " Gap " << setprecision(1) << setw(5) << cfg(DT) * i << " s = " << i << " pts.";
                }
                ilast = it->nend;
                while (ilast >= static_cast<int>(it->nbeg) && !(spdvector[ilast].flag & OK))
                {
        ...
        for (it = SegList.begin(); it != SegList.end(); it++)
        {
            for (i = it->nbeg; i <= it->nend; i++)
            {
                ...
                oss << "DSC" << label << " " << GDCUnique << " " << sat << " " << it->nseg << " "
                    << printTime(time(i), outFormat) << " " << setw(3) << spdvector[i].flag << fixed << setprecision(3)
                ...
                if (extra)
                {
                    oss << " " << setw(13) << spdvector[i].data[A1] << " " << setw(13) << spdvector[i].data[A2];
                }
                oss << " " << setw(4) << i;
                if (i == it->nbeg)
                {
...
void GDCPass::deleteSegment(list<Segment>::iterator &it, string msg)
{
    try
    {
        ...
        if (cfg(Debug) >= 6)
        {
            log << "Delete segment " << GDCUnique << " " << sat << " " << it->nseg << " pts " << it->npts << " indexes "
                << it->nbeg << " - " << it->nend << " start " << printTime(firstTime, outFormat) << " : " << msg
                << endl;
        }
