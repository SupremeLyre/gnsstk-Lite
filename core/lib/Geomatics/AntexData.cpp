...
/** @file AntexData.cpp
    ...
    receiver and satellite antennas, ANTEX file I/O, discrimination between
    different satellite antennas based on system, PRN and time, and computation
    of phase center offsets and variations. */
...
*/
const vector<string> AntexData::SatelliteTypes = {"BEIDOU-2G",
                                                  "BEIDOU-2I",
                                                  "BEIDOU-2M",
                                                  "BEIDOU-3I",
                                                  "BEIDOU-3SI-CAST",
...
bool AntexData::isValid(CommonTime &time) const
{
    ...
    if (time == CommonTime::BEGINNING_OF_TIME || (!(valid & validFromValid) && !(valid & validUntilValid)))
    {
        return true;
    }
...
// ----------------------------------------------------------------------------
double AntexData::getTotalPhaseCenterOffset(const string &freq, double azim, double elev_nadir) const
{
    try
    {
        ...
        return (-pcv + pco[0] * cosel * cosaz + pco[1] * cosel * sinaz + pco[2] * sinel);
    }
    catch (Exception &e)
    {
        GNSSTK_RETHROW(e);
...
Triple AntexData::getPhaseCenterOffset(const string &freq) const
{
    ...
    {
        gnsstk::Exception e("Frequency " + freq + " not found! System not supported or data corrupted.");
        GNSSTK_THROW(e);
    }
...
double AntexData::getPhaseCenterVariation(const string &freq, double azimuth, double elev_nadir) const
{
    if (!isValid())
    {
    ...
    */
    ...
    if (it == freqPCVmap.end())
    {
        Exception e("Frequency " + freq + " not found! System not supported or data corrupted.");
        GNSSTK_THROW(e);
    }
    ...
    {   // must bracket in azimuth
        ...
        else
        {
            ...
            retpco = (pco[0] * (az_hi - azim) * (zen - zn_lo) + pco[1] * (azim - az_lo) * (zen - zn_lo) +
                      pco[2] * (az_hi - azim) * (zn_hi - zen) + pco[3] * (azim - az_lo) * (zn_hi - zen)) /
...
void AntexData::dump(ostream &s, int detail) const
{
    ...
    s << "Method: " << method << "   Agency: " << agency << "   #Cal.Ant.s: " << noAntCalibrated << "   Date: " << date
      << endl;
    if (azimDelta > 0.0)
    {
        s << "Azimuth dependence, delta azimuth = " << fixed << setprecision(1) << azimDelta << endl;
    }
    else
    {
        s << "No azimuth dependence" << endl;
    }
    s << "Elevation dependence: from " << fixed << setprecision(1) << zenRange[0] << " to " << zenRange[1]
    ...
    if (detail == 1)
    {
        ...
        {
            const antennaPCOandPCVData &antpco = it->second;
            s << "PCO (" << (isRxAntenna ? "NEU from antenna reference position" : "body XYZ from center-of-mass")
              << ") (mm):"
              << " (freq " << it->first << ") " << fixed << setprecision(2) << setw(10) << antpco.PCOvalue[0] << ", "
              << setw(10) << antpco.PCOvalue[1] << ", " << setw(10) << antpco.PCOvalue[2] << endl;
        }
        return;
    }
    ...
    for (it = freqPCVmap.begin(); it != freqPCVmap.end(); it++)
    {
        s << "Offset values for frequency: " << it->first << " (" << (it->second.hasAzimuth ? "has" : "does not have")
          << " azimuths)" << endl;
        const antennaPCOandPCVData &antpco = it->second;

        // PCOs
        s << "  PCO (" << (isRxAntenna ? "NEU from antenna reference position" : "body XYZ from center-of-mass")
          << ") (mm):" << fixed << setprecision(2) << setw(10) << antpco.PCOvalue[0] << ", " << setw(10)
          << antpco.PCOvalue[1] << ", " << setw(10) << antpco.PCOvalue[2] << endl;

        // RMS PCOs
        if (valid & neuFreqRMSValid)
        {
            s << "  RMS PCO (" << (isRxAntenna ? "NEU from antenna reference position" : "body XYZ from center-of-mass")
              << " (mm):" << fixed << setprecision(2) << setw(10) << antpco.PCOrms[0] << ", " << setw(10)
              << antpco.PCOrms[1] << ", " << setw(10) << antpco.PCOrms[2] << endl;
        }
...
*/
void AntexData::evaluateZenithMap(double zen, const zenOffsetMap &eomap, double &zen_lo, double &zen_hi, double &pco_lo,
                                  double &pco_hi) const
{
    map<double, double>::const_iterator kt;
...
void AntexData::ParseDataRecord(string &line)
{
    try
    {
        ...
        { // "# OF FREQUENCIES"
            throwRecordOutOfOrder(validFromValid | validUntilValid | sinexCodeValid | dataCommentValid | startFreqValid,
                                  label);
        ...
        { // "VALID FROM"
            throwRecordOutOfOrder(validUntilValid | sinexCodeValid | dataCommentValid | startFreqValid, label);
        ...
        { // "START OF FREQUENCY"
            foundRMS = false;
            throwRecordOutOfOrder(startFreqRMSValid | neuFreqRMSValid | endOfFreqRMSValid | endOfAntennaValid, label);
        ...
        { // "END OF FREQUENCY"
            ...
            {
                FFStreamError fse("START/END OF FREQ confused: " + freq + " != " + line.substr(3, 3));
                GNSSTK_THROW(fse);
            }
        ...
        { // "END OF FREQ RMS"
            ...
            if (freq != line.substr(3, 3))
            {
                FFStreamError fse("START/END OF FREQ RMS confused: " + freq + " != " + line.substr(3, 3));
                GNSSTK_THROW(fse);
            }
...
CommonTime AntexData::parseTime(const string &line) const
{
    try
    {
        ...
        */
        if ((line.substr(0, 2) != string(2, ' ')) || (line.substr(6, 4) != string(4, ' ')) ||
            (line.substr(12, 4) != string(4, ' ')) || (line.substr(18, 4) != string(4, ' ')) ||
