...
/** @file SolarSystem.hpp
    ...
    The IERS convention (1996, 2003 or 2010) is also included in the class; it
    is required by EarthOrientation but must be consistent with the
...
//---------------------------------------------------------------------------------
/**
 Class SolarSystem provides the functionality of SolarSytemEphemeris in
 the Earth-centered Earth-fixed (ECEF) frame, and makes use of this to
 implement models for solid Earth tides (SETs) as well as geometry-related
 functions involving satellite, Sun, Moon and Earth, e.g. satellite
 attitude.

 The class publicly inherits two large classes:
    class SolarSystemEphemeris (the JPL solar system ephemeris) and
    class EOPStore (storage and retrieval of Earth orientiation
    parameters).
 The motivation for this design is that the fundamental routine of the
 ephemeris, SolarSystem::ECEFPositionVelocity(planet,time), always
 requires simultaneous EarthOrientiation data, and this class enforces
 that requirement. This design allows the class to retrieve EOPs and with
 them implement the transformation from the inertial (celestial) frame of
 the solar system ephemeris to the ECEF (terrestrial) frame (using class
 EarthOrientation).

 The class must first be initialized by initializing both
 SolarSystemEphemeris and EOPStore. SolarSystemEphemeris is initialized by
 calling initializeWithBinaryFile(filename), passing the name of a
 SolarSystem binary file (cf. the convertSSEph app that read JPL ASCII
 files and creates a binary file). EOPStore is initialized by calling
 addIERSFile(filename), passing it the finals2000A.data or similar file
 obtained, e.g. from USNO; see EOPStore. The IERS convention (1996, 2003
 or 2010) may be set at this time; is also included in the class, it is
 required by EarthOrientation but must be consistent with the
 SolarSystemEphemeris.
...
class SolarSystem : public SolarSystemEphemeris, public EOPStore
{
  ...
  public:
    /**
     Empty and only constructor. The IERS convention should be consistent
     with the SolarSystemEphemeris file when initializeWithBinaryFile() is
     called, otherwise a warning is issued.
    */
    ...
    /**
     Choose an IERS Convention. If the input IERS convention is
     inconsistent with the loaded ephemeris then a warning is issued.
    */
    ...
    /**
     Overloaded function to load ephemeris file. A check of the ephemeris
     number and the IERS convention for this object is made; if the IERS
     convention is inconsistent with the ephemeris file then a warning is
     issued. Cf. SolarSystemEphemeris::initializeWithBinaryFile(std::string
     filename).
     @throw Exception
    */
    ...
    /**
     Return the ECEF (terrestrial frame, relative to Earth's center)
     position of a Solar System body at the input time, with units meters.
    ...
    */
    ...
    /**
     Return the ECEF (terrestrial frame, relative to Earth's center)
     position and velocity of a Solar System body at the input time, with
    ...
    */
    Position solarPosition(const EphTime &tt)
    {
        try
        {
            return ECEFPosition(SolarSystemEphemeris::idSun, tt);
        }
        catch (Exception &e)
        {
            GNSSTK_RETHROW(e);
    ...
    */
    Position lunarPosition(const EphTime &tt)
    {
        try
        {
            return ECEFPosition(SolarSystemEphemeris::idMoon, tt);
        }
        catch (Exception &e)
        {
            GNSSTK_RETHROW(e);
    ...
    */
    ...
    /**
     Compute the satellite attitude, given the time and the satellite
     position SV Return a 3x3 Matrix which contains, as rows, the unit
     (ECEF) vectors X,Y,Z in the body frame of the satellite, namely
        ...
        X = completing the orthonormal triad. X will generally point toward
        the sun.
     Thus this rotation matrix R transforms an ECEF XYZ vector into the
     body frame of the satellite, so R * [ECEF XYZ Vector] = components in
     body frame. Also, R.transpose() * [satellite body frame Vector] = ECEF
     XYZ components. Also return the shadow factor, which is the fraction
     of the sun's area not visible to satellite; thus sf > 0 means the
    ...
    */
    ...
    /**
     Compute the angle between the Sun and the plane of the orbit of the
     satellite, given the time and the satellite position and velocity at
     that time. Return the angle in radians; it lies between +-pi/2 and has
     the sign of RxV. That is, the angle is positive if the Sun is out of
     the orbit plane in the direction of R cross V; then Sun "sees" the
     orbit motion as counter-clockwise. Also return, in phi, the angle, in
     the plane of the orbit, from the Sun to the satellite; this lies
     between 0 and 2pi and increases in the direction of Vel.
     ...
     @note NB. phi, beta and sesa, the satellite-earth-sun angle, form
          a right spherical triangle with sesa opposite the right angle. Thus
    ...
    */
    ...
    /**
     ...
     NB. Currently only the largest terms are implemented, yielding a
     result accurate to the millimeter level. Specifically, TN21 pg 61 eq 8
     and TN21 pg 65 eq 17.
    ...
    */
    Triple computeSolidEarthTides(const Position site, const EphTime &tt)
    {
        try
        {
            ...
            return gnsstk::computeSolidEarthTides(site, tt, Sun, Moon, EMRAT, SERAT, iersconv);
        }
        catch (Exception &e)
        {
            GNSSTK_RETHROW(e);
    ...
    /**
     Compute the site displacement due to rotational deformation due to
     polar motion for the given Position (assumed to fixed to the solid
     Earth) at the given time. Return a Triple containing the site
     displacement in ECEF XYZ coordinates with units meters. Reference IERS
     Conventions (1996) found in IERS Technical Note 21, ch. 7 pg 67.
  ...
  private:
    /**
     IERS convention in use with this instance of the class. This is
     determined either by reading the SolarSystemEphemeris number (403 ->
     IERS1996, 405 -> IERS2003 or IERS2010, the default), or by
     assignments; however if the IERS convention is inconsistent with the
     ephemeris then a warning will be issued at the reading of the
     ephemeris file or when the assignment is made.
    */
    ...
    void testIERSvsEphemeris(const IERSConvention conv, const int ephno)
    {
        if (ephno == -1)
        {
            return; // no ephemeris (yet)
        }

        if ((ephno == 403 && conv != IERSConvention::IERS1996) ||
            (ephno == 405 && conv != IERSConvention::IERS2003 && conv != IERSConvention::IERS2010))
        {

            LOG(WARNING) << "Warning - IERS convention (" << conv << ") is inconsistent with SolarSystemEphemeris ("
                         << ephno << ")";
        }
    }
