...
class AntexData : public AntexBase
{
  public:
    ...
    /// Constructor.
    AntexData()
        : valid(0), absolute(true), PRN(0), SVN(0), nFreq(0), validFrom(CommonTime::BEGINNING_OF_TIME),
  ...
  protected:
    ...
    void evaluateZenithMap(double zen, const zenOffsetMap &eomap, double &zen_lo, double &zen_hi, double &pco_lo,
                           double &pco_hi) const;
