...
namespace gnsstk
{
FFStream ::FFStream() : recordNumber(0)
{
}
...
FFStream ::FFStream(const char *fn, std::ios::openmode mode) : recordNumber(0), filename(fn)
{
...
FFStream ::FFStream(const std::string &fn, std::ios::openmode mode) : recordNumber(0), filename(fn)
{
    open(fn, mode);
}

void FFStream ::open(const std::string &fn, std::ios::openmode mode)
{
    open(fn.c_str(), mode);
}
...
void FFStream ::init(const char *fn, std::ios::openmode mode)
{
    close();
    clear();
...
bool FFStream ::isFFStream(std::istream &i)
{
    try
    {
...
{
    s << "filename:" << filename << ", recordNumber:" << recordNumber;
    s << ", exceptions:";

    if (exceptions() & std::ios::badbit)
        s << "bad ";
    if (exceptions() & std::ios::failbit)
        s << "fail ";
    if (exceptions() & std::ios::eofbit)
        s << "eof ";
    if (exceptions() == 0)
        s << "none";
...
void FFStream::tryFFStreamGet(FFData &rec)
{
    ...
    try
    {
        ...
        catch (EndOfFile &e)
        {
            ...
            e.addText("In record " + gnsstk::StringUtils::asString(recordNumber));
        ...
        {
            mostRecentException = FFStreamError("std::exception thrown: " + std::string(e.what()));
            mostRecentException.addText("In record " + gnsstk::StringUtils::asString(recordNumber));
    ...
    catch (std::ifstream::failure &e)
    {
        ...
        if (!fail())
        {
            mostRecentException = FFStreamError("ifstream::failure thrown: " + std::string(e.what()));
            mostRecentException.addText("In file " + filename);
            mostRecentException.addLocation(FILE_LOCATION);
        }
        conditionalThrow();
