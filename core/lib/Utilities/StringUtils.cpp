...
void hexDumpData(const std::string &data, std::ostream &s, const HexDumpDataConfig &cfg)
{
    ...
    {
        s << "hexDumpData: cfg.bytesPerLine % cfg.groupBy != 0" << std::endl;
        s.copyfmt(oldState);
        return;
    }
    if (cfg.group2By && ((cfg.bytesPerLine % cfg.group2By) != 0))
    {
        s << "hexDumpData: cfg.bytesPerLine % cfg.group2By != 0" << std::endl;
        s.copyfmt(oldState);
        return;
    }
    if (cfg.groupBy && ((cfg.group2By % cfg.groupBy) != 0))
    {
        s << "hexDumpData: cfg.group2By % cfg.groupBy != 0" << std::endl;
        s.copyfmt(oldState);
        return;
    }
    ...
    linesize = std::max(cfg.computeLineSize(cfg.bytesPerLine, false), cfg.computeLineSize(bytesOnLastLine, true));
    ...
    for (int i = 0; i < datasize; i++)
    {
        if (i % cfg.bytesPerLine == 0)
        {
            ...
            if (cfg.showIndex)
            {
                ...
                s << std::setfill('0') << (cfg.upperHex ? std::uppercase : std::nouppercase) << cfg.baseIndex()
                  << (cfg.hexIndex ? std::hex : std::dec) << std::setw(cfg.idxDigits) << i << cfg.indexSep
                  << std::noshowbase << std::dec << std::nouppercase;
                col += indexBase.length() + cfg.idxDigits + cfg.indexSep.length();
        ...
        s << (cfg.upperHex ? std::uppercase : std::nouppercase) << std::hex << std::setw(2) << std::setfill('0')
          << (int)c << std::dec << std::nouppercase << std::noshowbase;
        col += 2;
        DEBUG_COL('B', 2);
        if (((i % cfg.bytesPerLine) == (cfg.bytesPerLine - 1)) || (i == (datasize - 1)))
        {
            if (i == (datasize - 1))
            {
...
std::string floatFormat(double d, FFLead lead, unsigned mantissa, unsigned exponent, unsigned width, char expChar,
                        FFSign sign, FFAlign align)
{
    ...
    case FFLead::Zero:
        ...
        oss << std::scientific << std::setprecision(mantissa - 2) << (d * 10);
        rv = oss.str();
        // move the decimal
        pos = rv.find('.');
        rv[pos] = rv[pos - 1];
    ...
    case FFLead::Decimal:
        // because we're shifting the decimal point, we multiply by 10
        oss << std::scientific << std::setprecision(mantissa - 1) << (d * 10);
        rv = oss.str();
        // move the decimal
        pos = rv.find('.');
        rv[pos] = rv[pos - 1];
