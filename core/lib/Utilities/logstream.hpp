...
template <class T> std::string Log<T>::ToString(LogLevel level)
{
    ...
    static const char *const buffer[] = {"ERROR",  "WARNING", "INFO",   "VERBOSE", "DEBUG",  "DEBUG1",
                                         "DEBUG2", "DEBUG3",  "DEBUG4", "DEBUG5",  "DEBUG6", "DEBUG7"};
    return buffer[level];
}
...
template <class T> LogLevel Log<T>::FromString(const std::string &level)
{
    ...
    if (level == "ERROR")
        return ERROR;
    Log<T>().Put(WARNING) << "Unknown logging level '" << level << "'. Using INFO level instead.";
    return INFO;
}
...
#define LOG(level)                                                                                                     \
    if (level <= FILELOG_MAX_LEVEL && level <= ConfigureLOG::ReportingLevel() && ConfigureLOGstream::Stream())         \
    ConfigureLOG().Put(level)
