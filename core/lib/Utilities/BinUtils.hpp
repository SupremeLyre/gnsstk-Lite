 ...
 */
template <class T> inline T decodeVar(const std::string &str, std::string::size_type pos);
 ...
 * @warning This function does not check for appropriate string length.
 */
template <class T> inline T decodeVarLE(const std::string &str, std::string::size_type pos);
 ...
 * @warning This function does not check for appropriate string length.
 */
template <class T> inline T decodeVar(std::string &str);
 ...
 * @warning This function does not check for appropriate string length.
 */
template <class T> inline T decodeVarLE(std::string &str);
 ...
 */
template <class T> inline std::string encodeVar(const T &v);
 ...
 */
template <class T> inline void encodeVar(const T &v, std::string &str, size_t pos = 0);
 ...
 * @param[in] v the object of type T to convert to a string.
 */
template <class T> inline std::string encodeVarLE(const T &v);
 ...
 *   variable will be stored.
 */
template <class T> inline void encodeVarLE(const T &v, std::string &str, size_t pos = 0);
...
inline unsigned long reflect(unsigned long crc, int bitnum);
 ...
 */
inline uint32_t computeCRC(const unsigned char *data, unsigned long len, const CRCParam &params);
...
template <class T> inline void encodeVar(const T &v, std::string &str, size_t pos)
{
    str.replace(pos, sizeof(T), reinterpret_cast<const char *>(&v), sizeof(T));
#if BYTE_ORDER == LITTLE_ENDIAN
...
template <class T> inline void encodeVarLE(const T &v, std::string &str, size_t pos)
{
    str.replace(pos, sizeof(T), reinterpret_cast<const char *>(&v), sizeof(T));
#if BYTE_ORDER == BIG_ENDIAN
    std::reverse(str.begin() + pos, str.begin() + pos + sizeof(T));
...
inline uint32_t computeCRC(const unsigned char *data, unsigned long len, const CRCParam &params)
{
    ...
    // CRC high bit
    uint32_t crcmask = ((((uint32_t)1 << (params.order - 1)) - 1) << 1) | 1;
...
inline void buitohl(const void *p, uint32_t &v, unsigned pos)
{
    ...
    v = *tp;
#else
    v = (((*tp & 0x000000ff) << 24) | ((*tp & 0x0000ff00) << 8) | ((*tp & 0x00ff0000) >> 8) |
         ((*tp & 0xff000000) >> 24));
#endif
}
...
inline void buitohll(const void *p, uint64_t &v, unsigned pos)
{
    ...
    v = *tp;
#else
    v = (((*tp & 0x00000000000000ffULL) << 56) | ((*tp & 0x000000000000ff00ULL) << 40) |
         ((*tp & 0x0000000000ff0000ULL) << 24) | ((*tp & 0x00000000ff000000ULL) << 8) |
         ((*tp & 0x000000ff00000000ULL) >> 8) | ((*tp & 0x0000ff0000000000ULL) >> 24) |
         ((*tp & 0x00ff000000000000ULL) >> 40) | ((*tp & 0xff00000000000000ULL) >> 56));
#endif
}
...
inline void buitohsll(const void *p, int64_t &v, unsigned pos)
{
    ...
    v = (((*tp & 0x00000000000000ffULL) << 56) | ((*tp & 0x000000000000ff00ULL) << 40) |
         ((*tp & 0x0000000000ff0000ULL) << 24) | ((*tp & 0x00000000ff000000ULL) << 8) |
         ((*tp & 0x000000ff00000000ULL) >> 8) | ((*tp & 0x0000ff0000000000ULL) >> 24) |
         ((*tp & 0x00ff000000000000ULL) >> 40) | ((*tp >> 56) & 0x00000000000000ffULL));
#endif
}
...
inline void buhtoil(void *p, uint32_t v, unsigned pos)
{
    ...
    *tp = v;
#else
    *tp = (((v & 0x000000ff) << 24) | ((v & 0x0000ff00) << 8) | ((v & 0x00ff0000) >> 8) | ((v & 0xff000000) >> 24));
#endif
}
...
inline void buhtoill(void *p, uint64_t v, unsigned pos)
{
    ...
    *tp = v;
#else
    *tp = (((v & 0x00000000000000ffULL) << 56) | ((v & 0x000000000000ff00ULL) << 40) |
...
#else
    // mask after shift so sign extension doesn't break stuff
    *tp = (((v & 0x000000ff) << 24) | ((v & 0x0000ff00) << 8) | ((v & 0x00ff0000) >> 8) | ((v >> 24) & 0x000000ff));
#endif
}
...
inline void buhtoisll(void *p, int64_t v, unsigned pos)
{
    ...
    *tp = (((v & 0x00000000000000ffULL) << 56) | ((v & 0x000000000000ff00ULL) << 40) |
           ((v & 0x0000000000ff0000ULL) << 24) | ((v & 0x00000000ff000000ULL) << 8) |
           ((v & 0x000000ff00000000ULL) >> 8) | ((v & 0x0000ff0000000000ULL) >> 24) |
           ((v & 0x00ff000000000000ULL) >> 40) | ((v >> 56) & 0x00000000000000ffULL));
#endif
}
