...
#ifndef GNSSTK_ENUMITERATOR_HPP
#define GNSSTK_ENUMITERATOR_HPP
...
template <typename C, C beginVal, C endVal> class EnumIterator
{
  ...
  public:
    ...
    EnumIterator() : val(static_cast<ValType>(beginVal))
    {
    }
    /// Initialize the iterator to a specific value.
    EnumIterator(const C &f) : val(static_cast<ValType>(f))
    {
    }
