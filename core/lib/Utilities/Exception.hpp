...
class ExceptionLocation
{
  public:
     ...
     */
    ExceptionLocation(const std::string &filename = std::string(), const std::string &funcName = std::string(),
                      const unsigned long &lineNum = 0)
        : fileName(filename), functionName(funcName), lineNumber(lineNum)
    {
    }
...
#define NEW_EXCEPTION_CLASS(child, parent)                                                                             \
    ...
    {                                                                                                                  \
      public:                                                                                                          \
         ...
         * @param[in] c severity of exception (default unrecoverable)                                                  \
         */                                                                                                            \
        child(const char *a, unsigned long b = 0, gnsstk::Exception::Severity c = gnsstk::Exception::unrecoverable)    \
            : parent(a, b, c) {};                                                                                      \
        /** Destructor. */                                                                                             \
        ~child()                                                                                                       \
        {                                                                                                              \
