...
class FileFilterFrameWithHeader : public FileFilterFrame<FileStream, FileData>
{
  public:
    /** Default constructor
     * @throw Exception */
    FileFilterFrameWithHeader(const gnsstk::CommonTime &start = gnsstk::CommonTime::BEGINNING_OF_TIME,
    ...
    FileFilterFrameWithHeader &newSource(const FileSpec &filespec,
                                         ...
                                         const FileSpecFind::Filter &filter = FileSpecFind::Filter())
    {
        FileFilterFrame<FileStream, FileData>::newSource(filespec, start, end, filter);
        init(filter);
        return *this;
    }
    ...
    FileFilterFrameWithHeader &newSource(const std::string &filename,
                                         ...
                                         const gnsstk::CommonTime &end = gnsstk::CommonTime::END_OF_TIME)
    {
        FileFilterFrame<FileStream, FileData>::newSource(filename, start, end);
        init();
        return *this;
    }
    ...
    FileFilterFrameWithHeader &newSource(const std::vector<std::string> &fileList,
                                         ...
                                         const gnsstk::CommonTime &end = gnsstk::CommonTime::END_OF_TIME)
    {
        FileFilterFrame<FileStream, FileData>::newSource(fileList, start, end);
    ...
    std::list<FileData> halfDiff(const FileFilterFrameWithHeader<FileStream, FileData, FileHeader> &r,
                                 BinaryPredicate p, int precision) const
    {
        ...
        {
            if (rdvIt == r.dataVec.end() || p(*dvIt, this->headerList.front(), *rdvIt, r.headerList.front(),
                                              epsilon)) // dv less than
            {
                toReturn.push_back(*dvIt);
                dvIt++;
            }
            else if (p(*rdvIt, r.headerList.front(), *dvIt, this->headerList.front(),
    ...
    template <class Operation> FileFilterFrameWithHeader &touchHeader(Operation &op)
    {
...
template <class FileStream, class FileData, class FileHeader>
bool FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::writeFile(const std::string &outputFile,
...
template <class FileStream, class FileData, class FileHeader>
typename std::list<FileHeader>::const_iterator FileFilterFrameWithHeader<FileStream, FileData,
                                                                         FileHeader>::beginHeader() const
{
    try
    {
        chl("beginHeader");
...
template <class FileStream, class FileData, class FileHeader>
typename std::list<FileHeader>::const_iterator FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::endHeader()
    const
{
    try
    {
...
template <class FileStream, class FileData, class FileHeader>
typename std::list<FileHeader>::iterator FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::beginHeader()
{
    try
    {
        chl("beginHeader");
...
template <class FileStream, class FileData, class FileHeader>
typename std::list<FileHeader>::iterator FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::endHeader()
{
    try
    {
        chl("endHeader");
...
template <class FileStream, class FileData, class FileHeader>
FileHeader &FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::frontHeader()
{
    try
    {
        chl("frontHeader");
...
template <class FileStream, class FileData, class FileHeader>
const FileHeader &FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::frontHeader() const
{
    try
    {
        chl("frontHeader");
...
template <class FileStream, class FileData, class FileHeader>
FileHeader &FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::backHeader()
{
    try
    {
        chl("backHeader");
...
template <class FileStream, class FileData, class FileHeader>
const FileHeader &FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::backHeader() const
{
    try
    {
        chl("backHeader");
...
template <class FileStream, class FileData, class FileHeader>
void FileFilterFrameWithHeader<FileStream, FileData, FileHeader>::init(const FileSpecFind::Filter &filter)
{
    // find the files
    std::list<std::string> listOfFiles = FileSpecFind::find(this->fs, this->startTime, this->endTime, filter);
