 ...
 * gnsstk::FileSpecFind::find("/data/%04Y/%05n/%03j/nsh-%x-%5n-%1r-%04Y-%03j-%02H%02M%02S.xml", fromTime, toTime,
 * textStore);
 * @endcode
 *
 *
...
class FileSpecFind
{
  public:
     ...
     */
    static std::list<std::string> find(const std::string &fileSpec, const CommonTime &start, const CommonTime &end,
    ...
    static std::list<std::string> find(const FileSpec &fileSpec, const CommonTime &start, const CommonTime &end,
                                       const FileSpec::FSTStringMap &fsts = FileSpec::FSTStringMap())
    {
  ...
  private:
     ...
     * @return A list of matching file names.
     */
    static std::list<std::string> findGlob(const CommonTime &start, const CommonTime &end, const std::string &spec,
                                           const FileSpec::FSTStringMap &dummyFSTS, const Filter &filter,
                                           const std::string &matched = "", std::string::size_type pos = 0);
