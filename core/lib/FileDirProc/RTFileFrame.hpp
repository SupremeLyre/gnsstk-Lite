...
template <class FileStream, class FileData> class RTFileFrame;

/**
 * An iterator class for the RTFileFrame.  Note how much it looks
 * like std::istream_iterator except for a couple of differences..
 */
template <class FileStream, class FileData> class RTFileFrameIterator
{
  public:
    /// default constructor
    RTFileFrameIterator() : rtff(NULL)
    {
    }

    /// copy constructor
    RTFileFrameIterator(const RTFileFrameIterator &right) : rtff(right.rtff)
    {
    }
    ...
    bool operator==(const RTFileFrameIterator &right) const
    {
        if (rtff == right.rtff)
        {
            ...
            if ((rtff->isOK == right.rtff->isOK) || (!rtff->isOK && (right.rtff->fileStream == rtff->fileStream)))
                return true;
            return false;
        }
  ...
  private:
    ...
    RTFileFrameIterator(gnsstk::RTFileFrame<FileStream, FileData> &r) : rtff(&r)
    {
        rtff->getRecord();
    }
 ...
 *
 */
template <class FileStream, class FileData> class RTFileFrame : public RTFileFrameHelper
{
  public:
    /**
     * constructor
...
RTFileFrame<FileStream, FileData>::RTFileFrame<FileStream, FileData>(const gnsstk::FileSpec &fnFormat,
    ...
    : fileStream(NULL), fs(fnFormat), startTime(beginning), currentTime(beginning), endTime(ending), readMode(frm),
      getMode(grm)
{
    // zero out seconds
...
template <class FileStream, class FileData> RTFileFrame<FileStream, FileData>::~RTFileFrame()
{
    if (fileStream)
    {
        fileStream->close();
...
template <class FileStream, class FileData>
RTFileFrame<FileStream, FileData> &RTFileFrame<FileStream, FileData>::setFileReadingMode(
    const RTFileFrameHelper::FileReadingMode f)
{
    readMode = f;
    return *this;
}
...
/// Allows changing of the GetRecordMode
template <class FileStream, class FileData>
RTFileFrame<FileStream, FileData> &RTFileFrame<FileStream, FileData>::setGetRecordMode(
    const RTFileFrameHelper::GetRecordMode g)
{
    getMode = g;
    return *this;
}

template <class FileStream, class FileData> void RTFileFrame<FileStream, FileData>::waitForData(unsigned wait)
{
    sleep(wait);
...
template <class FileStream, class FileData> bool RTFileFrame<FileStream, FileData>::hasFileChanged()
{
...
template <class FileStream, class FileData> bool RTFileFrame<FileStream, FileData>::endOfDataSet()
{
...
template <class FileStream, class FileData>
RTFileFrameIterator<FileStream, FileData> RTFileFrame<FileStream, FileData>::end()
{
    return RTFileFrameIterator<FileStream, FileData>();
}

template <class FileStream, class FileData>
RTFileFrameIterator<FileStream, FileData> RTFileFrame<FileStream, FileData>::begin()
{
    if (readMode == AppendedData)
    {
        ;
...
template <class FileStream, class FileData> bool RTFileFrame<FileStream, FileData>::getRecord()
{
    if (!isOK)
        return false;
...
template <class FileStream, class FileData> void RTFileFrame<FileStream, FileData>::openNextDay()
{
...
template <class FileStream, class FileData> std::list<FileData> RTFileFrame<FileStream, FileData>::getList() const
{
...
template <class FileStream, class FileData>
FileFilter<FileData> RTFileFrame<FileStream, FileData>::getFileFilter() const
{
    RTFileFrameIterator<FileStream, FileData> inputStream;
...
template <class FileStream, class FileData> bool RTFileFrame<FileStream, FileData>::openCurrentFile()
{
    isOK = false;
...
template <class FileStream, class FileData> void RTFileFrame<FileStream, FileData>::closeCurrentFile()
{
    if (fileStream)
    {
        fileStream->close();
