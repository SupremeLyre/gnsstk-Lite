...
{
    return fst = (fst == FileSpec::unknown) ? FileSpec::end : FileSpec::FileSpecType(fst - 1);
}

// operator++ for FileSpecType
FileSpec::FileSpecType &operator++(FileSpec::FileSpecType &fst, int)
{
    return fst = (fst == FileSpec::end) ? FileSpec::unknown : FileSpec::FileSpecType(fst + 1);
}

bool FileSpec::FileSpecSort::operator()(const std::string &l, const std::string &r) const
{
        ...
        if (fileSpec.hasField(fst))
        {
            for (unsigned i = 0; i < fileSpec.fileSpecList.size(); i++)
            {
                ...
                if (fileSpec.fileSpecList[i].type == fst)
                {
                    ...
                    if (((sortDir == ascending) && (lsub < rsub)) || ((sortDir == descending) && (lsub > rsub)))
                    {
...
string FileSpec::convertFileSpecType(const FileSpecType fst)
{
    if (fst == station)
        return string("n");
    else if (fst == receiver)
        return string("r");
    else if (fst == prn)
        return string("p");
    else if (fst == selected)
        return string("t");
    else if (fst == sequence)
        return string("I");
    else if (fst == version)
        return string("v");
    else if (fst == fixed)
        ...
        return string("G");
    else if (fst == fullgpsweek)
        return string("F");
    else if (fst == gpssecond)
        return string("g");
    else if (fst == mjd)
        return string("Q");
    else if (fst == dayofweek)
        return string("w");
    else if (fst == day)
        return string("j");
    else if (fst == doysecond)
        return string("s");
    else if (fst == zcount)
        return string("Z");
    else if (fst == zcountfloor)
        return string("z");
    else if (fst == unixsec)
        return string("U");
    else if (fst == unixusec)
        return string("u");
    else if (fst == fullzcount)
        return string("C");
    else
    {
...
FileSpec::FileSpecType FileSpec::convertFileSpecType(const std::string &fst)
{
    if (fst == string("n"))
        return station;
    else if (fst == string("r"))
        return receiver;
    else if (fst == string("p"))
        return prn;
    else if (fst == string("t"))
        return selected;
    else if (fst == string("I"))
        return sequence;
    else if (fst == string("v"))
        return version;
    else if (fst == string("k"))
        return clock;
    else if (fst == string("x"))
        return text;

    else if (fst == string("Y") || fst == string("y"))
        return year;
    else if (fst == string("m"))
        return month;
    else if (fst == string("d"))
        return dayofmonth;
    else if (fst == string("H"))
        return hour;
    else if (fst == string("M"))
        return minute;
    else if (fst == string("S"))
        return second;
    else if (fst == string("f"))
        return fsecond;
    else if (fst == string("G"))
        return gpsweek;
    else if (fst == string("F"))
        return fullgpsweek;
    else if (fst == string("g"))
        return gpssecond;
    else if (fst == string("Q"))
        return mjd;
    else if (fst == string("w"))
        return dayofweek;
    else if (fst == string("j"))
        return day;
    else if (fst == string("s"))
        return doysecond;
    else if (fst == string("Z"))
        return zcount;
    else if (fst == string("z"))
        return zcountfloor;
    else if (fst == string("U"))
        return unixsec;
    else if (fst == string("u"))
        return unixusec;
    else if (fst == string("C") || fst == string("c"))
        return fullzcount;
    else
    {
...
string FileSpec::createSearchString() const
{
    ...
    while (itr != fileSpecList.end())
    {
        ...
        {
            FileSpecException fse("Unknown FileSpecType: " + asString((*itr).type));
            GNSSTK_THROW(fse);
        }
...
string FileSpec::extractField(const string &filename, const FileSpecType fst) const
{
    // stupidity check - is it a valid FST?
    if ((fst <= unknown) || (fst >= end))
    {
        FileSpecException fse("Unknown FileSpecType: " + convertFileSpecType(fst));
        GNSSTK_THROW(fse);
    }
    ...
    // oops - didn't find it.
    FileSpecException fse("Couldn't find specified FileSpecType: " + convertFileSpecType(fst));
    GNSSTK_THROW(fse);
}
...
std::string FileSpec::toString(const gnsstk::CommonTime &dt, const FSTStringMap &fstsMap) const
{
    string toReturn;
        ...
        if (fstsItr != fstsMap.end())
        {
            if ((*fstsItr).first == text)
            {
                ...
                else
                {
                    ...
                    toReturn += leftJustify((*fstsItr).second, (*fslItr).numCh);
                }
            }
            else
            {
...
void FileSpec::sortList(vector<string> &fileList, const FileSpecSortType fsst) const
{
    ...
    if (hasField(version))
    {
        ...
        for (index = 0; index < copyOfFileList.size(); index++)
        {
            ...
            copyOfFileList[index].erase(versionVec[0].offset, versionVec[0].numCh);
        }
...
void FileSpec::dump(ostream &o) const
{
    ...
    while (itr != fileSpecList.end())
    {
        o << setw(6) << (*itr).offset << setw(6) << (*itr).numCh << setw(6)
          << convertFileSpecType((*itr).type)
          // this makes the field bigger if the string is
          // bigger than 5 characters
          << setw(6 + ((*itr).field.size() > 5 ? ((*itr).field.size() - 5) : 0)) << (*itr).field << endl;
        itr++;
    }
}
