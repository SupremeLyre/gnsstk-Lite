 ...
 */
template <class FileData> class FileFilter
{
  public:
    /// Default constructor
    FileFilter(void);
    ...
    template <class Compare> FileFilter &sort(Compare comp)
    {
        // FIX: this someday...
    ...
    /// forte...
    template <class Compare> FileFilter &merge(const FileFilter &right, Compare bp)
    {
        merge(right);
        sort(bp);
        return *this;
    ...
    template <class BinaryPredicate> FileFilter &unique(BinaryPredicate bp)
    {
    ...
    template <class Predicate> FileFilter &filter(Predicate up)
    {
    ...
    template <class Operation> FileFilter &touch(Operation &op)
    {
        filtered = 0;
    ...
    template <class Operation> FileFilter &touch(const Operation &op)
    {
        Operation o(op);
        return touch(o);
    }
    ...
    std::pair<std::list<FileData>, std::list<FileData>> diff(const FileFilter<FileData> &r, BinaryPredicate p) const
    {
        ...
        std::set_difference(dataVec.begin(), dataVec.end(), r.dataVec.begin(), r.dataVec.end(),
                            std::inserter(toReturn.first, toReturn.first.begin()), p);

        std::set_difference(r.dataVec.begin(), r.dataVec.end(), dataVec.begin(), dataVec.end(),
                            std::inserter(toReturn.second, toReturn.second.begin()), p);

        return toReturn;
    }

    /// Returns a list of data matching the given unary predicate.
    template <class Predicate> std::list<FileData> findAll(Predicate p) const
    {
  ...
  protected:
    ...
    /// used elsewhere.
    template <class Compare> class SortAdapter : public std::binary_function<lItrType, lItrType, bool>
    {
      public:
        SortAdapter(Compare &c) : comp(c)
        {
        }
...
template <class FileData> FileFilter<FileData>::FileFilter(void) : filtered(0)
{
}

template <class FileData> FileFilter<FileData>::~FileFilter()
{
}

template <class FileData> FileFilter<FileData> &FileFilter<FileData>::addData(const FileData &ffd)
{
    dataVec.push_back(ffd);
    return *this;
}

template <class FileData> FileFilter<FileData> &FileFilter<FileData>::addData(const std::list<FileData> &datavec)
{
    std::copy(datavec.begin(), datavec.end(), std::inserter(dataVec, dataVec.begin()));
    return *this;
}

template <class FileData> FileFilter<FileData> &FileFilter<FileData>::merge(const FileFilter<FileData> &right)
{
    ...
    std::copy(rightData.begin(), rightData.end(), std::inserter(dataVec, dataVec.begin()));
