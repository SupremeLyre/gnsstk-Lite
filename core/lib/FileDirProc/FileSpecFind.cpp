        ...
        if (hFindFile != INVALID_HANDLE_VALUE)
        {
            do
            {
                ...
                if (winMatchRE(matchPattern, findFileData.cFileName))
                {
                    ...
                    std::string match(patternStr.substr(0, pos) + findFileData.cFileName);
                    // Add the remaining pattern, if any, for recursive search
                    std::string newPattern(match + gnsstk::getFileSep() + patternStr.substr(pos2 + 1));
                    // check if we have more directory layers to go in
                    // the pattern and the match is a directory.
                    if ((pos2 != std::string::npos) && (findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
                    {
...
namespace gnsstk
{
list<string> FileSpecFind ::find(const std::string &fileSpecString, const gnsstk::CommonTime &start,
                                 const gnsstk::CommonTime &end, const gnsstk::FileSpec::FSTStringMap &fsts)
{
    ...
    const gnsstk::FileSpec::FileSpecType fsType = gnsstk::FileSpec::text, fsSel = gnsstk::FileSpec::selected;
    ...
    // width by FileSpec.
    for (unsigned i = gnsstk::FileSpec::unknown; i < gnsstk::FileSpec::firstTime; i++)
    {
...
std::list<std::string> FileSpecFind ::find(const std::string &fileSpec, const CommonTime &start, const CommonTime &end,
                                           const Filter &filter)
{
...
string FileSpecFind ::transToken(const string &token)
{
    // chew through a FileSpec string changing tokens to glob patterns
    string rv;
    string::size_type spos = 0;
    string textTok = gnsstk::FileSpec::convertFileSpecType(gnsstk::FileSpec::text);
    string selTok = gnsstk::FileSpec::convertFileSpecType(gnsstk::FileSpec::selected);
    string stnTok = gnsstk::FileSpec::convertFileSpecType(gnsstk::FileSpec::station);
    while (true)
    {
 ...
 * pattern      :
 * /data/2018/85789/211/nsh-FOO-[0-9][0-9][0-9][0-9][0-9]-[0-9]-[0-9][0-9][0-9][0-9]-[0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9].xml
 * fromTimeMatch: 2018 211 0.000000
 * toTimeMatch  : 2018 212 0.000000
 * glob matches:
 ...
 */
list<string> FileSpecFind ::findGlob(const gnsstk::CommonTime &fromTime, const gnsstk::CommonTime &toTime,
                                     const string &spec, const gnsstk::FileSpec::FSTStringMap &dummyFSTS,
                                     const Filter &filter, const string &matched, string::size_type pos)
{
    list<string> rv;
    // level 0:
    ...
    string::size_type stoppos = min(stokpos, spec.find_last_of(PATH_SEP_STRING, stokpos));
    // srest is the first character of the "rest" of the path
    // i.e. lower depths in the tree.
    string::size_type srest =
        (stokpos == string::npos ? string::npos : spec.find_first_of(PATH_SEP_STRING, stokpos + 1));
    ...
    glob_t globbuf;
    int g = glob(pattern.c_str(), GLOB_ERR | GLOB_NOSORT | GLOB_TILDE, nullptr, &globbuf);
    for (size_t i = 0; i < globbuf.gl_pathc; i++)
    {
        ...
        if (checkTime)
        {
            ...
            // the search time
            gnsstk::CommonTime fileTime = specScanner.extractCommonTime(globbuf.gl_pathv[i]);
            timeMatched = ((fromTimeMatch <= fileTime) && (fileTime < toTimeMatch));
        }
        if (timeMatched)
        {
            ...
            if (!filter.empty())
            {
                ...
                while (fi != filter.end())
                {
                    ...
                    if ((fi->first == lastFST) || currentSpecScanner.hasField(fi->first))
                    {
                        ...
                        if (fi->first != lastFST)
                        {
                            ...
                            fieldVal = specScanner.extractField(globbuf.gl_pathv[i], fi->first);
                            lastFST = fi->first;
                        }
                        ...
                        else
                        {
                            ...
                            // Filtered field.
                            if ((fi == filter.end()) || (fi->first != lastFST))
                            {
                                matchedFilter = false;
                                break;
                            }
            ...
            if (matchedFilter)
            {
                ...
                else
                {
                    ...
                    list<string> toAdd =
                        findGlob(fromTime, toTime, spec, dummyFSTS, filter, globbuf.gl_pathv[i], thisSpec.length());
